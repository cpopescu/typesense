...
bool or_iterator_t::valid() const { return !its.empty(); }
...
void or_iterator_t::advance_smallest() {
  // we will advance the smallest value and point current_index to next smallest
  // value across the lists
...
uint32_t or_iterator_t::id() const { return its[curr_index].id(); }
...
bool or_iterator_t::take_id(result_iter_state_t& istate, uint32_t id,
  ...
  if (istate.filter_ids_length != 0) {
    ...
    size_t found_index =
        std::lower_bound(istate.filter_ids + istate.filter_ids_index,
...
or_iterator_t::or_iterator_t(std::vector<posting_list_t::iterator_t>& its)
    : its(std::move(its)) {
  curr_index = 0;
