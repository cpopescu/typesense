...
Tokenizer::Tokenizer(const std::string& input, bool normalize, bool no_op,
...
void Tokenizer::init(const std::string& input) {
  // init() can be called multiple times safely without leaking memory as we
  ...
  if (locale == "zh") {
    ...
    if (!transliterator) {
      transliterator = icu::Transliterator::createInstance(
          "Traditional-Simplified", UTRANS_FORWARD, translit_status);
    }
    if (U_FAILURE(translit_status)) {
      // LOG(ERROR) << "Unable to create transliteration instance for `zh`
      // locale.";
...
bool Tokenizer::next(std::string& token, size_t& token_index,
    ...
    while (end_pos != icu::BreakIterator::DONE) {
      ...
      if (locale == "ko") {
        ...
        icu::UnicodeString src =
            unicode_text.tempSubStringBetween(start_pos, end_pos);
      ...
      // `word` can be either a multi-byte unicode sequence or an ASCII
      // character ASCII character can be either a special character or English
      // alphabet
      ...
      if (is_ascii_char(word[0])) {
        if (std::isalnum(word[0])) {
          ...
          // special character:
          // a) present in `index_symbols` -> append word to out and continue
          // iteration b) present in `separator_symbols` -> skip word c) not
          // present in either -> skip word
  ...
  while (i < text.size()) {
    ...
    *p = 0;
    ...
    errno = 0;
    iconv(cd, &inptr, &insize, &outptr,
          &outsize);  // this can be handled by ICU via "Latin-ASCII"
...
bool Tokenizer::is_cyrillic(const std::string& locale) {
  return locale == "el" || locale == "ru" || locale == "sr" || locale == "uk" ||
         locale == "be";
}
