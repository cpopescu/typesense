...
Option<bool> stateful_remove_docs(deletion_state_t* deletion_state,
                                  size_t batch_size, bool& done) {
  bool removed = true;
  ...
  for (size_t i = 0; i < deletion_state->index_ids.size(); i++) {
    ...
    for (size_t j = start_index; j < batched_len; j++) {
      Option<bool> remove_op =
          deletion_state->collection->remove_if_found(ids[j], true);
...
Option<bool> stateful_export_docs(export_state_t* export_state,
                                  size_t batch_size, bool& done) {
  size_t batch_count = 0;
  ...
  for (size_t i = 0; i < export_state->index_ids.size(); i++) {
    ...
    for (size_t j = start_index; j < batched_len; j++) {
      ...
      nlohmann::json doc;
      Option<bool> get_op =
          export_state->collection->get_document_from_store(seq_id, doc);
