...
int64_t compact_posting_list_t::upsert(const uint32_t id,
...
int64_t compact_posting_list_t::upsert(const uint32_t id,
  ...
  if (length == 0 || id > last_id) {
      ...
      if (existing_id == id) {
        ...
        if (extra_length_needed > 0) {
          ...
          while (shift_index >= i && (shift_index - extra_length_needed) >= 0) {
            id_offsets[shift_index] =
                id_offsets[shift_index - extra_length_needed];
            shift_index--;
          }
    ...
    length += extra_length_needed;  // extra_length_needed can be negative here
                                    // but that's okay
  }
...
compact_posting_list_t* compact_posting_list_t::create(
    uint32_t num_ids, const uint32_t* ids, const uint32_t* offset_index,
    uint32_t num_offsets, const uint32_t* offsets) {
  // format: num_offsets, offset1,..,offsetn, id1 | num_offsets,
  // offset1,..,offsetn, id2

  size_t length_required = num_offsets + (2 * num_ids);
  compact_posting_list_t* pl = (compact_posting_list_t*)malloc(
      sizeof(compact_posting_list_t) + (length_required * sizeof(uint32_t)));
  ...
  for (size_t i = 0; i < num_ids; i++) {
    ...
    uint32_t next_start_offset =
        (i == num_ids - 1) ? num_offsets : offset_index[i + 1];
    pl->upsert(ids[i], offsets + start_offset,
...
uint32_t compact_posting_list_t::num_ids() const { return ids_length; }
...
bool compact_posting_list_t::contains_atleast_one(const uint32_t* target_ids,
  ...
  while (i < length && target_ids_index < target_ids_size) {
    ...
    size_t found_index =
        std::lower_bound(target_ids + target_ids_index,
                         target_ids + target_ids_size, existing_id) -
        target_ids;
...
void posting_t::upsert(void*& obj, uint32_t id,
                       const std::vector<uint32_t>& offsets) {
  if (IS_COMPACT_POSTING(obj)) {
    compact_posting_list_t* list =
        (compact_posting_list_t*)RAW_POSTING_PTR(obj);
    ...
    if ((list->capacity + extra_capacity_required) >
      ...
      size_t new_capacity =
          std::min<size_t>((list->capacity + extra_capacity_required) * 1.3,
                           COMPACT_LIST_THRESHOLD_LENGTH);

      size_t new_capacity_bytes =
          sizeof(compact_posting_list_t) + (new_capacity * sizeof(uint32_t));
      auto new_list =
          (compact_posting_list_t*)realloc(list, new_capacity_bytes);
      if (new_list == nullptr) {
        abort();
      }
...
void posting_t::erase(void*& obj, uint32_t id) {
  if (IS_COMPACT_POSTING(obj)) {
    ...
    if (list->num_blocks() == 1 && ((2 * list->get_root()->size()) +
                                    list->get_root()->offsets.getLength()) <=
                                       COMPACT_LIST_THRESHOLD_LENGTH) {
      ...
      compact_posting_list_t* compact_list = compact_posting_list_t::create(
          root_block->size(), ids, offset_index,
          root_block->offsets.getLength(), offsets);
...
bool posting_t::contains_atleast_one(const void* obj,
...
void posting_t::merge(const std::vector<void*>& raw_posting_lists,
...
void posting_t::intersect(const std::vector<void*>& raw_posting_lists,
                          std::vector<uint32_t>& result_ids,
...
void posting_t::to_expanded_plists(
    const std::vector<void*>& raw_posting_lists,
    std::vector<posting_list_t*>& plists,
    std::vector<posting_list_t*>& expanded_plists) {
...
void posting_t::get_array_token_positions(
    uint32_t id, const std::vector<void*>& raw_posting_lists,
...
void posting_t::get_exact_matches(const std::vector<void*>& raw_posting_lists,
  ...
  posting_list_t::get_exact_matches(its, field_is_array, ids, num_ids,
                                    exact_ids, num_exact_ids);
...
void posting_t::get_matching_array_indices(
    const std::vector<void*>& raw_posting_lists, uint32_t id,
...
void posting_t::get_phrase_matches(const std::vector<void*>& raw_posting_lists,
                                   bool field_is_array, const uint32_t* ids,
                                   uint32_t num_ids, uint32_t*& phrase_ids,
  ...
  posting_list_t::get_phrase_matches(its, field_is_array, ids, num_ids,
