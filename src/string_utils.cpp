...
std::string StringUtils::randstring(size_t length) {
  ...
  thread_local std::uniform_int_distribution<uint32_t> pick(0,
                                                            sizeof(chrs) - 2);
...
std::string StringUtils::hmac(const std::string& key, const std::string& msg) {
  ...
  HMAC(EVP_sha256(), key.c_str(), key.size(),
       reinterpret_cast<const unsigned char*>(msg.c_str()), msg.size(), hmac,
       &hmac_len);
...
std::string StringUtils::str2hex(const std::string& str, bool capital) {
  ...
  for (size_t i = 0, c = str[0] & 0xFF; i < hexstr.size();
       c = str[i / 2] & 0xFF) {
...
std::string StringUtils::hash_sha256(const std::string& str) {
  ...
  SHA256(reinterpret_cast<const unsigned char*>(str.c_str()), str.size(),
         hash_buf);
  return StringUtils::str2hex(
      std::string(reinterpret_cast<char*>(hash_buf), SHA256_SIZE));
}

void StringUtils::parse_query_string(
    const std::string& query, std::map<std::string, std::string>& query_map) {
  ...
  while (i < query_len) {
    // we have to support un-encoded "&&" in the query string value, which makes
...
void StringUtils::split_to_values(const std::string& vals_str,
...
void StringUtils::replace_all(std::string& subject, const std::string& search,
...
bool StringUtils::contains_word(const std::string& haystack,
...
char* StringUtils::get_ip_str(const struct sockaddr* sa, char* s,
                              size_t maxlen) {
  switch (sa->sa_family) {
    case AF_INET:
...
Option<bool> parse_reference_filter(const std::string& filter_query,
...
Option<bool> StringUtils::tokenize_filter_query(
    const std::string& filter_query, std::queue<std::string>& tokens) {
  auto size = filter_query.size();
...
Option<bool> StringUtils::split_include_fields(
    const std::string& include_fields, std::vector<std::string>& tokens) {
    ...
    if (range_pos == std::string::npos && comma_pos == std::string::npos) {
      ...
      if (end == std::string::npos ||
        ...
        return Option<bool>(400,
                            "Invalid reference in include_fields, expected "
                            "`$CollectionName(fieldA, ...)`.");
      }
...
size_t StringUtils::split_facet(const std::string& s,
                                std::vector<std::string>& result,
                                const bool keep_empty, const size_t start_index,
