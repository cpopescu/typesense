...
int64_t Index::get_points_from_doc(const nlohmann::json& document,
  ...
  if (document[default_sorting_field].is_number_float()) {
    ...
    points ^=
        ((points >> (std::numeric_limits<int32_t>::digits - 1)) | INT32_MIN);
    points = -1 * (INT32_MAX - points);
  } else if (document[default_sorting_field].is_string()) {
    // not much value in supporting default sorting field as string, so we will
    // just dummy it out
    points = 0;
  } else {
    points = document[default_sorting_field].is_boolean()
                 ? int64_t(document[default_sorting_field].get<bool>())
...
void Index::compute_token_offsets_facets(
    index_record& record, const tsl::htrie_map<char, field>& search_schema,
  ...
  for (const auto& the_field : search_schema) {
    ...
    if (the_field.is_string()) {
      if (the_field.type == field_types::STRING) {
        tokenize_string_with_facets(
            document[field_name], is_facet, the_field, local_symbols_to_index,
            local_token_separators, offset_facet_hashes.offsets,
            offset_facet_hashes.facet_hashes);
      } else {
        tokenize_string_array_with_facets(
...
bool doc_contains_field(const nlohmann::json& doc, const field& a_field,
  ...
  // check for a nested field, e.g. `foo.bar.baz` indexed but `foo.bar` present
  // in schema
...
void Index::validate_and_preprocess(
    Index* index, std::vector<index_record>& iter_batch,
    ...
    const std::vector<char>& token_separators,
    const std::vector<char>& symbols_to_index, const bool do_validation,
    ...
    try {
      ...
      if (do_validation) {
        Option<uint32_t> validation_op = validator_t::validate_index_in_memory(
            ...
            search_schema, embedding_fields, index_rec.operation,
            index_rec.is_update, fallback_field_type, index_rec.dirty_values,
            generate_embeddings);
      ...
      if (index_rec.is_update) {
        ...
        get_doc_changes(index_rec.operation, embedding_fields, index_rec.doc,
          ...
          for (auto& field : index_rec.doc.items()) {
            for (auto& embedding_field : embedding_fields) {
              if (!embedding_field.embed[fields::from].is_null()) {
                auto embed_from_vector = embedding_field.embed[fields::from]
      ...
      compute_token_offsets_facets(index_rec, search_schema, token_separators,
                                   symbols_to_index);
      ...
      if (index_rec.doc.count(default_sorting_field) == 0) {
        ...
        if (default_sorting_field_it != index->sort_index.end()) {
          auto seq_id_it =
              default_sorting_field_it->second->find(index_rec.seq_id);
  ...
  for (size_t thread_id = 0;
    ...
    index->thread_pool->enqueue([&, batch_index, batch_len]() {
      ...
      validate_and_preprocess(index, iter_batch, batch_index, batch_len,
                              ...
                              embedding_fields, fallback_field_type,
                              token_separators, symbols_to_index, do_validation,
                              remote_embedding_batch_size, generate_embeddings);
      ...
      try {
        ...
        for (auto& record : iter_batch) {
          record.index_failure(500,
                               "Unhandled Typesense error in index batch, "
    ...
    for (const auto& record : iter_batch) {
      ...
      if (afield.facet) {
        if (afield.is_array()) {
          ...
          for (size_t i = 0; i < field_index_it->second.facet_hashes.size();
               i++) {
          ...
          if (facet_dim_index == nullptr) {
            LOG(ERROR) << "Error, facet index not initialized for field "
                       << afield.name;
          } else {
          ...
          auto& facet_dim_index =
              single_val_facet_index_v3[afield.name][seq_id % ARRAY_FACET_DIM];
          if (facet_dim_index == nullptr) {
            LOG(ERROR) << "Error, facet index not initialized for field "
                       << afield.name;
          } else {
      ...
      for (auto& token_offsets : field_index_it->second.offsets) {
        ...
        if (afield.infix) {
          auto strhash = StringUtils::hash_wy(token_offsets.first.c_str(),
  ...
  if (!afield.is_string()) {
    if (afield.type == field_types::INT32) {
      ...
      iterate_and_index_numerical_field(
          iter_batch, afield,
          [&afield, num_tree](const index_record& record, uint32_t seq_id) {
      ...
      iterate_and_index_numerical_field(
          ...
          [&afield, &geo_array_index = geo_array_index, geo_index](
            ...
            // nested geopoint value inside an array of object will be a simple
            // array so must be treated as geopoint
            bool nested_obj_arr_geopoint =
                (afield.nested && afield.type == field_types::GEOPOINT_ARRAY &&
                 !record.doc[afield.name].empty() &&
                 record.doc[afield.name][0].is_number());

            if (afield.type == field_types::GEOPOINT ||
                nested_obj_arr_geopoint) {
              // this could be a nested gepoint array so can have more than 2
              // array values
              ...
              for (size_t li = 0; li < latlongs.size(); li += 2) {
                ...
                S2Point point =
                    S2LatLng::FromDegrees(latlongs[li], latlongs[li + 1])
                        .ToPoint();
              ...
              if (nested_obj_arr_geopoint) {
                int64_t* packed_latlongs =
                    new int64_t[(latlongs.size() / 2) + 1];
                packed_latlongs[0] = latlongs.size() / 2;
                size_t j_packed_latlongs = 0;

                for (size_t li = 0; li < latlongs.size(); li += 2) {
                  int64_t packed_latlong =
                      GeoPoint::pack_lat_lng(latlongs[li], latlongs[li + 1]);
      ...
      if (afield.type == field_types::FLOAT_ARRAY && afield.num_dim > 0) {
        ...
        const size_t window_size = (num_threads == 0)
                                       ? 0
                                       : (iter_batch.size() + num_threads - 1) /
        ...
        for (size_t thread_id = 0;
             thread_id < num_threads && result_index < iter_batch.size();
             thread_id++) {
          size_t batch_len = window_size;
          ...
          thread_pool->enqueue([thread_id, &afield, &vec_index,
                                &records = iter_batch, result_index, batch_len,
              ...
              try {
                ...
                if (float_vals.size() != afield.num_dim) {
                  ...
                  if (afield.vec_dist == cosine) {
                    ...
                    vec_index->addPoint(normalized_vals.data(),
                                        (size_t)record.seq_id, true);
                  } else {
                    vec_index->addPoint(float_vals.data(),
                                        (size_t)record.seq_id, true);
                  }
                }
              } catch (const std::exception& e) {
      ...
      iterate_and_index_numerical_field(
          iter_batch, afield,
          [&afield, num_tree](const index_record& record, uint32_t seq_id) {
            for (size_t arr_i = 0; arr_i < record.doc[afield.name].size();
                 arr_i++) {
      ...
      for (const auto& record : iter_batch) {
        ...
        if (is_integer) {
          ...
          doc_to_score->emplace(seq_id,
                                (int64_t)document[afield.name].get<bool>());
        } else if (is_geopoint) {
...
void Index::tokenize_string_with_facets(
    const std::string& text, bool is_facet, const field& a_field,
    const std::vector<char>& symbols_to_index,
    const std::vector<char>& token_separators,
...
void Index::tokenize_string_array_with_facets(
    const std::vector<std::string>& strings, bool is_facet,
    const field& a_field, const std::vector<char>& symbols_to_index,
    const std::vector<char>& token_separators,
    std::unordered_map<std::string, std::vector<uint32_t>>& token_to_offsets,
    std::vector<uint64_t>& facet_hashes) {
...
void Index::compute_facet_stats(facet& a_facet, uint64_t raw_value,
...
void Index::do_facets(std::vector<facet>& facets, facet_query_t& facet_query,
  ...
  for (size_t findex = 0; findex < facets.size(); findex++) {
    ...
    const auto& field_single_val_facet_mapping_it =
        single_val_facet_index_v3.find(a_facet.field_name);

    if ((field_facet_mapping_it == facet_index_v3.end()) &&
        (field_single_val_facet_mapping_it ==
         single_val_facet_index_v3.end())) {
      continue;
    }

    for (size_t i = 0; i < results_size; i++) {
      // if sampling is enabled, we will skip a portion of the results to speed
      // up things
      if (estimate_facets) {
        if (i % mod_value != 0) {
          continue;
      ...
      if (facet_field.is_array()) {
        ...
        facet_map_it =
            field_facet_mapping[doc_seq_id % ARRAY_FACET_DIM]->find(doc_seq_id);
        ...
        const auto& field_facet_mapping =
            field_single_val_facet_mapping_it->second;
        single_facet_map_it =
            field_facet_mapping[doc_seq_id % ARRAY_FACET_DIM]->find(doc_seq_id);

        if (single_facet_map_it ==
            field_facet_mapping[doc_seq_id % ARRAY_FACET_DIM]->end()) {
          continue;
        }
        facet_hash_count = 1;
      ...
      for (size_t j = 0; j < facet_hash_count; j++) {
        ...
        if (a_facet.is_range_query) {
          ...
          // LOG(INFO) << "field: " << a_facet.field_name << ", doc id: " <<
          // doc_seq_id << ", hash: " <<  fhash;
...
void Index::search_all_candidates(
    const size_t num_search_fields, const text_match_type_t match_type,
    const std::vector<search_field_t>& the_fields, const uint32_t* filter_ids,
    size_t filter_ids_length, const uint32_t* exclude_token_ids,
    ...
    int syn_orig_num_tokens, const int* sort_order,
  ...
  for (long long n = 0; n < N && n < combination_limit; ++n) {
    ...
    LOG(INFO) << "query: " << fullq.str() << ", total_cost: " << total_cost
              << ", all_result_ids_len: " << all_result_ids_len << ", bufsiz: "
    << id_buff.size();*/
    ...
    search_across_fields(
        query_suggestion, num_typos, prefixes, the_fields, num_search_fields,
        match_type, sort_fields, topster, groups_processed, searched_queries,
        qtoken_set, dropped_tokens, group_limit, group_by_fields,
        prioritize_exact_match, prioritize_token_position, filter_ids,
        filter_ids_length, total_cost, syn_orig_num_tokens, exclude_token_ids,
        exclude_token_ids_size, excluded_group_ids, sort_order, field_values,
...
void Index::search_candidates(
    const uint8_t& field_id, bool field_is_array, const uint32_t* filter_ids,
    size_t filter_ids_length, const uint32_t* exclude_token_ids,
    size_t exclude_token_ids_size, const std::vector<uint32_t>& curated_ids,
    std::vector<sort_by>& sort_fields,
    std::vector<token_candidates>& token_candidates_vec,
    std::vector<std::vector<art_leaf*>>& searched_queries, Topster* topster,
    spp::sparse_hash_map<uint64_t, uint32_t>& groups_processed,
    uint32_t** all_result_ids, size_t& all_result_ids_len,
    size_t& field_num_results, const size_t typo_tokens_threshold,
    const size_t group_limit, const std::vector<std::string>& group_by_fields,
    const std::vector<token_t>& query_tokens, bool prioritize_exact_match,
    const bool exhaustive_search, int syn_orig_num_tokens,
    const size_t concurrency, std::set<uint64>& query_hashes,
    std::vector<uint32_t>& id_buff) const {
  ...
  for (long long n = 0; n < N && n < combination_limit; ++n) {
    ...
    // every element in `query_suggestion` contains a token and its associated
    // hits
    ...
    if (topster == nullptr) {
      ...
      posting_t::block_intersector_t(posting_lists, iter_state)
          .intersect([&](uint32_t seq_id,
                         std::vector<posting_list_t::iterator_t>& its) {
            score_results(sort_fields, searched_queries.size(), field_id,
                          field_is_array, total_cost, topster, query_suggestion,
                          groups_processed, seq_id, sort_order, field_values,
                          geopoint_indices, group_limit, group_by_fields,
                          token_bits, prioritize_exact_match,
                          single_exact_query_token, syn_orig_num_tokens, its);
...
void Index::numeric_not_equals_filter(num_tree_t* const num_tree,
                                      ...
                                      size_t& ids_len, uint32_t*& ids) const {
  uint32_t* to_exclude_ids = nullptr;
  size_t to_exclude_ids_len = 0;

  if (context_ids_length != 0) {
    num_tree->contains(EQUALS, value, context_ids_length, context_ids,
                       to_exclude_ids_len, to_exclude_ids);
  } else {
  ...
  to_include_ids_len =
      ArrayUtils::exclude_scalar(all_ids, all_ids_size, to_exclude_ids,
  ...
  ids_len = ArrayUtils::or_scalar(ids, ids_len, to_include_ids,
                                  to_include_ids_len, &out);
...
Option<bool> Index::do_filtering(filter_node_t* const root,
  ...
  if (is_referenced_filter) {
    // Apply filter on referenced collection and get the sequence ids of current
    // collection from the filtered documents.
    auto& cm = CollectionManager::get_instance();
    auto collection = cm.get_collection(a_filter.referenced_collection_name);
    if (collection == nullptr) {
      return Option<bool>(400, "Referenced collection `" +
                                   a_filter.referenced_collection_name +
                                   "` not found.");
    }

    filter_result_t reference_filter_result;
    auto reference_filter_op = collection->get_reference_filter_ids(
        a_filter.field_name, reference_filter_result, collection_name);
    if (!reference_filter_op.ok()) {
      return Option<bool>(400,
                          "Failed to apply reference filter on `" +
                              a_filter.referenced_collection_name +
    ...
    if (context_ids_length != 0) {
      ...
      include_indexes.reserve(
          std::min(context_ids_length, reference_filter_result.count));

      size_t context_index = 0, reference_result_index = 0;
      while (context_index < context_ids_length &&
             reference_result_index < reference_filter_result.count) {
        if (context_ids[context_index] ==
            reference_filter_result.docs[reference_result_index]) {
          include_indexes.push_back(reference_result_index);
          context_index++;
          reference_result_index++;
        } else if (context_ids[context_index] <
                   reference_filter_result.docs[reference_result_index]) {
          context_index++;
        } else {
          reference_result_index++;
      ...
      auto& result_references =
          result.reference_filter_results[a_filter.referenced_collection_name];
      ...
      for (uint32_t i = 0; i < include_indexes.size(); i++) {
        ...
        result_references[i] =
            reference_filter_result
                .reference_filter_results[a_filter.referenced_collection_name]
                                         [include_indexes[i]];
      }
  ...
  if (a_filter.field_name == "id") {
    ...
    if (context_ids_length != 0) {
      uint32_t* out = nullptr;
      result.count =
          ArrayUtils::and_scalar(context_ids, context_ids_length, result_array,
                                 result_ids.size(), &out);
  ...
  if (f.is_integer()) {
    ...
    for (size_t fi = 0; fi < a_filter.values.size(); fi++) {
      ...
      if (a_filter.comparators[fi] == RANGE_INCLUSIVE &&
        ...
        if (context_ids_length != 0) {
          num_tree->range_inclusive_contains(value, range_end_value,
                                             context_ids_length, context_ids,
                                             result_ids_len, result_ids);
        } else {
          num_tree->range_inclusive_search(value, range_end_value, &result_ids,
                                           result_ids_len);
        }

        fi++;
      } else if (a_filter.comparators[fi] == NOT_EQUALS) {
        numeric_not_equals_filter(num_tree, value, context_ids_length,
                                  context_ids, result_ids_len, result_ids);
      } else {
        if (context_ids_length != 0) {
          num_tree->contains(a_filter.comparators[fi], value,
                             context_ids_length, context_ids, result_ids_len,
                             result_ids);
        } else {
    ...
    for (size_t fi = 0; fi < a_filter.values.size(); fi++) {
      ...
      if (a_filter.comparators[fi] == RANGE_INCLUSIVE &&
        ...
        const std::string& next_filter_value = a_filter.values[fi + 1];
        int64_t range_end_value =
            float_to_int64_t((float)std::atof(next_filter_value.c_str()));

        if (context_ids_length != 0) {
          num_tree->range_inclusive_contains(float_int64, range_end_value,
                                             context_ids_length, context_ids,
                                             result_ids_len, result_ids);
        } else {
        ...
        numeric_not_equals_filter(num_tree, float_int64, context_ids_length,
                                  context_ids, result_ids_len, result_ids);
      } else {
        if (context_ids_length != 0) {
          ...
          num_tree->search(a_filter.comparators[fi], float_int64, &result_ids,
                           result_ids_len);
        }
      }
    }
    ...
    for (const std::string& filter_value : a_filter.values) {
      ...
      if (a_filter.comparators[value_index] == NOT_EQUALS) {
        numeric_not_equals_filter(num_tree, bool_int64, context_ids_length,
                                  context_ids, result_ids_len, result_ids);
      } else {
        if (context_ids_length != 0) {
    ...
    for (const std::string& filter_value : a_filter.values) {
      ...
      for (const auto& term : indexer.GetQueryTerms(*query_region, "")) {
        ...
        if (ids_it != geo_index->end()) {
          geo_result_ids.insert(geo_result_ids.end(), ids_it->second.begin(),
                                ids_it->second.end());
        }
      }
      ...
      // `geo_result_ids` will contain all IDs that are within approximately
      // within query radius we still need to do another round of exact
      // filtering on them
      ...
      if (f.is_single_geopoint()) {
        ...
        for (auto result_id : geo_result_ids) {
          // no need to check for existence of `result_id` because of indexer
          // based pre-filtering above
      ...
      uint32_t* out = nullptr;
      result_ids_len = ArrayUtils::or_scalar(&exact_geo_result_ids[0],
    ...
    for (const std::string& filter_value : a_filter.values) {
      ...
      // there could be multiple tokens in a filter value, which we have to
      // treat as ANDs e.g. country: South Africa
      Tokenizer tokenizer(filter_value, true, false, f.locale, symbols_to_index,
                          token_separators);
      ...
      while (tokenizer.next(str_token, token_index)) {
        ...
        art_leaf* leaf = (art_leaf*)art_search(
            t, (const unsigned char*)str_token.c_str(), str_token.length() + 1);
        if (leaf == nullptr) {
          continue;
        }
      ...
      if (a_filter.comparators[0] == EQUALS ||
        ...
        posting_t::intersect(posting_lists, result_id_vec, context_ids_length,
                             context_ids);
        ...
        posting_t::intersect(posting_lists, f_id_buff, context_ids_length,
                             context_ids);
    ...
    if (!f_id_buff.empty()) {
      ...
      uint32_t* out = nullptr;
      or_ids_size = ArrayUtils::or_scalar(or_ids, or_ids_size, f_id_buff.data(),
                                          f_id_buff.size(), &out);
      delete[] or_ids;
      or_ids = out;
      std::vector<uint32_t>().swap(f_id_buff);  // clears out memory
  ...
  if (a_filter.apply_not_equals) {
    ...
    to_include_ids_len = ArrayUtils::exclude_scalar(
        all_ids, all_ids_size, result_ids, result_ids_len, &to_include_ids);
...
void Index::aproximate_numerical_match(num_tree_t* const num_tree,
  ...
  if (comparator == RANGE_INCLUSIVE) {
    num_tree->approx_range_inclusive_search_count(value, range_end_value,
                                                  filter_ids_length);
    return;
  }
...
Option<bool> Index::_approximate_filter_ids(
    const filter& a_filter, uint32_t& filter_ids_length,
  ...
  if (!a_filter.referenced_collection_name.empty()) {
    ...
    return collection->get_approximate_reference_filter_ids(a_filter.field_name,
                                                            filter_ids_length);
  }
  ...
  if (f.is_integer()) {
    ...
    for (size_t fi = 0; fi < a_filter.values.size(); fi++) {
      ...
      if (a_filter.comparators[fi] == RANGE_INCLUSIVE &&
        ...
        aproximate_numerical_match(num_tree, a_filter.comparators[fi], value,
                                   range_end_value, filter_ids_length);
        fi++;
      } else {
        aproximate_numerical_match(num_tree, a_filter.comparators[fi], value, 0,
                                   filter_ids_length);
      }
    }
  } else if (f.is_float()) {
    ...
    for (size_t fi = 0; fi < a_filter.values.size(); fi++) {
      ...
      if (a_filter.comparators[fi] == RANGE_INCLUSIVE &&
        ...
        const std::string& next_filter_value = a_filter.values[fi + 1];
        auto const range_end_value =
            float_to_int64_t((float)std::atof(next_filter_value.c_str()));

        aproximate_numerical_match(num_tree, a_filter.comparators[fi],
                                   float_int64, range_end_value,
                                   filter_ids_length);
        fi++;
      } else {
        aproximate_numerical_match(num_tree, a_filter.comparators[fi],
                                   float_int64, 0, filter_ids_length);
      }
    }
  } else if (f.is_bool()) {
    auto num_tree = numerical_index.at(a_filter.field_name);
    ...
    for (const std::string& filter_value : a_filter.values) {
      ...
      aproximate_numerical_match(num_tree, a_filter.comparators[value_index],
                                 bool_int64, 0, filter_ids_length);
      value_index++;
    }
  } else if (f.is_geopoint()) {
    ...
    for (const std::string& filter_value : a_filter.values) {
      ...
      while (tokenizer.next(str_token, token_index)) {
        auto const leaf = (art_leaf*)art_search(
            t, (const unsigned char*)str_token.c_str(), str_token.length() + 1);
        if (leaf == nullptr) {
          continue;
        }
...
Option<bool> Index::rearrange_filter_tree(
    filter_node_t* const root, uint32_t& approx_filter_ids_length,
    const std::string& collection_name) const {
  if (root == nullptr) {
    return Option(true);
  }
  ...
  if (root->isOperator) {
    ...
    if (root->left != nullptr) {
      auto rearrange_op = rearrange_filter_tree(root->left, l_filter_ids_length,
                                                collection_name);
      if (!rearrange_op.ok()) {
        return rearrange_op;
      }
    ...
    if (root->right != nullptr) {
      auto rearrange_op = rearrange_filter_tree(
          root->right, r_filter_ids_length, collection_name);
      if (!rearrange_op.ok()) {
        return rearrange_op;
      }
    }
    ...
    if (root->filter_operator == AND) {
      approx_filter_ids_length =
          std::min(l_filter_ids_length, r_filter_ids_length);
    } else {
  ...
  _approximate_filter_ids(root->filter_exp, approx_filter_ids_length,
                          collection_name);
  return Option(true);
}
...
Option<bool> Index::recursive_filter(filter_node_t* const root,
  ...
  return do_filtering(root, result, collection_name, context_ids_length,
                      context_ids);
}

Option<bool> Index::do_filtering_with_lock(
    filter_node_t* const filter_tree_root, filter_result_t& filter_result,
    const std::string& collection_name) const {
  std::shared_lock lock(mutex);

  auto filter_op =
      recursive_filter(filter_tree_root, filter_result, collection_name);
  if (!filter_op.ok()) {
    return filter_op;
  }
...
Option<bool> Index::do_reference_filtering_with_lock(
    filter_node_t* const filter_tree_root, filter_result_t& filter_result,
    const std::string& collection_name,
  ...
  for (uint32_t i = 0; i < reference_filter_result.count; i++) {
    ...
    auto doc_id =
        sort_index.at(reference_helper_field_name)->at(reference_doc_id);
  ...
  filter_result.reference_filter_results[collection_name] =
      new reference_filter_result_t[reference_map.size()];
  ...
  for (auto& item : reference_map) {
    ...
    auto& reference_result =
        filter_result.reference_filter_results[collection_name][doc_index];
...
Option<bool> Index::get_approximate_reference_filter_ids_with_lock(
    filter_node_t* const filter_tree_root, uint32_t& filter_ids_length) const {
  std::shared_lock lock(mutex);
...
Option<bool> Index::run_search(search_args* search_params,
                               const std::string& collection_name) {
  return search(
      search_params->field_query_tokens, search_params->search_fields,
      search_params->match_type, search_params->filter_tree_root,
      ...
      search_params->per_page, search_params->offset,
      search_params->token_order, search_params->prefixes,
      search_params->drop_tokens_threshold, search_params->all_result_ids_len,
      search_params->groups_processed, search_params->searched_queries,
      search_params->qtoken_set, search_params->raw_result_kvs,
      search_params->override_result_kvs, search_params->typo_tokens_threshold,
      ...
      search_params->max_extra_suffix, search_params->facet_query_num_typos,
      search_params->filter_curated_hits, search_params->split_join_tokens,
      search_params->vector_query, search_params->facet_sample_percent,
      search_params->facet_sample_threshold, collection_name);
}
...
bool Index::static_filter_query_eval(const override_t* override,
  ...
  if ((override->rule.match == override_t::MATCH_EXACT &&
    ...
    Option<bool> filter_op = filter::parse_filter_query(
        override->filter_by, search_schema, store, "", new_filter_tree_root);
    if (filter_op.ok()) {
      if (filter_tree_root == nullptr) {
        filter_tree_root = new_filter_tree_root;
      } else {
        filter_node_t* root =
            new filter_node_t(AND, filter_tree_root, new_filter_tree_root);
...
bool Index::resolve_override(const std::vector<std::string>& rule_tokens,
  ...
  while (i < rule_tokens.size()) {
    if (rule_tokens[i].front() == '{' && rule_tokens[i].back() == '}') {
      ...
      while (i < rule_tokens.size() &&
             (rule_tokens[i].front() == '{' && rule_tokens[i].back() == '}')) {
      ...
      // `i` now points to either end of array or at a non-placeholder rule
      // token end of array: add remaining query tokens as matched tokens
      // non-placeholder: skip query tokens until it matches a rule token

      while (j < query_tokens.size() &&
             (i == rule_tokens.size() || rule_tokens[i] != query_tokens[j])) {
      ...
      // rule token is not a placeholder, so we have to skip the query tokens
      // until it matches rule token
...
void Index::process_filter_overrides(
    ...
    std::vector<std::string>& query_tokens, token_ordering token_order,
  ...
  for (auto& override : filter_overrides) {
    if (!override->rule.dynamic_query) {
      ...
      // Check the original query and then the synonym variants until a rule
      // matches.
      bool resolved_override =
          static_filter_query_eval(override, query_tokens, filter_tree_root);
      ...
      // need to extract placeholder field names from the search query, filter
      // on them and rewrite query we will cover both original query and
      // synonyms
      ...
      std::set<std::string> absorbed_tokens;
      bool resolved_override =
          resolve_override(rule_parts, exact_rule_match, query_tokens,
                           token_order, absorbed_tokens, filter_by_clause);

      if (resolved_override) {
        filter_node_t* new_filter_tree_root = nullptr;
        Option<bool> filter_op = filter::parse_filter_query(
            filter_by_clause, search_schema, store, "", new_filter_tree_root);
        if (filter_op.ok()) {
...
bool Index::check_for_overrides(
    const token_ordering& token_order, const string& field_name,
    const bool slide_window, bool exact_rule_match,
    std::vector<std::string>& tokens, std::set<std::string>& absorbed_tokens,
    std::vector<std::string>& field_absorbed_tokens) const {
  for (size_t window_len = tokens.size(); window_len > 0; window_len--) {
    for (size_t start_index = 0; start_index + window_len - 1 < tokens.size();
         start_index++) {
      ...
      for (size_t i = start_index; i < start_index + window_len; i++) {
        ...
        window_tokens.emplace_back(i, tokens[i], is_prefix, tokens[i].size(),
                                   0);
      ...
      std::vector<sort_by> sort_fields;
      search_field(0, window_tokens, nullptr, 0, num_toks_dropped,
                   field_it.value(), field_name, nullptr, 0, {}, sort_fields,
                   -1, 0, searched_queries, topster, groups_processed,
                   &result_ids, result_ids_len, field_num_results, 0,
                   group_by_fields, false, 4, query_hashes, token_order, false,
                   0, 0, false, -1, 3, 7, 4);
      ...
      if (result_ids_len != 0) {
        ...
        posting_t::get_exact_matches(posting_lists, field_it.value().is_array(),
                                     result_ids, result_ids_len, exact_strt_ids,
                                     exact_strt_size);
...
void Index::search_infix(const std::string& query,
  ...
  for (auto infix_set : infix_sets) {
    thread_pool->enqueue([infix_set, &leaves, search_tree, &query,
                          max_extra_prefix, max_extra_suffix, &num_processed,
                          &m_process, &cv_process, &parent_search_begin,
                          &parent_search_stop_ms, &parent_search_cutoff]() {
      ...
      for (auto it = infix_set->begin(); it != infix_set->end(); it++) {
        ...
        // check for search cutoff but only once every 2^10 docs to reduce
        // overhead
        if (((num_iterated + 1) % (1 << 12)) == 0) {
          if ((std::chrono::duration_cast<std::chrono::microseconds>(
                   std::chrono::system_clock::now().time_since_epoch())
                   .count() -
               search_begin_us) > op_search_stop_ms) {
            search_cutoff = true;
            break;
          }
        }
...
Option<bool> Index::search(
    std::vector<query_tokens_t>& field_query_tokens,
    const std::vector<search_field_t>& the_fields,
    const text_match_type_t match_type, filter_node_t* filter_tree_root,
    std::vector<facet>& facets, facet_query_t& facet_query,
    const std::vector<std::pair<uint32_t, uint32_t>>& included_ids,
    const std::vector<uint32_t>& excluded_ids,
    std::vector<sort_by>& sort_fields_std,
    const std::vector<uint32_t>& num_typos, Topster* topster,
    Topster* curated_topster, const size_t per_page, const size_t offset,
    const token_ordering token_order, const std::vector<bool>& prefixes,
    ...
    size_t concurrency, size_t search_cutoff_ms, size_t min_len_1typo,
    size_t min_len_2typo, size_t max_candidates,
    ...
    const vector_query_t& vector_query, size_t facet_sample_percent,
    size_t facet_sample_threshold, const std::string& collection_name) const {
  std::shared_lock lock(mutex);

  uint32_t filter_ids_length = 0;
  auto rearrange_op = rearrange_filter_tree(filter_tree_root, filter_ids_length,
                                            collection_name);
  if (!rearrange_op.ok()) {
    return rearrange_op;
  }
  ...
  process_curated_ids(included_ids, excluded_ids, group_by_fields, group_limit,
                      filter_curated_hits, filter_result.docs,
                      filter_result.count, curated_ids, included_ids_map,
  ...
  if (!field_query_tokens[0].q_phrases.empty()) {
    do_phrase_search(
        num_search_fields, the_fields, field_query_tokens, sort_fields_std,
        searched_queries, group_limit, group_by_fields, topster, sort_order,
        field_values, geopoint_indices, curated_ids_sorted, all_result_ids,
        all_result_ids_len, groups_processed, curated_ids, excluded_result_ids,
        excluded_result_ids_size, excluded_group_ids, curated_topster,
        included_ids_map, is_wildcard_query, filter_result.docs,
        filter_result.count);
  ...
  // for phrase query, parser will set field_query_tokens to "*", need to handle
  // that
  if (is_wildcard_query && field_query_tokens[0].q_phrases.empty()) {
    ...
    bool no_filters_provided =
        (filter_tree_root == nullptr && filter_result.count == 0);
    ...
    if (no_filters_provided && facets.empty() && curated_ids.empty() &&
      ...
      while (it.valid()) {
        ...
        KV kv(searched_queries.size(), seq_id, distinct_id, match_score_index,
              scores, nullptr);
    ...
    curate_filtered_ids(curated_ids, excluded_result_ids,
                        excluded_result_ids_size, filter_result.docs,
                        filter_result.count, curated_ids_sorted);
    collate_included_ids({}, included_ids_map, curated_topster,
                         searched_queries);

    if (!vector_query.field_name.empty()) {
      auto k = vector_query.k == 0
                   ? std::max<size_t>(vector_query.k, fetch_size)
                   : vector_query.k;
      ...
      VectorFilterFunctor filterFunctor(filter_result.docs,
                                        filter_result.count);
      ...
      if (!no_filters_provided &&
          filter_result.count < vector_query.flat_search_cutoff) {
      ...
      for (const auto& dist_label : dist_labels) {
        ...
        compute_sort_scores(sort_fields_std, sort_order, field_values,
                            geopoint_indices, seq_id, 0, 0, scores,
      ...
      if (!nearest_ids.empty()) {
        std::sort(nearest_ids.begin(),
                  nearest_ids.end());  // seq_ids should be in ascending order
      ...
      search_wildcard(
          ...
          curated_topster, groups_processed, searched_queries, group_limit,
          group_by_fields, curated_ids, curated_ids_sorted, excluded_result_ids,
          excluded_result_ids_size, excluded_group_ids, all_result_ids,
          all_result_ids_len, filter_result.docs, filter_result.count,
          concurrency, sort_order, field_values, geopoint_indices);
    }
  } else {
    // Non-wildcard
    // In multi-field searches, a record can be matched across different fields,
    // so we use this for aggregation
    ...
    for (const auto& q_syn_vec : field_query_tokens[0].q_synonyms) {
      ...
      for (size_t j = 0; j < q_syn_vec.size(); j++) {
        ...
        q_pos_syn.emplace_back(j, q_syn_vec[j], is_prefix, q_syn_vec[j].size(),
                               0);
      }
    ...
    fuzzy_search_fields(
        the_fields, field_query_tokens[0].q_include_tokens, {}, match_type,
        excluded_result_ids, excluded_result_ids_size, filter_result.docs,
        filter_result.count, curated_ids_sorted, excluded_group_ids,
        sort_fields_std, num_typos, searched_queries, qtoken_set, topster,
        groups_processed, all_result_ids, all_result_ids_len, group_limit,
        group_by_fields, prioritize_exact_match, prioritize_token_position,
        query_hashes, token_order, prefixes, typo_tokens_threshold,
        exhaustive_search, max_candidates, min_len_1typo, min_len_2typo,
    ...
    if (split_join_tokens == always ||
      ...
      for (size_t i = 0; i < num_search_fields; i++) {
        ...
        resolve_space_as_typos(orig_q_include_tokens, the_fields[i].name,
      ...
      if (!space_resolved_queries.empty()) {
        ...
        for (size_t j = 0; j < resolved_query.size(); j++) {
          ...
          resolved_tokens.emplace_back(j, space_resolved_queries[0][j],
                                       is_prefix,
                                       space_resolved_queries[0][j].size(), 0);
        }

        fuzzy_search_fields(
            the_fields, resolved_tokens, {}, match_type, excluded_result_ids,
            excluded_result_ids_size, filter_result.docs, filter_result.count,
            curated_ids_sorted, excluded_group_ids, sort_fields_std, num_typos,
            ...
            max_candidates, min_len_1typo, min_len_2typo, syn_orig_num_tokens,
    ...
    do_synonym_search(
        the_fields, match_type, filter_tree_root, included_ids_map,
        sort_fields_std, curated_topster, token_order, 0, group_limit,
        group_by_fields, prioritize_exact_match, prioritize_token_position,
        exhaustive_search, concurrency, prefixes, min_len_1typo, min_len_2typo,
        max_candidates, curated_ids, curated_ids_sorted, excluded_result_ids,
        excluded_result_ids_size, excluded_group_ids, topster, q_pos_synonyms,
        syn_orig_num_tokens, groups_processed, searched_queries, all_result_ids,
        all_result_ids_len, filter_result.docs, filter_result.count,
        query_hashes, sort_order, field_values, geopoint_indices, qtoken_set);
    ...
    if (exhaustive_search || all_result_ids_len < drop_tokens_threshold) {
      for (size_t qi = 0; qi < all_queries.size(); qi++) {
        ...
        while (exhaustive_search ||
          ...
          // When atleast two tokens from the query are available we can drop
          // one
          ...
          if (orig_tokens.size() > 1 &&
            ...
            if (num_tokens_dropped < orig_tokens.size() - 1) {
              ...
              prefix_search = true;
              size_t start_index =
                  (num_tokens_dropped + 1) - orig_tokens.size() + 1;
              for (size_t i = 0; i < orig_tokens.size(); i++) {
                if (i >= start_index) {
                  truncated_tokens.emplace_back(orig_tokens[i]);
                } else {
            ...
            fuzzy_search_fields(
                the_fields, truncated_tokens, dropped_tokens, match_type,
                excluded_result_ids, excluded_result_ids_size,
                filter_result.docs, filter_result.count, curated_ids_sorted,
                excluded_group_ids, sort_fields_std, num_typos,
                searched_queries, qtoken_set, topster, groups_processed,
                all_result_ids, all_result_ids_len, group_limit,
                group_by_fields, prioritize_exact_match,
                prioritize_token_position, query_hashes, token_order, prefixes,
                typo_tokens_threshold, exhaustive_search, max_candidates,
                min_len_1typo, min_len_2typo, -1, sort_order, field_values,
                geopoint_indices);
    ...
    do_infix_search(
        num_search_fields, the_fields, infixes, sort_fields_std,
        searched_queries, group_limit, group_by_fields, max_extra_prefix,
        max_extra_suffix, field_query_tokens[0].q_include_tokens, topster,
        filter_result.docs, filter_result.count, sort_order, field_values,
        geopoint_indices, curated_ids_sorted, excluded_group_ids,
    ...
    if (!vector_query.field_name.empty()) {
      ...
      if (has_text_match) {
        // For hybrid search, we need to give weight to text match and vector
        // search
        ...
        std::sort(
            vec_results.begin(), vec_results.end(),
            [](const auto& a, const auto& b) { return a.second < b.second; });

        topster->sort();
        // Reciprocal rank fusion
        // Score is  sum of (1 / rank_of_document) * WEIGHT from each list (text
        // match and vector search)
        for (uint32_t i = 0; i < topster->size; i++) {
          ...
          result->scores[result->match_score_index] =
              float_to_int64_t((1.0 / (i + 1)) * TEXT_MATCH_WEIGHT);
        }

        std::vector<uint32_t>
            vec_search_ids;  // list of IDs found only in vector search
        ...
        for (size_t res_index = 0; res_index < vec_results.size();
          ...
          if (result_it != topster->kv_map.end()) {
            if (result_it->second->match_score_index < 0 ||
                result_it->second->match_score_index > 2) {
              continue;
            }

            // result overlaps with keyword search: we have to combine the
            // scores
            ...
            compute_sort_scores(sort_fields_std, sort_order, field_values,
                                geopoint_indices, seq_id, 0, match_score,
                                scores, match_score_index, vec_result.second);
            ...
            // Result has been found only in vector search: we have to add it to
            // both KV and result_ids (1 / rank_of_document) * WEIGHT)
            int64_t scores[3] = {0};
            int64_t match_score = float_to_int64_t((1.0 / (res_index + 1)) *
            ...
            KV kv(searched_queries.size(), seq_id, distinct_id,
        ...
        if (!vec_search_ids.empty()) {
          uint32_t* new_all_result_ids = nullptr;
          all_result_ids_len = ArrayUtils::or_scalar(
              all_result_ids, all_result_ids_len, &vec_search_ids[0],
...
process_search_results:
  ...
  bool estimate_facets = (facet_sample_percent < 100 &&
  ...
  if (!facets.empty()) {
    ...
    for (size_t thread_id = 0;
      ...
      thread_pool->enqueue([this, thread_id, &facet_batches, &facet_query,
                            ...
                            batch_res_len, &facet_infos, estimate_facets,
                            facet_sample_percent, &parent_search_begin,
                            &parent_search_stop_ms, &parent_search_cutoff,
                            &num_processed, &m_process, &cv_process]() {
        search_begin_us = parent_search_begin;
        ...
        auto fq = facet_query;
        do_facets(facet_batches[thread_id], fq, estimate_facets,
                  facet_sample_percent, facet_infos, group_limit,
                  group_by_fields, batch_result_ids, batch_res_len);
        std::unique_lock<std::mutex> lock(m_process);
        num_processed++;
        parent_search_cutoff = parent_search_cutoff || search_cutoff;
        cv_process.notify_one();
    ...
    for (auto& acc_facet : facets) {
      for (auto& facet_kv : acc_facet.result_map) {
        ...
        if (estimate_facets) {
          facet_kv.second.count = size_t(double(facet_kv.second.count) *
                                         (100.0f / facet_sample_percent));
        }
      }
  ...
  // std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now()
  // - begin).count(); LOG(INFO) << "Time taken for result calc: " << timeMillis
  // << "ms";
...
void Index::process_curated_ids(
  ...
  if (group_limit != 0) {
    // if one `id` of a group is present in curated hits, we have to exclude
    // that entire group from results
  ...
  // if `filter_curated_hits` is enabled, we will remove curated hits that don't
...
void Index::fuzzy_search_fields(
    const std::vector<search_field_t>& the_fields,
    const std::vector<token_t>& query_tokens,
    const std::vector<token_t>& dropped_tokens,
    const text_match_type_t match_type, const uint32_t* exclude_token_ids,
    size_t exclude_token_ids_size, const uint32_t* filter_ids,
    size_t filter_ids_length, const std::vector<uint32_t>& curated_ids,
    const std::unordered_set<uint32_t>& excluded_group_ids,
    const std::vector<sort_by>& sort_fields,
    const std::vector<uint32_t>& num_typos,
    std::vector<std::vector<art_leaf*>>& searched_queries,
    tsl::htrie_map<char, token_leaf>& qtoken_set, Topster* topster,
    spp::sparse_hash_map<uint64_t, uint32_t>& groups_processed,
    uint32_t*& all_result_ids, size_t& all_result_ids_len,
    const size_t group_limit, const std::vector<std::string>& group_by_fields,
    bool prioritize_exact_match, const bool prioritize_token_position,
    std::set<uint64>& query_hashes, const token_ordering token_order,
    const std::vector<bool>& prefixes, const size_t typo_tokens_threshold,
    const bool exhaustive_search, const size_t max_candidates,
    size_t min_len_1typo, size_t min_len_2typo, int syn_orig_num_tokens,
    const int* sort_order,
    std::array<spp::sparse_hash_map<uint32_t, int64_t>*, 3>& field_values,
    const std::vector<size_t>& geopoint_indices) const {
  // NOTE: `query_tokens` preserve original tokens, while `search_tokens` could
  // be a result of dropped tokens
  ...
  for (size_t stoken_index = 0; stoken_index < query_tokens.size();
    ...
    // etc.
    int bounded_cost =
        get_bounded_typo_cost(2, token.length(), min_len_1typo, min_len_2typo);
  ...
  while (n < N && n < combination_limit) {
    ...
    while (token_index < query_tokens.size()) {
      ...
      if (token_cost_cache.count(token_cost_hash) != 0) {
        ...
        // Prefix query with a preceding token should be handled in such a way
        // that we give preference to possible phrase continuation. Example:
        // "steve j" for "steve jobs" name field query. To do this, we will
        // first attempt to match the prefix with the most "popular" fields of
        // the preceding token. Tokens matched from popular fields will also be
        // searched across other query fields. Only when we find *no results*
        // for such an expansion, we will attempt cross field matching.
        ...
        std::vector<size_t> popular_field_ids;  // fields containing the token
                                                // most across documents

        if (last_token) {
          popular_fields_of_token(
              search_index, token_candidates_vec.back().candidates[0],
        ...
        const std::vector<size_t>& field_ids =
            last_token ? popular_field_ids : query_field_ids;

        for (size_t field_id : field_ids) {
          // NOTE: when accessing other field ordered properties like prefixes
          // or num_typos we have to index them by `the_field.orig_index` since
          // the original fields could be reordered on their weights.
          auto& the_field = the_fields[field_id];
          const bool field_prefix = (the_field.orig_index < prefixes.size())
                                        ? prefixes[the_field.orig_index]
                                        : prefixes[0];
          ;
          const bool prefix_search =
              field_prefix && query_tokens[token_index].is_prefix_searched;
          const size_t token_len =
              prefix_search ? (int)token.length() : (int)token.length() + 1;

          /*LOG(INFO) << "Searching for field: " << the_field.name << ", token:"
                    << token << " - cost: " << costs[token_index] << ",
             prefix_search: " << prefix_search;*/

          int64_t field_num_typos = (the_field.orig_index < num_typos.size())
          ...
          // LOG(INFO) << "Searching for field: " << the_field.name << ", found
          // token:" << token;
          const auto& prev_token =
              last_token ? token_candidates_vec.back().candidates[0] : "";

          std::vector<art_leaf*> field_leaves;
          art_fuzzy_search(search_index.at(the_field.name),
                           (const unsigned char*)token.c_str(), token_len,
                           costs[token_index], costs[token_index],
                           max_candidates, token_order, prefix_search,
                           last_token, prev_token, filter_ids,
        ...
        if (last_token && leaf_tokens.size() < max_candidates) {
          // field-wise matching with previous token has failed, have to look at
          // cross fields matching docs
          ...
          find_across_fields(token_candidates_vec.back().token,
                             ...
                             the_fields, num_search_fields, filter_ids,
                             filter_ids_length, exclude_token_ids,
                             exclude_token_ids_size, prev_token_doc_ids,
                             popular_field_ids);
          ...
          for (size_t field_id : query_field_ids) {
            ...
            art_fuzzy_search(search_index.at(the_field.name),
                             (const unsigned char*)token.c_str(), token_len,
                             costs[token_index], costs[token_index],
                             max_candidates, token_order, prefix_search, false,
                             "", filter_ids, filter_ids_length, field_leaves,
                             unique_tokens);
    ...
    token_done:
      ...
      if (!leaf_tokens.empty()) {
        ...
        token_candidates_vec.push_back(tok_candidates{
            query_tokens[token_index], costs[token_index],
            query_tokens[token_index].is_prefix_searched, leaf_tokens});
      } else {
        // No result at `cost = costs[token_index]`. Remove `cost` for token and
        // re-do combinations
        auto it =
            std::find(token_to_costs[token_index].begin(),
                      token_to_costs[token_index].end(), costs[token_index]);
        if (it != token_to_costs[token_index].end()) {
          ...
          if (token_to_costs[token_index].empty()) {
            // we cannot proceed further, as this token is not found within cost
            // limits and, dropping of tokens are done elsewhere.
            return;
          }
        }
        ...
        n = -1;
        N = std::accumulate(token_to_costs.begin(), token_to_costs.end(), 1LL,
                            product);
    ...
    if (token_candidates_vec.size() == query_tokens.size()) {
      ...
      search_all_candidates(
          num_search_fields, match_type, the_fields, filter_ids,
          filter_ids_length, exclude_token_ids, exclude_token_ids_size,
          excluded_group_ids, sort_fields, token_candidates_vec,
          searched_queries, qtoken_set, dropped_tokens, topster,
          groups_processed, all_result_ids, all_result_ids_len,
          typo_tokens_threshold, group_limit, group_by_fields, query_tokens,
          num_typos, prefixes, prioritize_exact_match,
          prioritize_token_position, exhaustive_search, max_candidates,
          syn_orig_num_tokens, sort_order, field_values, geopoint_indices,
      ...
      uint32_t* new_all_result_ids = nullptr;
      all_result_ids_len =
          ArrayUtils::or_scalar(all_result_ids, all_result_ids_len, &id_buff[0],
...
void Index::popular_fields_of_token(
    const spp::sparse_hash_map<std::string, art_tree*>& search_index,
    const std::string& previous_token,
    const std::vector<search_field_t>& the_fields,
    const size_t num_search_fields, std::vector<size_t>& popular_field_ids) {
  ...
  for (size_t i = 0; i < num_search_fields; i++) {
    ...
    auto leaf = static_cast<art_leaf*>(
        art_search(search_index.at(field_name), token_c_str, token_len));
...
void Index::find_across_fields(
    const token_t& previous_token, const std::string& previous_token_str,
    const std::vector<search_field_t>& the_fields,
    const size_t num_search_fields, const uint32_t* filter_ids,
    uint32_t filter_ids_length, const uint32_t* exclude_token_ids,
    size_t exclude_token_ids_size, std::vector<uint32_t>& prev_token_doc_ids,
...
void Index::search_across_fields(
    ...
    const std::vector<search_field_t>& the_fields,
    const size_t num_search_fields, const text_match_type_t match_type,
    const std::vector<sort_by>& sort_fields, Topster* topster,
    spp::sparse_hash_map<uint64_t, uint32_t>& groups_processed,
    std::vector<std::vector<art_leaf*>>& searched_queries,
    tsl::htrie_map<char, token_leaf>& qtoken_set,
    const std::vector<token_t>& dropped_tokens, const size_t group_limit,
    const std::vector<std::string>& group_by_fields,
    const bool prioritize_exact_match, const bool prioritize_token_position,
    ...
    const int* sort_order,
    std::array<spp::sparse_hash_map<uint32_t, int64_t>*, 3>& field_values,
    const std::vector<size_t>& geopoint_indices, std::vector<uint32_t>& id_buff,
  ...
  for (auto& dropped_token : dropped_tokens) {
    ...
    for (size_t i = 0; i < the_fields.size(); i++) {
      ...
      art_leaf* leaf = static_cast<art_leaf*>(
          art_search(tree, token_c_str, token.size() + 1));
  ...
  for (size_t ti = 0; ti < query_tokens.size(); ti++) {
    ...
    for (size_t i = 0; i < num_search_fields; i++) {
      ...
      const uint32_t field_num_typos =
          (the_fields[i].orig_index < num_typos.size())
              ? num_typos[the_fields[i].orig_index]
              : num_typos[0];
      const bool field_prefix = (the_fields[i].orig_index < prefixes.size())
                                    ? prefixes[the_fields[i].orig_index]
                                    : prefixes[0];

      if (token_num_typos > field_num_typos) {
        // since the token can come from any field, we still have to respect
        // per-field num_typos
        continue;
      }

      if (token_prefix && !field_prefix) {
        // even though this token is an outcome of prefix search, we can't use
        // it for this field, since this field has prefix search disabled.
        continue;
      }
  ...
  or_iterator_t::intersect(
      ...
      [&](uint32_t seq_id, const std::vector<or_iterator_t>& its) {
        ...
        for (size_t ti = 0; ti < dropped_token_its.size(); ti++) {
          ...
          if (token_fields_iters.skip_to(seq_id) &&
              token_fields_iters.id() == seq_id) {
            query_len++;
            const std::vector<posting_list_t::iterator_t>& field_iters =
                token_fields_iters.get_its();
            for (size_t fi = 0; fi < field_iters.size(); fi++) {
        ...
        for (size_t fi = 0; fi < field_to_tokens.size(); fi++) {
          ...
          score_results2(sort_fields, searched_queries.size(), fi,
                         field_is_array, total_cost, field_match_score, seq_id,
                         sort_order, prioritize_exact_match,
                         single_exact_query_token, prioritize_token_position,
                         query_tokens.size(), syn_orig_num_tokens,
                         token_postings);

          if (match_type == max_score &&
              field_match_score > best_field_match_score) {
        ...
        auto max_field_weight =
            std::min<size_t>(FIELD_MAX_WEIGHT, best_field_weight);
        num_matching_fields = std::min<size_t>(7, num_matching_fields);

        uint64_t aggregated_score =
            match_type == max_score ? ((int64_t(query_len) << 59) |
...
void Index::compute_sort_scores(
    const std::vector<sort_by>& sort_fields, const int* sort_order,
    std::array<spp::sparse_hash_map<uint32_t, int64_t>*, 3> field_values,
  ...
  const int64_t default_score = INT64_MIN;  // to handle field that doesn't
                                            // exist in document (e.g. optional)
  ...
  if (sort_fields.size() > 0) {
    if (field_values[0] == &text_match_sentinel_value) {
      ...
      // Returns iterator to the first element that is >= to value or last if no
      // such element is found.
      bool found = false;
      if (filter_index == 0 || filter_index < sort_fields[0].eval.size) {
        size_t found_index =
            std::lower_bound(sort_fields[0].eval.ids + filter_index,
      ...
      if (scores[0] == INT64_MIN &&
          sort_fields[0].missing_values == sort_by::missing_values_t::first) {
        // By default, missing numerical value are always going to be sorted to
        // be at the end because: -INT64_MIN == INT64_MIN. To account for
        // missing values config, we will have to change the default for missing
        // value based on whether it's asc or desc sort.
  ...
  if (sort_fields.size() > 1) {
    if (field_values[1] == &text_match_sentinel_value) {
      ...
      if (filter_index == 0 || filter_index < sort_fields[1].eval.size) {
        size_t found_index =
            std::lower_bound(sort_fields[1].eval.ids + filter_index,
  ...
  if (sort_fields.size() > 2) {
    if (field_values[2] == &text_match_sentinel_value) {
      ...
      if (filter_index == 0 || filter_index < sort_fields[2].eval.size) {
        size_t found_index =
            std::lower_bound(sort_fields[2].eval.ids + filter_index,
...
void Index::do_phrase_search(
    ...
    std::vector<std::vector<art_leaf*>>& searched_queries,
    const size_t group_limit, const std::vector<std::string>& group_by_fields,
    Topster* actual_topster, const int sort_order[3],
    std::array<spp::sparse_hash_map<uint32_t, int64_t>*, 3> field_values,
    const std::vector<size_t>& geopoint_indices,
    const std::vector<uint32_t>& curated_ids_sorted, uint32_t*& all_result_ids,
    size_t& all_result_ids_len,
    spp::sparse_hash_map<uint64_t, uint32_t>& groups_processed,
    const std::set<uint32_t>& curated_ids, const uint32_t* excluded_result_ids,
    size_t excluded_result_ids_size,
    const std::unordered_set<uint32_t>& excluded_group_ids,
    Topster* curated_topster,
    const std::map<size_t, std::map<size_t, uint32_t>>& included_ids_map,
    bool is_wildcard_query, uint32_t*& filter_ids,
    uint32_t& filter_ids_length) const {
  ...
  for (size_t i = 0; i < num_search_fields; i++) {
    ...
    for (const auto& phrase : field_query_tokens[i].q_phrases) {
      ...
      for (const std::string& token : phrase) {
        art_leaf* leaf = (art_leaf*)art_search(
            search_index.at(field_name), (const unsigned char*)token.c_str(),
            token.size() + 1);
        if (leaf) {
          posting_lists.push_back(leaf->values);
        }
      ...
      if (posting_lists.size() != phrase.size()) {
        // unmatched length means no matches will be found for this phrase, so
        // skip to next phrase
        continue;
      }
      ...
      posting_t::get_phrase_matches(posting_lists, is_array, &contains_ids[0],
                                    contains_ids.size(), this_phrase_ids,
                                    this_phrase_ids_size);
      ...
      if (field_phrase_match_ids_size == 0) {
        ...
        field_phrase_match_ids_size = ArrayUtils::and_scalar(
            this_phrase_ids, this_phrase_ids_size, field_phrase_match_ids,
    ...
    // upto 10K phrase match IDs per field will be weighted so that phrase match
    // against a higher weighted field is returned earlier in the results
    const size_t weight_score_base =
        100000;  // just to make score be a large number
    for (size_t pi = 0;
         pi < std::min<size_t>(10000, field_phrase_match_ids_size); pi++) {
      auto this_field_score = (weight_score_base + field_weight);
      auto existing_score = phrase_match_id_scores[field_phrase_match_ids[pi]];
      phrase_match_id_scores[field_phrase_match_ids[pi]] =
          std::max(this_field_score, existing_score);
    }
  ...
  if (filter_ids_length == 0) {
    ...
    filter_ids_length =
        ArrayUtils::and_scalar(filter_ids, filter_ids_length, phrase_match_ids,
  ...
  curate_filtered_ids(curated_ids, excluded_result_ids,
                      excluded_result_ids_size, filter_ids, filter_ids_length,
                      curated_ids_sorted);
  ...
  for (size_t i = 0; i < std::min<size_t>(10000, filter_ids_length); i++) {
    ...
    compute_sort_scores(sort_fields, sort_order, field_values, geopoint_indices,
                        seq_id, filter_index, match_score, scores,
                        match_score_index);
...
void Index::do_synonym_search(
    ...
    const token_ordering& token_order, const size_t typo_tokens_threshold,
    const size_t group_limit, const std::vector<std::string>& group_by_fields,
    bool prioritize_exact_match, const bool prioritize_token_position,
    const bool exhaustive_search, const size_t concurrency,
    const std::vector<bool>& prefixes, size_t min_len_1typo,
    size_t min_len_2typo, const size_t max_candidates,
    const std::set<uint32_t>& curated_ids,
    const std::vector<uint32_t>& curated_ids_sorted,
    const uint32_t* exclude_token_ids, size_t exclude_token_ids_size,
    const std::unordered_set<uint32_t>& excluded_group_ids,
    Topster* actual_topster, std::vector<std::vector<token_t>>& q_pos_synonyms,
    int syn_orig_num_tokens,
    spp::sparse_hash_map<uint64_t, uint32_t>& groups_processed,
    std::vector<std::vector<art_leaf*>>& searched_queries,
    uint32_t*& all_result_ids, size_t& all_result_ids_len,
    const uint32_t* filter_ids, const uint32_t filter_ids_length,
    std::set<uint64>& query_hashes, const int* sort_order,
    std::array<spp::sparse_hash_map<uint32_t, int64_t>*, 3>& field_values,
    const std::vector<size_t>& geopoint_indices,
  ...
  for (const auto& syn_tokens : q_pos_synonyms) {
    query_hashes.clear();
    fuzzy_search_fields(
        the_fields, syn_tokens, {}, match_type, exclude_token_ids,
        exclude_token_ids_size, filter_ids, filter_ids_length,
...
void Index::do_infix_search(
    ...
    std::vector<std::vector<art_leaf*>>& searched_queries,
    const size_t group_limit, const std::vector<std::string>& group_by_fields,
    const size_t max_extra_prefix, const size_t max_extra_suffix,
    ...
    if (field_infix == always ||
      ...
      if (!infix_ids.empty()) {
        ...
        for (size_t i = 0; i < raw_infix_ids_length; i++) {
          ...
          score_results2(sort_fields, searched_queries.size(), field_id,
                         field_is_array, 0, match_score, seq_id, sort_order,
          ...
          compute_sort_scores(sort_fields, sort_order, field_values,
                              geopoint_indices, seq_id, filter_index, 100,
...
void Index::handle_exclusion(const size_t num_search_fields,
  ...
  for (size_t i = 0; i < num_search_fields; i++) {
    ...
    for (const auto& q_exclude_phrase :
      ...
      // if phrase has multiple words, then we have to do exclusion of phrase
      // match results
      std::vector<void*> posting_lists;
      for (const std::string& exclude_token : q_exclude_phrase) {
        art_leaf* leaf =
            (art_leaf*)art_search(search_index.at(field_name),
    ...
    if (a_facet.field_name == facet_query.field_name &&
      ...
      for (size_t qtoken_index = 0; qtoken_index < query_tokens.size();
        ...
        qtokens.emplace_back(qtoken_index, query_tokens[qtoken_index],
                             is_prefix, query_tokens[qtoken_index].size(), 0);
      }
      ...
      search_field(
          0, qtokens, nullptr, 0, num_toks_dropped, facet_field,
          facet_field.faceted_name(), all_result_ids, all_result_ids_len, {},
          sort_fields, -1, facet_query_num_typos, searched_queries, topster,
          groups_processed, &field_result_ids, field_result_ids_len,
          field_num_results, 0, group_by_fields, false, 4, query_hashes,
          MAX_SCORE, true, 0, 1, false, -1, 3, 1000, max_candidates);

      // LOG(INFO) << "searched_queries.size: " << searched_queries.size();

      // NOTE: `field_result_ids` will consists of IDs across ALL queries in
      // searched_queries
        ...
        for (size_t i = 0; i < field_result_ids_len; i++) {
          ...
          for (auto pl : posting_lists) {
            if (!posting_t::contains(pl, seq_id)) {
              ...
              // searched_query tokens since `field_result_ids` contains
          ...
          if (facet_field.is_array()) {
            const auto doc_fvalues_it =
                field_facet_mapping_it->second[seq_id % ARRAY_FACET_DIM]->find(
                    seq_id);
            if (doc_fvalues_it ==
            ...
            posting_t::get_matching_array_indices(posting_lists, seq_id,
                                                  array_indices);
            ...
            for (size_t array_index : array_indices) {
              if (array_index < doc_fvalues_it->second.length) {
                ...
                /*LOG(INFO) << "seq_id: " << seq_id << ", hash: " << hash << ",
                   array index: "
                          << array_index;*/
            ...
            const auto doc_single_fvalues_it =
                field_single_val_facet_mapping_it
                    ->second[seq_id % ARRAY_FACET_DIM]
                    ->find(seq_id);
            if (doc_single_fvalues_it == field_single_val_facet_mapping_it
                                             ->second[seq_id % ARRAY_FACET_DIM]
                                             ->end()) {
              continue;
            }
...
void Index::curate_filtered_ids(
    const std::set<uint32_t>& curated_ids, const uint32_t* exclude_token_ids,
    size_t exclude_token_ids_size, uint32_t*& filter_ids,
...
void Index::search_wildcard(
    ...
    uint32_t*& all_result_ids, size_t& all_result_ids_len,
    const uint32_t* filter_ids, uint32_t filter_ids_length,
    const size_t concurrency, const int* sort_order,
    std::array<spp::sparse_hash_map<uint32_t, int64_t>*, 3>& field_values,
    const std::vector<size_t>& geopoint_indices) const {
  uint32_t token_bits = 0;
  ...
  for (size_t thread_id = 0;
    ...
    thread_pool->enqueue(
        [this, &parent_search_begin, &parent_search_stop_ms,
         ...
         &excluded_group_ids, &sort_order, field_values, &geopoint_indices,
         &plists, check_for_circuit_break, batch_result_ids, batch_res_len,
         &num_processed, &m_process, &cv_process]() {
          search_begin_us = parent_search_begin;
          search_stop_us = parent_search_stop_ms;
          search_cutoff = parent_search_cutoff;
          ...
          for (size_t i = 0; i < batch_res_len; i++) {
            ...
            score_results2(sort_fields, (uint16_t)searched_queries.size(), 0,
                           false, 0, match_score, seq_id, sort_order, false,
...
void Index::populate_sort_mapping(
    int* sort_order, std::vector<size_t>& geopoint_indices,
    std::vector<sort_by>& sort_fields_std,
    std::array<spp::sparse_hash_map<uint32_t, int64_t>*, 3>& field_values)
    const {
...
void Index::search_field(
    const uint8_t& field_id, const std::vector<token_t>& query_tokens,
    const uint32_t* exclude_token_ids, size_t exclude_token_ids_size,
    size_t& num_tokens_dropped, const field& the_field,
    const std::string& field_name,  // to handle faceted index
    const uint32_t* filter_ids, size_t filter_ids_length,
    const std::vector<uint32_t>& curated_ids, std::vector<sort_by>& sort_fields,
    const int last_typo, const int max_typos,
    std::vector<std::vector<art_leaf*>>& searched_queries, Topster* topster,
    spp::sparse_hash_map<uint64_t, uint32_t>& groups_processed,
    uint32_t** all_result_ids, size_t& all_result_ids_len,
    size_t& field_num_results, const size_t group_limit,
    const std::vector<std::string>& group_by_fields,
    bool prioritize_exact_match, const size_t concurrency,
    std::set<uint64>& query_hashes, const token_ordering token_order,
    const bool prefix, const size_t drop_tokens_threshold,
    const size_t typo_tokens_threshold, const bool exhaustive_search,
    int syn_orig_num_tokens, size_t min_len_1typo, size_t min_len_2typo,
  ...
  while (n < N && n < combination_limit) {
    ...
    for (long long i = (token_to_costs.size() - 1); 0 <= i; --i) {
      ...
      if (costs[i] == uint32_t(last_typo + 1)) {
        // to support progressive typo searching, there must be atleast one typo
        // that's greater than last_typo
        valid_combo = true;
      }
    }
    ...
    while (token_index < query_tokens.size()) {
      ...
      if (token_cost_cache.count(token_cost_hash) != 0) {
        ...
        // need less candidates for filtered searches since we already only pick
        // tokens with results
        art_fuzzy_search(search_index.at(field_name),
                         (const unsigned char*)token.c_str(), token_len,
                         costs[token_index], costs[token_index], max_candidates,
                         token_order, prefix_search, false, "", filter_ids,
                         filter_ids_length, leaves, unique_tokens);
      ...
      if (!leaves.empty()) {
        ...
        token_candidates_vec.push_back(
            token_candidates{query_tokens[token_index], costs[token_index],
    ...
    if (token_candidates_vec.size() == query_tokens.size()) {
      ...
      search_candidates(
          ...
          exclude_token_ids, exclude_token_ids_size, curated_ids, sort_fields,
          token_candidates_vec, searched_queries, topster, groups_processed,
          all_result_ids, all_result_ids_len, field_num_results,
          typo_tokens_threshold, group_limit, group_by_fields, query_tokens,
...
void Index::log_leaves(const int cost, const std::string& token,
...
int64_t Index::score_results2(
    const std::vector<sort_by>& sort_fields, const uint16_t& query_index,
    const size_t field_id, const bool field_is_array, const uint32_t total_cost,
    int64_t& match_score, const uint32_t seq_id, const int sort_order[3],
    const bool prioritize_exact_match, const bool single_exact_query_token,
    const bool prioritize_token_position, size_t num_query_tokens,
    int syn_orig_num_tokens,
  ...
  if (posting_lists.size() <= 1) {
    const uint8_t is_verbatim_match =
        uint8_t(prioritize_exact_match && single_exact_query_token &&
                posting_list_t::is_single_token_verbatim_match(posting_lists[0],
                                                               field_is_array));
    size_t words_present = (num_query_tokens == 1 && syn_orig_num_tokens != -1)
                               ? syn_orig_num_tokens
                               : 1;
    ...
    size_t max_offset =
        prioritize_token_position
            ? posting_list_t::get_last_offset(posting_lists[0], field_is_array)
            : 255;
    ...
    for (const auto& kv : array_token_positions) {
      ...
      // Within a field, only a subset of query tokens can match (unique_words),
      // but even a smaller set might be available within the window used for
      // proximity calculation (this_words_present)

      auto this_words_present = ((this_match_score >> 32) & 0xFF);
      auto unique_words = field_is_array ? this_words_present
      ...
      auto offset_score =
          prioritize_token_position ? ((this_match_score >> 0) & 0xFF) : 0;
      ...
      uint64_t mod_match_score =
          ((int64_t(this_words_present) << 40) | (int64_t(unique_words) << 32) |
           (int64_t(typo_score) << 24) | (int64_t(proximity) << 16) |
           (int64_t(verbatim) << 8) | (int64_t(offset_score) << 0));
...
void Index::score_results(
    const std::vector<sort_by>& sort_fields, const uint16_t& query_index,
    const uint8_t& field_id, const bool field_is_array,
    const uint32_t total_cost, Topster* topster,
    const std::vector<art_leaf*>& query_suggestion,
    spp::sparse_hash_map<uint64_t, uint32_t>& groups_processed,
    const uint32_t seq_id, const int sort_order[3],
    std::array<spp::sparse_hash_map<uint32_t, int64_t>*, 3> field_values,
    const std::vector<size_t>& geopoint_indices, const size_t group_limit,
    const std::vector<std::string>& group_by_fields, const uint32_t token_bits,
    const bool prioritize_exact_match, const bool single_exact_query_token,
    int syn_orig_num_tokens,
    const std::vector<posting_list_t::iterator_t>& posting_lists) const {
  int64_t geopoint_distances[3];
  ...
  if (posting_lists.size() <= 1) {
    ...
    // NOTE: tokens found returned by matcher is only within the best matched
    // window, so we have to still consider unique tokens found if they are
    ...
    for (const auto& kv : array_token_positions) {
      ...
      uint64_t mod_match_score =
          ((int64_t(unique_tokens_found) << 32) |
           (int64_t(this_words_present) << 24) | (int64_t(typo_score) << 16) |
           (int64_t(proximity) << 8) | (int64_t(verbatim) << 0));
...
uint64_t Index::get_distinct_id(const std::vector<std::string>& group_by_fields,
  ...
  for (const auto& field : group_by_fields) {
    ...
    const auto& field_single_val_facet_mapping_it =
        single_val_facet_index_v3.find(field);
    if ((field_facet_mapping_it == facet_index_v3.end()) &&
        (field_single_val_facet_mapping_it ==
         single_val_facet_index_v3.end())) {
      continue;
...
inline uint32_t Index::next_suggestion2(
    const std::vector<tok_candidates>& token_candidates_vec, long long int n,
    std::vector<token_t>& query_suggestion, uint64& qhash) {
  ...
  for (size_t i = 0; i < token_candidates_vec.size(); i++) {
    ...
    if (candidate.size() > 1 && !Tokenizer::is_ascii_char(candidate[0])) {
      ...
      if (code_point >= 0x600 && code_point <= 0x6ff) {
        // adjust typo cost for Arabic strings, since 1 byte difference makes no
        // sense
        if (typo_cost == 1) {
          typo_cost = 2;
        }
    ...
    // we assume that toke was found via prefix search if candidate is longer
    // than token's typo tolerance
    ...
    query_suggestion[i] =
        token_t(i, candidate, is_prefix_searched, token_size, typo_cost);

    uint64_t this_hash = StringUtils::hash_wy(query_suggestion[i].value.c_str(),
...
inline uint32_t Index::next_suggestion(
    const std::vector<token_candidates>& token_candidates_vec, long long int n,
    std::vector<art_leaf*>& actual_query_suggestion,
    std::vector<art_leaf*>& query_suggestion, const int syn_orig_num_tokens,
    uint32_t& token_bits, uint64& qhash) {
  uint32_t total_cost = 0;
  qhash = 1;
  ...
  for (long long i = 0; i < (long long)token_candidates_vec.size(); i++) {
    ...
    bool exact_match = token_candidates_vec[i].cost == 0 &&
    ...
    size_t actual_cost =
        (2 * token_candidates_vec[i].cost) + uint32_t(incr_for_prefix_search);
...
void Index::remove_field(uint32_t seq_id, const nlohmann::json& document,
                         const std::string& field_name) {
  ...
  // Go through all the field names and find the keys+values so that they can be
  // removed from in-memory index
  if (search_field.type == field_types::STRING_ARRAY ||
    ...
    for (int32_t value : values) {
      ...
      if (search_field.facet) {
        remove_facet_token(search_field, search_index, std::to_string(value),
                           seq_id);
      }
    }
    ...
    const std::vector<std::vector<double>>& latlongs =
        search_field.is_single_geopoint()
            ? std::vector<std::vector<double>>{document[field_name]
  ...
  const auto& field_single_val_facets_it =
      single_val_facet_index_v3.find(field_name);
  if (field_single_val_facets_it != single_val_facet_index_v3.end()) {
    const auto& fvalues_it =
        field_single_val_facets_it->second[seq_id % ARRAY_FACET_DIM]->find(
            seq_id);
    if (fvalues_it !=
        field_single_val_facets_it->second[seq_id % ARRAY_FACET_DIM]->end()) {
      field_single_val_facets_it->second[seq_id % ARRAY_FACET_DIM]->erase(
          fvalues_it);
    }
  }
...
Option<uint32_t> Index::remove(const uint32_t seq_id,
  ...
  // schema detection: Value indexed as Type T but later if field is dropped and
  // reindexed in another type X, the on-disk data will differ from the newly
  // detected type on schema. We've to log the error, but have to ignore the
  // field and proceed because there's no leak caused here.
...
const spp::sparse_hash_map<std::string, hnsw_index_t*>&
Index::_get_vector_index() const {
  return vector_index;
}
...
void Index::handle_doc_ops(const tsl::htrie_map<char, field>& search_schema,
  ...
  */
  ...
  if (ops_it != update_doc.end()) {
    ...
    if (operations.contains("increment") &&
        operations["increment"].is_object()) {
      for (const auto& item : operations["increment"].items()) {
        auto field_it = search_schema.find(item.key());
        if (field_it != search_schema.end()) {
          if (field_it->type == field_types::INT32 &&
...
void Index::get_doc_changes(const index_operation_t op,
  ...
  if (op == UPSERT) {
    new_doc = update_doc;
    // since UPSERT could replace a doc with lesser fields, we have to add those
    // missing fields to del_doc
    for (auto it = old_doc.begin(); it != old_doc.end(); ++it) {
      if (it.value().is_object() ||
          (it.value().is_array() &&
           (it.value().empty() || it.value()[0].is_object()))) {
        continue;
      }

      if (!update_doc.contains(it.key())) {
        // embedding field won't be part of upsert doc so populate new doc with
        // the value from old doc
...
Option<bool> Index::seq_ids_outside_top_k(
    const std::string& field_name, size_t k,
...
void Index::resolve_space_as_typos(
    std::vector<std::string>& qtokens, const string& field_name,
  ...
  if (qtokens.size() > 1) {
    ...
    // b) join 2 adjacent tokens in a sliding window (provided they are atleast
    // 2 tokens in size)
    ...
    for (size_t i = 0; i < qtokens_size - 1 && qtokens_size > 2; i++) {
      ...
      for (auto& token : candidate_tokens) {
        art_leaf* leaf = static_cast<art_leaf*>(art_search(
            t, (const unsigned char*)token.c_str(), token.length() + 1));
        if (leaf == nullptr) {
          break;
        }
...
bool Index::common_results_exist(std::vector<art_leaf*>& leaves,
  ...
  posting_t::get_phrase_matches(leaf_vals, false, &result_ids[0],
                                result_ids.size(), phrase_ids, num_phrase_ids);
  bool phrase_exists = (num_phrase_ids != 0);
  delete[] phrase_ids;
  return phrase_exists;
}

void Index::batch_embed_fields(
  ...
  for (const auto& field : embedding_fields) {
    ...
    auto indexing_prefix =
        TextEmbedderManager::get_instance().get_indexing_prefix(
            field.embed[fields::model_config]);
    for (auto& record : records) {
      ...
      const auto& embed_from =
          field.embed[fields::from].get<std::vector<std::string>>();
    ...
    auto embedder_op =
        embedder_manager.get_text_embedder(field.embed[fields::model_config]);

    if (!embedder_op.ok()) {
      LOG(ERROR) << "Error while getting embedder for model: "
                 << field.embed[fields::model_config];
    ...
    auto embeddings =
        embedder_op.get()->batch_embed(texts, remote_embedding_batch_size);
...
poly, const GeoCoord &point) { int i, j; bool c = false; for (i = 0, j =
poly.numVerts - 1; i < poly.numVerts; j = i++) { if ((((poly.verts[i].lat <=
point.lat) && (point.lat < poly.verts[j].lat))
