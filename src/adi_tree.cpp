...
struct adi_node_t {
  ...
  adi_node_t()
      : scions(0), num_children(0), chars(nullptr), children(nullptr) {}
...
adi_tree_t::adi_tree_t() { root = new adi_node_t(); }

void adi_tree_t::add_node(adi_node_t* node, const std::string& key,
...
bool adi_tree_t::rank_aggregate(adi_node_t* node, const std::string& key,
                                const size_t key_index, size_t& rank) {
  char c = (key_index == key.size()) ? '\0' : key[key_index];
...
adi_node_t* adi_tree_t::get_node(adi_node_t* node, const std::string& key,
                                 const size_t key_index,
...
void adi_tree_t::remove_node(adi_node_t* node, const std::string& key,
                             const size_t key_index) {
  char c = (key_index == key.size()) ? '\0' : key[key_index];
  ...
  for (size_t i = 0; i < node->num_children; i++) {
    if (node->chars[i] == c) {
      if (node->children[i]->scions > 1) {
        // skip to next character in trie as this character is shared by other
        // entries
        node->scions--;
        if (c != '\0') {
...
void adi_tree_t::remove(uint32_t id) {
  ...
  // LOG(INFO) << "Removing key: " << key << ", seq_id: " << id << ",
  // id_keys.size: " << id_keys.size()
...
const adi_node_t* adi_tree_t::get_root() { return root; }
