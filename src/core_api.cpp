...
bool handle_authentication(std::map<std::string, std::string>& req_params,
  ...
  if (collections.size() != embedded_params_vec.size()) {
    LOG(ERROR) << "Impossible error: size of collections and "
                  "embedded_params_vec don't match, "
               << "collections.size: " << collections.size()
               << ", embedded_params_vec.size: " << embedded_params_vec.size();
    return false;
  }

  return collectionManager.auth_key_matches(
      req_auth_key, rpath.action, collections, req_params, embedded_params_vec);
}

void stream_response(const std::shared_ptr<http_req>& req,
  ...
  server->get_message_dispatcher()->send_message(
      HttpServer::STREAM_RESPONSE_MESSAGE, req_res);
}

void defer_processing(const std::shared_ptr<http_req>& req,
                      const std::shared_ptr<http_res>& res, size_t timeout_ms) {
  defer_processing_t* defer =
      new defer_processing_t(req, res, timeout_ms, server);
  // LOG(INFO) << "core_api req " << req.get() << ", use count: " <<
  // req.use_count();
  server->get_message_dispatcher()->send_message(
      HttpServer::DEFER_PROCESSING_MESSAGE, defer);
}

// we cannot return errors here because that will end up as auth failure and
// won't convey bad schema errors
void get_collections_for_auth(
    std::map<std::string, std::string>& req_params, const string& body,
  ...
  if (rpath.handler == post_multi_search) {
    ...
    // If a `preset` parameter is present, we've to only load a pre-existing
    // search configuration and ignore the actual request body.
    ...
    if (!req_obj.is_discarded() && req_obj.count("searches") != 0 &&
        req_obj["searches"].is_array()) {
        ...
        if (el.is_object()) {
          ...
          if (el.count("collection") != 0 && el["collection"].is_string()) {
            ...
            if (el.count("preset") != 0) {
              ...
              auto preset_op = CollectionManager::get_instance().get_preset(
                  el["preset"].get<std::string>(), preset_obj);
          ...
          const std::string& access_key =
              (el.count("x-typesense-api-key") != 0 &&
...
bool get_collections(const std::shared_ptr<http_req>& req,
                     const std::shared_ptr<http_res>& res) {
  CollectionManager& collectionManager = CollectionManager::get_instance();
...
bool post_create_collection(const std::shared_ptr<http_req>& req,
                            const std::shared_ptr<http_res>& res) {
  ...
  /*if(res->is_alive && req_json.is_object() &&
  req_json.count("enable_nested_fields") == 0) {
      // This patch ensures that nested fields are only enabled for collections
  created on Typesense versions
      // which support nested fields. This ensures that ".*" schema does not end
  up duplicating fields on
      // manually flattened collection schemas that also contain nested versions
  for convenience.
  ...
  const Option<Collection*>& collection_op =
      req->params.count(SRC_COLL_NAME) != 0
          ? collectionManager.clone_collection(req->params[SRC_COLL_NAME],
                                               req_json)
...
bool patch_update_collection(const std::shared_ptr<http_req>& req,
                             const std::shared_ptr<http_res>& res) {
  nlohmann::json req_json;
...
bool del_drop_collection(const std::shared_ptr<http_req>& req,
                         const std::shared_ptr<http_res>& res) {
  std::string doc_id = req->params["id"];
  CollectionManager& collectionManager = CollectionManager::get_instance();
  Option<nlohmann::json> drop_op =
      collectionManager.drop_collection(req->params["collection"], true);
...
bool get_debug(const std::shared_ptr<http_req>& req,
               const std::shared_ptr<http_res>& res) {
...
bool get_health(const std::shared_ptr<http_req>& req,
                const std::shared_ptr<http_res>& res) {
  nlohmann::json result;
...
bool post_health(const std::shared_ptr<http_req>& req,
                 const std::shared_ptr<http_res>& res) {
  nlohmann::json result;
  bool alive = server->is_alive();
  result["ok"] = alive;
...
bool get_metrics_json(const std::shared_ptr<http_req>& req,
                      const std::shared_ptr<http_res>& res) {
  nlohmann::json result;

  CollectionManager& collectionManager = CollectionManager::get_instance();
  const std::string& data_dir_path =
      collectionManager.get_store()->get_state_dir_path();
...
bool get_stats_json(const std::shared_ptr<http_req>& req,
                    const std::shared_ptr<http_res>& res) {
  nlohmann::json result;
...
bool get_status(const std::shared_ptr<http_req>& req,
                const std::shared_ptr<http_res>& res) {
...
bool get_search(const std::shared_ptr<http_req>& req,
                const std::shared_ptr<http_res>& res) {
  const auto use_cache_it = req->params.find("use_cache");
  bool use_cache =
      (use_cache_it != req->params.end()) &&
    ...
    if (hit_it != res_cache.end()) {
      ...
      uint64_t seconds_elapsed =
          std::chrono::duration_cast<std::chrono::seconds>(
              ...
              cached_value.created_at)
              .count();

      if (seconds_elapsed < cached_value.ttl) {
        res->set_content(cached_value.status_code,
                         cached_value.content_type_header, cached_value.body,
                         true);
        return true;
      }
  ...
  Option<bool> search_op = CollectionManager::do_search(
      req->params, req->embedded_params_vec[0], results_json_str, req->conn_ts);
  ...
  if (use_cache) {
    ...
    if (cache_ttl_it != req->params.end() &&
        StringUtils::is_int32_t(cache_ttl_it->second)) {
      cache_ttl = std::stoul(cache_ttl_it->second);
    }

    cached_res_t cached_res;
    cached_res.load(res->status_code, res->content_type_header, res->body, now,
                    cache_ttl, req_hash);
...
bool post_multi_search(const std::shared_ptr<http_req>& req,
                       const std::shared_ptr<http_res>& res) {
  const auto use_cache_it = req->params.find("use_cache");
  bool use_cache =
      (use_cache_it != req->params.end()) &&
  ...
  if (first_embedded_param.count(LIMIT_MULTI_SEARCHES) != 0 &&
      first_embedded_param[LIMIT_MULTI_SEARCHES].is_number_integer()) {
    limit_multi_searches =
        first_embedded_param[LIMIT_MULTI_SEARCHES].get<size_t>();
  }

  if (req_json["searches"].size() > limit_multi_searches) {
    res->set_400(std::string("Number of multi searches exceeds `") +
                 LIMIT_MULTI_SEARCHES + "` parameter.");
    return false;
  }
  ...
  if (searches.size() != req->embedded_params_vec.size()) {
    LOG(ERROR) << "Embedded params parsing error: length does not match multi "
                  "search array, searches.size(): "
               << searches.size() << ", embedded_params_vec.size: "
               << req->embedded_params_vec.size()
               << ", req_body: " << req->body;
  ...
  if (!req->metadata.empty()) {
    ...
    // Check rate limiting first before doing any search, don't want to waste
    // time if we're rate limited
    for (size_t i = 0; i < searches.size(); i++) {
      if (RateLimitManager::getInstance()->is_rate_limited(
              {RateLimitedEntityType::api_key, api_key_ip.first},
              {RateLimitedEntityType::ip, api_key_ip.second})) {
  ...
  for (size_t i = 0; i < searches.size(); i++) {
    ...
    for (auto& search_item : search_params.items()) {
      if (search_item.key() == "cache_ttl") {
        // cache ttl can be applied only from an embedded key: cannot be a multi
        // search param
        continue;
      }

      // overwrite = false since req params will contain embedded params and so
      // has higher priority
      bool populated =
          AuthManager::add_item_to_params(req->params, search_item, false);
      if (!populated) {
    ...
    std::string results_json_str;
    Option<bool> search_op =
        CollectionManager::do_search(req->params, req->embedded_params_vec[i],
...
bool get_collection_summary(const std::shared_ptr<http_req>& req,
                            const std::shared_ptr<http_res>& res) {
  CollectionManager& collectionManager = CollectionManager::get_instance();
  auto collection = collectionManager.get_collection(req->params["collection"]);
  ...
  res->set_200(json_response.dump(-1, ' ', false,
                                  nlohmann::detail::error_handler_t::ignore));

  return true;
}

bool get_export_documents(const std::shared_ptr<http_req>& req,
                          const std::shared_ptr<http_res>& res) {
  // NOTE: this is a streaming response end-point so this handler will be called
  // multiple times
  CollectionManager& collectionManager = CollectionManager::get_instance();
  auto collection = collectionManager.get_collection(req->params["collection"]);
  ...
  if (req->data == nullptr) {
    ...
    if (req->params.count(INCLUDE_FIELDS) != 0) {
      ...
      include_fields = spp::sparse_hash_set<std::string>(
          include_fields_vec.begin(), include_fields_vec.end());
    }

    if (req->params.count(EXCLUDE_FIELDS) != 0) {
      std::vector<std::string> exclude_fields_vec;
      StringUtils::split(req->params[EXCLUDE_FIELDS], exclude_fields_vec, ",");
      exclude_fields = spp::sparse_hash_set<std::string>(
          exclude_fields_vec.begin(), exclude_fields_vec.end());
    }

    collection->populate_include_exclude_fields_lk(
        include_fields, exclude_fields, export_state->include_fields,
        export_state->exclude_fields);

    if (req->params.count(BATCH_SIZE) != 0 &&
        StringUtils::is_uint32_t(req->params[BATCH_SIZE])) {
      export_state->export_batch_size = std::stoul(req->params[BATCH_SIZE]);
    }

    if (simple_filter_query.empty()) {
      export_state->iter_upper_bound_key =
          collection->get_seq_id_collection_prefix() +
          "`";  // cannot inline this
      export_state->iter_upper_bound =
          new rocksdb::Slice(export_state->iter_upper_bound_key);
      export_state->it = collectionManager.get_store()->scan(
          seq_id_prefix, export_state->iter_upper_bound);
    } else {
      filter_result_t filter_result;
      auto filter_ids_op =
          collection->get_filter_ids(simple_filter_query, filter_result);
      ...
      export_state->index_ids.emplace_back(filter_result.count,
                                           filter_result.docs);
  ...
  if (export_state->it != nullptr) {
    ...
    while (it->Valid() && it->key().ToString().compare(0, seq_id_prefix.size(),
                                                       seq_id_prefix) == 0) {
      if (export_state->include_fields.empty() &&
          export_state->exclude_fields.empty()) {
        res->body += it->value().ToString();
      } else {
        nlohmann::json doc = nlohmann::json::parse(it->value().ToString());
        Collection::prune_doc(doc, export_state->include_fields,
                              export_state->exclude_fields);
        res->body += doc.dump();
      }
      ...
      // append a new line character if there is going to be one more record to
      // send
      if (it->Valid() && it->key().ToString().compare(0, seq_id_prefix.size(),
                                                      seq_id_prefix) == 0) {
        res->body += "\n";
...
bool post_import_documents(const std::shared_ptr<http_req>& req,
                           const std::shared_ptr<http_res>& res) {
  // LOG(INFO) << "Import, req->body_index=" << req->body_index << ", body size:
  // " << req->body.size(); LOG(INFO) << "req->first_chunk=" <<
  // req->first_chunk_aggregate << ", last_chunk=" << req->last_chunk_aggregate;
  ...
  if (req->params.count(DIRTY_VALUES) == 0) {
    req->params[DIRTY_VALUES] =
        "";  // set it empty as default will depend on `index_all_fields`
  }
  ...
  if (!StringUtils::is_uint32_t(req->params[BATCH_SIZE])) {
    res->final = true;
    res->set_400("Parameter `" + std::string(BATCH_SIZE) +
  ...
  if (req->params[ACTION] != "create" && req->params[ACTION] != "update" &&
      req->params[ACTION] != "upsert" && req->params[ACTION] != "emplace") {
    res->final = true;
    res->set_400("Parameter `" + std::string(ACTION) +
                 "` must be a create|update|upsert.");
    stream_response(req, res);
    return false;
  }
  ...
  if (req->params[RETURN_DOC] != "true" && req->params[RETURN_DOC] != "false") {
    res->final = true;
    res->set_400("Parameter `" + std::string(RETURN_DOC) +
                 "` must be a true|false.");
    stream_response(req, res);
    return false;
  }
  ...
  if (req->params[RETURN_ID] != "true" && req->params[RETURN_ID] != "false") {
    res->final = true;
    res->set_400("Parameter `" + std::string(RETURN_ID) +
                 "` must be a true|false.");
    stream_response(req, res);
    return false;
  }
  ...
  const size_t REMOTE_EMBEDDING_BATCH_SIZE_VAL =
      std::stoi(req->params[REMOTE_EMBEDDING_BATCH_SIZE]);
  ...
  if (REMOTE_EMBEDDING_BATCH_SIZE_VAL == 0) {
    res->final = true;
    res->set_400("Parameter `" + std::string(REMOTE_EMBEDDING_BATCH_SIZE) +
                 "` must be a positive integer.");
    stream_response(req, res);
    return false;
  }
  ...
  if (req->body_index == 0) {
    ...
    // LOG(INFO) << "Import, req->body.size=" << req->body.size() << ",
    // batch_size=" << IMPORT_BATCH_SIZE; int nminusten_pos = std::max(0,
    // int(req->body.size())-10); LOG(INFO) << "Last 10 chars: " <<
    // req->body.substr(nminusten_pos);
  }
  ...
  if (collection == nullptr) {
    // LOG(INFO) << "collection == nullptr, for collection: " <<
  ...
  // LOG(INFO) << "Import, " << "req->body_index=" << req->body_index << ",
  // req->body.size: " << req->body.size(); LOG(INFO) << "req body %: " <<
  // (float(req->body_index)/req->body.size())*100;

  std::vector<std::string> json_lines;
  StringUtils::split(req->body, json_lines, "\n", false, false);

  // LOG(INFO) << "json_lines.size before: " << json_lines.size() << ",
  // req->body_index: " << req->body_index;
  ...
  // stream_proceed: " << stream_proceed; LOG(INFO) << "json_lines.size: " <<
  // json_lines.size() << ", req->res_state: " << req->res_state;

  // When only one partial record arrives as a chunk, an empty body is pushed to
  // response stream
  ...
  if (!single_partial_record_body) {
    ...
    const auto& dirty_values =
        collection->parse_dirty_values_option(req->params[DIRTY_VALUES]);
    const bool& return_doc = req->params[RETURN_DOC] == "true";
    const bool& return_id = req->params[RETURN_ID] == "true";
    nlohmann::json json_res = collection->add_many(
        json_lines, document, operation, "", dirty_values, return_doc,
        return_id, REMOTE_EMBEDDING_BATCH_SIZE_VAL);
    // const std::string& import_summary_json = json_res->dump();
    // response_stream << import_summary_json << "\n";

    for (size_t i = 0; i < json_lines.size(); i++) {
      bool res_final =
          req->last_chunk_aggregate && (i == json_lines.size() - 1);
...
bool post_add_document(const std::shared_ptr<http_req>& req,
                       const std::shared_ptr<http_res>& res) {
  const char* ACTION = "action";
  ...
  if (req->params.count(DIRTY_VALUES_PARAM) == 0) {
    req->params[DIRTY_VALUES_PARAM] =
        "";  // set it empty as default will depend on whether schema is enabled
  }
  ...
  const index_operation_t operation = get_index_operation(req->params[ACTION]);
  const auto& dirty_values =
      collection->parse_dirty_values_option(req->params[DIRTY_VALUES_PARAM]);

  nlohmann::json document;
  std::vector<std::string> json_lines = {req->body};
  const nlohmann::json& inserted_doc_op = collection->add_many(
      json_lines, document, operation, "", dirty_values, false, false);
  ...
  if (!inserted_doc_op["success"].get<bool>()) {
    ...
    if (res_doc.count("error") != 0 &&
        !res_doc["error"].get<std::string>().empty()) {
  ...
  res->set_201(
      document.dump(-1, ' ', false, nlohmann::detail::error_handler_t::ignore));
  return true;
}

bool patch_update_document(const std::shared_ptr<http_req>& req,
                           const std::shared_ptr<http_res>& res) {
  std::string doc_id = req->params["id"];
  ...
  const auto& dirty_values =
      collection->parse_dirty_values_option(req->params[DIRTY_VALUES_PARAM]);
  Option<nlohmann::json> upserted_doc_op = collection->add(
      req->body, index_operation_t::UPDATE, doc_id, dirty_values);
...
bool patch_update_documents(const std::shared_ptr<http_req>& req,
                            const std::shared_ptr<http_res>& res) {
  const char* FILTER_BY = "filter_by";
  std::string filter_query;
  if (req->params.count(FILTER_BY) == 0) {
    res->set_400("Parameter `" + std::string(FILTER_BY) +
                 "` must be provided.");
    return false;
  } else {
  ...
  auto update_op = collection->update_matching_filter(
      filter_query, req->body, req->params[DIRTY_VALUES_PARAM]);
  if (update_op.ok()) {
...
bool get_fetch_document(const std::shared_ptr<http_req>& req,
                        const std::shared_ptr<http_res>& res) {
  std::string doc_id = req->params["id"];
...
bool del_remove_document(const std::shared_ptr<http_req>& req,
                         const std::shared_ptr<http_res>& res) {
  std::string doc_id = req->params["id"];
  ...
  res->set_200(
      doc.dump(-1, ' ', false, nlohmann::detail::error_handler_t::ignore));
  return true;
}

bool del_remove_documents(const std::shared_ptr<http_req>& req,
                          const std::shared_ptr<http_res>& res) {
  ...
  if (req->data == nullptr) {
    ...
    deletion_state->index_ids.emplace_back(filter_result.count,
                                           filter_result.docs);
    filter_result.docs = nullptr;
  ...
  bool done = true;
  Option<bool> remove_op =
      stateful_remove_docs(deletion_state, DELETE_BATCH_SIZE, done);

  // LOG(INFO) << "Deletion batch size: " << DELETE_BATCH_SIZE << ", done: " <<
  // done;
...
bool get_aliases(const std::shared_ptr<http_req>& req,
                 const std::shared_ptr<http_res>& res) {
  CollectionManager& collectionManager = CollectionManager::get_instance();
  const spp::sparse_hash_map<std::string, std::string>& symlinks =
      collectionManager.get_symlinks();
...
bool get_alias(const std::shared_ptr<http_req>& req,
               const std::shared_ptr<http_res>& res) {
  const std::string& alias = req->params["alias"];
  CollectionManager& collectionManager = CollectionManager::get_instance();
  Option<std::string> collection_name_op =
      collectionManager.resolve_symlink(alias);
...
bool put_upsert_alias(const std::shared_ptr<http_req>& req,
                      const std::shared_ptr<http_res>& res) {
  nlohmann::json req_json;
  ...
  if (req_json.count(COLLECTION_NAME) == 0) {
    res->set_400(std::string("Parameter `") + COLLECTION_NAME +
                 "` is required.");
    return false;
  }

  Option<bool> success_op =
      collectionManager.upsert_symlink(alias, req_json[COLLECTION_NAME]);
  if (!success_op.ok()) {
...
bool del_alias(const std::shared_ptr<http_req>& req,
               const std::shared_ptr<http_res>& res) {
  const std::string& alias = req->params["alias"];
  CollectionManager& collectionManager = CollectionManager::get_instance();
...
bool get_overrides(const std::shared_ptr<http_req>& req,
                   const std::shared_ptr<http_res>& res) {
  CollectionManager& collectionManager = CollectionManager::get_instance();
  auto collection = collectionManager.get_collection(req->params["collection"]);
  ...
  const std::map<std::string, override_t>& overrides =
      collection->get_overrides();
...
bool get_override(const std::shared_ptr<http_req>& req,
                  const std::shared_ptr<http_res>& res) {
  CollectionManager& collectionManager = CollectionManager::get_instance();
  auto collection = collectionManager.get_collection(req->params["collection"]);
...
bool put_override(const std::shared_ptr<http_req>& req,
                  const std::shared_ptr<http_res>& res) {
  CollectionManager& collectionManager = CollectionManager::get_instance();
  auto collection = collectionManager.get_collection(req->params["collection"]);
...
bool del_override(const std::shared_ptr<http_req>& req,
                  const std::shared_ptr<http_res>& res) {
  CollectionManager& collectionManager = CollectionManager::get_instance();
  auto collection = collectionManager.get_collection(req->params["collection"]);
...
bool get_keys(const std::shared_ptr<http_req>& req,
              const std::shared_ptr<http_res>& res) {
  CollectionManager& collectionManager = CollectionManager::get_instance();
...
bool post_create_key(const std::shared_ptr<http_req>& req,
                     const std::shared_ptr<http_res>& res) {
  ...
  const std::string& rand_key = (req_json.count("value") != 0)
                                    ? req_json["value"].get<std::string>()
                                    : req->metadata;

  api_key_t api_key(rand_key, req_json["description"].get<std::string>(),
...
bool get_key(const std::shared_ptr<http_req>& req,
             const std::shared_ptr<http_res>& res) {
  CollectionManager& collectionManager = CollectionManager::get_instance();
  AuthManager& auth_manager = collectionManager.getAuthManager();
...
bool del_key(const std::shared_ptr<http_req>& req,
             const std::shared_ptr<http_res>& res) {
  CollectionManager& collectionManager = CollectionManager::get_instance();
  AuthManager& auth_manager = collectionManager.getAuthManager();
  ...
  res->set_200(
      res_json.dump(-1, ' ', false, nlohmann::detail::error_handler_t::ignore));
  return true;
}

bool post_snapshot(const std::shared_ptr<http_req>& req,
                   const std::shared_ptr<http_res>& res) {
...
bool post_vote(const std::shared_ptr<http_req>& req,
               const std::shared_ptr<http_res>& res) {
  res->status_code = 200;
  res->content_type_header = "application/json";
...
bool post_config(const std::shared_ptr<http_req>& req,
                 const std::shared_ptr<http_res>& res) {
  nlohmann::json req_json;
...
bool post_clear_cache(const std::shared_ptr<http_req>& req,
                      const std::shared_ptr<http_res>& res) {
  {
    std::unique_lock lock(mutex);
    res_cache.clear();
...
bool post_compact_db(const std::shared_ptr<http_req>& req,
                     const std::shared_ptr<http_res>& res) {
  CollectionManager& collectionManager = CollectionManager::get_instance();
...
bool post_reset_peers(const std::shared_ptr<http_req>& req,
                      const std::shared_ptr<http_res>& res) {
  res->status_code = 200;
  res->content_type_header = "application/json";
...
bool get_synonyms(const std::shared_ptr<http_req>& req,
                  const std::shared_ptr<http_res>& res) {
  CollectionManager& collectionManager = CollectionManager::get_instance();
  auto collection = collectionManager.get_collection(req->params["collection"]);
...
bool get_synonym(const std::shared_ptr<http_req>& req,
                 const std::shared_ptr<http_res>& res) {
  CollectionManager& collectionManager = CollectionManager::get_instance();
  auto collection = collectionManager.get_collection(req->params["collection"]);
...
bool put_synonym(const std::shared_ptr<http_req>& req,
                 const std::shared_ptr<http_res>& res) {
  CollectionManager& collectionManager = CollectionManager::get_instance();
  auto collection = collectionManager.get_collection(req->params["collection"]);
  ...
  // These checks should be inside `add_synonym` but older versions of Typesense
  // wrongly persisted `root` as an array, so we have to do it here so that
  // on-disk synonyms are loaded properly
...
bool del_synonym(const std::shared_ptr<http_req>& req,
                 const std::shared_ptr<http_res>& res) {
  CollectionManager& collectionManager = CollectionManager::get_instance();
  auto collection = collectionManager.get_collection(req->params["collection"]);
...
bool is_doc_write_route(uint64_t route_hash) {
  route_path* rpath;
  bool found = server->get_route(route_hash, &rpath);
  return found && (rpath->handler == post_add_document ||
                   rpath->handler == patch_update_document);
}

bool is_doc_del_route(uint64_t route_hash) {
  route_path* rpath;
  bool found = server->get_route(route_hash, &rpath);
  return found && (rpath->handler == del_remove_document ||
                   rpath->handler == del_remove_documents);
}

bool get_presets(const std::shared_ptr<http_req>& req,
                 const std::shared_ptr<http_res>& res) {
  CollectionManager& collectionManager = CollectionManager::get_instance();
  const spp::sparse_hash_map<std::string, nlohmann::json>& presets =
      collectionManager.get_presets();
  nlohmann::json res_json = nlohmann::json::object();
...
bool get_preset(const std::shared_ptr<http_req>& req,
                const std::shared_ptr<http_res>& res) {
...
bool put_upsert_preset(const std::shared_ptr<http_req>& req,
                       const std::shared_ptr<http_res>& res) {
  nlohmann::json req_json;
  ...
  Option<bool> success_op =
      collectionManager.upsert_preset(preset_name, req_json[PRESET_VALUE]);
  if (!success_op.ok()) {
    res->set_500(success_op.error());
    return false;
  }
...
bool del_preset(const std::shared_ptr<http_req>& req,
                const std::shared_ptr<http_res>& res) {
  const std::string& preset_name = req->params["name"];
  CollectionManager& collectionManager = CollectionManager::get_instance();
...
bool get_active_throttles(const std::shared_ptr<http_req>& req,
                          const std::shared_ptr<http_res>& res) {
  RateLimitManager* rateLimitManager = RateLimitManager::getInstance();
...
bool del_throttle(const std::shared_ptr<http_req>& req,
                  const std::shared_ptr<http_res>& res) {
  RateLimitManager* rateLimitManager = RateLimitManager::getInstance();
  if (!StringUtils::is_uint32_t(req->params["id"])) {
    res->set_400("{\"message\": \"Invalid ID\"}");
...
bool del_exceed(const std::shared_ptr<http_req>& req,
                const std::shared_ptr<http_res>& res) {
  RateLimitManager* rateLimitManager = RateLimitManager::getInstance();
  if (!StringUtils::is_uint32_t(req->params["id"])) {
    res->set_400("{\"message\": \"Invalid ID\"}");
...
bool get_limit_exceed_counts(const std::shared_ptr<http_req>& req,
                             const std::shared_ptr<http_res>& res) {
  RateLimitManager* rateLimitManager = RateLimitManager::getInstance();
...
Option<std::pair<std::string, std::string>> get_api_key_and_ip(
  ...
  if (metadata.size() < std::stoul(metadata.substr(0, metadata.find(":"))) +
                            metadata.find(":") + 7) {
    return Option<std::pair<std::string, std::string>>(400, "Invalid metadata");
  }

  uint32_t api_key_length =
      static_cast<uint32_t>(std::stoul(metadata.substr(0, metadata.find(":"))));
  ...
  std::regex ip_pattern(
      "\\b(([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\\.){3}([0-9]|[1-9]["
      "0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\\b");
  if (!std::regex_match(ip, ip_pattern)) {
    return Option<std::pair<std::string, std::string>>(400, "Invalid metadata");
  }

  return Option<std::pair<std::string, std::string>>(
      std::make_pair(api_key, ip));
}

bool post_create_event(const std::shared_ptr<http_req>& req,
                       const std::shared_ptr<http_res>& res) {
  nlohmann::json req_json;
...
bool get_analytics_rules(const std::shared_ptr<http_req>& req,
                         const std::shared_ptr<http_res>& res) {
...
bool get_analytics_rule(const std::shared_ptr<http_req>& req,
                        const std::shared_ptr<http_res>& res) {
  auto rules_op =
      AnalyticsManager::get_instance().get_rule(req->params["name"]);
...
bool post_create_analytics_rules(const std::shared_ptr<http_req>& req,
                                 const std::shared_ptr<http_res>& res) {
  nlohmann::json req_json;
...
bool put_upsert_analytics_rules(const std::shared_ptr<http_req>& req,
                                const std::shared_ptr<http_res>& res) {
  nlohmann::json req_json;
...
bool del_analytics_rules(const std::shared_ptr<http_req>& req,
                         const std::shared_ptr<http_res>& res) {
...
bool post_proxy(const std::shared_ptr<http_req>& req,
                const std::shared_ptr<http_res>& res) {
  ...
  if (!req_json["url"].is_string() || !req_json["method"].is_string() ||
      req_json["url"].get<std::string>().empty() ||
      req_json["method"].get<std::string>().empty()) {
