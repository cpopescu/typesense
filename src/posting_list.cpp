...
uint32_t posting_list_t::block_t::upsert(
    const uint32_t id, const std::vector<uint32_t>& positions) {
  if (id > ids.last() || ids.getLength() == 0) {
    ...
    if (id_index == ids.getLength()) {
      ...
      uint32_t end_offset_index = (id == ids.last())
                                      ? offsets.getLength() - 1
                                      : offset_index.at(id_index + 1) - 1;
      ...
      if (num_offsets == positions.size()) {
        // no need to touch the offset index and need to just do inplace updates
        // of offsets
        bool find_new_min_max = false;
        for (size_t i = 0; i < positions.size(); i++) {
          if ((curr_offsets[start_offset_index + i] == m ||
               curr_offsets[start_offset_index + i] == M) &&
              curr_offsets[start_offset_index + i] != positions[i]) {
            // when an existing min/max is affected we will have to find the new
            // min/max
        ...
        int64_t size_diff = int64_t(positions.size()) -
                            num_offsets;  // size_diff can be negative
        size_t new_offsets_length = offsets.getLength() + size_diff;
        uint32_t* new_offsets = new uint32_t[new_offsets_length];
        std::memmove(new_offsets, curr_offsets,
                     sizeof(uint32_t) * start_offset_index);
        ...
        std::memmove(
            new_offsets + start_offset_index + positions.size(),
...
void posting_list_t::block_t::remove_and_shift_offset_index(
    const uint32_t* indices_sorted, const uint32_t num_indices) {
  ...
  while (curr_index < offset_index.getLength()) {
    if (indices_counter < num_indices &&
        curr_index >= indices_sorted[indices_counter]) {
      // skip copying
      if (curr_index == indices_sorted[indices_counter]) {
        curr_index++;
        const uint32_t diff = curr_index == offset_index.getLength()
                                  ? 0
                                  : (offset_index.at(curr_index) -
                                     offset_index.at(curr_index - 1));
...
void posting_list_t::block_t::insert_and_shift_offset_index(
    const uint32_t index, const uint32_t num_offsets) {
  ...
  memmove(&curr_array[index + 1], &curr_array[index],
          sizeof(uint32_t) * (length - index));
...
bool posting_list_t::block_t::contains(uint32_t id) { return ids.contains(id); }

/* posting_list_t operations */

posting_list_t::posting_list_t(uint16_t max_block_elements)
    : BLOCK_MAX_ELEMENTS(max_block_elements) {
...
void posting_list_t::merge_adjacent_blocks(posting_list_t::block_t* block1,
  ...
  std::memmove(new_ids + block1->size(), ids2,
               sizeof(uint32_t) * num_block2_ids_to_move);

  block1->ids.load(new_ids, block1->size() + num_block2_ids_to_move);
  if (block2->size() != num_block2_ids_to_move) {
    block2->ids.load(ids2 + num_block2_ids_to_move,
                     block2->size() - num_block2_ids_to_move);
  } else {
  ...
  uint32_t* new_offset_index =
      new uint32_t[block1_orig_size + block2_orig_size];

  size_t num_block2_offsets_to_move =
      (num_block2_ids_to_move == block2_orig_size)
          ? block2->offsets.getLength()
          : offset_index2[num_block2_ids_to_move];

  std::memmove(new_offset_index, offset_index1,
               sizeof(uint32_t) * block1->offset_index.getLength());
  ...
  block1->offset_index.load(new_offset_index, block1->offset_index.getLength() +
                                                  num_block2_ids_to_move);

  if (block2->offset_index.getLength() != num_block2_ids_to_move) {
    const uint32_t offset_index2_base_index =
        offset_index2[num_block2_ids_to_move];
    ...
    block2->offset_index.load(offset_index2 + num_block2_ids_to_move,
                              block2_orig_size - num_block2_ids_to_move);
  } else {
  ...
  size_t new_block1_offsets_size =
      block1->offsets.getLength() + num_block2_offsets_to_move;
  ...
  if (block2->offsets.getLength() != num_block2_offsets_to_move) {
    const size_t block2_new_offsets_length =
        (block2->offsets.getLength() - num_block2_offsets_to_move);
    ...
    block2->offsets.load(block2_new_raw_offsets, block2_new_offsets_length, min,
                         max);
  ...
  if (block1->offsets.getLength() < block1->offset_index.getLength()) {
    LOG(ERROR) << "Block offset length is smaller than offset index length "
                  "after merging.";
  }
...
void posting_list_t::split_block(posting_list_t::block_t* src_block,
                                 posting_list_t::block_t* dst_block) {
  if (src_block->size() <= 1) {
    return;
  }
  ...
  size_t offset_indices_first_half_length =
      (src_block->offset_index.getLength() / 2);
  size_t offset_indices_second_half_length =
      (src_block->offset_index.getLength() - offset_indices_first_half_length);
  src_block->offset_index.load(raw_offset_indices,
                               offset_indices_first_half_length);

  // update second half to use zero based index
  uint32_t base_index_diff =
      raw_offset_indices[offset_indices_first_half_length];
  ...
  dst_block->offset_index.load(
      raw_offset_indices + offset_indices_first_half_length,
      offset_indices_second_half_length);
  ...
  size_t offsets_second_half_length =
      src_offsets_length - offset_first_half_length;
  dst_block->offsets.load(raw_offsets + offset_first_half_length,
                          offsets_second_half_length, min, max);
...
void posting_list_t::upsert(const uint32_t id,
  ...
  if (id_block_map.empty()) {
    ...
    upsert_block =
        (it == id_block_map.end()) ? id_block_map.rbegin()->second : it->second;
  ...
  if (upsert_block->size() < BLOCK_MAX_ELEMENTS) {
    ...
    if (upsert_block->next == nullptr && upsert_block->ids.last() < id) {
      // appending to the end of the last block where the id will reside on a
      // newly block
...
void posting_list_t::erase(const uint32_t id) {
  ...
  if (new_ids_length == 0) {
    ...
    if (erase_block != &root_block) {
      // since we will be deleting the empty node, set the previous node's next
      // pointer to null
      std::prev(it)->second->next = nullptr;
      delete erase_block;
    } else {
      // The root block cannot be empty if there are other blocks so we will
      // pull some contents from next block This is only an issue for blocks
      // with max size of 2
      if (root_block.next != nullptr) {
        auto next_block_last_id = erase_block->next->ids.last();
        merge_adjacent_blocks(erase_block, erase_block->next,
                              erase_block->next->size() / 2);
  ...
  if (new_ids_length >= BLOCK_MAX_ELEMENTS / 2 ||
      erase_block->next == nullptr) {
  ...
  // block is less than 50% of max capacity and contains a next node which we
  // can refill from
  ...
  if (erase_block->size() + next_block->size() <= BLOCK_MAX_ELEMENTS) {
    // we can merge the contents of next block with `erase_block` and delete the
    // next block
    ...
    // We will move only 50% of max elements to ensure that we don't end up
    // "flipping" adjacent blocks: 1, 5 -> 5, 1
    size_t num_block2_ids = BLOCK_MAX_ELEMENTS / 2;
    merge_adjacent_blocks(erase_block, next_block, num_block2_ids);
    // NOTE: we don't have to update `id_block_map` for `next_block` as last
...
posting_list_t::block_t* posting_list_t::get_root() { return &root_block; }

size_t posting_list_t::num_blocks() const { return id_block_map.size(); }
...
void posting_list_t::merge(const std::vector<posting_list_t*>& posting_lists,
...
bool posting_list_t::take_id(result_iter_state_t& istate, uint32_t id) {
  ...
  if (istate.filter_ids_length != 0) {
    return std::binary_search(istate.filter_ids,
                              istate.filter_ids + istate.filter_ids_length, id);
  }

  return true;
}

bool posting_list_t::get_offsets(
  ...
  // Array string format:
  // offset1, ... , offsetn, offsetn, array_index, 0 (if token is the last
  // offset for the document) NOTE 1: last offset is repeated to indicate end of
  // offsets for a given array index) NOTE 2: offsets are 1-index based (since 0
  // is used as last offset marking)
  ...
  for (size_t j = 0; j < its.size(); j++) {
    ...
    /*LOG(INFO) << "id: " << its[j].id() << ", start_offset: " << start_offset
    << ", end_offset: " << end_offset; for(size_t x = 0; x < end_offset; x++) {
    ...
    while (start_offset < end_offset) {
      ...
      if (pos == prev_pos) {  // indicates end of array index
        if (!positions.empty()) {
          ...
          array_token_pos[array_index].push_back(
              token_positions_t{is_last_token, positions});
          positions.clear();
        }

        start_offset++;  // skip current value which is the array index or flag
                         // for last index
        prev_pos = -1;
        continue;
      }
...
bool posting_list_t::is_single_token_verbatim_match(
    const posting_list_t::iterator_t& it, bool field_is_array) {
  ...
  if (field_is_array) {
    ...
    while (start_offset < end_offset) {
      ...
      if (pos == prev_pos && pos == 1 && start_offset + 1 < end_offset &&
          offsets[start_offset + 1] == 0) {
        return true;
      }
...
bool posting_list_t::at_end(
    const std::vector<posting_list_t::iterator_t>& its) {
...
posting_list_t::iterator_t posting_list_t::new_iterator(block_t* start_block,
                                                        block_t* end_block,
                                                        uint32_t field_id) {
  start_block = (start_block == nullptr) ? &root_block : start_block;
  return posting_list_t::iterator_t(&id_block_map, start_block, end_block, true,
                                    field_id);
}
...
void posting_list_t::advance_all2(
    std::vector<posting_list_t::iterator_t>& its) {
  its[0].next();
  its[1].next();
}

void posting_list_t::advance_non_largest(
    std::vector<posting_list_t::iterator_t>& its) {
  // we will find the iter with greatest value and then advance the rest until
  // their value catches up
...
void posting_list_t::advance_non_largest2(
    std::vector<posting_list_t::iterator_t>& its) {
...
uint32_t posting_list_t::advance_smallest(
    std::vector<posting_list_t::iterator_t>& its) {
  // we will advance the iterator(s) with the smallest value and then return
  // that value
...
uint32_t posting_list_t::advance_smallest2(
    std::vector<posting_list_t::iterator_t>& its) {
...
size_t posting_list_t::num_ids() const { return ids_length; }
...
bool posting_list_t::contains_atleast_one(const uint32_t* target_ids,
                                          size_t target_ids_size) {
  ...
  while (target_ids_index < target_ids_size && it.valid()) {
    ...
    if (id == target_ids[target_ids_index]) {
      ...
      if (id > target_ids[target_ids_index]) {
        while (target_ids_index < target_ids_size &&
               target_ids[target_ids_index] < id) {
          target_ids_index++;
        }
      } else {
...
void posting_list_t::get_exact_matches(std::vector<iterator_t>& its,
                                       const bool field_is_array,
                                       const uint32_t* ids,
                                       const uint32_t num_ids,
                                       uint32_t*& exact_ids,
                                       size_t& num_exact_ids) {
  ...
  if (its.size() == 1) {
    ...
    if (!field_is_array) {
        ...
        for (int j = its.size() - 1; j >= 0; j--) {
          ...
          if (j == its.size() - 1) {
            ...
            if (offsets[end_offset_index - 1] != 0 ||
                (end_offset_index - 2 >= 0 &&
                 offsets[end_offset_index - 2] != its.size())) {
          ...
          // looping handles duplicate query tokens, e.g. "hip hip hurray
          // hurray"
      ...
      for (size_t i = 0; i < num_ids; i++) {
        ...
        for (int j = its.size() - 1; j >= 0; j--) {
          ...
          while (start_offset_index < end_offset_index) {
            ...
            if (pos == prev_pos) {  // indicates end of array index
              ...
              start_offset_index++;  // skip current value which is the array
                                     // index or flag for last index
              prev_pos = -1;
          ...
          // check if the last query token is the last offset of ANY array
          // element
          ...
          if (num_matching_index == 0) {
            // not even a single matching index found: can never be an exact
            // match
            premature_exit = true;
            break;
          }
        ...
        if (!premature_exit) {
          // iterate array index to token index to check if atleast 1 array
          // position contains all tokens
          for (auto& kv : array_index_to_token_index) {
            if (kv.second.token_index.count() == its.size() &&
                kv.second.has_last_token) {
              exact_ids[exact_id_index++] = id;
              break;
            }
          }
...
bool posting_list_t::found_token_sequence(
    const std::vector<token_positions_t>& token_positions,
    const size_t token_index, const uint16_t target_pos) {
  if (token_index == token_positions.size()) {
    return true;
  }

  // iterate through the positions and see if `target_pos` is found in token
  // positions
...
bool posting_list_t::has_phrase_match(
    const std::vector<token_positions_t>& token_positions) {
...
void posting_list_t::get_phrase_matches(
    std::vector<iterator_t>& its, bool field_is_array, const uint32_t* ids,
  ...
  if (its.size() == 1) {
    ...
    for (size_t i = 0; i < num_ids; i++) {
      ...
      for (auto& kv : array_token_positions) {
        ...
        if (token_positions.size() == its.size() &&
            has_phrase_match(token_positions)) {
          phrase_ids[phrase_id_index] = ids[i];
          phrase_id_index++;
          break;
        }
...
void posting_list_t::get_matching_array_indices(uint32_t id,
  ...
  for (int j = its.size() - 1; j >= 0; j--) {
    ...
    while (start_offset_index < end_offset_index) {
      ...
      if (pos == prev_pos) {  // indicates end of array index
        ...
        start_offset_index++;  // skip current value which is the array index or
                               // flag for last index
        prev_pos = -1;
        continue;
      }
  ...
  // iterate array index to token index to check if atleast 1 array position
  // contains all tokens
  for (auto& kv : array_index_to_token_index) {
...
bool posting_list_t::all_ended2(
    const std::vector<posting_list_t::iterator_t>& its) {
  // if both iterators are at end, we return true
  return !its[0].valid() && !its[1].valid();
}

size_t posting_list_t::get_last_offset(const posting_list_t::iterator_t& it,
                                       bool field_is_array) {
  block_t* curr_block = it.block();
  uint32_t curr_index = it.index();
  uint32_t* offsets = it.offsets;
  ...
  if (field_is_array) {
    ...
    return offsets[end_offset - 1] == 0 ? offsets[end_offset - 2]
                                        : offsets[end_offset - 1];
  }
...
uint32_t posting_list_t::iterator_t::id() const { return ids[curr_index]; }

uint32_t posting_list_t::iterator_t::index() const { return curr_index; }
...
posting_list_t::iterator_t& posting_list_t::iterator_t::operator=(
    posting_list_t::iterator_t&& rhs) noexcept {
  id_block_map = rhs.id_block_map;
  curr_block = rhs.curr_block;
  curr_index = rhs.curr_index;
  end_block = rhs.end_block;
...
uint32_t posting_list_t::iterator_t::get_field_id() const { return field_id; }
