...
int ReplicationState::start(const butil::EndPoint& peering_endpoint,
                            const int api_port, int election_timeout_ms,
  ...
  while (true) {
    std::string actual_nodes_config =
        to_nodes_config(peering_endpoint, api_port, nodes);
  ...
  // do snapshot only when the gap between applied index and last snapshot index
  // is >= this number
  ...
  // automatic snapshot is disabled since it caused issues during slow follower
  // catch-ups
  ...
  braft::Node* node = new braft::Node(
      "default_group", braft::PeerId(peering_endpoint, api_port));

  std::string snapshot_dir = raft_dir + "/" + snapshot_dir_name;
  bool snapshot_exists = dir_enum_count(snapshot_dir) > 0;

  if (snapshot_exists) {
    // we will be assured of on_snapshot_load() firing and we will wait for that
    // to init_db()
  } else {
    LOG(INFO)
...
std::string ReplicationState::to_nodes_config(
    const butil::EndPoint& peering_endpoint, const int api_port,
    const std::string& nodes_config) {
  if (nodes_config.empty()) {
    std::string ip_str = butil::ip2str(peering_endpoint.ip).c_str();
    return ip_str + ":" + std::to_string(peering_endpoint.port) + ":" +
...
string ReplicationState::resolve_node_hosts(const string& nodes_config) {
  ...
  for (const auto& node_str : node_strings) {
    ...
    if (status == 0) {
      final_nodes_vec.push_back(std::string(butil::ip2str(ip).c_str()) + ":" +
...
void ReplicationState::write(const std::shared_ptr<http_req>& request,
  ...
  // Serialize request to replicated WAL so that all the nodes in the group
  // receive it as well. NOTE: actual write must be done only on the `on_apply`
...
void ReplicationState::write_to_leader(
  ...
  if (!node || node->leader_id().is_empty()) {
    ...
    if (response->proxied_stream) {
      // streaming in progress: ensure graceful termination (cannot start
      // response again)
  ...
  if (response->proxied_stream) {
    ...
    // LOG(INFO) << "Inflight proxied request, returning control to caller,
  ...
  const std::string& scheme =
      std::string(raw_req->scheme->name.base, raw_req->scheme->name.len);
  ...
  thread_pool->enqueue([request, response, server, path, url, this]() {
    ...
    if (request->http_method == "POST") {
      ...
      if (path_parts.back().rfind("import", 0) == 0) {
        ...
        std::string api_res;
        long status = HttpClient::post_response(
            url, request->body, api_res, res_headers, {}, 10 * 1000, true);
        response->content_type_header = res_headers["content-type"];
      ...
      long status =
          HttpClient::delete_response(url, api_res, res_headers, 0, true);
      response->content_type_header = res_headers["content-type"];
      response->set_body(status, api_res);
      ...
      long status = HttpClient::patch_response(url, request->body, api_res,
                                               res_headers, timeout_ms, true);
      response->content_type_header = res_headers["content-type"];
      response->set_body(status, api_res);
    } else {
      const std::string& err =
          "Forwarding for http method not implemented: " + request->http_method;
      LOG(ERROR) << err;
...
std::string ReplicationState::get_node_url_path(
    const std::string& node_addr, const std::string& path,
...
void ReplicationState::on_apply(braft::Iterator& iter) {
  ...
  //  NOTE: this is executed on a different thread and runs concurrent to http
  //  thread A batch of tasks are committed, which must be processed through
  //  |iter|
  for (; iter.valid(); iter.next()) {
    // Guard invokes replication_arg->done->Run() asynchronously to avoid the
    // callback blocking the main thread
    braft::AsyncClosureGuard closure_guard(iter.done());

    // LOG(INFO) << "Apply entry";

    const std::shared_ptr<http_req>& request_generated =
        iter.done()
            ? dynamic_cast<ReplicationClosure*>(iter.done())->get_request()
            : std::make_shared<http_req>();

    // LOG(INFO) << "Post assignment " << request_generated.get() << ", use
    // count: " << request_generated.use_count();

    const std::shared_ptr<http_res>& response_generated =
        iter.done()
    ...
    // To avoid blocking the serial Raft write thread persist the log entry in
    // local storage. Actual operations will be done in collection-sharded batch
    // indexing threads.
...
void* ReplicationState::save_snapshot(void* arg) {
  ...
  for (butil::FilePath file = dir_enum.Next(); !file.empty();
       file = dir_enum.Next()) {
    std::string file_name =
        std::string(db_snapshot_name) + "/" + file.BaseName().value();
  ...
  // if an external snapshot is requested, copy latest snapshot directory into
  // that
  if (!sa->ext_snapshot_path.empty()) {
    // temp directory will be moved to final snapshot directory, so let's wait
    // for that to happen
    while (butil::DirectoryExists(butil::FilePath(temp_snapshot_dir))) {
      std::this_thread::sleep_for(std::chrono::milliseconds(100));
    }

    LOG(INFO) << "Copying system snapshot to external snapshot directory at "
              << sa->ext_snapshot_path;

    const butil::FilePath& dest_state_dir =
        butil::FilePath(sa->ext_snapshot_path + "/state");

    if (!butil::DirectoryExists(dest_state_dir)) {
      butil::CreateDirectory(dest_state_dir, true);
    }

    const butil::FilePath& src_snapshot_dir =
        butil::FilePath(sa->state_dir_path + "/snapshot");
    const butil::FilePath& src_meta_dir =
        butil::FilePath(sa->state_dir_path + "/meta");

    bool snapshot_copied =
    ...
    sa->replication_state->ext_snapshot_succeeded =
        snapshot_copied && meta_copied;

    // notify on demand closure that external snapshotting is done
    sa->replication_state->notify();
  }

  // NOTE: *must* do a dummy write here since snapshots cannot be triggered if
  // no write has happened since the last snapshot. By doing a dummy write right
  // after a snapshot, we ensure that this can never be the case.
...
// this method is serial to on_apply so guarantees a snapshot view of the state
// machine
void ReplicationState::on_snapshot_save(braft::SnapshotWriter* writer,
                                        braft::Closure* done) {
...

  {
    ...
    store->insert(CollectionManager::BATCHED_INDEXER_STATE_KEY,
                  batch_index_state.dump());
    ...
    rocksdb::Status status =
        store->create_check_point(&checkpoint, db_snapshot_path);
    std::unique_ptr<rocksdb::Checkpoint> checkpoint_guard(checkpoint);

    if (!status.ok()) {
      LOG(ERROR) << "Failure during checkpoint creation, msg:"
                 << status.ToString();
  ...
  // Start a new bthread to avoid blocking StateMachine for slower operations
  // that don't need a blocking view
  bthread_t tid;
...
int ReplicationState::init_db() {
  ...
  if (init_op.ok()) {
    ...
    LOG(ERROR) << "Typesense failed to start. "
               << "Could not load collections from disk: " << init_op.error();
    return 1;
  }
...
int ReplicationState::on_snapshot_load(braft::SnapshotReader* reader) {
  std::shared_lock lock(node_mutex);
  CHECK(!node || !node->is_leader())
      << "Leader is not supposed to load snapshot";
  lock.unlock();

  LOG(INFO) << "on_snapshot_load";

  // ensures that reads and writes are rejected, as `store->reload()` unique
  // locks the DB handle
...
void ReplicationState::refresh_nodes(
    const std::string& nodes, const size_t raft_counter,
  ...
  if (node->is_leader()) {
    ...
    if (node->leader_id().is_empty()) {
      // When node is not a leader, does not have a leader and is also a
      // single-node cluster, we forcefully reset its peers. NOTE:
      // `reset_peers()` is not a safe call to make as we give up on consistency
      // and consensus guarantees. We are doing this solely to handle single
      // node cluster whose IP changes. Examples: Docker container IP change,
      // local DHCP leased IP change etc.

      std::vector<braft::PeerId> latest_nodes;
      new_conf.list_peers(&latest_nodes);

      if (latest_nodes.size() == 1 ||
          (raft_counter > 0 && reset_peers_on_error)) {
        LOG(WARNING) << "Node with no leader. Resetting peers of size: "
                     << latest_nodes.size();
...
void ReplicationState::refresh_catchup_status(bool log_msg) {
  ...
  // `known_applied_index` guaranteed to be atleast 1 if raft log is available
  // (after snapshot loading etc.)
  ...
  // work around for:
  // https://github.com/baidu/braft/issues/277#issuecomment-823080171
  int64_t current_index = (n_status.applying_index == 0)
                              ? n_status.known_applied_index
                              : n_status.applying_index;
  int64_t apply_lag = n_status.last_index - current_index;

  // in addition to raft level lag, we should also account for internal batched
  // write queue
  int64_t num_queued_writes = batched_indexer->get_queued_writes();

  // LOG(INFO) << "last_index: " << n_status.applying_index << ",
  // known_applied_index: " << n_status.known_applied_index; LOG(INFO) <<
  // "apply_lag: " << apply_lag;
  ...
  if (apply_lag > healthy_read_lag) {
    LOG_IF(ERROR, log_msg) << apply_lag
                           << " lagging entries > healthy read lag of "
                           << healthy_read_lag;
    this->read_caught_up = false;
  } else {
    if (num_queued_writes > healthy_read_lag) {
      LOG_IF(ERROR, log_msg)
          << num_queued_writes << " queued writes > healthy read lag of "
          << healthy_read_lag;
      this->read_caught_up = false;
    } else {
  ...
  if (apply_lag > healthy_write_lag) {
    LOG_IF(ERROR, log_msg) << apply_lag
                           << " lagging entries > healthy write lag of "
                           << healthy_write_lag;
    this->write_caught_up = false;
  } else {
    if (num_queued_writes > healthy_write_lag) {
      LOG_IF(ERROR, log_msg)
          << num_queued_writes << " queued writes > healthy write lag of "
          << healthy_write_lag;
      this->write_caught_up = false;
    } else {
  ...
  if (node->leader_id().is_empty()) {
    LOG(ERROR)
        << "Could not get leader status, as node does not have a leader!";
    return;
  }
  ...
  std::map<std::string, std::string> res_headers;
  long status_code =
      HttpClient::get_response(url, api_res, res_headers, {}, 5 * 1000, true);
  if (status_code == 200) {
    ...
    if (leader_status.contains("committed_index")) {
      int64_t leader_committed_index =
          leader_status["committed_index"].get<int64_t>();
      ...
      this->read_caught_up = ((leader_committed_index -
                               n_status.committed_index) < healthy_read_lag);
    } else {
      // we will refrain from changing current status
      LOG(ERROR) << "Error, `committed_index` key not found in /status "
                    "response from leader.";
    }
  } else {
    // we will again refrain from changing current status
    LOG(ERROR) << "Error, /status end-point returned bad status code "
               << status_code;
  }
}
...
bool ReplicationState::is_alive() const {
  // for general health check we will only care about the `read_caught_up`
  // threshold
...
void ReplicationState::do_snapshot(const std::string& snapshot_path,
  ...
  thread_pool->enqueue([&snapshot_path, req, res, this]() {
    OnDemandSnapshotClosure* snapshot_closure =
...
void ReplicationState::do_dummy_write() {
  ...
  std::map<std::string, std::string> res_headers;
  long status_code =
      HttpClient::post_response(url, "", api_res, res_headers, {}, 4000, true);
...
bool ReplicationState::trigger_vote() {
  ...
  if (node) {
    ...
    LOG(INFO) << "Triggered vote. Ok? " << status.ok()
              << ", status: " << status;
    return status.ok();
  }
...
bool ReplicationState::reset_peers() {
  ...
  if (node) {
    const Option<std::string>& refreshed_nodes_op =
        Config::fetch_nodes_config(config->get_nodes());
    if (!refreshed_nodes_op.ok()) {
      LOG(WARNING) << "Error while fetching peer configuration: "
                   << refreshed_nodes_op.error();
      return false;
    }

    const std::string& nodes_config = ReplicationState::to_nodes_config(
        peering_endpoint, Config::get_instance().get_api_port(),
...
Store* ReplicationState::get_store() { return store; }
...
void ReplicationState::shutdown() {
  ...
  LOG(INFO) << "Replication state shutdown, store sequence: "
...
void ReplicationState::do_snapshot(const std::string& nodes) {
  ...
  if (is_leader()) {
    ...
    for (const auto& peer : peers) {
      ...
      if (!peer_healthy) {
        LOG(WARNING) << "Peer " << peer_addr
                     << " reported unhealthy during snapshot pre-check.";
      }
    ...
    if (!all_peers_healthy) {
      LOG(WARNING) << "Unable to trigger snapshot as one or more of the peers "
                      "reported unhealthy.";
      return;
    }
  }
...
void TimedSnapshotClosure::Run() {
  ...
  if (status().ok()) {
    ...
    LOG(ERROR) << "Timed snapshot failed, error: " << status().error_str()
...
void OnDemandSnapshotClosure::Run() {
  ...
  replication_state->get_message_dispatcher()->send_message(
