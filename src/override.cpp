...
Option<bool> override_t::parse(const nlohmann::json& override_json,
  ...
  if (override_json["rule"].count("filter_by") == 0 &&
      (override_json["rule"].count("query") == 0 ||
       override_json["rule"].count("match") == 0)) {
    return Option<bool>(
        400, "The `rule` definition must contain a `query` and `match`.");
  }

  if (override_json.count("includes") == 0 &&
      override_json.count("excludes") == 0 &&
      override_json.count("filter_by") == 0 &&
      override_json.count("sort_by") == 0 &&
  ...
  if (override_json.count("includes") != 0) {
    ...
    for (const auto& include_obj : override_json["includes"]) {
      if (!include_obj.is_object()) {
        return Option<bool>(
            400, "The `includes` value must be an array of objects.");
      }
  ...
  if (override_json.count("excludes") != 0) {
    ...
    for (const auto& exclude_obj : override_json["excludes"]) {
      if (!exclude_obj.is_object()) {
        return Option<bool>(
            400, "The `excludes` value must be an array of objects.");
      }
  ...
  if (override_json.count("remove_matched_tokens") != 0) {
    if (!override_json["remove_matched_tokens"].is_boolean()) {
      return Option<bool>(400,
                          "The `remove_matched_tokens` must be a boolean.");
    }
  }
  ...
  override.rule.query = json_rule.count("query") == 0
                            ? ""
                            : json_rule["query"].get<std::string>();
  override.rule.match = json_rule.count("match") == 0
                            ? ""
                            : json_rule["match"].get<std::string>();
  ...
  if (!override.rule.query.empty()) {
    ...
    symbols.push_back('}');
    symbols.push_back('*');
    Tokenizer tokenizer(override.rule.query, true, false, locale, symbols,
                        token_separators);
  ...
  if (json_rule.count("filter_by") != 0) {
    ...
    override.rule.filter_by =
        override_json["rule"]["filter_by"].get<std::string>();
  }
  ...
  if (override_json.count("replace_query") != 0) {
    if (override_json.count("remove_matched_tokens") != 0 &&
        override_json["remove_matched_tokens"].get<bool>()) {
      return Option<bool>(400,
                          "Only one of `replace_query` or "
                          "`remove_matched_tokens` can be specified.");
    }
  ...
  if (override_json.count("filter_curated_hits") != 0) {
    override.filter_curated_hits =
        override_json["filter_curated_hits"].get<bool>();
  }
  ...
  if (override_json.count("effective_from_ts") != 0) {
    override.effective_from_ts =
        override_json["effective_from_ts"].get<int64_t>();
  }
    ...
    if (override.rule.normalized_query[i] == '{') {
      ...
      while (i < override.rule.normalized_query.size()) {
        if (override.rule.normalized_query[i] == '}') {
          ...
          override.rule.normalized_query =
