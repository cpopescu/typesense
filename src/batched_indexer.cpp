...
BatchedIndexer::BatchedIndexer(HttpServer* server, Store* store,
                               Store* meta_store, const size_t num_threads,
                               const Config& config,
...
void BatchedIndexer::enqueue(const std::shared_ptr<http_req>& req,
                             const std::shared_ptr<http_res>& res) {
  // Called by the raft write thread: goal is to quickly send the request to a
  // queue and move on NOTE: it's ok to access `req` and `res` in this function
  // without synchronization because the read thread for *this* request is
  // paused now and resumes only messaged at the end
  ...
  const std::string& request_chunk_key =
      req_key_prefix + StringUtils::serialize_uint32_t(chunk_sequence);

  // LOG(INFO) << "request_chunk_key: " << req->start_ts << "_" <<
  // chunk_sequence << ", req body: " << req->body;
  ...
  if (req->last_chunk_aggregate) {
...

    {
      ...
      uint64_t queue_id =
          StringUtils::hash_wy(coll_name.c_str(), coll_name.size()) %
          num_threads;
      req->body = "";
    ...
    // IMPORTANT: must not read `req` variables (except _req) henceforth to
    // prevent data races with indexing thread

    if (is_old_serialized_request) {
      // Indicates a serialized request from a version that did not support
      // batching (v0.21 and below). We can only do serial writes as we cannot
      // reliably distinguish one streaming request from another. So, wait for
      // `req_res_map` to be empty before proceeding
      while (true) {
        {
          std::unique_lock lk(mutex);
  ...
  if (read_more_input) {
    ...
    deferred_req_res_t* req_res =
        new deferred_req_res_t(req, res, server, true);
    server->get_message_dispatcher()->send_message(
        HttpServer::REQUEST_PROCEED_MESSAGE, req_res);
  }
}

std::string BatchedIndexer::get_collection_name(
  ...
  if (coll_name.empty()) {
    ...
    // ensure that collection creation is sent to the same queue as writes to
    // that collection
    if (route_found && rpath->handler == post_create_collection) {
      nlohmann::json obj = nlohmann::json::parse(req->body, nullptr, false);

      if (!obj.is_discarded() && obj.is_object() && obj.count("name") != 0 &&
          obj["name"].is_string()) {
        coll_name = obj["name"];
      }
    }
...
void BatchedIndexer::run() {
  ...
  skip_index_iter =
      meta_store->scan(SKIP_INDICES_PREFIX, skip_index_iter_upper_bound);
  populate_skip_index();
    ...
    thread_pool->enqueue([&queue, &queue_mutex, this, i]() {
      while (!quit) {
        ...
        /*  Format of the key: $RL_reqId_chunkId
            NOTE: we use an explicit `next_chunk_index` so that the reads can
           resume from a partially request.
        */
        const std::string& req_key_start_prefix =
            req_key_prefix +
            StringUtils::serialize_uint32_t(orig_req_res.next_chunk_index);

        const std::string& req_key_upper_bound =
            get_req_suffix_key(req_id);  // cannot inline this
        rocksdb::Slice upper_bound(req_key_upper_bound);
        rocksdb::Iterator* iter =
            store->scan(req_key_start_prefix, &upper_bound);
        ...
        bool route_found =
            server->get_route(orig_req->route_hash, &found_rpath);
        ...
        while (iter->Valid() && iter->key().starts_with(req_key_prefix)) {
          ...
          if (write_log_index == skip_index) {
            ...
            // LOG(INFO) << "index req " << req_id << ", chunk index: " <<
            ...
            if (resource_check != cached_resource_stat_t::OK &&
                orig_req->http_method != "DELETE" &&
                found_rpath->handler != post_health) {
              const std::string& err_msg =
                  "Rejecting write: running out of resource type: " +
                  std::string(magic_enum::enum_name(resource_check));
              LOG(ERROR) << err_msg;
              orig_res->set_422(err_msg);
              orig_res->final = true;
              async_req_res_t* async_req_res =
                  new async_req_res_t(orig_req, orig_res, true);
              server->get_message_dispatcher()->send_message(
                  HttpServer::STREAM_RESPONSE_MESSAGE, async_req_res);
              goto end;
            }
        ...
        // LOG(INFO) << "Erasing request data from disk and memory for request "
        // << req_id;
  ...
  while (!quit) {
    ...
    uint64_t seconds_elapsed =
        std::chrono::duration_cast<std::chrono::seconds>(
            std::chrono::high_resolution_clock::now() - last_gc_run)
            .count();

    if (seconds_elapsed > GC_INTERVAL_SECONDS) {
      std::unique_lock lk(mutex);
      LOG(INFO) << "Running GC for aborted requests, req map size: "
                << req_res_map.size();

      // iterate through all map entries and delete ones which are not complete
      // but > GC_PRUNE_MAX_SECONDS
      for (auto it = req_res_map.cbegin(); it != req_res_map.cend();) {
        uint64_t seconds_since_batch_update =
            std::chrono::duration_cast<std::chrono::seconds>(
                std::chrono::system_clock::now().time_since_epoch())
                .count() -
        ...
        // LOG(INFO) << "Seconds since last batch update: " <<
        // seconds_since_batch_update;

        if (!it->second.is_complete &&
            seconds_since_batch_update > GC_PRUNE_MAX_SECONDS) {
          LOG(INFO) << "Deleting partial upload for req id "
                    << it->second.start_ts;

          const std::string& req_key_prefix =
              get_req_prefix_key(it->second.start_ts);
          store->delete_range(
              req_key_prefix,
              req_key_prefix + StringUtils::serialize_uint32_t(UINT32_MAX));
          ...
          if (it->second.res->is_alive) {
            ...
            async_req_res_t* async_req_res =
                new async_req_res_t(it->second.req, it->second.res, true);
            server->get_message_dispatcher()->send_message(
                HttpServer::STREAM_RESPONSE_MESSAGE, async_req_res);
          }
...
std::string BatchedIndexer::get_req_suffix_key(uint64_t req_id) {
  const std::string& req_key_prefix =
      RAFT_REQ_LOG_PREFIX + StringUtils::serialize_uint64_t(req_id) + "`";
  return req_key_prefix;
}
...
void BatchedIndexer::stop() { quit = true; }

int64_t BatchedIndexer::get_queued_writes() { return queued_writes; }

void BatchedIndexer::populate_skip_index() {
  if (skip_index_iter->Valid() &&
      skip_index_iter->key().starts_with(SKIP_INDICES_PREFIX)) {
...
void BatchedIndexer::serialize_state(nlohmann::json& state) {
  ...
  for (auto& kv : req_res_map) {
    ...
    // LOG(INFO) << "req_key: " << req_key << ", next_chunk_index: " <<
    // kv.second.next_chunk_index;
  }

  LOG(INFO) << "Serialized " << num_reqs_stored
            << " in-flight requests for snapshot.";
}
...
void BatchedIndexer::load_state(const nlohmann::json& state) {
  ...
  // need to sort on `start_ts` to preserve original order before notifying
  // queues
  ...
  LOG(INFO) << "Restored " << num_reqs_restored
            << " in-flight requests from snapshot.";
}

std::shared_mutex& BatchedIndexer::get_pause_mutex() { return pause_mutex; }
