...
HttpServer::HttpServer(const std::string& version,
                       ...
                       bool cors_enabled,
                       const std::set<std::string>& cors_domains,
...
void HttpServer::on_metrics_refresh_timeout(h2o_timer_t* entry) {
  h2o_custom_timer_t* custom_timer =
      reinterpret_cast<h2o_custom_timer_t*>(entry);
  ...
  h2o_timer_link(hs->ctx.loop, AppMetrics::METRICS_REFRESH_INTERVAL_MS,
...
void HttpServer::on_ssl_refresh_timeout(h2o_timer_t* entry) {
  ...
  bool refresh_success = initialize_ssl_ctx(
      hs->ssl_cert_path.c_str(), hs->ssl_cert_key_path.c_str(), hs->accept_ctx);

  if (refresh_success) {
    // delete the old SSL context but after some time, to allow existing
    // connections to drain
    h2o_custom_timer_t* ssl_ctx_delete_timer =
        new h2o_custom_timer_t(old_ssl_ctx);
    h2o_timer_init(&ssl_ctx_delete_timer->timer, on_ssl_ctx_delete_timeout);
    uint64_t delete_lag =
        std::max<uint64_t>(60 * 1000, hs->SSL_REFRESH_INTERVAL_MS / 2);
  ...
  // link the timer for the next cycle
  h2o_timer_link(hs->ctx.loop, hs->SSL_REFRESH_INTERVAL_MS,
...
int HttpServer::setup_ssl(const char* cert_file, const char* key_file) {
  // Set up a timer to refresh SSL config from disk. Also, initializing upfront
  // so that destructor works
  ...
  LOG(INFO) << "SSL cert refresh interval: " << (SSL_REFRESH_INTERVAL_MS / 1000)
            << "s";
...
int HttpServer::run(ReplicationState* replication_state) {
  ...
  h2o_timer_link(ctx.loop, AppMetrics::METRICS_REFRESH_INTERVAL_MS,
                 &metrics_refresh_timer.timer);
...
std::string HttpServer::get_version() { return version; }

void HttpServer::clear_timeouts(const std::vector<h2o_timer_t*>& timers,
...
h2o_pathconf_t* HttpServer::register_handler(h2o_hostconf_t* hostconf,
                                             const char* path,
...
uint64_t HttpServer::find_route(const std::vector<std::string>& path_parts,
                                const std::string& http_method,
                                route_path** found_rpath) {
  for (const auto& index_route : route_hash_to_path) {
    const route_path& rpath = index_route.second;

    if (rpath.path_parts.size() != path_parts.size() ||
        rpath.http_method != http_method) {
      continue;
    }
...
void HttpServer::on_res_generator_dispose(void* self) {
  ...
  h2o_custom_generator_t* custom_generator =
      *static_cast<h2o_custom_generator_t**>(self);

  // locking to ensure dispose does not happen while the h2o req object is being
  // written to
  {
...
int HttpServer::catch_all_handler(h2o_handler_t* _h2o_handler, h2o_req_t* req) {
  h2o_custom_req_handler_t* h2o_handler =
      (h2o_custom_req_handler_t*)_h2o_handler;

  const std::string& http_method =
      std::string(req->method.base, req->method.len);
  const std::string& path = std::string(req->path.base, req->path.len);
  std::vector<std::string> path_with_query_parts;

  // These guards have been added to debug a strange issue of
  // `path_with_query_parts` being empty sometimes
  if (req->path.len == 0 || path.empty()) {
    LOG(ERROR) << "Request path is empty: path.len=" << req->path.len
               << ", path: " << path;
    nlohmann::json resp;
  ...
  if (Config::get_instance().get_enable_access_logging()) {
    ...
    AppMetrics::get_instance().write_access_log(epoch_millis, client_ip.c_str(),
                                                metric_identifier);
  }
  ...
  if (h2o_handler->http_server->cors_enabled) {
    ...
    if (!h2o_handler->http_server->cors_domains.empty()) {
      auto acl_origin_cursor =
          h2o_find_header(&req->headers, H2O_TOKEN_ORIGIN, -1);
      if (acl_origin_cursor != -1) {
        response_origin = req->headers.entries[acl_origin_cursor].value;
        std::string origin_str =
            std::string(response_origin.base, response_origin.len);
    ...
    if (response_origin.len != 0) {
      ...
      h2o_add_header_by_str(&req->pool, &req->res.headers,
                            ...
                            response_origin.base, response_origin.len);
    }

    if (http_method == "OPTIONS") {
      // locate request access control headers
      const char* ACL_REQ_HEADERS = "access-control-request-headers";
      ssize_t acl_header_cursor = h2o_find_header_by_str(
          &req->headers, ACL_REQ_HEADERS, strlen(ACL_REQ_HEADERS), -1);

      if (acl_header_cursor != -1) {
        h2o_iovec_t& acl_req_headers =
            req->headers.entries[acl_header_cursor].value;
        ...
        h2o_add_header_by_str(
            ...
            H2O_STRLIT("POST, GET, DELETE, PUT, PATCH, OPTIONS"));
        h2o_add_header_by_str(&req->pool, &req->res.headers,
                              H2O_STRLIT("access-control-allow-headers"), 0,
                              NULL, acl_req_headers.base, acl_req_headers.len);
        h2o_add_header_by_str(&req->pool, &req->res.headers,
  ...
  h2o_iovec_t query = req->query_at != SIZE_MAX
                          ? h2o_iovec_init(req->path.base + req->query_at,
                                           req->path.len - req->query_at)
                          : h2o_iovec_init(H2O_STRLIT(""));

  if (query.len > 4000 && http_method == "GET" && !path_parts.empty() &&
      path_parts.back() == "search") {
    nlohmann::json resp;
    resp["message"] =
        "Query string exceeds max allowed length of 4000. Use the "
        "/multi_search end-point for larger payloads.";
    return send_response(req, 400, resp.dump());
  }
  ...
  // Extract auth key from header. If that does not exist, look for a GET
  // parameter.
  ssize_t auth_header_cursor = h2o_find_header_by_str(
      &req->headers, http_req::AUTH_HEADER, strlen(http_req::AUTH_HEADER), -1);
  ...
  ssize_t user_header_cursor = h2o_find_header_by_str(
      &req->headers, http_req::USER_HEADER, strlen(http_req::USER_HEADER), -1);
  ...
  uint64_t route_hash =
      h2o_handler->http_server->find_route(path_parts, http_method, &rpath);
  ...
  bool needs_readiness_check =
      (root_resource == "collections") ||
      !(root_resource == "health" || root_resource == "debug" ||
        root_resource == "proxy" || root_resource == "stats.json" ||
        root_resource == "metrics.json" || root_resource == "sequence" ||
        root_resource == "operations" || root_resource == "config" ||
        root_resource == "status");
  ...
  if (needs_readiness_check) {
    ...
    if (read_op && !h2o_handler->http_server->get_replication_state()
                        ->is_read_caught_up()) {
      return send_response(req, 503, message);
    }

    else if (write_op && !h2o_handler->http_server->get_replication_state()
                              ->is_write_caught_up()) {
      return send_response(req, 503, message);
    }
  }
  ...
  if (RateLimitManager::getInstance()->is_rate_limited(
          {RateLimitedEntityType::api_key, api_auth_key_sent},
  ...
  if (root_resource != "multi_search") {
    // multi_search needs to be handled later because the API key could be part
    // of request body and the whole request body might not be available right
    // now.
    bool authenticated = h2o_handler->http_server->auth_handler(
        query_map, embedded_params_vec, body, *rpath, api_auth_key_sent);
    if (!authenticated) {
      std::string message =
          std::string("{\"message\": \"Forbidden - a valid `") +
          http_req::AUTH_HEADER + "` header must be sent.\"}";
      return send_response(req, 401, message);
    }
  }

  std::shared_ptr<http_req> request = std::make_shared<http_req>(
      req, rpath->http_method, path_without_query, route_hash, query_map,
      embedded_params_vec, api_auth_key_sent, body, client_ip);
  ...
  h2o_custom_generator_t** allocated_generator =
      static_cast<h2o_custom_generator_t**>(h2o_mem_alloc_shared(
          &req->pool, sizeof(*allocated_generator), on_res_generator_dispose));
  *allocated_generator = custom_gen;

  // ensures that the first response need not wait for previous chunk to be done
  // sending
  response->notify();

  // LOG(INFO) << "Init res: " << custom_gen->response << ", ref count: " <<
  // custom_gen->response.use_count();

  if (root_resource == "multi_search") {
    // format is <length of api_auth_key_sent>:<api_auth_key_sent><client_ip>
    std::string multi_search_key = std::to_string(api_auth_key_sent.length()) +
                                   ":" + api_auth_key_sent + client_ip;
  ...
  if (rpath->action == "keys:create") {
    // we enrich incoming request with a random API key here so that leader and
    // replicas will use the same key
    request->metadata = StringUtils::randstring(AuthManager::GENERATED_KEY_LEN);
  }

  if (req->proceed_req == nullptr) {
    // Full request body is already available, so we don't care if handler is
    // async or not
    // LOG(INFO) << "Full request body is already available: " <<
    // req->entity.len;
    request->last_chunk_aggregate = true;
    return process_request(request, response, rpath, h2o_handler,
                           use_meta_thread_pool);
  } else {
    // Only partial request body is available.
    // If rpath->async_req is true, the request handler function will be invoked
    // multiple times, for each chunk
...
bool HttpServer::is_write_request(const std::string& root_resource,
                                  const std::string& http_method) {
  if (http_method == "GET") {
    return false;
  }

  bool write_free_request =
      (root_resource == "multi_search" || root_resource == "proxy" ||
...
int HttpServer::async_req_cb(void* ctx, int is_end_stream) {
  h2o_custom_generator_t* custom_generator =
      static_cast<h2o_custom_generator_t*>(ctx);
  ...
  bool is_http_v1 =
      (0x101 <= request->_req->version && request->_req->version < 0x200);
  ...
  if (!is_http_v1 && async_req && request->first_chunk_aggregate &&
      request->chunk_len == 0 &&
      request->path_without_query.find("import") != std::string::npos) {
    ssize_t agent_header_cursor =
        h2o_find_header_by_str(&request->_req->headers, http_req::AGENT_HEADER,
                               strlen(http_req::AGENT_HEADER), -1);
    if (agent_header_cursor != -1) {
      h2o_iovec_t& slot =
          request->_req->headers.entries[agent_header_cursor].value;
      ...
      if (user_agent.find("curl/") != std::string::npos) {
        ...
        int major_version =
            version_num[0] - 48;  // convert ascii char to integer
        if (major_version <= 7 &&
            std::stoll(version_num) < 7710) {  // allow >= v7.71.0
          std::string message =
              "{ \"message\": \"HTTP2 is not supported by your curl client. "
              "You need to use atleast Curl v7.71.0.\"}";
          h2o_iovec_t body =
              h2o_strdup(&request->_req->pool, message.c_str(), SIZE_MAX);
  ...
  /*LOG(INFO) << "entity: " << std::string(request->req->entity.base,
     std::min<size_t>(40, request->req->entity.len))
            << ", chunk len: " << std::string(chunk.base, std::min<size_t>(40,
     chunk.len));*/

  // std::this_thread::sleep_for(std::chrono::seconds(30));

  // LOG(INFO) << "request->body.size(): " << request->body.size() << ",
  // request->chunk_len=" << request->chunk_len;
  //  LOG(INFO) << "req->entity.len: " << request->req->entity.len << ",
  //  request->chunk_len=" << request->chunk_len;
  ...
  if (can_process_async || is_end_stream) {
    // For async streaming requests, handler should be invoked for every
    // aggregated chunk For a non streaming request, buffer body and invoke only
    // at the end
    ...
    process_request(request, response, custom_generator->rpath,
                    custom_generator->h2o_handler, false);
    return 0;
  }
...
int HttpServer::process_request(const std::shared_ptr<http_req>& request,
  ...
  const std::string& root_resource =
      (rpath->path_parts.empty()) ? "" : rpath->path_parts[0];

  if (root_resource == "multi_search") {
    // We can authenticate only when the full request body is available
    bool authenticated = handler->http_server->auth_handler(
        request->params, request->embedded_params_vec, request->body, *rpath,
        request->api_auth_key);
    if (!authenticated) {
      std::string message =
          std::string("{\"message\": \"Forbidden - a valid `") +
          http_req::AUTH_HEADER + "` header must be sent.\"}";
  ...
  auto thread_pool = use_meta_thread_pool
                         ? handler->http_server->get_meta_thread_pool()
  ...
  thread_pool->enqueue([rpath, message_dispatcher, request, response]() {
    ...
    // LOG(INFO) << "Wait for response " << response.get() << ", action: " <<
    // rpath->_get_action();
    ...
    if (!rpath->async_res) {
      ...
      message_dispatcher->send_message(HttpServer::STREAM_RESPONSE_MESSAGE,
                                       req_res);
    }
...
void HttpServer::on_deferred_process_request(h2o_timer_t* entry) {
  h2o_custom_timer_t* custom_timer =
      reinterpret_cast<h2o_custom_timer_t*>(entry);
  deferred_req_res_t* deferred_req_res =
      static_cast<deferred_req_res_t*>(custom_timer->data);
  // LOG(INFO) << "on_deferred_process_request " << deferred_req_res->req.get();

  route_path* found_rpath = nullptr;
  deferred_req_res->server->get_route(deferred_req_res->req->route_hash,
                                      &found_rpath);
  ...
  if (found_rpath) {
    ...
    server->thread_pool->enqueue([found_rpath, request, response]() {
      // LOG(INFO) << "Sleeping for 5s req count " <<
      // deferred_req_res->req.use_count();
      // std::this_thread::sleep_for(std::chrono::seconds(5));
      // LOG(INFO) << "on_deferred_process_request, calling handler, req use
      // count " << request.use_count();
      found_rpath->handler(request, response);
    });
  }
}

void HttpServer::defer_processing(const std::shared_ptr<http_req>& req,
                                  const std::shared_ptr<http_res>& res,
                                  size_t timeout_ms) {
  // LOG(INFO) << "defer_processing, exit_loop: " << exit_loop << ", req: " <<
  // req.get() << ", use count: " << req.use_count();
...
int HttpServer::send_response(h2o_req_t* req, int status_code,
  ...
  h2o_add_header(&req->pool, &req->res.headers, H2O_TOKEN_CONTENT_TYPE, nullptr,
...
void HttpServer::response_abort(h2o_generator_t* generator, h2o_req_t* req) {
  ...
  h2o_custom_generator_t* custom_generator =
      reinterpret_cast<h2o_custom_generator_t*>(generator);
...
void HttpServer::response_proceed(h2o_generator_t* generator, h2o_req_t* req) {
  ...
  // LOG(INFO) << "proxied_stream: " <<
  // custom_generator->response->proxied_stream; LOG(INFO) << "response.final: "
  ...
  // if the request itself is NOT async, call the handler since it will be the
  // handler that will be producing content (streaming response but not request)
  if (!custom_generator->rpath->async_req) {
    // call the handler since it will be the handler that will be producing
    // content
    custom_generator->h2o_handler->http_server->defer_processing(
        custom_generator->req(), custom_generator->res(), 1);
  }
}
...
void HttpServer::stream_response(stream_response_state_t& state) {
  ...
  // ***IMPORTANT***
  // We must ensure that fields of `state.req` are not written to for preventing
  // race conditions with indexing thread Check `async_req_res_t` constructor
  ...
  if (start_of_res) {
    h2o_add_header(&req->pool, &req->res.headers, H2O_TOKEN_CONTENT_TYPE, NULL,
                   state.res_content_type.data(),
                   state.res_content_type.size());
    req->res.status = state.status;
    req->res.reason = state.reason;
  }

  if (state.is_req_early_exit) {
    // premature termination of async request: handle this explicitly as
    // otherwise, request is not being closed
...
void HttpServer::set_auth_handler(bool (*handler)(
...
void HttpServer::post(const std::string& path,
                      bool (*handler)(const std::shared_ptr<http_req>&,
                                      const std::shared_ptr<http_res>&),
                      bool async_req, bool async_res) {
  std::vector<std::string> path_parts;
...
void HttpServer::put(const std::string& path,
                     bool (*handler)(const std::shared_ptr<http_req>&,
                                     const std::shared_ptr<http_res>&),
                     bool async_req, bool async_res) {
  std::vector<std::string> path_parts;
...
void HttpServer::patch(const std::string& path,
                       bool (*handler)(const std::shared_ptr<http_req>&,
                                       const std::shared_ptr<http_res>&),
                       bool async_req, bool async_res) {
  std::vector<std::string> path_parts;
...
void HttpServer::del(const std::string& path,
                     bool (*handler)(const std::shared_ptr<http_req>&,
                                     const std::shared_ptr<http_res>&),
                     bool async_req, bool async_res) {
  std::vector<std::string> path_parts;
...
bool HttpServer::is_alive() const { return replication_state->is_alive(); }
...
bool HttpServer::on_stream_response_message(void* data) {
  ...
  // NOTE: access to `req` and `res` objects must be synchronized and wrapped by
  // `req_res`

  if (req_res->is_alive()) {
    stream_response(req_res->get_res_state());
  } else {
    // serialized request or generator has been disposed (underlying request is
    // probably dead)
    req_res->req_notify();
    req_res->res_notify();
  }
...
bool HttpServer::on_request_proceed_message(void* data) {
  ...
  //  This callback will run concurrently to batch indexer's run() so care must
  //  be taken to protect access to variables that are written to by the batch
  //  indexer, which for now is only: last_chunk_aggregate (atomic)
  deferred_req_res_t* req_res = static_cast<deferred_req_res_t*>(data);
  if (req_res->res->is_alive) {
    auto stream_state = (req_res->req->last_chunk_aggregate)
...
bool HttpServer::on_deferred_processing_message(void* data) {
  ...
  // LOG(INFO) << "req use count: " << defer->req.use_count() << ", req " <<
  // defer->req.get();
  delete defer;
  return true;
}

bool HttpServer::has_exited() const { return exit_loop; }

void HttpServer::do_snapshot(const std::string& snapshot_path,
                             const std::shared_ptr<http_req>& req,
                             const std::shared_ptr<http_res>& res) {
  return replication_state->do_snapshot(snapshot_path, req, res);
}

bool HttpServer::trigger_vote() { return replication_state->trigger_vote(); }

bool HttpServer::reset_peers() { return replication_state->reset_peers(); }

ThreadPool* HttpServer::get_thread_pool() const { return thread_pool; }

bool HttpServer::initialize_ssl_ctx(const char* cert_file, const char* key_file,
  ...
  SSL_CTX_set_cipher_list(
      ...
      "ECDHE-RSA-AES128-GCM-SHA256:ECDHE-RSA-AES256-GCM-SHA384:ECDHE-RSA-"
      ...
      "DHE-RSA-AES128-SHA:DHE-RSA-AES256-SHA:DHE-RSA-AES128-SHA256:DHE-RSA-"
      "AES256-SHA256");
  ...
  if (SSL_CTX_use_PrivateKey_file(new_ctx, key_file, SSL_FILETYPE_PEM) != 1) {
    LOG(ERROR) << "An error occurred while trying to load private key file: "
               << key_file;
    SSL_CTX_free(new_ctx);
    return false;
  }
...
bool HttpServer::is_leader() const { return replication_state->is_leader(); }
