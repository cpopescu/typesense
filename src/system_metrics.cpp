...
void SystemMetrics::get(const std::string &data_dir_path,
  ...
  result["system_memory_total_bytes"] =
      std::to_string(get_memory_total_bytes());
...
float SystemMetrics::used_memory_ratio() {
  ...
  uint64_t memory_consumed_bytes =
      get_memory_active_bytes() + get_memory_non_proc_bytes();
...
uint64_t SystemMetrics::get_memory_used_bytes() {
  ...
  if (KERN_SUCCESS == host_page_size(mach_port, &mach_page_size) &&
    ...
    memory_used_bytes =
        ((int64_t)(vm_stats.active_count + vm_stats.wire_count) *
         (int64_t)mach_page_size);
  }
#elif __linux__
  memory_used_bytes =
      get_memory_total_bytes() - linux_get_mem_available_bytes();
#endif
...
void SystemMetrics::linux_get_network_data(const std::string &stat_path,
  ...
  /*
     ...
     multicast|bytes    packets errs drop fifo colls carrier compressed ens5:
     324278716  897631    0    0    0     0          0         0 93933882 575535
     0    0    0     0       0          0
  */
