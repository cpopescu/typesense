...
void master_server_routes() {
  ...
  server->del("/collections/:collection/documents", del_remove_documents, false,
              true);

  server->post("/collections/:collection/documents/import",
               post_import_documents, true, true);
  server->get("/collections/:collection/documents/export", get_export_documents,
              false, true);

  server->get("/collections/:collection/documents/:id", get_fetch_document);
  server->patch("/collections/:collection/documents/:id",
...
void (*backward::SignalHandling::_callback)(int sig,
                                            backward::StackTrace&) = nullptr;
...
void crash_callback(int sig, backward::StackTrace& st) {
  ...
  LOG(ERROR) << "Typesense " << TYPESENSE_VERSION
             << " is terminating abruptly.";
}
...
int main(int argc, char** argv) {
  ...
  // We have to force the presence of these symbols on macOS by explicitly
  // calling this method. See these issues:
  ...
  if (!config_validitation.ok()) {
    ...
    std::cerr << "Invalid configuration: " << config_validitation.error()
              << std::endl;
    std::cerr << "Command line " << options.usage() << std::endl;
    std::cerr
        << "You can also pass these arguments as environment variables such as "
