...
int main(int argc, char *argv[]) {
  ...
  Collection *collection =
      collectionManager.get_collection("github_top1k").get();
  if (collection == nullptr) {
    collection =
        collectionManager
            .create_collection("github_top1k", 4, fields_to_index, "stars")
            .get();
  }
  ...
  while (j < 1000) {
    ...
    while (std::getline(infile, json_line)) {
      ...
      document["id"] = document["org"].get<std::string>() + ":" +
                       document["repo_name"].get<std::string>();
    ...
    std::cout << "Time taken for insertion: " << timeMillis << "ms"
              << std::endl;
    ...
    timeMillis = std::chrono::duration_cast<std::chrono::milliseconds>(
                     std::chrono::high_resolution_clock::now() - begin)
                     .count();
