...
std::string get_query(StringUtils& string_utils, std::string& text) {
  ...
  size_t rand_index =
      0 + (rand() % static_cast<int>(tokens.size() - 1 - 0 + 1));
...
void benchmark_hn_titles(char* file_path) {
  ...
  Collection* collection =
      collectionManager.get_collection("hnstories_direct").get();
  if (collection == nullptr) {
    collection =
        collectionManager
            .create_collection("hnstories_direct", 4, fields_to_index, "points")
            .get();
  }
  ...
  infile.close();
  long long int timeMillis0 =
      std::chrono::duration_cast<std::chrono::milliseconds>(
          std::chrono::high_resolution_clock::now() - begin0)
          .count();
  ...
  for (size_t i = 0; i < queries.size(); i++) {
    auto results_op = collection->search(queries[i], search_fields, "", {},
                                         {sort_by("points", "DESC")}, {2}, 10,
                                         1, MAX_SCORE, {true});
...
void benchmark_reactjs_pages(char* file_path) {
  ...
  Collection* collection =
      collectionManager.get_collection("reactjs_pages").get();
  if (collection == nullptr) {
    collection = collectionManager
                     .create_collection("reactjs_pages", 4, fields_to_index,
                                        "dummy_sorting_field")
                     .get();
  }
  ...
  std::vector<std::string> search_fields = {"h1", "h2", "h3", "h4",
                                            "h5", "h6", "p"};
  uint64_t results_total = 0;  // to prevent no-op optimization!
  ...
  for (size_t i = 0; i < queries.size(); i++) {
    auto results_op = collection->search(
        queries[i], search_fields, "", {},
        {sort_by("dummy_sorting_field", "DESC")}, {2}, 10, 1, MAX_SCORE, {true},
        10, spp::sparse_hash_set<std::string>(), {"p"});
    if (results_op.ok() != true) {
      exit(2);
    }
    auto results = results_op.get();
...
void generate_word_freq() {
  ...
  while (std::getline(infile, json_line)) {
    try {
      nlohmann::json obj = nlohmann::json::parse(json_line);
      obj["count"] = uint64_t(
          (double(obj["count"].get<uint64_t>()) / 23135851162) * 1000000000);
