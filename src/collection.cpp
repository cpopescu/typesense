...
Collection::Collection(const std::string& name, const uint32_t collection_id,
                       const uint64_t created_at, const uint32_t next_seq_id,
                       Store* store, const std::vector<field>& fields,
    ...
    : name(name),
      ...
      dynamic_fields({}),
      symbols_to_index(to_char_array(symbols_to_index)),
      token_separators(to_char_array(token_separators)),
      index(init_index()) {
...
Option<doc_seq_id_t> Collection::to_doc(const std::string& json_str,
  ...
  if (!document.is_object()) {
    return Option<doc_seq_id_t>(400,
                                "Bad JSON: not a properly formed document.");
  }

  if (document.count("id") != 0 && id != "" && document["id"] != id) {
    return Option<doc_seq_id_t>(
        400,
        "The `id` of the resource does not match the `id` in the JSON body.");
  }

  if (document.count("id") == 0 && !id.empty()) {
    // use the explicit ID (usually from a PUT request) if document body does
    // not have it
    document["id"] = id;
  }
  ...
  if (document.count("id") == 0) {
    ...
    // for UPSERT, EMPLACE or CREATE, if a document does not have an ID, we will
    // treat it as a new doc
    ...
    for (auto const& pair : reference_fields) {
      ...
      if (!optional && document.count(field_name) != 1) {
        return Option<doc_seq_id_t>(
            400, "Missing the required reference field `" + field_name +
                     "` in the document.");
      ...
      if (collection == nullptr) {
        return Option<doc_seq_id_t>(400, "Referenced collection `" +
                                             reference_collection_name +
                                             "` not found.");
      }

      if (collection->get_schema().count(reference_field_name) == 0) {
        return Option<doc_seq_id_t>(400, "Referenced field `" +
      ...
      if (!collection->get_schema().at(reference_field_name).index) {
        return Option<doc_seq_id_t>(
            400, "Referenced field `" + reference_field_name +
                     "` in the collection `" + reference_collection_name +
                     "` must be indexed.");
      }
      ...
      collection->get_filter_ids(reference_field_name + ":=" + value,
                                 filter_result);

      if (filter_result.count != 1) {
        auto match = " `" + reference_field_name + ": " + value + "` ";

        // Constraints similar to foreign key apply here. The reference match
        // must be unique and not null.
        return Option<doc_seq_id_t>(400,
                                    filter_result.count < 1
                                        ? "Referenced document having" + match +
                                              "not found in the collection `" +
                                              reference_collection_name + "`."
                                        : "Multiple documents having" + match +
                                              "found in the collection `" +
                                              reference_collection_name + "`.");
      }

      document[field_name + REFERENCE_HELPER_FIELD_SUFFIX] =
          filter_result.docs[0];
    }
    ...
    if (seq_id_status == StoreStatus::ERROR) {
      return Option<doc_seq_id_t>(
          500,
          "Error fetching the sequence key for document with id: " + doc_id);
    }

    if (seq_id_status == StoreStatus::FOUND) {
      if (operation == CREATE) {
        return Option<doc_seq_id_t>(409, std::string("A document with id ") +
      ...
      if (operation == UPDATE) {
        ...
        return Option<doc_seq_id_t>(
            404, "Could not find a document with id: " + doc_id);
      } else {
        // for UPSERT, EMPLACE or CREATE, if a document with given ID is not
        // found, we will treat it as a new doc
        uint32_t seq_id = get_next_seq_id();
  ...
  for (const field& coll_field : fields) {
    ...
    if (coll_field.embed.count(fields::from) != 0) {
      ...
      if (field_json[fields::embed].count(fields::model_config) != 0) {
        hide_credential(field_json[fields::embed][fields::model_config],
                        "api_key");
        hide_credential(field_json[fields::embed][fields::model_config],
                        "access_token");
        hide_credential(field_json[fields::embed][fields::model_config],
...
Option<nlohmann::json> Collection::add(const std::string& json_str,
  ...
  const nlohmann::json& res =
      add_many(json_lines, document, operation, id, dirty_values, false, false);
  ...
  if (!res["success"].get<bool>()) {
    ...
    return Option<nlohmann::json>(res_doc["code"].get<size_t>(),
                                  res_doc["error"].get<std::string>());
  }

  return Option<nlohmann::json>(document);
}

nlohmann::json Collection::add_many(
    std::vector<std::string>& json_lines, nlohmann::json& document,
    const index_operation_t& operation, const std::string& id,
    const DIRTY_VALUES& dirty_values, const bool& return_doc,
    const bool& return_id, const size_t remote_embedding_batch_size) {
  // LOG(INFO) << "Memory ratio. Max = " << max_memory_ratio << ", Used = " <<
  // SystemMetrics::used_memory_ratio();
  ...
  for (size_t i = 0; i < json_lines.size(); i++) {
    ...
    Option<doc_seq_id_t> doc_seq_id_op =
        to_doc(json_line, document, operation, dirty_values, id);
    ...
    // NOTE: we overwrite the input json_lines with result to avoid memory
    // pressure
    ...
    if (!doc_seq_id_op.ok()) {
      ...
      if (repeated_doc) {
        // when a document repeats, we send the batch until this document so
        // that we can deal with conflicts
        i--;
      ...
      // if `fallback_field_type` or `dynamic_fields` is enabled, update schema
      // first before indexing
      if (!fallback_field_type.empty() || !dynamic_fields.empty() ||
          !nested_fields.empty()) {
        std::vector<field> new_fields;
        std::unique_lock lock(mutex);

        Option<bool> new_fields_op = detect_new_fields(
            record.doc, dirty_values, search_schema, dynamic_fields,
            nested_fields, fallback_field_type, record.is_update, new_fields,
...
Option<nlohmann::json> Collection::update_matching_filter(
    const std::string& filter_query, const std::string& json_str,
    std::string& req_dirty_values, const int batch_size) {
  ...
  if (_filter_query == "*") {
    // Get an iterator from rocksdb and iterate over all the documents present
    // in the collection.
    ...
    rocksdb::Iterator* it =
        collectionManager.get_store()->scan(seq_id_prefix, iter_upper_bound);

    while (it->Valid()) {
      // Generate a batch of documents to be ingested by add_many.
      for (int buffer_counter = 0;
           buffer_counter < batch_size && it->Valid();) {
      ...
      auto res =
          add_many(buffer, dummy, index_operation_t::UPDATE, "", dirty_values);
    ...
    if (!filter_ids_op.ok()) {
      return Option<nlohmann::json>(filter_ids_op.code(),
                                    filter_ids_op.error());
    }

    for (size_t i = 0; i < filter_result.count;) {
      for (int buffer_counter = 0;
           buffer_counter < batch_size && i < filter_result.count;) {
        uint32_t seq_id = filter_result.docs[i++];
        nlohmann::json existing_document;

        auto get_doc_op =
            get_document_from_store(get_seq_id_key(seq_id), existing_document);
        if (!get_doc_op.ok()) {
          continue;
        }
...
void Collection::batch_index(std::vector<index_record>& index_records,
                             std::vector<std::string>& json_out,
                             size_t& num_indexed, const bool& return_doc,
                             const bool& return_id,
  ...
  for (auto& index_record : index_records) {
    ...
    if (index_record.indexed.ok()) {
      if (index_record.is_update) {
        ...
        const std::string& serialized_json = index_record.new_doc.dump(
            -1, ' ', false, nlohmann::detail::error_handler_t::ignore);
        bool write_ok =
            store->insert(get_seq_id_key(index_record.seq_id), serialized_json);
        ...
        if (!write_ok) {
          ...
          index_in_memory(index_record.old_doc, index_record.seq_id,
                          index_record.operation, index_record.dirty_values);
          index_record.index_failure(500,
        ...
        const std::string& serialized_json = index_record.doc.dump(
            -1, ' ', false, nlohmann::detail::error_handler_t::ignore);
      ...
      if (return_doc & index_record.indexed.ok()) {
        res["document"] =
            index_record.is_update ? index_record.new_doc : index_record.doc;
      }
      if (return_id & index_record.indexed.ok()) {
        res["id"] = index_record.is_update ? index_record.new_doc["id"]
                                           : index_record.doc["id"];
      }
      if (!index_record.indexed.ok()) {
    ...
    json_out[index_record.position] =
        res.dump(-1, ' ', false, nlohmann::detail::error_handler_t::ignore);
  }
}

Option<uint32_t> Collection::index_in_memory(nlohmann::json& document,
                                             uint32_t seq_id,
                                             const index_operation_t op,
                                             const DIRTY_VALUES& dirty_values) {
  std::unique_lock lock(mutex);

  Option<uint32_t> validation_op = validator_t::validate_index_in_memory(
      document, seq_id, default_sorting_field, search_schema, embedding_fields,
      op, false, fallback_field_type, dirty_values);
  ...
  Index::batch_memory_index(index, index_batch, default_sorting_field,
                            search_schema, embedding_fields,
...
size_t Collection::batch_index_in_memory(
    std::vector<index_record>& index_records,
    const size_t remote_embedding_batch_size, const bool generate_embeddings) {
  std::unique_lock lock(mutex);
  size_t num_indexed = Index::batch_memory_index(
      index, index_records, default_sorting_field, search_schema,
      embedding_fields, fallback_field_type, token_separators, symbols_to_index,
      true, remote_embedding_batch_size, generate_embeddings);
  num_documents += num_indexed;
  return num_indexed;
}

void Collection::curate_results(
    string& actual_query, const string& filter_query, bool enable_overrides,
    bool already_segmented,
    const std::map<size_t, std::vector<std::string>>& pinned_hits,
    const std::vector<std::string>& hidden_hits,
    std::vector<std::pair<uint32_t, uint32_t>>& included_ids,
    std::vector<uint32_t>& excluded_ids,
    std::vector<const override_t*>& filter_overrides, bool& filter_curated_hits,
  ...
  if (enable_overrides && !overrides.empty()) {
    ...
    if (actual_query == "*") {
      ...
      Tokenizer tokenizer(actual_query, true, false, "", symbols_to_index,
                          token_separators);
    ...
    for (const auto& override_kv : overrides) {
      ...
      if (override.effective_from_ts != -1 &&
          now_epoch < override.effective_from_ts) {
        continue;
      }

      if (override.effective_to_ts != -1 &&
          now_epoch > override.effective_to_ts) {
        continue;
      }

      // ID-based overrides are applied first as they take precedence over
      // filter-based overrides
      if (!override.filter_by.empty()) {
        filter_overrides.push_back(&override);
      }

      bool filter_by_match =
          (override.rule.query.empty() && override.rule.match.empty() &&
           !override.rule.filter_by.empty() &&
           override.rule.filter_by == filter_query);

      bool query_match =
          (override.rule.match == override_t::MATCH_EXACT &&
           override.rule.normalized_query == query) ||
      ...
      if (filter_by_match || query_match) {
        if (!override.rule.filter_by.empty() &&
            override.rule.filter_by != filter_query) {
          continue;
        }
        ...
        } else if (override.remove_matched_tokens &&
                   override.filter_by.empty()) {
          // don't prematurely remove tokens from query because dynamic
          // filtering will require them
...
Option<bool> Collection::validate_and_standardize_sort_fields(
    const std::vector<sort_by>& sort_fields,
    std::vector<sort_by>& sort_fields_std, const bool is_wildcard_query,
    const bool is_vector_query, const bool is_group_by_query) const {
  ...
  for (size_t i = 0; i < sort_fields.size(); i++) {
    ...
    if (sort_field_std.name.back() == ')') {
      ...
      while (paran_start < sort_field_std.name.size() &&
             sort_field_std.name[paran_start] != '(') {
        paran_start++;
      }

      const std::string& actual_field_name =
          sort_field_std.name.substr(0, paran_start);
      ...
      if (actual_field_name == sort_field_const::text_match) {
        ...
        const std::string& match_config = sort_field_std.name.substr(
            paran_start + 1, sort_field_std.name.size() - paran_start - 2);
        StringUtils::split(match_config, match_parts, ":");
        if (match_parts.size() != 2 || match_parts[0] != "buckets") {
          return Option<bool>(
              400, "Invalid sorting parameter passed for _text_match.");
        }

        if (!StringUtils::is_uint32_t(match_parts[1])) {
          return Option<bool>(
              400,
              "Invalid value passed for _text_match `buckets` configuration.");
        }
        ...
        const std::string& filter_exp = sort_field_std.name.substr(
            paran_start + 1, sort_field_std.name.size() - paran_start - 2);
        if (filter_exp.empty()) {
          return Option<bool>(400, "The eval expression in sort_by is empty.");
        }

        Option<bool> parse_filter_op =
            filter::parse_filter_query(filter_exp, search_schema, store, "",
                                       sort_field_std.eval.filter_tree_root);
        if (!parse_filter_op.ok()) {
          return Option<bool>(
              parse_filter_op.code(),
              "Error parsing eval expression in sort_by clause.");
        }
        ...
        if (field_it == search_schema.end()) {
          std::string error = "Could not find a field named `" +
                              actual_field_name +
        ...
        std::string error =
            "Bad syntax for sorting field `" + actual_field_name + "`";

        if (!field_it.value().is_geopoint()) {
          // check for null value order
          const std::string& sort_params_str = sort_field_std.name.substr(
              paran_start + 1, sort_field_std.name.size() - paran_start - 2);
          ...
          auto missing_values_op =
              magic_enum::enum_cast<sort_by::missing_values_t>(param_parts[1]);
          ...
          const std::string& geo_coordstr = sort_field_std.name.substr(
              paran_start + 1, sort_field_std.name.size() - paran_start - 2);
          ...
          if (!StringUtils::is_float(geo_parts[0]) ||
              !StringUtils::is_float(geo_parts[1])) {
            return Option<bool>(400, error);
          }
          ...
          if (geo_parts.size() == 3) {
            ...
            if (StringUtils::begins_with(geo_parts[2],
                                         sort_field_const::exclude_radius)) {
              is_exclude_option = true;
            } else if (StringUtils::begins_with(geo_parts[2],
                                                sort_field_const::precision)) {
            ...
            std::string unit =
                param_parts[1].substr(param_parts[1].size() - 2, 2);
            ...
            if (unit == "km") {
              ...
              return Option<bool>(400,
                                  "Sort field's parameter "
                                  "unit must be either `km` or `mi`.");
            }

            if (value_meters <= 0) {
              return Option<bool>(
                  400, "Sort field's parameter must be a positive number.");
            }
    ...
    if (sort_field_std.name != sort_field_const::text_match &&
        sort_field_std.name != sort_field_const::eval &&
        sort_field_std.name != sort_field_const::seq_id &&
        sort_field_std.name != sort_field_const::group_found &&
        sort_field_std.name != sort_field_const::vector_distance) {
      const auto field_it = search_schema.find(sort_field_std.name);
      if (field_it == search_schema.end() || !field_it.value().sort ||
          !field_it.value().index) {
        std::string error = "Could not find a field named `" +
                            sort_field_std.name +
                            "` in the schema for sorting.";
        return Option<bool>(404, error);
    ...
    if (sort_field_std.name == sort_field_const::group_found &&
        is_group_by_query == false) {
      std::string error =
          "group_by parameters should not be empty when using sort_by "
          "group_found";
      return Option<bool>(404, error);
    }

    if (sort_field_std.name == sort_field_const::vector_distance &&
        !is_vector_query) {
      std::string error =
          "sort_by vector_distance is only supported for vector queries, "
    ...
    if (sort_field_std.order != sort_field_const::asc &&
        sort_field_std.order != sort_field_const::desc) {
      std::string error = "Order for field` " + sort_field_std.name +
  ...
  */
  if (sort_fields_std.empty()) {
    if (!is_wildcard_query) {
      sort_fields_std.emplace_back(sort_field_const::text_match,
                                   sort_field_const::desc);
    }

    if (is_vector_query) {
      sort_fields_std.emplace_back(sort_field_const::vector_distance,
                                   sort_field_const::asc);
    }

    if (!default_sorting_field.empty()) {
      sort_fields_std.emplace_back(default_sorting_field,
                                   sort_field_const::desc);
    } else {
      sort_fields_std.emplace_back(sort_field_const::seq_id,
                                   sort_field_const::desc);
    }
  }
...
Option<bool> Collection::extract_field_name(
    const std::string& field_name,
    const tsl::htrie_map<char, field>& search_schema,
    std::vector<std::string>& processed_search_fields,
    const bool extract_only_string_fields, const bool enable_nested_fields,
  ...
  // If wildcard, remove *
  auto prefix_it = search_schema.equal_prefix_range(
      field_name.substr(0, field_name.size() - is_wildcard));
  ...
  for (auto kv = prefix_it.first; kv != prefix_it.second; ++kv) {
    ...
    bool text_embedding =
        kv.value().type == field_types::FLOAT_ARRAY && kv.value().num_dim > 0;
    ...
    if (exact_primitive_match || is_wildcard || text_embedding ||
        // field_name prefix must be followed by a "." to indicate an object
        // search
        (enable_nested_fields && kv.key().size() > field_name.size() &&
  ...
  if (is_wildcard && extract_only_string_fields && !field_found) {
    std::string error =
        "No string or string array field found matching the pattern `" +
        field_name + "` in the schema.";
    return Option<bool>(404, error);
  } else if (!field_found) {
    std::string error = is_wildcard ? "No field found matching the pattern `"
...
Option<nlohmann::json> Collection::search(
    std::string raw_query, const std::vector<std::string>& raw_search_fields,
    const std::string& filter_query,
    const std::vector<std::string>& facet_fields,
    const std::vector<sort_by>& sort_fields,
    const std::vector<uint32_t>& num_typos, const size_t per_page,
    const size_t page, token_ordering token_order,
    ...
    const size_t max_facet_values, const std::string& simple_facet_query,
    const size_t snippet_threshold, const size_t highlight_affix_num_tokens,
    const std::string& highlight_full_fields, size_t typo_tokens_threshold,
    const std::string& pinned_hits_str, const std::string& hidden_hits_str,
    const std::vector<std::string>& raw_group_by_fields, size_t group_limit,
    const std::string& highlight_start_tag,
    const std::string& highlight_end_tag,
    std::vector<uint32_t> raw_query_by_weights, size_t limit_hits,
    bool prioritize_exact_match, bool pre_segmented_query,
    bool enable_overrides, const std::string& highlight_fields,
    const bool exhaustive_search, const size_t search_stop_millis,
    const size_t min_len_1typo, const size_t min_len_2typo,
    enable_t split_join_tokens, const size_t max_candidates,
    const std::vector<enable_t>& infixes, const size_t max_extra_prefix,
    const size_t max_extra_suffix, const size_t facet_query_num_typos,
    const size_t filter_curated_hits_option,
    const bool prioritize_token_position, const std::string& vector_query_str,
    const bool enable_highlight_v1, const uint64_t search_time_start_us,
    const text_match_type_t match_type, const size_t facet_sample_percent,
    const size_t facet_sample_threshold, const size_t page_offset,
  ...
  search_begin_us =
      (search_time_start_us != 0)
          ? search_time_start_us
  ...
  if (raw_query != "*" && raw_search_fields.empty()) {
    return Option<nlohmann::json>(400,
                                  "No search fields specified for the query.");
  }
  ...
  if (!raw_search_fields.empty() && !raw_query_by_weights.empty() &&
    ...
    return Option<nlohmann::json>(
        400,
        "Number of weights in `query_by_weights` does not match "
        "number of `query_by` fields.");
  }

  if (!raw_group_by_fields.empty() &&
      (group_limit == 0 || group_limit > GROUP_LIMIT_MAX)) {
    return Option<nlohmann::json>(
        400, "Value of `group_limit` must be between 1 and " +
                 std::to_string(GROUP_LIMIT_MAX) + ".");
  }

  if (!raw_search_fields.empty() &&
      raw_search_fields.size() != num_typos.size()) {
    if (num_typos.size() != 1) {
      return Option<nlohmann::json>(
          400,
          "Number of values in `num_typos` does not match "
          "number of `query_by` fields.");
    }
  }

  if (!raw_search_fields.empty() &&
      raw_search_fields.size() != prefixes.size()) {
    if (prefixes.size() != 1) {
      return Option<nlohmann::json>(
          400,
          "Number of prefix values in `prefix` does not match "
          "number of `query_by` fields.");
    }
  }

  if (!raw_search_fields.empty() &&
      raw_search_fields.size() != infixes.size()) {
    if (infixes.size() != 1) {
      return Option<nlohmann::json>(
          400,
          "Number of infix values in `infix` does not match "
          "number of `query_by` fields.");
    }
  }
  ...
  if (facet_sample_percent > 100) {
    return Option<nlohmann::json>(
        400, "Value of `facet_sample_percent` must be less than 100.");
  }
  ...
  if (!vector_query_str.empty()) {
    ...
    auto parse_vector_op = VectorQueryOps::parse_vector_query_str(
        vector_query_str, vector_query, is_wildcard_query, this);
    ...
    if (vector_field_it == search_schema.end() ||
        vector_field_it.value().num_dim == 0) {
      return Option<nlohmann::json>(
          400, "Field `" + vector_query.field_name +
                   "` does not have a vector query index.");
    }

    if (is_wildcard_query &&
        vector_field_it.value().num_dim != vector_query.values.size()) {
      return Option<nlohmann::json>(
          400, "Query field `" + vector_query.field_name + "` must have " +
  ...
  for (size_t i = 0; i < raw_search_fields.size(); i++) {
    ...
    auto field_op =
        extract_field_name(field_name, search_schema, expanded_search_fields,
                           true, enable_nested_fields);
    if (!field_op.ok()) {
    ...
    for (const auto& expanded_search_field : expanded_search_fields) {
      ...
      if (search_field.num_dim > 0) {
        ...
        if (num_embed_fields > 1 ||
            (!vector_query.field_name.empty() &&
             search_field.name != vector_query.field_name)) {
          std::string error =
        ...
        //     std::string error = "Text embedding is not enabled. Please set
        //     `model-dir` at startup."; return Option<nlohmann::json>(400,
        //     error);
        // }
        ...
        TextEmbedderManager& embedder_manager =
            TextEmbedderManager::get_instance();
        auto embedder_op = embedder_manager.get_text_embedder(
            search_field.embed[fields::model_config]);
        if (!embedder_op.ok()) {
        ...
        if ((std::chrono::duration_cast<std::chrono::microseconds>(
                 std::chrono::system_clock::now().time_since_epoch())
                 .count() -
             search_begin_us) > remote_embedding_timeout_us) {
        ...
        if (embedder->is_remote()) {
          ...
          if ((prefixes.size() == 1 && prefixes[0] == true) ||
              (prefixes.size() > 1 && prefixes[i] == true)) {
            std::string error =
                "Prefix search is not supported for remote embedders. Please "
                "set `prefix=false` as an additional search parameter to "
                "disable prefix searching.";
            return Option<nlohmann::json>(400, error);
          }

          if (remote_embedding_num_tries == 0) {
            std::string error =
                "`remote_embedding_num_tries` must be greater than 0.";
            return Option<nlohmann::json>(400, error);
          }
        }

        std::string embed_query =
            embedder_manager.get_query_prefix(
                search_field.embed[fields::model_config]) +
            raw_query;
        auto embedding_op =
            embedder->Embed(embed_query, remote_embedding_timeout_ms,
                            remote_embedding_num_tries);
        if (!embedding_op.success) {
          if (!embedding_op.error["error"].get<std::string>().empty()) {
            return Option<nlohmann::json>(
                400, embedding_op.error["error"].get<std::string>());
          } else {
            return Option<nlohmann::json>(400, embedding_op.error.dump());
          }
        }
        std::vector<float> embedding = embedding_op.embedding;
        // params could have been set for an embed field, so we take a backup
        // and restore
  ...
  if (!vector_query.field_name.empty() && vector_query.values.empty() &&
      num_embed_fields == 0) {
  ...
  if (!query_by_weights.empty() &&
      processed_search_fields.size() != query_by_weights.size()) {
  ...
  for (const std::string& field_name : processed_search_fields) {
    ...
    if (!search_field.index) {
      std::string error = "Field `" + field_name +
                          "` is marked as a non-indexed field in the schema.";
      return Option<nlohmann::json>(400, error);
    }
  ...
  for (const std::string& field_name : raw_group_by_fields) {
    auto field_op =
        extract_field_name(field_name, search_schema, group_by_fields, false,
  ...
  auto include_exclude_op = populate_include_exclude_fields(
      include_fields, exclude_fields, include_fields_full, exclude_fields_full);

  if (!include_exclude_op.ok()) {
    return Option<nlohmann::json>(include_exclude_op.code(),
                                  include_exclude_op.error());
  }
  ...
  process_search_field_weights(processed_search_fields, query_by_weights,
                               weighted_search_fields, reordered_search_fields);

  const std::vector<std::string>& search_fields =
      reordered_search_fields.empty() ? processed_search_fields
                                      : reordered_search_fields;

  const std::string doc_id_prefix =
      std::to_string(collection_id) + "_" + DOC_ID_PREFIX + "_";
  filter_node_t* filter_tree_root = nullptr;
  Option<bool> parse_filter_op = filter::parse_filter_query(
      filter_query, search_schema, store, doc_id_prefix, filter_tree_root);
  std::unique_ptr<filter_node_t> filter_tree_root_guard(filter_tree_root);

  if (!parse_filter_op.ok()) {
    return Option<nlohmann::json>(parse_filter_op.code(),
                                  parse_filter_op.error());
  }
  ...
  if (!simple_facet_query.empty()) {
    ...
    if (found_colon_index == std::string::npos) {
      std::string error =
          "Facet query must be in the `facet_field: value` format.";
      return Option<nlohmann::json>(400, error);
    }

    if (facet_fields.empty()) {
      std::string error =
          "The `facet_query` parameter is supplied without a `facet_by` "
          "parameter.";
      return Option<nlohmann::json>(400, error);
    }

    std::string&& facet_query_fname =
        simple_facet_query.substr(0, found_colon_index);
    StringUtils::trim(facet_query_fname);

    std::string&& facet_query_value =
        simple_facet_query.substr(found_colon_index + 1, std::string::npos);
    ...
    if (facet_query_value.empty()) {
      ...
      if (std::find(facet_fields.begin(), facet_fields.end(),
                    facet_query.field_name) == facet_fields.end()) {
        std::string error = "Facet query refers to a facet field `" +
                            facet_query.field_name + "` " +
                            "that is not part of `facet_by` parameter.";
        return Option<nlohmann::json>(400, error);
      }

      if (search_schema.count(facet_query.field_name) == 0 ||
          !search_schema.at(facet_query.field_name).facet) {
        std::string error = "Could not find a facet field named `" +
                            facet_query.field_name + "` in the schema.";
  ...
  if (per_page > PER_PAGE_MAX) {
    std::string message = "Only upto " + std::to_string(PER_PAGE_MAX) +
                          " hits can be fetched per page.";
  ...
  if (fetch_size > limit_hits) {
    std::string message =
        "Only upto " + std::to_string(limit_hits) + " hits can be fetched. " +
  ...
  curate_results(query, filter_query, enable_overrides, pre_segmented_query,
                 pinned_hits, hidden_hits, included_ids, excluded_ids,
                 filter_overrides, filter_curated_hits, curated_sort_by);

  if (filter_curated_hits_option == 0 || filter_curated_hits_option == 1) {
    // When query param has explicit value set, override level configuration
  ...
  */
  ...
  if (curated_sort_by.empty()) {
    auto sort_validation_op = validate_and_standardize_sort_fields(
        sort_fields, sort_fields_std, is_wildcard_query, is_vector_query,
        is_group_by_query);
    if (!sort_validation_op.ok()) {
      return Option<nlohmann::json>(sort_validation_op.code(),
                                    sort_validation_op.error());
    }
  } else {
    std::vector<sort_by> curated_sort_fields;
    bool parsed_sort_by = CollectionManager::parse_sort_by_str(
        curated_sort_by, curated_sort_fields);
    if (!parsed_sort_by) {
      return Option<nlohmann::json>(400, "Parameter `sort_by` is malformed.");
    }

    auto sort_validation_op = validate_and_standardize_sort_fields(
        curated_sort_fields, sort_fields_std, is_wildcard_query,
  ...
  for (size_t i = 0; i < sort_fields_std.size(); i++) {
    if (sort_fields_std[i].name == sort_field_const::text_match &&
        sort_fields_std[i].text_match_buckets != 0) {
      match_score_index = i;
      break;
    }
  }
  ...
  if (search_fields.size() == 0) {
    ...
    parse_search_query(query, q_include_tokens,
                       ...
                       field_query_tokens[0].q_phrases, "", false);
    for (size_t i = 0; i < q_include_tokens.size(); i++) {
      auto& q_include_token = q_include_tokens[i];
      field_query_tokens[0].q_include_tokens.emplace_back(
          i, q_include_token, (i == q_include_tokens.size() - 1),
          q_include_token.size(), 0);
    }
  } else {
    field_query_tokens.emplace_back(query_tokens_t{});
    const std::string& field_locale =
        search_schema.at(weighted_search_fields[0].name).locale;
    parse_search_query(
        query, q_include_tokens, field_query_tokens[0].q_exclude_tokens,
        field_query_tokens[0].q_phrases, field_locale, pre_segmented_query);

    // process filter overrides first, before synonyms (order is important)

    // included_ids, excluded_ids
    process_filter_overrides(filter_overrides, q_include_tokens, token_order,
                             filter_tree_root, included_ids, excluded_ids);
  ...
  size_t index_id = 0;
  search_args* search_params = new search_args(
      field_query_tokens, weighted_search_fields, match_type, filter_tree_root,
      facets, included_ids, excluded_ids, sort_fields_std, facet_query,
      num_typos, max_facet_values, max_hits, per_page, offset, token_order,
      prefixes, drop_tokens_threshold, typo_tokens_threshold, group_by_fields,
      group_limit, default_sorting_field, prioritize_exact_match,
      prioritize_token_position, exhaustive_search, 4, search_stop_millis,
      min_len_1typo, min_len_2typo, max_candidates, infixes, max_extra_prefix,
      max_extra_suffix, facet_query_num_typos, filter_curated_hits,
      split_join_tokens, vector_query, facet_sample_percent,
  ...
  populate_result_kvs(&topster, raw_result_kvs, search_params->groups_processed,
                      sort_fields_std);
  populate_result_kvs(&curated_topster, override_result_kvs,
                      search_params->groups_processed, sort_fields_std);
  ...
  if (search_cutoff && total == 0) {
    // this can happen if other requests stopped this request from being
    // processed we should return an error so that request can be retried by
    // client
    return Option<nlohmann::json>(408, "Request Timeout");
  }

  if (match_score_index >= 0 &&
      sort_fields_std[match_score_index].text_match_buckets > 0) {
    size_t num_buckets = sort_fields_std[match_score_index].text_match_buckets;
    const size_t max_kvs_bucketed =
        std::min<size_t>(DEFAULT_TOPSTER_SIZE, raw_result_kvs.size());

    if (max_kvs_bucketed >= num_buckets) {
      spp::sparse_hash_map<uint64_t, int64_t> result_scores;

      // only first `max_kvs_bucketed` elements are bucketed to prevent
      // pagination issues past 250 records
      size_t block_len = (max_kvs_bucketed / num_buckets);
      size_t i = 0;
      while (i < max_kvs_bucketed) {
        int64_t anchor_score =
            raw_result_kvs[i][0]
                ->scores[raw_result_kvs[i][0]->match_score_index];
        size_t j = 0;
        while (j < block_len && i + j < max_kvs_bucketed) {
          result_scores[raw_result_kvs[i + j][0]->key] =
              raw_result_kvs[i + j][0]
                  ->scores[raw_result_kvs[i + j][0]->match_score_index];
          raw_result_kvs[i + j][0]
              ->scores[raw_result_kvs[i + j][0]->match_score_index] =
              anchor_score;
          j++;
      ...
      std::partial_sort(raw_result_kvs.begin(),
                        raw_result_kvs.begin() + max_kvs_bucketed,
                        raw_result_kvs.end(), Topster::is_greater_kv_group);
  ...
  std::sort(override_result_kvs.begin(), override_result_kvs.end(),
  ...
  while (raw_results_index < raw_result_kvs.size()) {
    if (override_kv_index < override_result_kvs.size()) {
      ...
      uint64_t override_position =
          override_result_kvs[override_kv_index][0]->distinct_key;
      if (result_position == override_position) {
        override_result_kvs[override_kv_index][0]->match_score_index =
            CURATED_RECORD_IDENTIFIER;
  ...
  if (!facet_query.query.empty()) {
    ...
    Tokenizer(facet_query.query, normalise, !fq_field.is_string(),
              fq_field.locale, symbols_to_index, token_separators)
        .tokenize(facet_query_tokens);

    facet_query_num_tokens = facet_query_tokens.size();
    facet_query_last_token =
        facet_query_tokens.empty() ? "" : facet_query_tokens.back();
  }

  const long start_result_index = offset;

  // `end_result_index` could be -1 when max_hits is 0
  const long end_result_index =
      std::min(fetch_size, std::min(max_hits, result_group_kvs.size())) - 1;
  ...
  if (query != "*") {
    process_highlight_fields(
        weighted_search_fields, raw_search_fields, include_fields_full,
        exclude_fields_full, highlight_field_names, highlight_full_field_names,
  ...
  for (long result_kvs_index = start_result_index;
       result_kvs_index <= end_result_index; result_kvs_index++) {
    ...
    nlohmann::json& hits_array =
        group_limit ? group_hits["hits"] : result["hits"];
    nlohmann::json group_key = nlohmann::json::array();

    for (const KV* field_order_kv : kv_group) {
      const std::string& seq_id_key =
          get_seq_id_key((uint32_t)field_order_kv->key);

      nlohmann::json document;
      const Option<bool>& document_op =
          get_document_from_store(seq_id_key, document);
      ...
      if (!highlight_items.empty()) {
        copy_highlight_doc(highlight_items, enable_nested_fields, document,
                           highlight_res);
      ...
      for (size_t i = 0; i < highlight_items.size(); i++) {
        ...
        if (query != "*") {
          ...
          highlight_result(
              raw_query, search_field, i, highlight_item.qtoken_leaves,
              field_order_kv, document, highlight_res, string_utils,
              snippet_threshold, highlight_affix_num_tokens,
      ...
      // explicit highlight fields could be parent of searched fields, so we
      // will take a pass at that
      ...
      if (enable_highlight_v1) {
        ...
        for (const auto& highlight : highlights) {
          ...
          if (field_it == search_schema.end() || field_it->nested) {
            // nested field highlighting will be available only in the new
            // highlight structure.
            continue;
          }
      ...
      auto prune_op =
          prune_doc(document, include_fields_full, exclude_fields_full, "", 0,
      ...
      if (field_order_kv->match_score_index == CURATED_RECORD_IDENTIFIER) {
        ...
        if (!vector_query.field_name.empty()) {
          ...
          wrapper_doc["hybrid_search_info"]["rank_fusion_score"] =
              Index::int64_t_to_float(
      ...
      for (size_t sort_field_index = 0;
           sort_field_index < sort_fields_std.size(); sort_field_index++) {
        const auto& sort_field = sort_fields_std[sort_field_index];
        if (sort_field.geopoint != 0) {
          geo_distances[sort_field.name] =
              std::abs(field_order_kv->scores[sort_field_index]);
        }
      }
    ...
    if (group_limit) {
      ...
      const auto& itr =
          search_params->groups_processed.find(kv_group[0]->distinct_key);
  ...
  for (facet& a_facet : facets) {
    ...
    if (a_facet.is_range_query) {
      for (auto kv : a_facet.result_map) {
        ...
        if (facet_range_iter != a_facet.facet_range_map.end()) {
          ...
          facet_value_t facet_value = {facet_range_iter->second, std::string(),
                                       facet_count.count};
    ...
    std::nth_element(facet_hash_counts.begin(),
                     facet_hash_counts.begin() + max_facets,
    ...
    for (size_t fi = 0; fi < max_facets; fi++) {
      ...
      const std::string& seq_id_key =
          get_seq_id_key((uint32_t)facet_count.doc_id);
      nlohmann::json document;
      const Option<bool>& document_op =
          get_document_from_store(seq_id_key, document);
      if (!document_op.ok()) {
      ...
      const std::string& last_full_q_token =
          ftokens.empty() ? "" : ftokens.back();
      // 2 passes: first identify tokens that need to be highlighted and then
      // construct highlighted text
      bool is_cyrillic = Tokenizer::is_cyrillic(the_field.locale);
      bool normalise = is_cyrillic ? false : true;
      Tokenizer tokenizer(value, normalise, !the_field.is_string(),
                          the_field.locale, symbols_to_index, token_separators);
      // secondary tokenizer used for specific languages that requires
      // transliteration we use 2 tokenizers so that the original text offsets
      // are available for highlighting
      Tokenizer word_tokenizer("", true, false, the_field.locale,
                               symbols_to_index, token_separators);
      std::string raw_token;
      size_t raw_token_index = 0, tok_start = 0, tok_end = 0;
      // need an ordered map here to ensure that it is ordered by the key (start
      // offset)
        ...
        if (offset_it != token_offsets.end()) {
          if (i == offset_it->first) {
            ...
            size_t token_len = (i == prefix_token_start_index &&
                                token_offsets.size() == facet_query_num_tokens)
                                   ? facet_query_last_token.size()
                                   : (offset_it->second - i + 1);
            if (i == prefix_token_start_index &&
                token_offsets.size() == facet_query_num_tokens) {
              token_len = std::min((offset_it->second - i + 1),
                                   facet_query_last_token.size());
            } else {
      ...
      facet_value_t facet_value = {value, highlightedss.str(),
                                   facet_count.count};
      facet_values.emplace_back(facet_value);
    }

    std::stable_sort(facet_values.begin(), facet_values.end(),
                     Collection::facet_count_str_compare);
    ...
    if (a_facet.stats.fvcount != 0) {
      ...
      facet_result["stats"]["avg"] =
          (a_facet.stats.fvsum / a_facet.stats.fvcount);
    }
...
void Collection::copy_highlight_doc(
    std::vector<highlight_field_t>& hightlight_items,
    const bool nested_fields_enabled, const nlohmann::json& src,
    nlohmann::json& dst) {
  for (const auto& hightlight_item : hightlight_items) {
    ...
    if (dst.count(root_field_name) != 0) {
      // skip if parent "foo" has already has been copied over in e.g. foo.bar,
      // foo.baz
      continue;
    }

    // root field name might not exist if object has primitive field values with
    // "."s in the name
...
void Collection::process_search_field_weights(
  ...
  for (size_t i = 0; i < raw_search_fields.size(); i++) {
    if (!weights_given) {
      ...
      auto prev_weight =
          (i == 0) ? query_by_weights[0] : query_by_weights[i - 1];
      weights_in_desc_order =
          weights_in_desc_order && (query_by_weights[i] <= prev_weight);
      weights_undex_max =
          weights_undex_max && (query_by_weights[i] <= Index::FIELD_MAX_WEIGHT);
    }
  }
  ...
  if (weights_given && (!weights_in_desc_order || !weights_undex_max)) {
    ...
    std::sort(field_index_and_weights.begin(), field_index_and_weights.end(),
    ...
    for (size_t i = 0; i < field_index_and_weights.size(); i++) {
      ...
      // we have to also normalize weights to 0 to Index::FIELD_MAX_WEIGHT
      // range.
      if (i == 0) {
        ...
        if (curr_weight == prev_weight) {
          ...
          // bound to be lesser than prev_weight since weights have been sorted
          // desc
          uint32_t bounded_weight =
              std::max(0, int(query_by_weights[i - 1]) - 1);
...
uint64_t Collection::extract_bits(uint64_t value, unsigned lsb_offset,
                                  unsigned n) {
...
void Collection::populate_text_match_info(
    nlohmann::json& info, uint64_t match_score,
    const text_match_type_t match_type) const {
  // MAX_SCORE
  // [ sign | tokens_matched | max_field_score | max_field_weight |
  // num_matching_fields ] [   1  |        4       |        48       |       8
  // |         3           ]  (64 bits)

  // MAX_WEIGHT
  // [ sign | tokens_matched | max_field_weight | max_field_score  |
  // num_matching_fields ] [   1  |        4       |        8         |      48
  // |         3           ]  (64 bits)
  ...
  if (match_type == max_score) {
    ...
    info["best_field_score"] =
        std::to_string(extract_bits(match_score, 11, 48));
...
void Collection::process_highlight_fields(
    ...
    const std::vector<enable_t>& infixes, std::vector<std::string>& q_tokens,
  ...
  for (const std::string& highlight_full_field : highlight_full_field_names) {
    extract_field_name(highlight_full_field, search_schema,
                       fields_highlighted_fully_expanded, true,
                       enable_nested_fields);
  }
  ...
  for (size_t i = 0; i < search_fields.size(); i++) {
    ...
    enable_t field_infix = (search_fields[i].orig_index < infixes.size())
                               ? infixes[search_fields[i].orig_index]
                               : infixes[0];
    if (field_infix != off) {
  ...
  if (highlight_field_names.empty()) {
    ...
    for (size_t i = 0; i < raw_search_fields.size(); i++) {
      extract_field_name(raw_search_fields[i], search_schema,
                         highlight_field_names_expanded, false,
                         enable_nested_fields);
    }
    ...
    for (size_t i = 0; i < highlight_field_names_expanded.size(); i++) {
      ...
      if (exclude_fields.count(field_name) != 0) {
        // should not pick excluded field for highlighting (only for implicit
        // highlighting)
        continue;
      }
      ...
      bool fully_highlighted =
          (fields_highlighted_fully_set.count(field_name) != 0);
      bool infixed = (fields_infixed_set.count(field_name) != 0);
      auto schema_it = search_schema.find(field_name);
      bool is_string =
          (schema_it != search_schema.end()) && schema_it->is_string();
    ...
    for (size_t i = 0; i < highlight_field_names.size(); i++) {
      extract_field_name(highlight_field_names[i], search_schema,
                         highlight_field_names_expanded, false,
                         enable_nested_fields);
    }
    ...
    for (size_t i = 0; i < highlight_field_names_expanded.size(); i++) {
      ...
      bool fully_highlighted =
          (fields_highlighted_fully_set.count(highlight_field_name) != 0);
      bool infixed = (fields_infixed_set.count(highlight_field_name) != 0);
      bool is_string = schema_it->is_string();
      highlight_items.emplace_back(highlight_field_name, fully_highlighted,
                                   infixed, is_string);
    }
  }
  ...
  for (auto it = qtoken_set.begin(); it != qtoken_set.end(); ++it) {
    ...
    for (auto& highlight_item : highlight_items) {
      ...
      art_leaf* leaf = index->get_token_leaf(
          field_name, (const unsigned char*)qtoken.c_str(), qtoken.size() + 1);
      if (leaf) {
        highlight_item.qtoken_leaves.insert(
            qtoken, token_leaf(leaf, it.value().root_len, it.value().num_typos,
                               it.value().is_prefix));
      }
    }
  }
  ...
  // This helps handle highlighting of tokens which were dropped from the query
  // to return results.
  for (auto& q_token : q_tokens) {
    if (qtoken_set.find(q_token) == qtoken_set.end()) {
      for (auto& highlight_item : highlight_items) {
        ...
        const auto& field_name = highlight_item.name;
        art_leaf* leaf = index->get_token_leaf(
            field_name, (const unsigned char*)q_token.c_str(),
            q_token.size() + 1);
        if (leaf) {
          highlight_item.qtoken_leaves.insert(
              q_token, token_leaf(leaf, q_token.size(), 0, false));
        }
      }
    }
  }
...
void Collection::process_filter_overrides(
    std::vector<const override_t*>& filter_overrides,
    std::vector<std::string>& q_include_tokens, token_ordering token_order,
    filter_node_t*& filter_tree_root,
    std::vector<std::pair<uint32_t, uint32_t>>& included_ids,
    std::vector<uint32_t>& excluded_ids) const {
  std::vector<const override_t*> matched_dynamic_overrides;
  index->process_filter_overrides(filter_overrides, q_include_tokens,
                                  token_order, filter_tree_root,
                                  matched_dynamic_overrides);

  // we will check the dynamic overrides to see if they also have
  // include/exclude
  std::set<uint32_t> excluded_set;
...
void Collection::parse_search_query(
    const std::string& query, std::vector<std::string>& q_include_tokens,
    std::vector<std::vector<std::string>>& q_exclude_tokens,
    std::vector<std::vector<std::string>>& q_phrases, const std::string& locale,
    const bool already_segmented) const {
  if (query == "*") {
    q_exclude_tokens = {};
    ...
    if (already_segmented) {
      ...
      Tokenizer(query, true, false, locale, custom_symbols, token_separators)
          .tokenize(tokens);
    }
    ...
    auto symbols_to_index_has_minus =
        std::find(symbols_to_index.begin(), symbols_to_index.end(), '-') !=
        symbols_to_index.end();
    ...
    for (auto& token : tokens) {
      ...
      if (!token.empty() &&
          (token.back() == '"' || (token[0] == '"' && token.size() == 1))) {
        if (phrase_search_op_prior) {
          // handles single token phrase and a phrase with padded space, like:
          // "some query "
          end_of_phrase = true;
      ...
      // retokenize using collection config (handles hyphens being part of the
      // query)
      ...
      if (already_segmented) {
        ...
        Tokenizer(token, true, false, locale, symbols_to_index,
                  token_separators)
            .tokenize(sub_tokens);
      }
...
void Collection::populate_result_kvs(
    Topster* topster, std::vector<std::vector<KV*>>& result_kvs,
  ...
  if (topster->distinct) {
    ...
    for (size_t i = 0; i < gtopster.size; i++) {
      ...
      const std::vector<KV*> group_kvs(
          ...
          topster->group_kv_map[kv->distinct_key]->kvs +
              topster->group_kv_map[kv->distinct_key]->size);
...
Option<std::string> Collection::get_reference_field(
  ...
  if (reference_field_name.empty()) {
    return Option<std::string>(400, "Could not find any field in `" + name +
...
Option<bool> Collection::get_approximate_reference_filter_ids(
    const std::string& filter_query, uint32_t& filter_ids_length) const {
  std::shared_lock lock(mutex);

  const std::string doc_id_prefix =
      std::to_string(collection_id) + "_" + DOC_ID_PREFIX + "_";
  filter_node_t* filter_tree_root = nullptr;
  Option<bool> parse_op = filter::parse_filter_query(
      filter_query, search_schema, store, doc_id_prefix, filter_tree_root);
  std::unique_ptr<filter_node_t> filter_tree_root_guard(filter_tree_root);
  ...
  return index->get_approximate_reference_filter_ids_with_lock(
      filter_tree_root, filter_ids_length);
}

Option<bool> Collection::get_reference_filter_ids(
    const std::string& filter_query, filter_result_t& filter_result,
    const std::string& collection_name) const {
  std::shared_lock lock(mutex);
  ...
  return index->do_reference_filtering_with_lock(
      filter_tree_root, filter_result, name, field_name);
}

bool Collection::facet_value_to_string(const facet& a_facet,
                                       const facet_count_t& facet_count,
  ...
  if (document.count(a_facet.field_name) == 0) {
    ...
    LOG(ERROR) << "Could not find field " << a_facet.field_name
               << " in document during faceting.";
    LOG(ERROR) << "Facet field type: "
               << search_schema.at(a_facet.field_name).type;
  ...
  if (search_schema.at(a_facet.field_name).is_array()) {
    ...
    if (facet_count.array_pos >= array_sz) {
      LOG(ERROR) << "Facet field array size " << array_sz
                 << " lesser than array pos " << facet_count.array_pos
  ...
  if (search_schema.at(a_facet.field_name).type == field_types::STRING) {
  ...
  } else if (search_schema.at(a_facet.field_name).type ==
             field_types::STRING_ARRAY) {
    ...
    value = std::to_string(raw_val);
             ...
             field_types::INT32_ARRAY) {
    int32_t raw_val =
        document[a_facet.field_name][facet_count.array_pos].get<int32_t>();
    value = std::to_string(raw_val);
    ...
    value = std::to_string(raw_val);
             ...
             field_types::INT64_ARRAY) {
    int64_t raw_val =
        document[a_facet.field_name][facet_count.array_pos].get<int64_t>();
    value = std::to_string(raw_val);
    ...
    if (value != "0") {
      value.erase(value.find_last_not_of('0') + 1,
             ...
             field_types::FLOAT_ARRAY) {
    float raw_val =
        document[a_facet.field_name][facet_count.array_pos].get<float>();
    value = StringUtils::float_to_str(raw_val);
    value.erase(value.find_last_not_of('0') + 1,
                std::string::npos);  // remove trailing zeros
    ...
    value = (value == "1") ? "true" : "false";
             ...
             field_types::BOOL_ARRAY) {
    value = std::to_string(
        document[a_facet.field_name][facet_count.array_pos].get<bool>());
    value = (value == "1") ? "true" : "false";
  }
...
bool Collection::is_nested_array(const nlohmann::json& obj,
  ...
  if (child_it.value().is_array() && !child_it.value().empty() &&
      child_it.value().at(0).is_object()) {
    return true;
  }
...
void Collection::highlight_result(
    ...
    nlohmann::json& highlight_doc, StringUtils& string_utils,
    const size_t snippet_threshold, const size_t highlight_affix_num_tokens,
    bool highlight_fully, bool is_infix_search,
    const std::string& highlight_start_tag,
    const std::string& highlight_end_tag, const uint8_t* index_symbols,
    highlight_t& highlight, bool& found_highlight,
  ...
  bool use_word_tokenizer = search_field.locale == "th" ||
                            search_field.locale == "ja" ||
                            Tokenizer::is_cyrillic(search_field.locale);
  bool normalise = !use_word_tokenizer;

  std::vector<std::string> raw_query_tokens;
  Tokenizer(raw_query, normalise, false, search_field.locale, symbols_to_index,
            token_separators)
      .tokenize(raw_query_tokens);
  ...
  if (is_infix_search) {
    ...
    if (document.contains(search_field.name)) {
      ...
      for (size_t i = 0; i < array_len; i++) {
        std::string text = field_is_array ? document[search_field.name][i]
                                          : document[search_field.name];
        StringUtils::tolowercase(text);
        if (text.size() < 100 &&
            text.find(raw_query_tokens.front()) != std::string::npos) {
          const Match& this_match =
              Match(field_order_kv->key, empty_offsets, false, false);
    ...
    for(auto it = qtoken_leaves.begin(); it != qtoken_leaves.end(); ++it) {
        it.key(qtok_buff);
        LOG(INFO) << "Token: " << qtok_buff << ", root_len: " <<
    it.value().root_len;
    }*/
    ...
    if (!qtoken_leaves.empty()) {
      ...
      posting_t::get_array_token_positions(field_order_kv->key, posting_lists,
      ...
      for (const auto& kv : array_token_positions) {
        ...
        const Match& this_match =
            Match(field_order_kv->key, token_positions, true, true);
        uint64_t this_match_score =
            this_match.get_match_score(1, token_positions.size());
        match_indices.emplace_back(this_match, this_match_score, array_index);

        /*LOG(INFO) << "doc_id: " << document["id"] << ", search_field: " <<
           search_field.name
  ...
  const size_t max_array_matches =
      std::min((size_t)MAX_ARRAY_MATCHES, match_indices.size());
  std::partial_sort(match_indices.begin(),
                    match_indices.begin() + max_array_matches,
                    match_indices.end());

  highlight_nested_field(
      highlight_doc, highlight_doc, path_parts, 0, false, -1,
      [&](nlohmann::json& h_obj, bool is_arr_obj_ele, int array_i) {
        ...
        if (!is_arr_obj_ele) {
          // Since we will iterate on both matching and non-matching array
          // elements for highlighting, we need to check if `array_i`exists
          // within match_indices vec.

          for (size_t match_index = 0; match_index < match_indices.size();
               match_index++) {
          ...
          if (matched_index == -1) {
            // If an element does not belong to an array of object field and
            // also does not have a matching index we know that there cannot be
            // any matching tokens for highlighting
          ...
          // array of object element indices will not match indexed offsets, so
          // we will use dummy match the highlighting logic will ignore this and
          // try to do exhaustive highlighting (look at all tokens)
          match_indices.clear();
        ...
        for (size_t match_offset_i = 0;
             match_offset_i < match_index.match.offsets.size();
             match_offset_i++) {
        ...
        handle_highlight_text(
            text, normalise, search_field, symbols_to_index, token_separators,
            array_highlight, string_utils, use_word_tokenizer,
            highlight_affix_num_tokens, qtoken_leaves, last_valid_offset_index,
            prefix_token_num_chars, highlight_fully, snippet_threshold,
            is_infix_search, raw_query_tokens, last_valid_offset,
            highlight_start_tag, highlight_end_tag, index_symbols, match_index);

        if (array_highlight.snippets.empty() &&
            array_highlight.values.empty()) {
          h_obj["snippet"] = text;
          h_obj["matched_tokens"] = nlohmann::json::array();
        }
  ...
  for (size_t array_i = 0; array_i < max_array_matches; array_i++) {
    std::sort(match_indices[array_i].match.offsets.begin(),
              match_indices[array_i].match.offsets.end());
    ...
    for (size_t match_offset_index = 0;
         match_offset_index < match.offsets.size(); match_offset_index++) {
    ...
    /*LOG(INFO) << "field: " << document[search_field.name] << ", id: " <<
       field_order_kv->key
    ...
    if (search_field.type == field_types::STRING) {
      ...
      // since we try to do manual prefix matching on the first array value, we
      // have to check for an empty array
...
bool Collection::handle_highlight_text(
    std::string& text, bool normalise, const field& search_field,
    const std::vector<char>& symbols_to_index,
    const std::vector<char>& token_separators, highlight_t& highlight,
    StringUtils& string_utils, bool use_word_tokenizer,
    const size_t highlight_affix_num_tokens,
    const tsl::htrie_map<char, token_leaf>& qtoken_leaves,
    int last_valid_offset_index, const size_t prefix_token_num_chars,
    bool highlight_fully, const size_t snippet_threshold, bool is_infix_search,
    std::vector<std::string>& raw_query_tokens, size_t last_valid_offset,
    const std::string& highlight_start_tag,
    const std::string& highlight_end_tag, const uint8_t* index_symbols,
    const match_index_t& match_index) const {
  const Match& match = match_index.match;

  Tokenizer tokenizer(text, normalise, false, search_field.locale,
                      symbols_to_index, token_separators);

  // word tokenizer is a secondary tokenizer used for specific languages that
  // requires transliteration
  Tokenizer word_tokenizer("", true, false, search_field.locale,
                           symbols_to_index, token_separators);
  ...
  size_t text_len = Tokenizer::is_ascii_char(text[0])
                        ? text.size()
                        : StringUtils::get_num_chars(text);
  ...
  while (tokenizer.next(raw_token, raw_token_index, tok_start, tok_end)) {
    ...
    // ensures that the `snippet_start_offset` is always from a matched token,
    // and not from query suggestion
    bool match_offset_found =
        (found_first_match && token_already_found) ||
        (match_offset_index <= last_valid_offset_index &&
         match.offsets[match_offset_index].offset == raw_token_index);

    // Token might not appear in the best matched window, which is limited to a
    // size of 10. If field is marked to be highlighted fully, or field length
    // exceeds snippet_threshold, we will locate all tokens that appear in the
    // query / query candidates
    bool raw_token_found =
        !match_offset_found &&
        (highlight_fully || text_len < snippet_threshold * 6) &&
    ...
    if (match_offset_found || raw_token_found) {
      if (qtoken_it != qtoken_leaves.end() && qtoken_it.value().is_prefix &&
        ...
        // if length diff is within 2, we still might not want to highlight
        // partially in some cases e.g. "samsng" vs "samsung" -> full highlight
        // is preferred, unless it's a full prefix match
        ...
        while (k <= tok_end) {
          ...
          if (tokenizer.should_skip_char(text[k])) {
            // used to handle special characters inside a tokenized word, e.g.
            // `foo-bar`
            continue;
          }
      ...
      if (match_offset_found) {
        ...
        do {
          match_offset_index++;
                 ...
                 match.offsets[match_offset_index - 1].offset ==
                     match.offsets[match_offset_index].offset);
  ...
  if (raw_token_index <= snippet_threshold - 1) {
    // fully highlight field whose token size is less than given snippet
    // threshold
  ...
  // tokens from query might occur before actual snippet start offset: we skip
  // that
  auto offset_it = token_offsets.begin();
  while (offset_it != token_offsets.end() &&
         offset_it->first < snippet_start_offset) {
    offset_it++;
  }
...
void Collection::highlight_text(
    const string& highlight_start_tag, const string& highlight_end_tag,
    ...
    std::stringstream& highlighted_text, const uint8_t* index_symbols,
  ...
  while (snippet_start_offset <= snippet_end_offset) {
    if (offset_it != token_offsets.end()) {
      if (snippet_start_offset == offset_it->first) {
        ...
        // if a token ends with one or more puncutation chars, we should not
        // highlight them
        ...
        const std::string& text_token =
            text.substr(snippet_start_offset, token_len);
        ...
        for (size_t j = 0; j < token_len; j++) {
          if ((snippet_start_offset + j) >= text.size()) {
            LOG(ERROR) << "??? snippet_start_offset: " << snippet_start_offset
                       ...
                       << ", end_offset: " << end_offset << ", j: " << j
                       << ", token_len: " << token_len << ", text: " << text;
            break;
          }
...
void Collection::remove_document(const nlohmann::json& document,
...
Option<std::string> Collection::remove(const std::string& id,
  ...
  if (!get_doc_op.ok()) {
    if (get_doc_op.code() == 404) {
      LOG(ERROR) << "Sequence ID exists, but document is missing for id: "
                 << id;
      return Option<std::string>(404,
                                 "Could not find a document with id: " + id);
    }
...
Option<bool> Collection::remove_if_found(uint32_t seq_id,
                                         const bool remove_from_store) {
  nlohmann::json document;
  auto get_doc_op = get_document_from_store(get_seq_id_key(seq_id), document);
...
Option<uint32_t> Collection::add_override(const override_t& override,
  ...
  if (write_to_store) {
    bool inserted =
        store->insert(Collection::get_override_key(name, override.id),
                      override.to_json().dump());
    if (!inserted) {
...
Option<uint32_t> Collection::remove_override(const std::string& id) {
  if (overrides.count(id) != 0) {
    ...
    if (!removed) {
      return Option<uint32_t>(500,
                              "Error while deleting the override from disk.");
    }
...
std::string Collection::get_next_seq_id_key(
...
std::string Collection::get_seq_id_key(uint32_t seq_id) const {
  // We can't simply do std::to_string() because we want to preserve the byte
  // order. & 0xFF masks all but the lowest eight bits.
...
uint64_t Collection::get_created_at() const { return created_at.load(); }

size_t Collection::get_num_documents() const { return num_documents.load(); }

uint32_t Collection::get_collection_id() const { return collection_id.load(); }
...
std::string Collection::get_override_key(const std::string& collection_name,
                                         const std::string& override_id) {
  return std::string(COLLECTION_OVERRIDE_PREFIX) + "_" + collection_name + "_" +
         override_id;
}
...
Option<bool> Collection::get_document_from_store(const std::string& seq_id_key,
  ...
  if (json_doc_status != StoreStatus::FOUND) {
    ...
    if (json_doc_status == StoreStatus::NOT_FOUND) {
      return Option<bool>(
          404, "Could not locate the JSON document for sequence ID: " + seq_id);
    }

    return Option<bool>(
        500, "Error while fetching JSON document for sequence ID: " + seq_id);
  }
  ...
  try {
    ...
    return Option<bool>(
        500,
        "Error while parsing stored document with sequence ID: " + seq_id_key);
  }
...
const Index* Collection::_get_index() const { return index; }

Option<bool> Collection::parse_pinned_hits(
...
void Collection::synonym_reduction(
...
SynonymIndex* Collection::get_synonym_index() { return synonym_index; }

spp::sparse_hash_map<std::string, reference_pair>
Collection::get_reference_fields() {
...
Option<bool> Collection::persist_collection_meta() {
  ...
  StoreStatus status =
      store->get(Collection::get_meta_key(name), coll_meta_json);
  ...
  Option<bool> fields_json_op =
      field::fields_to_json_fields(fields, default_sorting_field, fields_json);
  ...
  collection_meta[Collection::COLLECTION_DEFAULT_SORTING_FIELD_KEY] =
...
Option<bool> Collection::batch_alter_data(
    const std::vector<field>& alter_fields,
  ...
  std::string upper_bound_key =
      get_seq_id_collection_prefix() + "`";  // cannot inline this
  ...
  while (iter->Valid() && iter->key().starts_with(seq_id_prefix)) {
    num_found_docs++;
    const uint32_t seq_id =
        Collection::get_seq_id_from_key(iter->key().ToString());
    ...
    try {
      ...
      return Option<bool>(
          400, "Bad JSON in document: " +
                   document.dump(-1, ' ', false,
    ...
    index_record record(num_found_docs, seq_id, document,
                        index_operation_t::CREATE, DIRTY_VALUES::REJECT);
    iter_batch.emplace_back(std::move(record));

    // Peek and check for last record right here so that we handle batched
    // indexing correctly Without doing this, the "last batch" would have to be
    // indexed outside the loop.
    iter->Next();
    bool last_record =
        !(iter->Valid() && iter->key().starts_with(seq_id_prefix));

    if (num_found_docs % index_batch_size == 0 || last_record) {
      // put delete first because a field could be deleted and added in the same
      // change set
      if (!del_fields.empty()) {
      ...
      Index::batch_memory_index(index, iter_batch, default_sorting_field,
                                schema_additions, embedding_fields,
                                fallback_field_type, token_separators,
                                symbols_to_index, true, 200, false);
  ...
  for (auto& del_field : del_fields) {
    ...
    auto new_end = std::remove_if(
        ...
        [&del_field](const field& f) { return f.name == del_field.name; });
...
Option<bool> Collection::alter(nlohmann::json& alter_payload) {
  ...
  // Validate that all stored documents are compatible with the proposed schema
  // changes.
  ...
  auto validate_op =
      validate_alter_payload(alter_payload, addition_fields, reindex_fields,
  ...
  auto batch_alter_op =
      batch_alter_data(addition_fields, del_fields, fallback_field_type);
...
void Collection::remove_flat_fields(nlohmann::json& document) {
  if (document.count(".flat") != 0) {
    for (const auto& flat_key :
         document[".flat"].get<std::vector<std::string>>()) {
...
Option<bool> Collection::prune_doc(
    nlohmann::json& doc, const tsl::htrie_set<char>& include_names,
    const tsl::htrie_set<char>& exclude_names, const std::string& parent_name,
    size_t depth, const reference_filter_result_t* reference_filter_result) {
  // doc can only be an object
  auto it = doc.begin();
  while (it != doc.end()) {
    std::string nested_name =
        parent_name + (parent_name.empty() ? it.key() : "." + it.key());

    // LOG(INFO) << "it.key(): " << it.key() << ", nested_name: " <<
    // nested_name;
    ...
    if (!include_names.empty() && prefix_it.first == prefix_it.second) {
      // prefix not found in allowed list of highlight field names, so can trim
      // early
      it = doc.erase(it);
      continue;
    }
    ...
    if (exclude_names.empty() && !include_names.empty() &&
        include_names.count(nested_name) != 0) {
      // without exclusions, we can pick the sub-tree early if parent name is
      // found in include names
      it++;
      continue;
    }
    ...
    if (it.value().is_object()) {
      ...
      prune_doc(it.value(), include_names, exclude_names, nested_name,
                depth + 1);
      ...
      while (arr_it != it.value().end()) {
        ...
        if (arr_it.value().is_object()) {
          ...
          prune_doc(arr_it.value(), include_names, exclude_names, nested_name,
                    depth + 1);
      ...
      if (!orig_array_empty && it.value().empty()) {
        // only drop field if array became empty because of pruning (and not
        // empty already)
        it = doc.erase(it);
        continue;
      }
    ...
    if (!include_names.empty() && include_names.count(nested_name) == 0) {
      // at this point, name should match fully, otherwise we should erase the
      // value
      it = doc.erase(it);
      continue;
    }
  ...
  for (auto reference = reference_it.first; reference != reference_it.second;
       reference++) {
    auto ref = reference.key();
    size_t parenthesis_index = ref.find('(');

    auto ref_collection_name = ref.substr(1, parenthesis_index - 1);
    auto reference_fields =
        ref.substr(parenthesis_index + 1, ref.size() - parenthesis_index - 2);
    ...
    auto include_exclude_op = collection->populate_include_exclude_fields(
        include_fields, exclude_fields, include_fields_full,
        exclude_fields_full);
    if (!include_exclude_op.ok()) {
    ...
    for (size_t i = 0; i < reference_filter_result->count; i++) {
      ...
      auto get_doc_op =
          collection->get_document_from_store(doc_seq_id, ref_doc);
      if (!get_doc_op.ok()) {
        return get_doc_op;
      }

      auto prune_op =
          prune_doc(ref_doc, include_fields_full, exclude_fields_full);
      if (!prune_op.ok()) {
        return prune_op;
      }
...
Option<bool> Collection::validate_alter_payload(
    nlohmann::json& schema_changes, std::vector<field>& addition_fields,
    std::vector<field>& reindex_fields, std::vector<field>& del_fields,
  ...
  const std::string err_msg =
      "The `fields` value should be an array of objects containing "
      "the field `name` and other properties.";

  if (!schema_changes.contains("fields") ||
      !schema_changes["fields"].is_array() ||
  ...
  // we will first do a pass at basic validations and pick out fields to be
  // deleted
  ...
  for (const auto& kv : schema_changes["fields"].items()) {
    ...
    if (kv.value().contains("drop")) {
      if (!kv.value()["drop"].is_boolean() || !kv.value()["drop"].get<bool>()) {
        return Option<bool>(400, "Field `" + field_name +
                                     "` must have a drop value of `true`.");
      }
      ...
      if (found_field) {
        ...
        if (field_it.value().nested && enable_nested_fields) {
          ...
          for (auto prefix_kv = prefix_it.first; prefix_kv != prefix_it.second;
               ++prefix_kv) {
            bool exact_key_match =
                (prefix_kv.key().size() == field_name.size());
            if (!exact_key_match) {
      ...
      // NOTE: fields with type "auto" or "string*" will exist in both
      // `search_schema` and `dynamic_fields`
      if (found_dyn_field) {
        del_fields.push_back(dyn_field_it->second);
        // we will also have to resolve the actual field names which match the
        ...
        for (auto& a_field : search_schema) {
          if (std::regex_match(a_field.name, std::regex(dyn_field_it->first))) {
            ...
            // if schema contains explicit fields that match dynamic field
            // that're going to be removed, we will have to remove them from the
            // schema so that validation can occur properly
      ...
      if (is_addition || is_reindex) {
        ...
        auto parse_op = field::json_field_to_field(
            enable_nested_fields, kv.value(), diff_fields, fallback_field_type,
        ...
        if (!f.embed.empty()) {
          auto validate_res = field::validate_and_init_embed_field(
              search_schema, schema_changes["fields"][json_array_index],
        ...
        return Option<bool>(400, "Field `" + field_name +
                                     "` is already part of the schema: To "
                                     "change this field, drop it first before "
                                     "adding it back to the schema.");
      }
    }
  }
  ...
  // data validations: here we ensure that already stored data is compatible
  // with requested schema changes
  const std::string seq_id_prefix = get_seq_id_collection_prefix();
  std::string upper_bound_key =
      get_seq_id_collection_prefix() + "`";  // cannot inline this
  rocksdb::Slice upper_bound(upper_bound_key);
  ...
  while (iter->Valid() && iter->key().starts_with(seq_id_prefix)) {
    ...
    if (!fallback_field_type.empty() || !new_dynamic_fields.empty() ||
        !updated_nested_fields.empty()) {
      std::vector<field> new_fields;
      Option<bool> new_fields_op = detect_new_fields(
          document, DIRTY_VALUES::DROP, updated_search_schema,
          new_dynamic_fields, updated_nested_fields, fallback_field_type, false,
      ...
      for (auto& new_field : new_fields) {
        if (updated_search_schema.find(new_field.name) ==
            updated_search_schema.end()) {
    ...
    // validate existing data on disk for compatibility via
    // updated_search_schema
    auto validate_op = validator_t::validate_index_in_memory(
        document, seq_id, default_sorting_field, updated_search_schema,
        updated_embedding_fields, index_operation_t::CREATE, false,
        fallback_field_type, DIRTY_VALUES::COERCE_OR_REJECT);
    if (!validate_op.ok()) {
      std::string err_message = validate_op.error();

      // we've to message the error message to suite the schema alter context
      if (err_message.find("but is not found in the document.") !=
          std::string::npos) {
        // missing field
        err_message.pop_back();  // delete trailing dot
        err_message +=
            "s already present in the collection. If you still want to add "
            "this field, "
        ...
        std::string type_error =
            "Schema change is incompatible with the type of documents already "
            "stored "
            "in this collection.";
        ...
        if (err_parts.size() == 2) {
          ...
          type_error += " Existing data for " + err_parts[0] +
                        " cannot be coerced into " + err_parts[1];
        }
...
Option<bool> Collection::resolve_field_type(
    field& new_field, nlohmann::detail::iter_impl<nlohmann::basic_json<>>& kv,
    nlohmann::json& document, const DIRTY_VALUES& dirty_values,
    const bool found_dynamic_field, const std::string& fallback_field_type,
    const bool enable_nested_fields, std::vector<field>& new_fields) {
  if (!new_field.index) {
    return Option<bool>(true);
  }
  ...
  const std::string& test_field_type =
      found_dynamic_field ? new_field.type : fallback_field_type;

  if (test_field_type == field_types::AUTO ||
      field_types::is_string_or_array(test_field_type)) {
    if (kv.key() == ".*") {
      return Option<bool>(true);
    }
    ...
    if (!parseable) {
      ...
      if (dirty_values == DIRTY_VALUES::REJECT ||
        ...
        return Option<bool>(400,
                            "Type of field `" + kv.key() + "` is invalid.");
      } else {
...
Option<bool> Collection::detect_new_fields(
    nlohmann::json& document, const DIRTY_VALUES& dirty_values,
    const tsl::htrie_map<char, field>& schema,
    const std::unordered_map<std::string, field>& dyn_fields,
    tsl::htrie_map<char, field>& nested_fields,
    const std::string& fallback_field_type, bool is_update,
    std::vector<field>& new_fields, const bool enable_nested_fields) {
    ...
    if (schema.count(kv.key()) == 0 && kv.key() != "id") {
      ...
      for (auto dyn_field_it = dyn_fields.begin();
        ...
        if (std::regex_match(kv.key(), std::regex(dynamic_field.name))) {
          // to prevent confusion we also disallow dynamic field names that
          // contain ".*"
          ...
          if (kv->is_object() &&
              dynamic_field.name.find(".*") == kv.key().size()) {
            // e.g. { name => price.*, type: float } to match price.USD,
            // price.UK etc. top-level price field should be treated as type
            // `object` and NOT `float`.
            new_field.nested = true;
      ...
      if (!found_dynamic_field && fallback_field_type.empty()) {
        // we will not auto detect schema for non-dynamic fields if auto
        // detection is not enabled
        kv++;
        continue;
      }

      auto add_op = resolve_field_type(new_field, kv, document, dirty_values,
                                       found_dynamic_field, fallback_field_type,
                                       enable_nested_fields, new_fields);
      if (!add_op.ok()) {
        return add_op;
      }
  ...
  if (enable_nested_fields) {
    ...
    std::vector<field> flattened_fields;
    auto flatten_op = field::flatten_doc(document, nested_fields, dyn_fields,
                                         is_update, flattened_fields);
    if (!flatten_op.ok()) {
      return flatten_op;
    }
...
Index* Collection::init_index() {
  for (const field& field : fields) {
    ...
    if (!field.reference.empty()) {
      ...
      reference_fields.emplace(field.name,
                               reference_pair(collection_name, field_name));
    }
  }
  ...
  return new Index(name + std::to_string(0), collection_id, store,
                   synonym_index,
                   CollectionManager::get_instance().get_thread_pool(),
                   search_schema, symbols_to_index, token_separators);
}

DIRTY_VALUES Collection::parse_dirty_values_option(
    std::string& dirty_values) const {
  std::shared_lock lock(mutex);
  ...
  if (dirty_values_op.has_value()) {
    ...
    dirty_values_action =
        (fallback_field_type.empty() && dynamic_fields.empty())
            ? DIRTY_VALUES::REJECT
...
std::vector<char> Collection::to_char_array(
...
bool Collection::get_enable_nested_fields() { return enable_nested_fields; }

Option<bool> Collection::parse_facet(const std::string& facet_field,
  ...
  if (facet_field.find(":") != std::string::npos) {  // range based facet
    ...
    if (search_schema.count(field_name) == 0) {
      std::string error = "Could not find a facet field named `" + field_name +
                          "` in the schema.";
      return Option<bool>(404, error);
    }
    ...
    if (!a_field.is_integer() && !a_field.is_float()) {
      std::string error =
          "Range facet is restricted to only integer and float fields.";
      return Option<bool>(400, error);
    }
    ...
    auto range_string =
        std::string(facet_field.begin() + startpos + 1, facet_field.end() - 1);
    ...
    while (index < range_string.size()) {
      if (range_string[index] == ']') {
        if (range_open == true) {
          std::string range =
              range_string.substr(startpos, index + 1 - startpos);
    ...
    for (const auto& tup : tupVec) {
      ...
      if ((!range_map.empty()) &&
          (range_map.find(lower_range) == range_map.end())) {
    ...
    if (facet_field[facet_field.size() - 1] != '*') {
      return Option<bool>(404,
                          "Only prefix matching with a wildcard is allowed.");
    }
    ...
    if (pair.first == pair.second) {
      // not found
      std::string error = "Could not find a facet field for `" + facet_field +
                          "` in the schema.";
      return Option<bool>(404, error);
    }
    ...
    if (search_schema.count(facet_field) == 0 ||
        !search_schema.at(facet_field).facet) {
      std::string error = "Could not find a facet field named `" + facet_field +
                          "` in the schema.";
      return Option<bool>(404, error);
    }
...
Option<bool> Collection::populate_include_exclude_fields(
    const spp::sparse_hash_set<std::string>& include_fields,
    const spp::sparse_hash_set<std::string>& exclude_fields,
    tsl::htrie_set<char>& include_fields_full,
  ...
  for (auto& f_name : include_fields) {
    auto field_op = extract_field_name(
        f_name, search_schema, include_fields_vec, false, enable_nested_fields);
    if (!field_op.ok()) {
      if (field_op.code() == 404) {
        // field need not be part of schema to be included (could be a stored
        // value in the doc)
  ...
  for (auto& f_name : exclude_fields) {
    ...
    auto field_op = extract_field_name(
        f_name, search_schema, exclude_fields_vec, false, enable_nested_fields);
    if (!field_op.ok()) {
      if (field_op.code() == 404) {
        // field need not be part of schema to be excluded (could be a stored
        // value in the doc)
...
Option<bool> Collection::populate_include_exclude_fields_lk(
    const spp::sparse_hash_set<std::string>& include_fields,
    const spp::sparse_hash_set<std::string>& exclude_fields,
    tsl::htrie_set<char>& include_fields_full,
    tsl::htrie_set<char>& exclude_fields_full) const {
  std::shared_lock lock(mutex);
  return populate_include_exclude_fields(
      include_fields, exclude_fields, include_fields_full, exclude_fields_full);
}

// Removes the dropped field from embed_from of all embedding fields.
void Collection::process_remove_field_for_embedding_fields(
  ...
  for (auto& field : fields) {
    ...
    if (found_field) {
      // mark this embedding field as "garbage" if it has no more embed_from
      // fields
      if (embed_from_names.empty()) {
        garbage_embed_fields.push_back(field);
      } else {
        // the dropped field was present in `embed_from`, so we have to update
        // the field objects
...
void Collection::hide_credential(nlohmann::json& json,
  ...
  if (json.count(credential_name) != 0) {
    ...
    if (credential_name_str.size() > 5) {
      ...
      json[credential_name] = credential_name_str.replace(
          5, num_chars_to_replace, num_chars_to_replace, '*');
    } else {
...
Option<bool> Collection::truncate_after_top_k(const string& field_name,
                                              size_t k) {
