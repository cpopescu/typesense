...
TextEmbedder::TextEmbedder(const std::string& model_name) {
  ...
  std::string abs_path =
      TextEmbedderManager::get_absolute_model_path(model_name);
  LOG(INFO) << "Loading model from disk: " << abs_path;
  session_ =
      std::make_unique<Ort::Session>(env_, abs_path.c_str(), session_options);
  std::ifstream config_file(
      TextEmbedderManager::get_absolute_config_path(model_name));
  nlohmann::json config;
  config_file >> config;
  TokenizerType tokenizer_type =
      TextEmbedderManager::get_tokenizer_type(config);
  auto vocab_path = TextEmbedderManager::get_absolute_vocab_path(
      model_name, config["vocab_file_name"].get<std::string>());
  ...
  for (size_t i = 0; i < output_tensor_count; i++) {
    ...
    if (shape.size() == 3 && shape[0] == -1 && shape[1] == -1 && shape[2] > 0) {
      ...
      output_tensor_name =
          std::string(session_->GetOutputNameAllocated(i, allocator).get());
      num_dim = shape[2];
      break;
    }
  }
...
TextEmbedder::TextEmbedder(const nlohmann::json& model_config,
  ...
  if (model_namespace == "openai") {
    ...
    remote_embedder_ =
        std::make_unique<GCPEmbedder>(project_id, model_name, access_token,
...
std::vector<float> TextEmbedder::mean_pooling(
...
embedding_res_t TextEmbedder::Embed(const std::string& text,
  ...
  if (is_remote()) {
    return remote_embedder_->Embed(text, remote_embedder_timeout_ms,
    ...
    // If model is DistilBERT or sentencepiece, it has 2 inputs, else it has 3
    // inputs
    if (session_->GetInputCount() == 3) {
      input_node_names.push_back("token_type_ids");
    }
    input_shapes.push_back(
        {1, static_cast<int64_t>(encoded_input.input_ids.size())});
    input_shapes.push_back(
        {1, static_cast<int64_t>(encoded_input.attention_mask.size())});
    if (session_->GetInputCount() == 3) {
      // edge case: xlm_roberta does not have token_type_ids, but if the model
      // has it as input, we need to fill it with 0s
      if (encoded_input.token_type_ids.size() == 0) {
        encoded_input.token_type_ids.resize(encoded_input.input_ids.size(), 0);
      }

      input_shapes.push_back(
          {1, static_cast<int64_t>(encoded_input.token_type_ids.size())});
    }
    input_tensors.push_back(Ort::Value::CreateTensor<int64_t>(
        memory_info, encoded_input.input_ids.data(),
        encoded_input.input_ids.size(), input_shapes[0].data(),
        input_shapes[0].size()));
    input_tensors.push_back(Ort::Value::CreateTensor<int64_t>(
        memory_info, encoded_input.attention_mask.data(),
        encoded_input.attention_mask.size(), input_shapes[1].data(),
        input_shapes[1].size()));
    if (session_->GetInputCount() == 3) {
      input_tensors.push_back(Ort::Value::CreateTensor<int64_t>(
          memory_info, encoded_input.token_type_ids.data(),
          encoded_input.token_type_ids.size(), input_shapes[2].data(),
          input_shapes[2].size()));
    }
...
std::vector<embedding_res_t> TextEmbedder::batch_embed(
  ...
  if (!is_remote()) {
    ...
    for (int i = 0; i < inputs.size(); i += 8) {
      auto input_batch = std::vector<std::string>(
          inputs.begin() + i,
          inputs.begin() + std::min(i + 8, static_cast<int>(inputs.size())));
      ...
      input_shapes.push_back(
          {static_cast<int64_t>(encoded_inputs.input_ids.size()),
           static_cast<int64_t>(encoded_inputs.input_ids[0].size())});
      input_shapes.push_back(
          {static_cast<int64_t>(encoded_inputs.attention_mask.size()),
           static_cast<int64_t>(encoded_inputs.attention_mask[0].size())});
      if (session_->GetInputCount() == 3) {
        input_shapes.push_back(
            {static_cast<int64_t>(encoded_inputs.token_type_ids.size()),
             static_cast<int64_t>(encoded_inputs.token_type_ids[0].size())});
      }
      ...
      if (session_->GetInputCount() == 3) {
        for (int i = 0; i < encoded_inputs.token_type_ids.size(); i++) {
          for (int j = 0; j < encoded_inputs.token_type_ids[i].size(); j++) {
            token_type_ids_flatten.push_back(
                encoded_inputs.token_type_ids[i][j]);
          }
        }
      }

      input_tensors.push_back(Ort::Value::CreateTensor<int64_t>(
          memory_info, input_ids_flatten.data(), input_ids_flatten.size(),
          input_shapes[0].data(), input_shapes[0].size()));
      input_tensors.push_back(Ort::Value::CreateTensor<int64_t>(
          memory_info, attention_mask_flatten.data(),
          attention_mask_flatten.size(), input_shapes[1].data(),
          input_shapes[1].size()));
      if (session_->GetInputCount() == 3) {
        input_tensors.push_back(Ort::Value::CreateTensor<int64_t>(
            memory_info, token_type_ids_flatten.data(),
            token_type_ids_flatten.size(), input_shapes[2].data(),
            input_shapes[2].size()));
      }
      ...
      if (input_shapes[0][1] == 0) {
        for (int i = 0; i < input_batch.size(); i++) {
          outputs.push_back(embedding_res_t(
              400,
              nlohmann::json({{"error", "Invalid input: empty sequence"}})));
        }
        continue;
      }
    ...
    outputs = std::move(
        remote_embedder_->batch_embed(inputs, remote_embedding_batch_size));
  }
...
batch_encoded_input_t TextEmbedder::batch_encode(
    const std::vector<std::string>& inputs) {
  ...
  if (session_->GetInputCount() == 3) {
    ...
    if (std::strcmp(token_type_ids_name.get(), "token_type_ids") != 0) {
      ...
      return Option<bool>(400,
                          "Invalid model: token_type_ids tensor not found");
    }
  }
...
const size_t TextEmbedder::get_num_dim() const { return num_dim; }
