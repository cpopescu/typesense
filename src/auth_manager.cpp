...
Option<bool> AuthManager::init(Store* store,
                               const std::string& bootstrap_auth_key) {
  ...
  StoreStatus next_api_key_id_status =
      store->get(API_KEY_NEXT_ID_KEY, next_api_key_id_str);

  if (next_api_key_id_status == StoreStatus::ERROR) {
    return Option<bool>(
        500, "Error while fetching the next API key id from the store.");
  }

  if (next_api_key_id_status == StoreStatus::FOUND) {
    next_api_key_id =
        (uint32_t)StringUtils::deserialize_uint32_t(next_api_key_id_str);
  } else {
    next_api_key_id = 0;
  }
  ...
  LOG(INFO) << "Indexing " << api_key_json_strs.size()
            << " API key(s) found on disk.";
...
Option<api_key_t> AuthManager::get_key(uint32_t id, bool truncate_value) const {
  ...
  std::string api_key_store_key =
      std::string(API_KEYS_PREFIX) + "_" + std::to_string(id);
...
Option<api_key_t> AuthManager::create_key(api_key_t& api_key) {
  ...
  if (api_keys.count(api_key.value) != 0 ||
      api_key.value == bootstrap_auth_key) {
  ...
  std::string api_key_store_key =
      std::string(API_KEYS_PREFIX) + "_" + std::to_string(api_key.id);
...
bool AuthManager::authenticate(
  ...
  for (size_t i = 0; i < collection_keys.size(); i++) {
    ...
    if (key_it != api_keys.end()) {
      ...
      Option<bool> auth_op =
          authenticate_parse_params(coll_key, action, embedded_params);
      if (!auth_op.ok()) {
        return false;
      }
    }
    ...
    // lengths of embedded_params_vec and collection_keys are guaranteed by
    // upstream to be the same
...
bool AuthManager::regexp_match(const std::string& value,
                               const std::string& regexp) {
  try {
    return std::regex_match(value, std::regex(regexp));
  } catch (const std::exception& e) {
    LOG(ERROR) << "Error while matching regexp " << regexp << " against value "
               << value;
    return false;
  }
}

bool AuthManager::auth_against_key(const std::string& req_collection,
                                   const std::string& action,
                                   const api_key_t& api_key,
                                   const bool search_only) const {
  ...
  if (search_only) {
    ...
    if (api_key.actions.size() != 1 ||
        api_key.actions[0] != DOCUMENTS_SEARCH_ACTION) {
      LOG(ERROR) << fmt_error(std::string("Parent API key must allow only `") +
                                  DOCUMENTS_SEARCH_ACTION + "` action.",
                              api_key.value);
      return false;
    }
  } else {
    ...
    for (const std::string& allowed_action : api_key.actions) {
      if (allowed_action == "*" ||
          (action != "*" && allowed_action == action)) {
        action_is_allowed = true;
        break;
      }

      // e.g. collections:create or documents:create
      if (allowed_action.size() >= 2 &&
          allowed_action[allowed_action.size() - 2] == ':' &&
          allowed_action.back() == '*') {
        std::string allowed_resource =
            allowed_action.substr(0, allowed_action.size() - 2);
  ...
  for (const std::string& allowed_collection : api_key.collections) {
    if (allowed_collection == "*" || (allowed_collection == req_collection) ||
        req_collection.empty() ||
...
Option<bool> AuthManager::authenticate_parse_params(
    const collection_key_t& scoped_api_key, const std::string& action,
  ...
  const std::string& key_payload =
      StringUtils::base64_decode(scoped_api_key.api_key);
  ...
  const std::string& key_prefix =
      key_payload.substr(HMAC_BASE64_LEN, api_key_t::PREFIX_LEN);
  const std::string& custom_params =
      key_payload.substr(HMAC_BASE64_LEN + api_key_t::PREFIX_LEN);

  // Calculate and verify hmac against matching api key.
  // There could be several matching keys since we look up only on a 4-char
  // prefix.
  ...
  for (auto it = prefix_range.first; it != prefix_range.second; ++it) {
    ...
    bool auth_success =
        auth_against_key(scoped_api_key.collection, action, root_api_key, true);
    ...
    if (digest == hmacSHA256) {
      ...
      if (embedded_params.count("expires_at") != 0) {
        ...
        uint64_t expiry_ts =
            std::min(root_api_key.expires_at,
                     embedded_params["expires_at"].get<uint64_t>());
...
std::string AuthManager::fmt_error(std::string&& error,
                                   const std::string& key) {
  std::stringstream ss;
  ss << error << " Key prefix: " << key.substr(0, api_key_t::PREFIX_LEN)
     << ", SHA256: " << StringUtils::hash_sha256(key);
  return ss.str();
}

Option<uint32_t> api_key_t::validate(const nlohmann::json& key_obj) {
  auto mandatory_keys = {"description", "actions", "collections"};

  for (auto key : mandatory_keys) {
    if (key_obj.count(key) == 0) {
      return Option<uint32_t>(
          400, std::string("Could not find a `") + key + "` key.");
    }
  }
  ...
  if (key_obj.count("description") != 0 &&
      !key_obj["description"].is_string()) {
    return Option<uint32_t>(400,
                            std::string("Key description must be a string."));
  }

  if (!key_obj["actions"].is_array() || key_obj["actions"].empty()) {
    return Option<uint32_t>(
        400, "Wrong format for `actions`. It should be an array of string.");
  }
  ...
  if (key_obj.count("expires_at") != 0) {
    if (!key_obj["expires_at"].is_number_integer() ||
        key_obj["expires_at"].get<int64_t>() < 0) {
      return Option<uint32_t>(
          400,
...
bool AuthManager::add_item_to_params(
    std::map<std::string, std::string>& req_params,
    const nlohmann::detail::iteration_proxy_value<nlohmann::json::iterator>&
        item,
    bool overwrite) {
