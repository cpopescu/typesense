...
Option<uint32_t> validator_t::coerce_element(
    const field& a_field, nlohmann::json& document, nlohmann::json& doc_ele,
    const std::string& fallback_field_type, const DIRTY_VALUES& dirty_values) {
  ...
  if (a_field.type == field_types::STRING && !doc_ele.is_string()) {
    Option<uint32_t> coerce_op =
        coerce_string(dirty_values, fallback_field_type, a_field, document,
    ...
    if (!doc_ele.is_number_integer()) {
      ...
      if (!coerce_op.ok()) {
        return coerce_op;
             ...
             !doc_ele.is_number_integer()) {
    Option<uint32_t> coerce_op =
        coerce_int64_t(dirty_values, a_field, document, field_name, dummy_iter,
                       false, array_ele_erased);
    if (!coerce_op.ok()) {
      return coerce_op;
    }
    ...
    Option<uint32_t> coerce_op =
        coerce_float(dirty_values, a_field, document, field_name, dummy_iter,
                     false, array_ele_erased);
    if (!coerce_op.ok()) {
      return coerce_op;
    }
    ...
    Option<uint32_t> coerce_op =
        coerce_bool(dirty_values, a_field, document, field_name, dummy_iter,
                    false, array_ele_erased);
    if (!coerce_op.ok()) {
      return coerce_op;
    }
    ...
    if (!(doc_ele[0].is_number() && doc_ele[1].is_number())) {
      ...
      Option<uint32_t> coerce_op = coerce_geopoint(
          dirty_values, a_field, document, field_name, doc_ele[0], doc_ele[1],
          dummy_iter, false, array_ele_erased);
      if (!coerce_op.ok()) {
        return coerce_op;
      }
    ...
    // have to differentiate the geopoint[] type of a nested array object's
    // geopoint[] vs a simple nested field geopoint[] type of an array of
    // objects field won't be an array of array
    if (a_field.nested && a_field.type == field_types::GEOPOINT_ARRAY &&
        it != doc_ele.end() && it->is_number()) {
      if (!doc_ele.empty() && doc_ele.size() % 2 != 0) {
        return Option<>(400, "Nested field `" + field_name +
                                 "` does not contain valid geopoint values.");
      }
      ...
      for (size_t ai = 0; ai < doc_ele.size(); ai += 2) {
        if (!(doc_ele[ai].is_number() && doc_ele[ai + 1].is_number())) {
          ...
          Option<uint32_t> coerce_op = coerce_geopoint(
              dirty_values, a_field, document, field_name, doc_ele[ai],
              doc_ele[ai + 1], it, true, array_ele_erased);
          if (!coerce_op.ok()) {
            return coerce_op;
          }
        }
    ...
    if (a_field.type == field_types::FLOAT_ARRAY && a_field.num_dim != 0 &&
        a_field.num_dim != doc_ele.size()) {
      return Option<uint32_t>(400, "Field `" + a_field.name + "` must have " +
                                       std::to_string(a_field.num_dim) +
                                       " dimensions.");
    }
    ...
    for (; it != doc_ele.end();) {
      ...
      if (a_field.type == field_types::STRING_ARRAY && !item.is_string()) {
        ...
        Option<uint32_t> coerce_op =
            coerce_int32_t(dirty_values, a_field, document, field_name, it,
        ...
        if (!coerce_op.ok()) {
          return coerce_op;
                 ...
                 !item.is_number_integer()) {
        Option<uint32_t> coerce_op =
            coerce_int64_t(dirty_values, a_field, document, field_name, it,
                           true, array_ele_erased);
        if (!coerce_op.ok()) {
          return coerce_op;
        }
                 ...
                 !item.is_number()) {
        // we check for `is_number` to allow whole numbers to be passed into
        // float fields
        Option<uint32_t> coerce_op =
            coerce_float(dirty_values, a_field, document, field_name, it, true,
                         array_ele_erased);
        if (!coerce_op.ok()) {
          return coerce_op;
        }
                 ...
                 !item.is_boolean()) {
        Option<uint32_t> coerce_op =
            coerce_bool(dirty_values, a_field, document, field_name, it, true,
                        array_ele_erased);
        if (!coerce_op.ok()) {
          return coerce_op;
        }
        ...
        if (!item.is_array() || item.size() != 2) {
          return Option<>(
              400, "Field `" + field_name +
                       "` must contain 2 element arrays: [ [lat, lng],... ].");
        }
        ...
        if (!(item[0].is_number() && item[1].is_number())) {
          ...
          Option<uint32_t> coerce_op =
              coerce_geopoint(dirty_values, a_field, document, field_name,
                              item[0], item[1], it, true, array_ele_erased);
          if (!coerce_op.ok()) {
            return coerce_op;
          }
        }
...
Option<uint32_t> validator_t::coerce_string(
    const DIRTY_VALUES& dirty_values, const std::string& fallback_field_type,
  ...
  if (dirty_values == DIRTY_VALUES::REJECT) {
    if (a_field.nested && item.is_array()) {
      return Option<>(400, "Field `" + field_name +
                               ...
                               "Hint: field inside an array of objects must be "
                               "an array type as well.");
    }
    return Option<>(
  ...
  if (dirty_values == DIRTY_VALUES::DROP) {
    if (!a_field.optional) {
      if (a_field.nested && item.is_array()) {
        return Option<>(400, "Field `" + field_name +
                                 "` has an incorrect type. "
                                 "Hint: field inside an array of objects must "
                                 "be an array type as well.");
      }
      return Option<>(
          400, "Field `" + field_name + "` must be " + suffix + " string.");
    }
...
Option<uint32_t> validator_t::coerce_int32_t(
    const DIRTY_VALUES& dirty_values, const field& a_field,
  ...
  if (dirty_values == DIRTY_VALUES::REJECT) {
    ...
    return Option<>(400,
                    "Field `" + field_name + "` must be " + suffix + " int32.");
  }
  ...
  if (document.contains(field_name) &&
    ...
    if (a_field.optional && (dirty_values == DIRTY_VALUES::DROP ||
      ...
      return Option<>(
          400, "Field `" + field_name + "` exceeds maximum value of int32.");
    }
  }
...
Option<uint32_t> validator_t::coerce_int64_t(
    const DIRTY_VALUES& dirty_values, const field& a_field,
    nlohmann::json& document, const std::string& field_name,
    nlohmann::json::iterator& array_iter, bool is_array,
    bool& array_ele_erased) {
  ...
  if (dirty_values == DIRTY_VALUES::REJECT) {
    ...
    return Option<>(400,
                    "Field `" + field_name + "` must be " + suffix + " int64.");
  }
...
Option<uint32_t> validator_t::coerce_bool(const DIRTY_VALUES& dirty_values,
                                          const field& a_field,
  ...
  if (dirty_values == DIRTY_VALUES::REJECT) {
    ...
    return Option<>(400,
                    "Field `" + field_name + "` must be " + suffix + " bool.");
  }
...
Option<uint32_t> validator_t::coerce_geopoint(
    const DIRTY_VALUES& dirty_values, const field& a_field,
    nlohmann::json& document, const std::string& field_name,
...
Option<uint32_t> validator_t::coerce_float(const DIRTY_VALUES& dirty_values,
                                           const field& a_field,
                                           nlohmann::json& document,
                                           const std::string& field_name,
                                           nlohmann::json::iterator& array_iter,
  ...
  if (dirty_values == DIRTY_VALUES::REJECT) {
    ...
    return Option<>(400,
                    "Field `" + field_name + "` must be " + suffix + " float.");
  }
...
Option<uint32_t> validator_t::validate_index_in_memory(
    nlohmann::json& document, uint32_t seq_id,
    const std::string& default_sorting_field,
    const tsl::htrie_map<char, field>& search_schema,
    const tsl::htrie_map<char, field>& embedding_fields,
    const index_operation_t op, const bool is_update,
    const std::string& fallback_field_type, const DIRTY_VALUES& dirty_values,
    const bool validate_embedding_fields) {
  ...
  if ((op == CREATE || op == UPSERT) && missing_default_sort_field) {
    return Option<>(400, "Field `" + default_sorting_field +
  ...
  for (const auto& a_field : search_schema) {
    ...
    if ((a_field.optional || op == UPDATE || (op == EMPLACE && is_update)) &&
    ...
    if (a_field.optional && doc_ele.is_null()) {
      ...
      if (!is_update) {
        // for updates, the erasure is done later since we need to keep the key
        // for overwrite
        document.erase(field_name);
      }
      continue;
    ...
    auto coerce_op = coerce_element(a_field, document, doc_ele,
  ...
  if (validate_embedding_fields) {
    ...
    auto validate_embed_op = validate_embed_fields(document, embedding_fields,
                                                   search_schema, is_update);
...
Option<bool> validator_t::validate_embed_fields(
    const nlohmann::json& document,
    const tsl::htrie_map<char, field>& embedding_fields,
    const tsl::htrie_map<char, field>& search_schema, const bool& is_update) {
  for (const auto& field : embedding_fields) {
    if (document.contains(field.name) && !is_update) {
      const auto& field_vec = document[field.name];
      if (!field_vec.is_array() || field_vec.empty() ||
          !field_vec[0].is_number() || field_vec.size() != field.num_dim) {
        return Option<bool>(
            400, "Field `" + field.name + "` contains an invalid embedding.");
      }
      ...
      while (it != field_vec.end()) {
        if (!it.value().is_number()) {
          return Option<bool>(
              400, "Field `" + field.name + "` contains invalid float values.");
        }
        it++;
      }
      ...
      if (schema_field_it == search_schema.end()) {
        return Option<bool>(
            400, "Field `" + field.name +
                     "` has invalid fields to create embeddings from.");
      }
      if (doc_field_it == document.end()) {
        if (!is_update && !schema_field_it->optional) {
          return Option<bool>(
              400, "Field `" + field_name + "` is needed to create embedding.");
        } else {
          continue;
        }
      }
      all_optional_and_null = false;
      if ((schema_field_it.value().type == field_types::STRING &&
           !doc_field_it.value().is_string()) ||
          (schema_field_it.value().type == field_types::STRING_ARRAY &&
           !doc_field_it.value().is_array())) {
        return Option<bool>(400,
                            "Field `" + field_name + "` has malformed data.");
      }
      if (doc_field_it.value().is_array()) {
        for (const auto& val : doc_field_it.value()) {
          if (!val.is_string()) {
            return Option<bool>(
                400, "Field `" + field_name + "` has malformed data.");
          }
        }
      }
    }
    if (all_optional_and_null && !field.optional && !is_update) {
      return Option<bool>(
          400, "No valid fields found to create embedding for `" + field.name +
                   "`, please provide at least one valid field or make the "
