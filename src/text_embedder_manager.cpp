...
Option<bool> TextEmbedderManager::validate_and_init_model(
    const nlohmann::json& model_config, size_t& num_dims) {
...
Option<bool> TextEmbedderManager::validate_and_init_remote_model(
    const nlohmann::json& model_config, size_t& num_dims) {
  const std::string& model_name = model_config["model_name"].get<std::string>();
  ...
  std::string model_key = is_remote_model(model_name)
                              ? RemoteEmbedder::get_model_key(model_config)
                              : model_name;
  auto text_embedder_it = text_embedders.find(model_key);
  if (text_embedder_it == text_embedders.end()) {
    text_embedders.emplace(
        model_key, std::make_shared<TextEmbedder>(model_config, num_dims));
  }

  return Option<bool>(true);
}

Option<bool> TextEmbedderManager::validate_and_init_local_model(
    const nlohmann::json& model_config, size_t& num_dims) {
  const std::string& model_name = model_config["model_name"].get<std::string>();
  Option<bool> public_model_op =
      TextEmbedderManager::get_instance().init_public_model(model_name);
  ...
  if (!is_public_model) {
    if (!std::filesystem::exists(
            TextEmbedderManager::get_absolute_config_path(model_name))) {
    ...
    if (!config["model_type"].is_string() ||
        !config["vocab_file_name"].is_string()) {
      LOG(ERROR) << "Invalid config file: "
                 << TextEmbedderManager::get_absolute_config_path(model_name);
      return Option<bool>(400, "Invalid config file");
    }
    ...
    if (!std::filesystem::exists(
            TextEmbedderManager::get_model_subdir(model_name) + "/" +
            config["vocab_file_name"].get<std::string>())) {
      LOG(ERROR) << "Vocab file not found: "
                 << TextEmbedderManager::get_model_subdir(model_name) + "/" +
                        config["vocab_file_name"].get<std::string>();
      return Option<bool>(400, "Vocab file not found");
    }

    if (config["model_type"].get<std::string>() != "bert" &&
        config["model_type"].get<std::string>() != "xlm_roberta" &&
        config["model_type"].get<std::string>() != "distilbert") {
      LOG(ERROR) << "Invalid model type: "
                 << config["model_type"].get<std::string>();
  ...
  const std::shared_ptr<TextEmbedder>& embedder =
      std::make_shared<TextEmbedder>(
...
Option<TextEmbedder*> TextEmbedderManager::get_text_embedder(
...
const TokenizerType TextEmbedderManager::get_tokenizer_type(
    const nlohmann::json& model_config) {
  if (model_config.find("model_type") == model_config.end()) {
    return TokenizerType::bert;
  } else {
    std::string tokenizer_type =
        model_config.at("model_type").get<std::string>();
...
const std::string TextEmbedderManager::get_indexing_prefix(
    const nlohmann::json& model_config) {
  std::string val;
  if (is_public_model(model_config["model_name"].get<std::string>())) {
    std::unique_lock<std::mutex> lock(text_embedders_mutex);
    val = public_models[model_config["model_name"].get<std::string>()]
              .indexing_prefix;
  } else {
    val = model_config.count("indexing_prefix") == 0
              ? ""
              : model_config["indexing_prefix"].get<std::string>();
  }
  if (!val.empty()) {
    val += " ";
  }
...
const std::string TextEmbedderManager::get_query_prefix(
    const nlohmann::json& model_config) {
  std::string val;
  if (is_public_model(model_config["model_name"].get<std::string>())) {
    std::unique_lock<std::mutex> lock(text_embedders_mutex);
    ...
    val = model_config.count("query_prefix") == 0
              ? ""
              : model_config["query_prefix"].get<std::string>();
  }
  if (!val.empty()) {
    val += " ";
  }
...
const std::string& TextEmbedderManager::get_model_dir() { return model_dir; }

TextEmbedderManager::~TextEmbedderManager() {}

const std::string TextEmbedderManager::get_absolute_model_path(
    const std::string& model_name) {
  return get_model_subdir(model_name) + "/model.onnx";
}
const std::string TextEmbedderManager::get_absolute_vocab_path(
    const std::string& model_name, const std::string& vocab_file_name) {
  return get_model_subdir(model_name) + "/" + vocab_file_name;
}

const std::string TextEmbedderManager::get_absolute_config_path(
    const std::string& model_name) {
  return get_model_subdir(model_name) + "/config.json";
}

const bool TextEmbedderManager::check_md5(const std::string& file_path,
                                          const std::string& target_md5) {
...
Option<bool> TextEmbedderManager::download_public_model(
    const text_embedding_model& model) {
  ...
  if (!check_md5(
          get_absolute_vocab_path(actual_model_name, model.vocab_file_name),
          model.vocab_md5)) {
    long res = httpClient.download_file(
        get_vocab_url(model),
        get_absolute_vocab_path(actual_model_name, model.vocab_file_name));
    if (res != 200) {
      LOG(INFO) << "Failed to download default vocab for model: "
                << model.model_name;
...
Option<bool> TextEmbedderManager::init_public_model(
    const std::string& model_name) {
  std::unique_lock<std::mutex> lock(text_embedders_mutex);
  ...
  auto download_op =
      TextEmbedderManager::get_instance().download_public_model(model);
  if (!download_op.ok()) {
...
const std::string TextEmbedderManager::get_model_subdir(
    const std::string& model_name) {
  if (model_dir.back() != '/') {
...
Option<std::string> TextEmbedderManager::get_namespace(
    const std::string& model_name) {
...
const std::string TextEmbedderManager::get_model_name_without_namespace(
    const std::string& model_name) {
  // <namespace>/<model_name> if / is present in model_name
  if (model_name.find("/") != std::string::npos) {
...
Option<nlohmann::json> TextEmbedderManager::get_public_model_config(
    const std::string& model_name) {
  // check cache first
  ...
  long res = httpClient.get_response(
      MODELS_REPO_URL + actual_model_name + "/" + MODEL_CONFIG_FILE,
      response_body, response_headers, headers);
...
const std::string TextEmbedderManager::get_model_url(
    const text_embedding_model& model) {
  return MODELS_REPO_URL + model.model_name + "/model.onnx";
}

const std::string TextEmbedderManager::get_vocab_url(
    const text_embedding_model& model) {
  return MODELS_REPO_URL + model.model_name + "/" + model.vocab_file_name;
}

const std::string TextEmbedderManager::get_model_namespace(
    const std::string& model_name) {
  if (model_name.find("/") != std::string::npos) {
...
bool TextEmbedderManager::is_remote_model(const std::string& model_name) {
  auto model_namespace = get_namespace(model_name);
  return model_namespace.ok() &&
         (model_namespace.get() == "openai" ||
          model_namespace.get() == "google" || model_namespace.get() == "gcp");
}
