...
Option<bool> VectorQueryOps::parse_vector_query_str(
    const std::string& vector_query_str, vector_query_t& vector_query,
    const bool is_wildcard_query, const Collection* coll) {
  // FORMAT:
    ...
    if (vector_query_str[i] != ':') {
      ...
      if (vector_query_str[i] != ':') {
        ...
        return Option<bool>(400,
                            "Malformed vector query string: `:` is missing.");
      }
      ...
      for (auto& svalue : svalues) {
        if (!StringUtils::is_float(svalue)) {
          return Option<bool>(400,
                              "Malformed vector query string: one of the "
                              "vector values is not a float.");
        }
      ...
      if (i == vector_query_str.size() - 1) {
        ...
        if (vector_query.values.empty()) {
          // when query values are missing, atleast the `id` parameter must be
          // present
          return Option<bool>(400,
                              "When a vector query value is empty, an `id` "
      ...
      std::string param_str =
          vector_query_str.substr(i, (vector_query_str.size() - i));
      ...
      for (auto& param_kv_str : param_kvs) {
        ...
        if (param_kv[0] == "id") {
          if (!vector_query.values.empty()) {
            ...
            return Option<bool>(
                400,
                "Malformed vector query string: cannot pass both vector query "
                "and `id` parameter.");
          }
          ...
          if (!id_op.ok()) {
            return Option<bool>(
                400, "Document id referenced in vector query is not found.");
          }
          ...
          if (!document.contains(vector_query.field_name) ||
              !document[vector_query.field_name].is_array()) {
            return Option<bool>(
                400,
                "Document referenced in vector query does not contain a valid "
                "vector field.");
          }
          ...
          for (auto& fvalue : document[vector_query.field_name]) {
            if (!fvalue.is_number_float()) {
              return Option<bool>(400,
                                  "Document referenced in vector query does "
                                  "not contain a valid "
                                  "vector field.");
            }
        ...
        if (param_kv[0] == "k") {
          if (!StringUtils::is_uint32_t(param_kv[1])) {
            return Option<bool>(400,
                                "Malformed vector query string: `k` parameter "
                                "must be an integer.");
          }
        ...
        if (param_kv[0] == "distance_threshold") {
          if (!StringUtils::is_float(param_kv[1]) ||
              std::stof(param_kv[1]) < 0.0 || std::stof(param_kv[1]) > 2.0) {
            return Option<bool>(400,
                                "Malformed vector query string: "
                                "`distance_threshold` parameter must be a "
                                "float between 0.0-2.0.");
          }
      ...
      if (is_wildcard_query && !vector_query.query_doc_given &&
          vector_query.values.empty()) {
        return Option<bool>(400,
                            "When a vector query value is empty, an `id` "
                            "parameter must be present.");
      }
