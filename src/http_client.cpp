...
long HttpClient::post_response_async(const std::string &url,
                                     const std::shared_ptr<http_req> request,
                                     const std::shared_ptr<http_res> response,
                                     HttpServer *server,
                                     bool send_ts_api_header) {
  deferred_req_res_t *req_res =
      new deferred_req_res_t(request, response, server, false);
...
long HttpClient::put_response(const std::string &url, const std::string &body,
                              std::string &response,
                              std::map<std::string, std::string> &res_headers,
                              long timeout_ms, bool send_ts_api_header) {
  CURL *curl = init_curl(url, response, timeout_ms);
...
long HttpClient::patch_response(const std::string &url, const std::string &body,
                                std::string &response,
                                std::map<std::string, std::string> &res_headers,
                                long timeout_ms, bool send_ts_api_header) {
  CURL *curl = init_curl(url, response, timeout_ms);
...
long HttpClient::delete_response(
...
long HttpClient::perform_curl(CURL *curl,
  ...
  if (send_ts_api_header) {
    std::string api_key_header =
        std::string("x-typesense-api-key: ") + HttpClient::api_key;
  ...
  if (res != CURLE_OK) {
    ...
    if (res == CURLE_OPERATION_TIMEDOUT) {
      ...
      LOG(ERROR) << "CURL timeout. Time taken: " << total_time
                 << ", URL: " << url;
      status_code = 408;
    } else {
      LOG(ERROR) << "CURL failed. URL: " << url << ", Code: " << res
...
void HttpClient::extract_response_headers(
    CURL *curl, std::map<std::string, std::string> &res_headers) {
  char *content_type;
...
size_t HttpClient::curl_req_send_callback(char *buffer, size_t size,
  ...
  if (!req_res->res->is_alive) {
    // underlying client request is dead, don't proxy anymore data to upstream
    // (leader)
  ...
  size_t available_body_bytes =
      (req_res->req->body.size() - req_res->req->body_index);
  ...
  /*LOG(INFO) << "Wrote " << bytes_to_read << " bytes to request body
  (max_buffer_bytes=" << max_req_bytes << ")"; LOG(INFO) <<
  "req_res->req->body_index: " << req_res->req->body_index
  ...
  if (req_res->req->body_index == req_res->req->body.size()) {
    ...
    if (!req_res->req->last_chunk_aggregate) {
      ...
      // LOG(INFO) << "Buffer refilled, unpausing request forwarding,
      // body_size=" << req_res->req->body.size();
    }
  }
...
size_t HttpClient::curl_write_async(char *buffer, size_t size, size_t nmemb,
                                    void *context) {
  ...
  if (req_res->res->status_code == 0) {
    client_state_t *client_state =
        dynamic_cast<client_state_t *>(req_res->req->data);
  ...
  // we've got response from remote host: write to client and ask for more
  // request body
  ...
  async_req_res_t *async_req_res =
      new async_req_res_t(req_res->req, req_res->res, true);
  req_res->server->get_message_dispatcher()->send_message(
...
size_t HttpClient::curl_write_async_done(void *context, curl_socket_t item) {
  ...
  // wait until final response is flushed or response object will be destroyed
  // by caller
  // LOG(INFO) << "Waiting on req_res " << req_res->res;
  req_res->res->wait();
...
CURL *HttpClient::init_curl_async(const std::string &url,
                                  deferred_req_res_t *req_res,
                                  curl_slist *&chunk, bool send_ts_api_header) {
  ...
  req_res->req->data = new client_state_t(
      curl);  // destruction of data is managed by req destructor
  ...
  std::string content_length_header =
      ...
      std::to_string(req_res->req->_req->content_length);
  chunk = curl_slist_append(chunk, content_length_header.c_str());

  curl_easy_setopt(curl, CURLOPT_HTTPHEADER, chunk);

  // Enabling this causes issues in mixed mode: client using http/1 but follower
  // -> leader using http/2
  ...
  curl_easy_setopt(curl, CURLOPT_READFUNCTION,
                   HttpClient::curl_req_send_callback);
  ...
  curl_easy_setopt(curl, CURLOPT_CLOSESOCKETFUNCTION,
                   HttpClient::curl_write_async_done);
...
size_t HttpClient::curl_write(char *contents, size_t size, size_t nmemb,
                              std::string *s) {
  s->append(contents, size * nmemb);
  return size * nmemb;
}

size_t HttpClient::curl_write_download(void *ptr, size_t size, size_t nmemb,
                                       FILE *stream) {
  size_t written = fwrite(ptr, size, nmemb, stream);
  return written;
}

long HttpClient::download_file(const std::string &url,
  ...
  if (res_code != CURLE_OK) {
    LOG(ERROR) << "Unable to download file: " << url << " to " << file_path
