...
void num_tree_t::insert(int64_t value, uint32_t id) {
  if (int64map.count(value) == 0) {
    int64map.emplace(value,
                     SET_COMPACT_IDS(compact_id_list_t::create(1, {id})));
  } else {
...
void num_tree_t::range_inclusive_search(int64_t start, int64_t end,
                                        uint32_t** ids, size_t& ids_len) {
  if (int64map.empty()) {
    return;
  }
...
void num_tree_t::approx_range_inclusive_search_count(int64_t start, int64_t end,
                                                     uint32_t& ids_len) {
  if (int64map.empty()) {
    return;
  }
...
bool num_tree_t::range_inclusive_contains(const int64_t& start,
                                          const int64_t& end,
...
void num_tree_t::range_inclusive_contains(const int64_t& start,
                                          const int64_t& end,
  ...
  uint32_t* out = nullptr;
  result_ids_len =
      ArrayUtils::or_scalar(&consolidated_ids[0], consolidated_ids.size(),
...
void num_tree_t::search(NUM_COMPARATOR comparator, int64_t value,
                        uint32_t** ids, size_t& ids_len) {
  if (int64map.empty()) {
    return;
  }
  ...
  if (comparator == EQUALS) {
    ...
    if (it != int64map.end()) {
      ...
      ids_len = ArrayUtils::or_scalar(val_ids, ids_t::num_ids(it->second), *ids,
                                      ids_len, &out);
      delete[] *ids;
      *ids = out;
      delete[] val_ids;
    ...
    gfx::timsort(consolidated_ids.begin(), consolidated_ids.end());
    consolidated_ids.erase(
        unique(consolidated_ids.begin(), consolidated_ids.end()),
        consolidated_ids.end());

    uint32_t* out = nullptr;
    ids_len = ArrayUtils::or_scalar(
        &consolidated_ids[0], consolidated_ids.size(), *ids, ids_len, &out);
    ...
    *ids = out;
    ...
    if (it != int64map.end() && comparator == LESS_THAN_EQUALS &&
        it->first == value) {
      ids_t::uncompress(it->second, consolidated_ids);
    }
...
void num_tree_t::approx_search_count(NUM_COMPARATOR comparator, int64_t value,
                                     uint32_t& ids_len) {
  if (int64map.empty()) {
    return;
  }
...
void num_tree_t::contains(const NUM_COMPARATOR& comparator,
                          const int64_t& value,
                          const uint32_t& context_ids_length,
                          uint32_t* const& context_ids, size_t& result_ids_len,
                          uint32_t*& result_ids) const {
  if (int64map.empty()) {
    return;
  }
...
void num_tree_t::seq_ids_outside_top_k(size_t k,
...
size_t num_tree_t::size() { return int64map.size(); }
