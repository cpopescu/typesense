...
cached_resource_stat_t::has_enough_resources(
    const std::string& data_dir_path, const int disk_used_max_percentage,
  ...
  double disk_used_percentage =
      (double(disk_used_bytes) / double(disk_total_bytes)) * 100;
  if (disk_used_percentage > disk_used_max_percentage) {
    LOG(INFO) << "disk_total_bytes: " << disk_total_bytes
  ...
  uint64_t all_memory_used = (memory_total_bytes - memory_available_bytes) +
  ...
  // compare with 500M or `100 - memory_used_max_percentage` of total memory,
  // whichever is lower
  uint64_t memory_free_min_bytes = std::min<uint64_t>(
      500ULL * 1024 * 1024,
  ...
  if (free_mem < memory_free_min_bytes) {
    LOG(INFO) << "memory_total: " << memory_total_bytes
              << ", memory_available: " << memory_available_bytes
              << ", all_memory_used: " << all_memory_used
              << ", free_mem: " << free_mem
