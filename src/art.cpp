...
static void art_fuzzy_recurse(unsigned char p, unsigned char c,
                              const art_node *n, int depth,
                              const unsigned char *term, const int term_len,
                              const int *irow, const int *jrow,
                              const int min_cost, const int max_cost,
                              const bool prefix,
                              std::vector<const art_node *> &results);

void art_int_fuzzy_recurse(art_node *n, int depth, const unsigned char *int_str,
                           int int_str_len, NUM_COMPARATOR comparator,
...
bool compare_art_node_score(const art_node *a, const art_node *b) {
  int64_t a_value = std::numeric_limits<int64_t>::min(),
          b_value = std::numeric_limits<int64_t>::min();
...
void compare_and_match_leaf(const unsigned char *int_str, int int_str_len,
...
static art_node **find_child(art_node *n, unsigned char c) {
  ...
  switch (n->type) {
    case NODE4:
      p.p1 = (art_node4 *)n;
      for (i = 0; i < n->num_children; i++) {
        if (p.p1->keys[i] == c) return &p.p1->children[i];
      }
      break;
...
static art_leaf *make_leaf(const unsigned char *key, uint32_t key_len,
  ...
  if ((2 + document->offsets.size()) <=
      posting_t::COMPACT_LIST_THRESHOLD_LENGTH) {
    compact_posting_list_t *list = compact_posting_list_t::create(
        1, ids, offset_index, document->offsets.size(), &document->offsets[0]);
...
static void add_child256(art_node256 *n, art_node **ref, unsigned char c,
  ...
  n->n.max_score =
      MAX(n->n.max_score, ((art_leaf *)LEAF_RAW(child))->max_score);
}
...
static void add_child16(art_node16 *n, art_node **ref, unsigned char c,
  ...
  if (n->n.num_children < 16) {
    ...
    memcpy(new_n->children, n->children, sizeof(void *) * n->n.num_children);
...
static void *recursive_insert(art_node *n, art_node **ref,
                              ...
                              const int64_t docs_max_score,
                              std::vector<art_document> &documents, int depth,
...
RECURSE_SEARCH:;
  ...
  if (child) {
    return recursive_insert(*child, child, key, key_len, docs_max_score,
                            documents, depth + 1, path, old);
  }
...
void *art_inserts(art_tree *t, const unsigned char *key, int key_len,
                  const int64_t docs_max_score,
  ...
  void *old = recursive_insert(t->root, &t->root, key, key_len, docs_max_score,
                               documents, 0, path, &old_val);
  if (!old_val) t->size++;
...
const uint32_t *get_allowed_doc_ids(art_tree *t, const std::string &prev_token,
                                    ...
                                    const size_t filter_ids_length,
                                    size_t &prev_token_doc_ids_len) {
  art_leaf *prev_leaf = static_cast<art_leaf *>(
      art_search(t, reinterpret_cast<const unsigned char *>(prev_token.c_str()),
                 prev_token.size() + 1));
  ...
  if (filter_ids_length != 0) {
    prev_token_doc_ids_len = ArrayUtils::and_scalar(
        prev_leaf_ids.data(), prev_leaf_ids.size(), filter_ids,
        filter_ids_length, &prev_token_doc_ids);
  } else {
...
bool validate_and_add_leaf(art_leaf *leaf, const bool last_token,
                           const std::string &prev_token,
                           const uint32_t *allowed_doc_ids,
                           const size_t allowed_doc_ids_len,
...
int art_topk_iter(const art_node *root, token_ordering token_order,
  ...
  while (!q.empty() && results.size() < max_results * 4) {
    ...
    if (IS_LEAF(n)) {
      ...
      validate_and_add_leaf(l, last_token, prev_token, allowed_doc_ids,
                            allowed_doc_ids_len, exclude_leaves, exact_leaf,
                            results);

      if (++num_processed % 1024 == 0 &&
          (microseconds(std::chrono::system_clock::now().time_since_epoch())
               .count() -
...
static inline void levenshtein_dist(const int depth, const unsigned char p,
                                    const unsigned char c,
                                    const unsigned char *term,
                                    const int term_len, const int *irow,
                                    const int *jrow, int *krow) {
  krow[0] = jrow[0] + 1;

  // Calculate levenshtein distance incrementally (term => b, column => j, c =>
  // a[i], p => a[i-1], irow => d[i-1]):
  // https://en.wikipedia.org/wiki/Damerau%E2%80%93Levenshtein_distance#Optimal_string_alignment_distance

  for (int column = 1; column <= term_len; column++) {
    int cost = (c == term[column - 1])
                   ? 0
                   : 1;  // column-1 used because of zero-based char array
    ...
    if (depth > 1 && column > 1 && c == term[column - 1 - 1] &&
        p == term[column - 1]) {
...
static inline void art_fuzzy_children(unsigned char p, const art_node *n,
                                      int depth, const unsigned char *term,
                                      const int term_len, const int *irow,
                                      const int *jrow, const int min_cost,
                                      const int max_cost, const bool prefix,
                                      std::vector<const art_node *> &results) {
  char child_char;
  art_node *child;
  ...
  switch (n->type) {
    case NODE4:
      ...
      for (int i = n->num_children - 1; i >= 0; i--) {
        ...
        art_fuzzy_recurse(p, child_char, child, depth, term, term_len, irow,
                          jrow, min_cost, max_cost, prefix, results);
      }
      break;
    case NODE16:
      printf("\nNODE16\n");
...
static inline int fuzzy_search_state(const bool prefix, int key_index,
                                     bool last_key_char, const int query_len,
                                     const int *cost_row, int min_cost,
                                     int max_cost) {
  ...
  if (last_key_char) {
    ...
    // Special case used to match q=strawberries on key=strawberry (query_len >
    // key_len) but limit to larger keys to prevent eager matches
    if (key_len > 5 && query_len > key_len &&
        (query_len - key_len) <= max_cost && cost_row[key_len] >= min_cost &&
        cost_row[key_len] <= max_cost - 1) {
      return 1;
    }
  ...
  // `key_len` can't exceed `query_len` since length of `cost_row` is `query_len
  // + 1`
  ...
  if (key_len >= query_len && prefix) {
    // Case b)
    // For prefix queries
    // - we can return early if key_len reaches query_len and cost is within
    // bounds.
  ...
  // We have to account for the case that `cost` could momentarily exceed
  // max_cost but resolve later. e.g. key=example, query=exZZample, after 5
  // chars, cost is 3 but drops to 2 at the end. But we will limit this for
  // longer keys for performance.
  return cost > max_cost && (key_len > 3 ? cost > (max_cost * 2) : true) ? -1
                                                                         : 0;
}
...
static void art_fuzzy_recurse(unsigned char p, unsigned char c,
  ...
  if (depth == -1) {
    ...
    int action = fuzzy_search_state(prefix, depth, last_key_char, term_len,
                                    rows[j], min_cost, max_cost);
    if (1 == action) {
      results.push_back(n);
      return;
    }
  ...
  if (IS_LEAF(n)) {
    ...
    // look past term_len to deal with trailing typo, e.g. searching "pltinum"
    // on "platinum" @ max_cost = 1
    const int iter_len = std::min(int(l->key_len), term_len + max_cost);

    if (depth >= iter_len) {
      // when a preceding partial node completely contains the whole leaf (e.g.
      // "[raspberr]y" on "raspberries")
      int action = fuzzy_search_state(prefix, depth, true, term_len, rows[j],
                                      min_cost, max_cost);
      if (action == 1) {
        results.push_back(n);
      }
    ...
    while (depth < iter_len) {
      ...
      if (!prefix || !last_key_char) {
        levenshtein_dist(depth, p, c, term, term_len, rows[i], rows[j],
                         rows[k]);
  ...
  // std::string partial_str(reinterpret_cast<const char *>(n->partial),
  // n->partial_len);
  ...
  for (int idx = 0; idx < partial_len; idx++) {
    ...
    int action = fuzzy_search_state(prefix, depth, false, term_len, rows[j],
                                    min_cost, max_cost);
    if (action == 1) {
      results.push_back(n);
      return;
  ...
  // Some intermediate path may have been left out if partial_len is truncated:
  // progress the levenshtein matrix
  ...
  art_fuzzy_children(c, n, depth, term, term_len, rows[i], rows[j], min_cost,
                     max_cost, prefix, results);
}
...
int art_fuzzy_search(art_tree *t, const unsigned char *term, const int term_len,
                     ...
                     const bool prefix, bool last_token,
                     const std::string &prev_token, const uint32_t *filter_ids,
                     const size_t filter_ids_length,
                     std::vector<art_leaf *> &results,
                     std::set<std::string> &exclude_leaves) {
  ...
  if (IS_LEAF(t->root)) {
    ...
    art_fuzzy_recurse(0, l->key[0], t->root, 0, term, term_len, irow, jrow,
                      min_cost, max_cost, prefix, nodes);
  } else {
    if (t->root == nullptr) {
      return 0;
    }
    ...
    art_fuzzy_recurse(0, 0, t->root, -1, term, term_len, irow, jrow, min_cost,
                      max_cost, prefix, nodes);
  }

  // long long int time_micro =
  // microseconds(std::chrono::high_resolution_clock::now() - begin).count();
  //! LOG(INFO) << "Time taken for fuzz: " << time_micro << "us, size of nodes:
  //! " << nodes.size();
  ...
  // LOG(INFO) << "exact_leaf: " << exact_leaf << ", term: " << term << ",
  // term_len: " << term_len;

  // documents that contain the previous token and/or filter ids
  size_t allowed_doc_ids_len = 0;
  const uint32_t *allowed_doc_ids = get_allowed_doc_ids(
      t, prev_token, filter_ids, filter_ids_length, allowed_doc_ids_len);

  for (auto node : nodes) {
    art_topk_iter(node, token_order, max_words, exact_leaf, last_token,
                  prev_token, allowed_doc_ids, allowed_doc_ids_len, t,
  ...
  if (exact_leaf && min_cost == 0) {
    std::string tok(reinterpret_cast<char *>(exact_leaf->key),
                    exact_leaf->key_len - 1);
    if (exclude_leaves.count(tok) == 0) {
  ...
  /*auto time_micro = microseconds(std::chrono::high_resolution_clock::now() -
  begin).count();
...
void encode_int32(int32_t n, unsigned char *chars) {
  unsigned char symbols[16] = {0, 1, 2,  3,  4,  5,  6,  7,
...
// See:
// https://github.com/apache/hbase/blob/master/hbase-common/src/main/java/org/apache/hadoop/hbase/util/OrderedBytes.java#L1372
...
// Implements ==, <= and >=
recurse_progress matches(unsigned char a, unsigned char b,
                         NUM_COMPARATOR comparator) {
  switch (comparator) {
    case LESS_THAN:
...
static void art_iter(const art_node *n, const unsigned char *int_str,
                     int int_str_len, NUM_COMPARATOR comparator,
                     std::vector<const art_leaf *> &results) {
  // Handle base cases
  if (!n) return;
  ...
  switch (n->type) {
    case NODE4:
      for (int i = 0; i < n->num_children; i++) {
        art_iter(((art_node4 *)n)->children[i], int_str, int_str_len,
                 comparator, results);
      }
      break;
    ...
    case NODE16:
      for (int i = 0; i < n->num_children; i++) {
        art_iter(((art_node16 *)n)->children[i], int_str, int_str_len,
                 comparator, results);
      }
      break;
    ...
    case NODE48:
      for (int i = 0; i < 256; i++) {
        idx = ((art_node48 *)n)->keys[i];
        if (!idx) continue;
        art_iter(((art_node48 *)n)->children[idx - 1], int_str, int_str_len,
                 comparator, results);
      }
      break;
    ...
    case NODE256:
      for (int i = 0; i < 256; i++) {
        if (!((art_node256 *)n)->children[i]) continue;
        art_iter(((art_node256 *)n)->children[i], int_str, int_str_len,
                 comparator, results);
      }
      break;
...
static inline void art_int_fuzzy_children(
    const art_node *n, int depth, const unsigned char *int_str, int int_str_len,
    ...
    case NODE4:
      ...
      for (int i = n->num_children - 1; i >= 0; i--) {
        ...
        printf("4!child_char: %c, %d, depth: %d\n", child_char, child_char,
               depth);
        child = ((art_node4 *)n)->children[i];
        recurse_progress progress =
            matches(child_char, int_str[depth], comparator);
        if (progress == RECURSE) {
          art_int_fuzzy_recurse(child, depth + 1, int_str, int_str_len,
                                comparator, results);
...
int art_int32_search(art_tree *t, int32_t value, NUM_COMPARATOR comparator,
                     std::vector<const art_leaf *> &results) {
...
int art_int64_search(art_tree *t, int64_t value, NUM_COMPARATOR comparator,
                     std::vector<const art_leaf *> &results) {
  unsigned char chars[8];
...
int art_float_search(art_tree *t, float value, NUM_COMPARATOR comparator,
                     std::vector<const art_leaf *> &results) {
  unsigned char chars[8];
