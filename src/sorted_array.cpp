...
uint32_t sorted_array::at(uint32_t index) { return for_select(in, index); }
...
// returns the first element in the sequence which does not compare less than
// |value|.
uint32_t sorted_array::lower_bound_search_bits(const uint8_t *in, uint32_t imin,
                                               uint32_t imax, uint32_t base,
...
// returns the first element in the sequence which does not compare less than
// |value|.
uint32_t sorted_array::lower_bound_search(const uint32_t *in, uint32_t imin,
                                          uint32_t imax, uint32_t value,
                                          uint32_t *actual) {
  uint32_t imid;
  uint32_t v;
...
void sorted_array::binary_search_indices(const uint32_t *values, int low_vindex,
                                         int high_vindex, int low_index,
                                         int high_index, uint32_t base,
                                         uint32_t bits, uint32_t *indices) {
  uint32_t actual_value = 0;

  if (high_vindex >= low_vindex && high_index >= low_index) {
    size_t pivot_vindex = (low_vindex + high_vindex) / 2;

    uint32_t in_index = lower_bound_search_bits(
        in + METADATA_OVERHEAD, low_index, high_index, base, bits,
    ...
    binary_search_indices(values, low_vindex, pivot_vindex - 1, low_index,
                          in_index, base, bits, indices);
    binary_search_indices(values, pivot_vindex + 1, high_vindex, in_index,
                          high_index, base, bits, indices);
  }
}
...
void sorted_array::indexOf(const uint32_t *values, const size_t values_len,
                           uint32_t *indices) {
  if (values_len == 0) {
    return;
  }
  ...
  do {
    head++;
    low_index =
        lower_bound_search_bits(in + METADATA_OVERHEAD, 0, length - 1, base,
                                bits, values[head], &actual_value);
  ...
  binary_search_indices(values, head, tail, low_index, high_index, base, bits,
                        indices);
}
...
void sorted_array::remove_value(uint32_t value) {
  ...
  // A lower bound search returns the first element in the sequence that is >=
  // `value` So, `found_val` will be either equal or greater than `value`
  uint32_t found_val;
  ...
  if (found_index + 1 < length) {
    memmove(&curr_array[found_index], &curr_array[found_index + 1],
            sizeof(uint32_t) * (length - found_index - 1));
  }
...
void sorted_array::remove_values(uint32_t *sorted_values,
                                 uint32_t sorted_values_length) {
  uint32_t *curr_array = uncompress();
  ...
  while (curr_index < length) {
    if (sorted_values_index < sorted_values_length &&
        sorted_values[sorted_values_index] == curr_array[curr_index]) {
      curr_index++;
      sorted_values_index++;
    } else {
...
size_t sorted_array::numFoundOf(const uint32_t *values,
  ...
  if (length > values_len) {
    ...
    // recursively search within the bounds for all values
    binary_count_indices(values, head, tail, low_index, high_index, base, bits,
                         num_found);
  } else {
    // identify the upper and lower bounds of the search space
    ...
    do {
      head++;
      low_index = lower_bound_search(values, 0, values_len - 1, src[head],
                                     &actual_value);
    ...
    do {
      tail--;
      high_index = lower_bound_search(values, 0, values_len - 1, src[tail],
                                      &actual_value);
    } while (tail > 0 && actual_value < src[tail]);

    // recursively search within the bounds for all values
    binary_count_indices(src, head, tail, values, low_index, high_index,
                         num_found);
...
void sorted_array::binary_count_indices(const uint32_t *values, int low_vindex,
                                        int high_vindex, int low_index,
                                        int high_index, uint32_t base,
                                        uint32_t bits, size_t &num_found) {
  uint32_t actual_value = 0;
  ...
  if (high_vindex >= low_vindex && high_index >= low_index) {
    ...
    // LOG(INFO) << "pivot_vindex: " << pivot_vindex << ", values[pivot_vindex]:
    // " << values[pivot_vindex];
    if (actual_value == values[pivot_vindex]) {
    ...
    binary_count_indices(values, low_vindex, pivot_vindex - 1, low_index,
                         in_index, base, bits, num_found);
    binary_count_indices(values, pivot_vindex + 1, high_vindex, in_index,
                         high_index, base, bits, num_found);
  }
}

void sorted_array::binary_count_indices(const uint32_t *values, int low_vindex,
                                        int high_vindex, const uint32_t *src,
  ...
  if (high_vindex >= low_vindex && high_index >= low_index) {
    ...
    uint32_t in_index = lower_bound_search(src, low_index, high_index,
                                           values[pivot_vindex], &actual_value);
    ...
    binary_count_indices(values, low_vindex, pivot_vindex - 1, src, low_index,
                         in_index, num_found);
    binary_count_indices(values, pivot_vindex + 1, high_vindex, src, in_index,
                         high_index, num_found);
  }
}

uint32_t sorted_array::last() { return (length == 0) ? UINT32_MAX : max; }
