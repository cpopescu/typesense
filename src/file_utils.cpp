...
bool copy_dir(const std::string& from_path, const std::string& to_path) {
  ...
  butil::FileEnumerator dir_enum(butil::FilePath(from_path), false,
                                 butil::FileEnumerator::FILES);
  for (butil::FilePath name = dir_enum.Next(); !name.empty();
       name = dir_enum.Next()) {
    ...
    if (0 != link(src_file.c_str(), dst_file.c_str())) {
      if (!butil::CopyFile(butil::FilePath(src_file),
                           butil::FilePath(dst_file))) {
...
bool mv_dir(const std::string& from_path, const std::string& to_path) {
  ...
  butil::FileEnumerator file_enum(
      butil::FilePath(from_path), false,
      butil::FileEnumerator::FILES | butil::FileEnumerator::DIRECTORIES);
  for (butil::FilePath name = file_enum.Next(); !name.empty();
       name = file_enum.Next()) {
    std::string src_file(from_path);
    std::string dst_file(to_path);
    ...
    if (!butil::ReplaceFile(butil::FilePath(src_file),
                            butil::FilePath(dst_file), &error)) {
      LOG(WARNING) << "move " << src_file << " to " << dst_file
                   << " failed: " << error;
      return false;
    }
  }
