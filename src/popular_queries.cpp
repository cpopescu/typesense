...
void PopularQueries::add(const std::string& key, const bool live_query,
                         const std::string& user_id, uint64_t now_ts_us) {
  if (live_query) {
    // live query must be aggregated first to their final form as they could be
    // prefix queries
    if (now_ts_us == 0) {
    ...
    if (!umutex.try_lock()) {
      // instead of locking we just skip incrementing keys during consolidation
      // time
      return;
    }
...
void PopularQueries::serialize_as_docs(std::string& docs) {
  ...
  for (auto it = local_counts.begin(); it != local_counts.end(); ++it) {
    it.key(key_buffer);
    nlohmann::json doc;
    doc["id"] = std::to_string(
        StringUtils::hash_wy(key_buffer.c_str(), key_buffer.size()));
    doc["q"] = key_buffer;
...
size_t PopularQueries::get_k() { return k; }
...
void PopularQueries::compact_user_queries(uint64_t now_ts_us) {
  ...
  for (auto& kv : user_prefix_queries) {
    ...
    for (uint32_t i = 0; i < queries.size(); i++) {
      if (now_ts_us - queries[i].timestamp <
          QUERY_FINALIZATION_INTERVAL_MICROS) {
        break;
      }

      uint64_t diff_micros =
          (i == queries.size() - 1)
              ? (now_ts_us - queries[i].timestamp)
    ...
    queries.erase(queries.begin(),
                  queries.begin() + last_consolidated_index + 1);
...
std::unordered_map<std::string, std::vector<PopularQueries::QWithTimestamp>>
PopularQueries::get_user_prefix_queries() {
  std::unique_lock lk(umutex);
