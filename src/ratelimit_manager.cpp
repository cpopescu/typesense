...
void RateLimitManager::temp_ban_entity(const rate_limit_entity_t& entity,
                                       const uint64_t number_of_hours) {
  // lock mutex
  std::unique_lock<std::shared_mutex> lock(rate_limit_mutex);
  temp_ban_entity_wrapped(entity, number_of_hours);
}

bool RateLimitManager::is_rate_limited(
    const rate_limit_entity_t& api_key_entity,
    const rate_limit_entity_t& ip_entity) {
  // lock mutex
  std::unique_lock<std::shared_mutex> lock(rate_limit_mutex);
  ...
  std::sort(rules_bucket.begin(), rules_bucket.end(),
            [](rate_limit_rule_t* rule1, rate_limit_rule_t* rule2) {
  ...
  while (throttle_key.ok()) {
    ...
    std::string ban_key = std::string(BANS_PREFIX) + "_" +
                          std::to_string(throttled_entities.at(key).status_id);
  ...
  auto request_counter_key =
      get_request_counter_key(rule, ip_entity, api_key_entity);
  ...
  if (request_counts.last_reset_time_minute <= get_current_time() - 60) {
    request_counts.previous_requests_count_minute =
  ...
  if (request_counts.last_reset_time_hour <= get_current_time() - 3600) {
    request_counts.previous_requests_count_hour =
  ...
  auto current_rate_for_minute =
      (60 - (get_current_time() - request_counts.last_reset_time_minute)) / 60 *
      request_counts.previous_requests_count_minute;
  current_rate_for_minute += request_counts.current_requests_count_minute;
  if (rule.max_requests.minute_threshold >= 0 &&
      current_rate_for_minute >= rule.max_requests.minute_threshold) {
    bool auto_ban_is_enabled =
        (rule.auto_ban_1m_threshold > 0 && rule.auto_ban_1m_duration_hours > 0);
    // If key is not in exceed map that means, it is a new exceed, not a
    // continued exceed
    if (rate_limit_exceeds.count(request_counter_key) == 0) {
      rate_limit_exceeds.insert(
          {request_counter_key,
           rate_limit_exceed_t{last_throttle_id++, request_counter_key, 1}});
    ...
    if (auto_ban_is_enabled) {
      if (request_counts.threshold_exceed_count_minute >
          rule.auto_ban_1m_threshold) {
        temp_ban_entity_wrapped(
            request_counter_key.substr(0, request_counter_key.find("_")) == ".*"
                ? WILDCARD_API_KEY
                : api_key_entity,
            rule.auto_ban_1m_duration_hours,
            (request_counter_key.substr((request_counter_key.find("_") + 1)) ==
                 ".*" &&
             !rule.apply_limit_per_entity)
                ? nullptr
                : &ip_entity);
      }
    }
  ...
  auto current_rate_for_hour =
      (3600 - (get_current_time() - request_counts.last_reset_time_hour)) /
      3600 * request_counts.previous_requests_count_hour;
  current_rate_for_hour += request_counts.current_requests_count_hour;
  if (rule.max_requests.hour_threshold >= 0 &&
      current_rate_for_hour >= rule.max_requests.hour_threshold) {
    if (rate_limit_exceeds.count(request_counter_key) == 0) {
      rate_limit_exceeds.insert(
          {request_counter_key,
           rate_limit_exceed_t{last_throttle_id++, request_counter_key, 1}});
  ...
  request_counts.current_requests_count_minute++;
  request_counts.current_requests_count_hour++;
  // If key is in exceed map that means, it is no longer exceed, so remove it
  // from the map
  if (rate_limit_exceeds.count(request_counter_key) > 0) {
...
bool RateLimitManager::delete_rule_by_id(const uint64_t id) {
  ...
  if (rule_store.count(id) > 0) {
    ...
    rule_store.erase(id);
    for (auto it = rate_limit_entities.begin();
         it != rate_limit_entities.end();) {
      if (it->second.empty()) {
...
const std::vector<rate_limit_status_t> RateLimitManager::get_banned_entities(
    const RateLimitedEntityType entity_type) {
  std::shared_lock<std::shared_mutex> lock(rate_limit_mutex);
...
void RateLimitManager::temp_ban_entity_wrapped(
    const rate_limit_entity_t& entity, const uint64_t number_of_hours,
    const rate_limit_entity_t* and_entity) {
  std::string key = entity.entity_id + "_" +
                    (and_entity != nullptr ? and_entity->entity_id : ".*");
  ...
  rate_limit_status_t status(
      last_ban_id, now, now + (number_of_hours * 60 * 60), entity, and_entity);
...
const nlohmann::json rate_limit_status_t::to_json() const {
  ...
  if (and_entity.ok()) {
    ...
    status["and_entity"]["entity_type"] =
        magic_enum::enum_name(and_entity_value.entity_type);
  }
  return status;
}
...
void rate_limit_status_t::parse_json(const nlohmann::json& json) {
  ...
  entity.entity_type = magic_enum::enum_cast<RateLimitedEntityType>(
                           json["entity_type"].get<std::string>())
                           .value();
  if (json.contains("and_entity")) {
    and_entity = Option<rate_limit_entity_t>(rate_limit_entity_t{
        magic_enum::enum_cast<RateLimitedEntityType>(
            json["and_entity"]["entity_type"].get<std::string>())
            .value(),
        json["and_entity"]["value"]});
  } else {
    and_entity = Option<rate_limit_entity_t>(404, "No and_entity found");
  }
}

Option<nlohmann::json> RateLimitManager::add_rule(
    const nlohmann::json& rule_json) {
  std::unique_lock<std::shared_mutex> lock(rate_limit_mutex);
  auto rule_validation_result = is_valid_rule(rule_json);
  if (!rule_validation_result.ok()) {
    return Option<nlohmann::json>(rule_validation_result.code(),
                                  rule_validation_result.error());
  }
  auto parsed_rule_option = parse_rule(rule_json);
  if (!parsed_rule_option.ok()) {
    return Option<nlohmann::json>(parsed_rule_option.code(),
                                  parsed_rule_option.error());
  }
  ...
  if (!inserted) {
    return Option<nlohmann::json>(500,
                                  "Failed to insert rule into the DB store");
  }
...
Option<nlohmann::json> RateLimitManager::edit_rule(
    const uint64_t id, const nlohmann::json& rule_json) {
  std::unique_lock<std::shared_mutex> lock(rate_limit_mutex);
...
Option<bool> RateLimitManager::is_valid_rule(const nlohmann::json& rule_json) {
  ...
  if (rule_json.count("apply_limit_per_entity") > 0 &&
      rule_json["apply_limit_per_entity"].is_boolean() == false) {
    return Option<bool>(
        400, "Parameter `apply_limit_per_entity` must be a boolean.");
  }
  if ((rule_json.count("ip_addresses") == 0 &&
       rule_json.count("api_keys") == 0)) {
    return Option<bool>(400,
                        "Parameter `ip_addresses` or `api_keys` is required.");
  }
  if (rule_json.count("ip_addresses") > 0 &&
      (!rule_json["ip_addresses"].is_array() ||
       !rule_json["ip_addresses"][0].is_string())) {
    return Option<bool>(
        400, "Parameter `ip_addresses` must be an array of strings.");
  }
  if (rule_json.count("api_keys") > 0 &&
      (!rule_json["api_keys"].is_array() ||
       !rule_json["api_keys"][0].is_string())) {
    return Option<bool>(400,
                        "Parameter `api_keys` must be an array of strings.");
  }
  if (rule_json.count("api_keys") > 0 && rule_json.count("ip_addresses") > 0 &&
      rule_json["api_keys"].size() == 0 &&
      rule_json["ip_addresses"].size() == 0) {
    return Option<bool>(
        400,
        "Parameter `ip_addresses` or `api_keys` must have at least one value.");
  }
  if (rule_json.count("api_keys") > 0 && rule_json.count("ip_addresses") > 0 &&
      rule_json["api_keys"].size() > 1 &&
      rule_json["ip_addresses"].size() > 1) {
  ...
  if (rule_json["action"] == "allow") {
    ...
    if (rule_json.count("max_requests_1m") == 0 &&
        rule_json.count("max_requests_1h") == 0) {
      return Option<bool>(400,
                          "At least  one of `max_requests_1m` or "
                          "`max_requests_1h` is required.");
    }
    if (rule_json.count("max_requests_1m") > 0 &&
        !rule_json["max_requests_1m"].is_number_integer()) {
      return Option<bool>(400,
                          "Parameter `max_requests_1m` must be an integer.");
    }
    if (rule_json.count("max_requests_1h") > 0 &&
        !rule_json["max_requests_1h"].is_number_integer()) {
      return Option<bool>(400,
                          "Parameter `max_requests_1h` must be an integer.");
    }
    if ((rule_json.count("auto_ban_1m_threshold") > 0 &&
         rule_json.count("auto_ban_1m_duration_hours") == 0) ||
        (rule_json.count("auto_ban_1m_threshold") == 0 &&
         rule_json.count("auto_ban_1m_duration_hours") > 0)) {
      return Option<bool>(
          400,
          "Both `auto_ban_1m_threshold` and `auto_ban_1m_duration_hours` are "
          "required ifeither is specified.");
    }
    if (rule_json.count("auto_ban_1m_threshold") > 0 &&
        rule_json.count("auto_ban_1m_duration_hours") > 0) {
      if (!rule_json["auto_ban_1m_threshold"].is_number_integer() ||
          !rule_json["auto_ban_1m_duration_hours"].is_number_integer()) {
        return Option<bool>(400,
                            "Parameters `auto_ban_1m_threshold` and "
                            "`auto_ban_1m_duration_hours` must be integers.");
      }
      if (rule_json["auto_ban_1m_threshold"].get<int>() < 0 ||
          rule_json["auto_ban_1m_duration_hours"].get<int>() < 0) {
        return Option<bool>(
            400,
            "Both `auto_ban_1m_threshold` and `auto_ban_1m_duration_hours` "
...
Option<rate_limit_rule_t> RateLimitManager::parse_rule(
    const nlohmann::json& rule_json) {
  rate_limit_rule_t new_rule;
  new_rule.action = magic_enum::enum_cast<RateLimitAction>(
                        rule_json["action"].get<std::string>())
                        .value();
  if (rule_json.count("ip_addresses") > 0) {
    for (const auto& ip : rule_json["ip_addresses"]) {
      new_rule.entities.push_back(
          rate_limit_entity_t{RateLimitedEntityType::ip, ip});
    }
  }
  if (rule_json.count("api_keys") > 0) {
    for (const auto& api_key : rule_json["api_keys"]) {
      new_rule.entities.push_back(
          rate_limit_entity_t{RateLimitedEntityType::api_key, api_key});
    }
  }
  if (rule_json.count("max_requests_1m") > 0) {
  ...
  if (rule_json.count("auto_ban_1m_threshold") > 0 &&
    ...
    new_rule.auto_ban_1m_duration_hours =
        rule_json["auto_ban_1m_duration_hours"];
  }
  if (rule_json.count("apply_limit_per_entity") > 0) {
    new_rule.apply_limit_per_entity =
        rule_json["apply_limit_per_entity"].get<bool>();
  }
...
Option<bool> RateLimitManager::init(Store* store) {
  ...
  StoreStatus last_rule_id_status =
      store->get(std::string(RULES_NEXT_ID), last_rule_id_str);
  if (last_rule_id_status == StoreStatus::ERROR) {
    return Option<bool>(500,
                        "Error while fetching rule next id from database.");
  ...
  store->scan_fill(std::string(RULES_PREFIX) + "_",
                   std::string(RULES_PREFIX) + "`", rule_json_strs);
  ...
  store->scan_fill(std::string(BANS_PREFIX) + "_",
                   std::string(BANS_PREFIX) + "`", ban_json_strs);
  ...
  for (const auto& ban_json_str : ban_json_strs) {
    ...
    std::string key =
        ban_status.entity.entity_id + "_" +
        (ban_status.and_entity.ok() ? ban_status.and_entity.get().entity_id
                                    : ".*");
...
Option<std::string> RateLimitManager::get_throttle_key(
    const rate_limit_entity_t& ip_entity,
    const rate_limit_entity_t& api_key_entity) {
  if (throttled_entities.count(api_key_entity.entity_id + "_" +
                               ip_entity.entity_id) > 0) {
...
const std::string RateLimitManager::get_request_counter_key(
    const rate_limit_rule_t& rule, const rate_limit_entity_t& ip_entity,
    const rate_limit_entity_t& api_key_entity) {
  bool has_api_key = false, has_ip = false, has_wildcard_ip = false,
       has_wildcard_api_key = false;
  for (const auto& entity : rule.entities) {
...
const nlohmann::json RateLimitManager::get_throttled_entities_json() {
  ...
  for (const auto& entity : throttled_entities) {
    ...
    json[json["entity_type"].get<std::string>() == "ip" ? "ip_address"
                                                        : "api_key"] =
        json["value"];
    json.erase("entity_type");
    json.erase("value");
    if (json["and_entity"].is_object()) {
      json[json["and_entity"]["entity_type"].get<std::string>() == "ip"
               ? "ip_address"
               : "api_key"] = json["and_entity"]["value"];
...
bool RateLimitManager::delete_ban_by_id(const uint64_t id) {
  ...
  auto ban = std::find_if(
      throttled_entities.begin(), throttled_entities.end(),
      [id](const auto& ban) { return ban.second.status_id == id; });
...
void RateLimitManager::fill_bucket(
    const rate_limit_entity_t& target_entity,
    const rate_limit_entity_t& other_entity,
    std::vector<rate_limit_rule_t*>& rules_bucket) {
  ...
  for (const auto& rule : it->second) {
    ...
    if (std::find(rules_bucket.begin(), rules_bucket.end(), rule) !=
        rules_bucket.end()) {
      continue;
    }
    // Add the rule only If:
    // A. it has no entity with type of other_entity
    // B. it has an entity with type of other_entity and it's value is equal to
    // other_entity's value C. it has an entity with type of other_entity and
...
bool RateLimitManager::delete_throttle_by_id(const uint32_t id) {
  ...
  for (auto it = rate_limit_exceeds.begin(); it != rate_limit_exceeds.end();
       it++) {
