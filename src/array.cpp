...
uint32_t array::at(uint32_t index) { return for_select(in, index); }
...
void array::load(const uint32_t *sorted_array, const uint32_t array_length,
                 const uint32_t m, const uint32_t M) {
  min = m;
  max = M;

  uint32_t size_required =
      (uint32_t)(unsorted_append_size_required(max, array_length) *
                 FOR_GROWTH_FACTOR);
...
bool array::insert(size_t index, const uint32_t *values, size_t num_values) {
  ...
  memmove(&curr_array[index + num_values], &curr_array[index],
...
void array::remove_index(uint32_t start_index, uint32_t end_index) {
  ...
  uint32_t size_required =
      (uint32_t)(unsorted_append_size_required(max, new_index) *
                 FOR_GROWTH_FACTOR);
  uint8_t *out = (uint8_t *)malloc(size_required * sizeof *out);
  memset(out, 0, size_required);
