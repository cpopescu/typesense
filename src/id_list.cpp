...
bool id_list_t::block_t::contains(uint32_t id) { return ids.contains(id); }
...
id_list_t::iterator_t::iterator_t(id_list_t::block_t* start,
                                  id_list_t::block_t* end,
...
void id_list_t::iterator_t::previous() {
  curr_index--;
  if (curr_index < 0) {
    // since block stores only the next pointer, we have to use `id_block_map`
...
uint32_t id_list_t::iterator_t::id() const { return ids[curr_index]; }

uint32_t id_list_t::iterator_t::index() const { return curr_index; }

id_list_t::block_t* id_list_t::iterator_t::block() const { return curr_block; }
...
void id_list_t::iterator_t::skip_to(uint32_t id) {
  ...
  while (curr_block != end_block && curr_index < curr_block->size() &&
         this->id() < id) {
    curr_index++;
  }
}
...
id_list_t::id_list_t(uint16_t max_block_elements)
...
void id_list_t::merge_adjacent_blocks(id_list_t::block_t* block1,
                                      id_list_t::block_t* block2,
...
void id_list_t::split_block(id_list_t::block_t* src_block,
                            id_list_t::block_t* dst_block) {
  if (src_block->size() <= 1) {
    return;
  }
...
id_list_t::block_t* id_list_t::get_root() { return &root_block; }

size_t id_list_t::num_blocks() const { return id_block_map.size(); }
...
void id_list_t::merge(const std::vector<id_list_t*>& id_lists,
...
void id_list_t::intersect(const std::vector<id_list_t*>& id_lists,
                          std::vector<uint32_t>& result_ids) {
  if (id_lists.empty()) {
    return;
  }
...
id_list_t::iterator_t id_list_t::new_iterator(block_t* start_block,
                                              block_t* end_block) {
...
id_list_t::iterator_t id_list_t::new_rev_iterator() {
  ...
  auto rev_it =
      id_list_t::iterator_t(start_block, nullptr, &id_block_map, true);
  return rev_it;
}
...
size_t id_list_t::num_ids() const { return ids_length; }
...
bool id_list_t::contains_atleast_one(const uint32_t* target_ids,
