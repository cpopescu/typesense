...
compact_id_list_t* compact_id_list_t::create(uint32_t num_ids,
...
compact_id_list_t* compact_id_list_t::create(uint32_t num_ids,
  ...
  compact_id_list_t* pl = (compact_id_list_t*)malloc(
      sizeof(compact_id_list_t) + (num_ids * sizeof(uint32_t)));
...
uint32_t compact_id_list_t::num_ids() const { return length; }
...
bool compact_id_list_t::contains_atleast_one(const uint32_t* target_ids,
  ...
  if (IS_COMPACT_IDS(obj)) {
    ...
    if ((list->capacity + extra_capacity_required) >
      ...
      size_t new_capacity_bytes =
          sizeof(compact_id_list_t) + (new_capacity * sizeof(uint32_t));
...
void ids_t::erase(void*& obj, uint32_t id) {
  if (IS_COMPACT_IDS(obj)) {
    ...
    if (list->num_blocks() == 1 &&
        list->get_root()->size() <= COMPACT_LIST_THRESHOLD_LENGTH) {
      // convert to compact posting format
      auto root_block = list->get_root();
      auto ids = root_block->ids.uncompress();

      compact_id_list_t* compact_list =
          compact_id_list_t::create(root_block->size(), ids);
...
bool ids_t::contains_atleast_one(const void* obj, const uint32_t* target_ids,
                                 size_t target_ids_size) {
  if (IS_COMPACT_IDS(obj)) {
    compact_id_list_t* list = COMPACT_IDS_PTR(obj);
...
void ids_t::merge(const std::vector<void*>& raw_posting_lists,
...
void ids_t::intersect(const std::vector<void*>& raw_posting_lists,
                      std::vector<uint32_t>& result_ids) {
  // we will have to convert the compact posting list (if any) to full form
  std::vector<id_list_t*> id_lists;
  std::vector<id_list_t*> expanded_id_lists;
...
void ids_t::to_expanded_id_lists(const std::vector<void*>& raw_posting_lists,
                                 std::vector<id_list_t*>& id_lists,
                                 std::vector<id_list_t*>& expanded_id_lists) {
...
void ids_t::block_intersector_t::split_lists(
  ...
  while (curr_block != nullptr) {
    ...
    if (blocks_traversed % window_size == 0 || blocks_traversed == num_blocks) {
      ...
      std::vector<id_list_t::iterator_t>& partial_its =
          partial_its_vec[window_index];
      ...
      for (size_t i = 0; i < this->id_lists.size(); i++) {
        ...
        if (i == 0) {
          ...
          id_list_t::block_t* last_block =
              this->id_lists[i]->block_of(end_block_last_id);
