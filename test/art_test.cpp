...
#define words_file_path                                                    \
  (std::string(ROOT_DIR) + std::string("external/libart/tests/words.txt")) \
      .c_str()
#define uuid_file_path                                                    \
  (std::string(ROOT_DIR) + std::string("external/libart/tests/uuid.txt")) \
      .c_str()
#define skus_file_path \
  (std::string(ROOT_DIR) + std::string("test/skus.txt")).c_str()
#define ill_file_path \
  (std::string(ROOT_DIR) + std::string("test/ill.txt")).c_str()
...
TEST(ArtTest, test_art_iter_prefix) {
  ...
  ASSERT_TRUE(!art_iter_prefix(&t, (unsigned char *)"api.foo.bar", 11,
                               test_prefix_cb, &p5));
  ...
  ASSERT_TRUE(
      !art_iter_prefix(&t, (unsigned char *)"api.end", 7, test_prefix_cb, &p6));
...
TEST(ArtTest, test_art_long_prefix) {
  ...
  // Search for the keys
  s = "this:key:has:a:long:common:prefix:1";
  EXPECT_EQ(1, posting_t::first_id(((art_leaf *)art_search(
                                        &t, (unsigned char *)s, strlen(s) + 1))
                                       ->values));

  s = "this:key:has:a:long:common:prefix:2";
  EXPECT_EQ(2, posting_t::first_id(((art_leaf *)art_search(
                                        &t, (unsigned char *)s, strlen(s) + 1))
                                       ->values));

  s = "this:key:has:a:long:prefix:3";
  EXPECT_EQ(3, posting_t::first_id(((art_leaf *)art_search(
                                        &t, (unsigned char *)s, strlen(s) + 1))
                                       ->values));
  ...
  ASSERT_TRUE(!art_iter_prefix(&t, (unsigned char *)"this:key:has", 12,
                               test_prefix_cb, &p));
  ASSERT_TRUE(p.count == p.max_count);
...
TEST(ArtTest, test_art_insert_search_uuid) {
  ...
  while (fgets(buf, sizeof buf, f)) {
    ...
    uintptr_t id = posting_t::first_id(
        ((art_leaf *)art_search(&t, (unsigned char *)buf, len))->values);
    ASSERT_TRUE(line == id);
    line++;
  }
...
TEST(ArtTest, test_art_max_prefix_len_scan_prefix) {
  ...
  art_document doc = get_document((uint32_t)1);
  ASSERT_TRUE(NULL ==
              art_insert(&t, (unsigned char *)key1, strlen(key1) + 1, &doc));

  const char *key2 = "foobarbaz1-test1-bar";
  auto doc2 = get_document((uint32_t)2);
  ASSERT_TRUE(NULL ==
              art_insert(&t, (unsigned char *)key2, strlen(key2) + 1, &doc2));

  const char *key3 = "foobarbaz1-test2-foo";
  auto doc3 = get_document((uint32_t)3);
  ASSERT_TRUE(NULL ==
              art_insert(&t, (unsigned char *)key3, strlen(key3) + 1, &doc3));
  ...
  ASSERT_TRUE(!art_iter_prefix(&t, (unsigned char *)prefix, strlen(prefix),
                               test_prefix_cb, &p));
  ASSERT_TRUE(p.count == p.max_count);
...
TEST(ArtTest, test_art_search_out_of_bounds) {
  ...
  art_leaf *l =
      (art_leaf *)art_search(&t, (const unsigned char *)prefix, strlen(prefix));
  ASSERT_EQ(NULL, l);
...
TEST(ArtTest, test_art_insert_multiple_ids_for_same_token) {
  ...
  art_document doc3 = get_document((uint32_t)3);
  void *reinsert_value =
      art_insert(&t, (unsigned char *)key1, strlen(key1) + 1, &doc3);
...
TEST(ArtTest, test_art_fuzzy_search_single_leaf) {
  ...
  art_document doc = get_document((uint32_t)1);
  ASSERT_TRUE(NULL == art_insert(&t, (unsigned char *)implement_key,
                                 strlen(implement_key) + 1, &doc));

  art_leaf *l = (art_leaf *)art_search(&t, (const unsigned char *)implement_key,
                                       strlen(implement_key) + 1);
  EXPECT_EQ(1, posting_t::first_id(l->values));

  std::vector<art_leaf *> leaves;
  art_fuzzy_search(&t, (const unsigned char *)implement_key,
                   strlen(implement_key) + 1, 0, 0, 10, FREQUENCY, false, false,
                   "", nullptr, 0, leaves, exclude_leaves);
  ...
  art_fuzzy_search(&t, (const unsigned char *)implement_key_typo1,
                   strlen(implement_key_typo1) + 1, 0, 0, 10, FREQUENCY, false,
                   false, "", nullptr, 0, leaves, exclude_leaves);
  ASSERT_EQ(0, leaves.size());

  leaves.clear();
  exclude_leaves.clear();
  art_fuzzy_search(&t, (const unsigned char *)implement_key_typo1,
                   strlen(implement_key_typo1) + 1, 0, 1, 10, FREQUENCY, false,
                   false, "", nullptr, 0, leaves, exclude_leaves);
  ASSERT_EQ(1, leaves.size());

  leaves.clear();
  exclude_leaves.clear();
  art_fuzzy_search(&t, (const unsigned char *)implement_key_typo2,
                   strlen(implement_key_typo2) + 1, 0, 2, 10, FREQUENCY, false,
                   false, "", nullptr, 0, leaves, exclude_leaves);
  ASSERT_EQ(1, leaves.size());
...
TEST(ArtTest, test_art_fuzzy_search_single_leaf_prefix) {
  ...
  art_document doc = get_document((uint32_t)1);
  ASSERT_TRUE(NULL ==
              art_insert(&t, (unsigned char *)key, strlen(key) + 1, &doc));

  art_leaf *l =
      (art_leaf *)art_search(&t, (const unsigned char *)key, strlen(key) + 1);
  EXPECT_EQ(1, posting_t::first_id(l->values));

  std::vector<art_leaf *> leaves;
  std::string term = "aplication";
  art_fuzzy_search(&t, (const unsigned char *)(term.c_str()), term.size(), 0, 1,
                   10, FREQUENCY, true, false, "", nullptr, 0, leaves,
                   exclude_leaves);
  ASSERT_EQ(1, leaves.size());

  leaves.clear();
  exclude_leaves.clear();
  art_fuzzy_search(&t, (const unsigned char *)(term.c_str()), term.size(), 0, 2,
                   10, FREQUENCY, true, false, "", nullptr, 0, leaves,
                   exclude_leaves);
  ASSERT_EQ(1, leaves.size());
...
TEST(ArtTest, test_art_fuzzy_search_single_leaf_non_prefix) {
  ...
  art_fuzzy_search(&t, (const unsigned char *)(term.c_str()), term.size() + 1,
                   0, 1, 10, FREQUENCY, false, false, "", nullptr, 0, leaves,
                   exclude_leaves);
  ASSERT_EQ(0, leaves.size());
...
TEST(ArtTest, test_art_prefix_larger_than_key) {
  ...
  std::vector<art_leaf *> leaves;
  art_fuzzy_search(&t, (const unsigned char *)(term.c_str()), term.size() + 1,
                   0, 2, 10, FREQUENCY, false, false, "", nullptr, 0, leaves,
                   exclude_leaves);
  ASSERT_EQ(0, leaves.size());
...
TEST(ArtTest, test_art_fuzzy_search_prefix_token_ordering) {
  ...
  std::vector<const char *> keys = {"enter",   "elephant", "enamel", "ercot",
                                    "enyzme",  "energy",   "epoch",  "epyc",
                                    "express", "everest",  "end",    "e"};

  for (size_t i = 0; i < keys.size(); i++) {
    art_document doc(i, keys.size() - i, {0});
    ASSERT_TRUE(NULL == art_insert(&t, (unsigned char *)keys[i],
                                   strlen(keys[i]) + 1, &doc));
  }

  std::vector<art_leaf *> leaves;
  art_fuzzy_search(&t, (const unsigned char *)"e", 1, 0, 0, 3, MAX_SCORE, true,
                   false, "", nullptr, 0, leaves, exclude_leaves);

  std::string first_key(reinterpret_cast<char *>(leaves[0]->key),
                        leaves[0]->key_len - 1);
  ASSERT_EQ("e", first_key);

  std::string second_key(reinterpret_cast<char *>(leaves[1]->key),
                         leaves[1]->key_len - 1);
  ASSERT_EQ("enter", second_key);

  std::string third_key(reinterpret_cast<char *>(leaves[2]->key),
                        leaves[2]->key_len - 1);
  ASSERT_EQ("elephant", third_key);

  leaves.clear();
  exclude_leaves.clear();
  art_fuzzy_search(&t, (const unsigned char *)"enter", 5, 1, 1, 3, MAX_SCORE,
                   true, false, "", nullptr, 0, leaves, exclude_leaves);
...
TEST(ArtTest, test_art_fuzzy_search) {
  ...
  leaves.clear();
  exclude_leaves.clear();
  art_fuzzy_search(&t, (const unsigned char *)"pltinum", strlen("pltinum"), 0,
                   1, 10, FREQUENCY, true, false, "", nullptr, 0, leaves,
                   exclude_leaves);
  ...
  // extra char
  art_fuzzy_search(&t, (const unsigned char *)"higghliving",
                   strlen("higghliving") + 1, 0, 1, 10, FREQUENCY, false, false,
                   "", nullptr, 0, leaves, exclude_leaves);
  ASSERT_EQ(1, leaves.size());
  ...
  leaves.clear();
  exclude_leaves.clear();
  art_fuzzy_search(&t, (const unsigned char *)"zymosthneic",
                   strlen("zymosthneic") + 1, 0, 1, 10, FREQUENCY, false, false,
                   "", nullptr, 0, leaves, exclude_leaves);
  ASSERT_EQ(1, leaves.size());
  ...
  leaves.clear();
  exclude_leaves.clear();
  art_fuzzy_search(&t, (const unsigned char *)"dacrcyystlgia",
                   strlen("dacrcyystlgia") + 1, 0, 2, 10, FREQUENCY, false,
                   false, "", nullptr, 0, leaves, exclude_leaves);
  ASSERT_EQ(1, leaves.size());
  ...
  exclude_leaves.clear();
  art_fuzzy_search(&t, (const unsigned char *)"dacrcyystlgia",
                   strlen("dacrcyystlgia") + 1, 1, 2, 10, FREQUENCY, false,
                   false, "", nullptr, 0, leaves, exclude_leaves);
  ASSERT_EQ(1, leaves.size());
  ASSERT_STREQ("dacrycystalgia", (const char *)leaves.at(0)->key);
  ...
  leaves.clear();
  exclude_leaves.clear();
  art_fuzzy_search(&t, (const unsigned char *)"gaberlunze",
                   strlen("gaberlunze") + 1, 0, 1, 10, FREQUENCY, false, false,
                   "", nullptr, 0, leaves, exclude_leaves);
  ASSERT_EQ(1, leaves.size());
  ...
  leaves.clear();
  exclude_leaves.clear();
  art_fuzzy_search(&t, (const unsigned char *)"eacemiferous",
                   strlen("eacemiferous") + 1, 0, 1, 10, FREQUENCY, false,
                   false, "", nullptr, 0, leaves, exclude_leaves);
  ASSERT_EQ(1, leaves.size());
  ...
  leaves.clear();
  exclude_leaves.clear();
  art_fuzzy_search(&t, (const unsigned char *)"Sarbruckken",
                   strlen("Sarbruckken") + 1, 0, 2, 10, FREQUENCY, false, false,
                   "", nullptr, 0, leaves, exclude_leaves);
  ASSERT_EQ(1, leaves.size());
  ...
  leaves.clear();
  exclude_leaves.clear();
  art_fuzzy_search(&t, (const unsigned char *)"hown", strlen("hown") + 1, 0, 1,
                   10, FREQUENCY, false, false, "", nullptr, 0, leaves,
                   exclude_leaves);
  ASSERT_EQ(10, leaves.size());

  std::set<std::string> expected_words = {"town", "sown", "mown", "lown",
                                          "howl", "howk", "howe", "how",
                                          "horn", "hoon"};
  ...
  leaves.clear();
  exclude_leaves.clear();
  art_fuzzy_search(&t, (const unsigned char *)"lionhear", strlen("lionhear"), 0,
                   0, 10, FREQUENCY, true, false, "", nullptr, 0, leaves,
                   exclude_leaves);
  ASSERT_EQ(3, leaves.size());

  leaves.clear();
  exclude_leaves.clear();
  art_fuzzy_search(&t, (const unsigned char *)"lineage", strlen("lineage"), 0,
                   0, 10, FREQUENCY, true, false, "", nullptr, 0, leaves,
                   exclude_leaves);
  ASSERT_EQ(2, leaves.size());
  ...
  leaves.clear();
  exclude_leaves.clear();
  art_fuzzy_search(&t, (const unsigned char *)"liq", strlen("liq"), 0, 0, 50,
                   FREQUENCY, true, false, "", nullptr, 0, leaves,
                   exclude_leaves);
  ASSERT_EQ(39, leaves.size());

  leaves.clear();
  exclude_leaves.clear();
  art_fuzzy_search(&t, (const unsigned char *)"antitraditiana",
                   strlen("antitraditiana"), 0, 1, 10, FREQUENCY, true, false,
                   "", nullptr, 0, leaves, exclude_leaves);
  ASSERT_EQ(1, leaves.size());

  leaves.clear();
  exclude_leaves.clear();
  art_fuzzy_search(&t, (const unsigned char *)"antisocao", strlen("antisocao"),
                   0, 2, 10, FREQUENCY, true, false, "", nullptr, 0, leaves,
                   exclude_leaves);
...
TEST(ArtTest, test_art_fuzzy_search_unicode_chars) {
  ...
  std::vector<const char *> keys = {"роман", "обладать",  "роисхождения",
                                    "без",   "பஞ்சமம்",     "சுதந்திரமாகவே",
                                    "அல்லது", "அடிப்படையில்"};
  ...
  for (const char *key : keys) {
    ...
    std::vector<art_leaf *> leaves;
    art_fuzzy_search(&t, (unsigned char *)key, strlen(key), 0, 0, 10, FREQUENCY,
                     true, false, "", nullptr, 0, leaves, exclude_leaves);
    ASSERT_EQ(1, leaves.size());
  }
...
TEST(ArtTest, test_art_fuzzy_search_extra_chars) {
  ...
  std::vector<const char *> keys = {"abbviation"};
  ...
  std::vector<art_leaf *> leaves;
  art_fuzzy_search(&t, (unsigned char *)query, strlen(query), 0, 2, 10,
                   FREQUENCY, true, false, "", nullptr, 0, leaves,
                   exclude_leaves);
  ASSERT_EQ(1, leaves.size());
...
TEST(ArtTest, test_art_search_sku_like_tokens) {
  ...
  // exact search
  art_leaf *l =
      (art_leaf *)art_search(&t, (const unsigned char *)key1, strlen(key1) + 1);
  ...
  for (const auto &key : keys) {
    std::vector<art_leaf *> leaves;
    art_fuzzy_search(&t, (const unsigned char *)key.c_str(), key.size(), 0, 0,
                     10, FREQUENCY, true, false, "", nullptr, 0, leaves,
                     exclude_leaves);
    ASSERT_EQ(1, leaves.size());
    ...
    // non prefix
    art_fuzzy_search(&t, (const unsigned char *)key.c_str(), key.size() + 1, 0,
                     0, 10, FREQUENCY, false, false, "", nullptr, 0, leaves,
                     exclude_leaves);
    ASSERT_EQ(1, leaves.size());
    ASSERT_STREQ(key.c_str(), (const char *)leaves.at(0)->key);
  }
...
TEST(ArtTest, test_art_search_ill_like_tokens) {
  ...
  std::map<std::string, size_t> key_to_count{
      std::make_pair("input", 2),    std::make_pair("illustration", 2),
      std::make_pair("image", 7),    std::make_pair("instrument", 2),
      std::make_pair("in", 10),      std::make_pair("info", 2),
      std::make_pair("inventor", 2), std::make_pair("imageresize", 2),
      std::make_pair("id", 5),       std::make_pair("insect", 2),
      std::make_pair("ice", 2),
  };
  ...
  for (const auto &key : keys) {
    art_leaf *l = (art_leaf *)art_search(&t, (const unsigned char *)key.c_str(),
                                         key.size() + 1);
...
TEST(ArtTest, test_art_search_ill_like_tokens2) {
  ...
  art_document doc = get_document((uint32_t)1);
  ASSERT_TRUE(NULL == art_insert(&t, (unsigned char *)keys[0].c_str(),
                                 keys[0].size() + 1, &doc));

  art_document doc2 = get_document((uint32_t)2);
  ASSERT_TRUE(NULL == art_insert(&t, (unsigned char *)keys[1].c_str(),
                                 keys[1].size() + 1, &doc2));

  art_document doc3 = get_document((uint32_t)3);
  ASSERT_TRUE(NULL == art_insert(&t, (unsigned char *)keys[2].c_str(),
                                 keys[2].size() + 1, &doc3));
...
TEST(ArtTest, test_art_search_roche_chews) {
  ...
  art_fuzzy_search(&t, (const unsigned char *)keys[0].c_str(),
                   keys[0].size() + 1, 0, 0, 10, FREQUENCY, false, false, "",
                   nullptr, 0, leaves, exclude_leaves);
...
TEST(ArtTest, test_art_search_raspberry) {
  ...
  for (const auto &key : keys) {
    art_document doc = get_document((uint32_t)1);
    ASSERT_TRUE(NULL == art_insert(&t, (unsigned char *)key.c_str(),
                                   key.size() + 1, &doc));
  }
  ...
  art_fuzzy_search(&t, (const unsigned char *)q_raspberries.c_str(),
                   q_raspberries.size(), 0, 2, 10, FREQUENCY, true, false, "",
                   nullptr, 0, leaves, exclude_leaves);
  ASSERT_EQ(2, leaves.size());
  ...
  art_fuzzy_search(&t, (const unsigned char *)q_raspberry.c_str(),
                   q_raspberry.size(), 0, 2, 10, FREQUENCY, true, false, "",
                   nullptr, 0, leaves, exclude_leaves);
  ASSERT_EQ(2, leaves.size());
...
TEST(ArtTest, test_art_search_highliving) {
  ...
  art_fuzzy_search(&t, (const unsigned char *)query.c_str(), query.size() + 1,
                   0, 1, 10, FREQUENCY, false, false, "", nullptr, 0, leaves,
                   exclude_leaves);
  ASSERT_EQ(1, leaves.size());
  ...
  art_fuzzy_search(&t, (const unsigned char *)query.c_str(), query.size(), 0, 2,
                   10, FREQUENCY, true, false, "", nullptr, 0, leaves,
                   exclude_leaves);
  ASSERT_EQ(1, leaves.size());
...
TEST(ArtTest, test_encode_int64) {
  ...
  unsigned char chars_large_neg_num[8] = {255, 255, 255, 255,
                                          127, 255, 255, 57};
...
TEST(ArtTest, test_encode_float_positive) {
  ...
  for (size_t i = 0; i < 6; i++) {
    ...
    art_document doc = get_document(i);
    ASSERT_TRUE(NULL ==
                art_insert(&t, (unsigned char *)chars0, CHAR_LEN, &doc));
  }
