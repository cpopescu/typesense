...
TEST(StringUtilsTest, HMAC) {
  std::string digest1 =
      StringUtils::hmac("KeyVal", "{\"filter_by\": \"user_id:1080\"}");
...
TEST(StringUtilsTest, ShouldSplitString) {
  ...
  size_t end_index =
      StringUtils::split("a b c d e f", lines_limited, " ", false, true, 0, 3);
  ...
  end_index = StringUtils::split("a b c d e f", lines_custom_start, " ", false,
                                 true, 2, 100);
...
TEST(StringUtilsTest, ShouldComputeSHA256) {
  ...
  ASSERT_STREQ(
      "d8705968091d40b60436675240712c584c187eef091514d4092483dc342ca3de",
      StringUtils::hash_sha256("some random key").c_str());

  ASSERT_STREQ(
      "6613f67d3d78d48e2678faf55c33fabc5895c538ce70ea10218ce9b7eccbf394",
      StringUtils::hash_sha256(
          "791a27668b3e01fc6ab3482b6e6a36255154df3ecd7dcec")
          .c_str());
}
...
TEST(StringUtilsTest, ShouldParseQueryString) {
  ...
  qs = "filter_by=points:>100 && brand:= "
       "nike&bazinga=true&filter_by=points:<=200";
  qmap.clear();
  StringUtils::parse_query_string(qs, qmap);
  ASSERT_EQ(2, qmap.size());
...
TEST(StringUtilsTest, ShouldTrimCurlySpaces) {
  ...
  ASSERT_EQ("foo {bar} {baz}",
            StringUtils::trim_curly_spaces("foo { bar } {  baz}"));
}
...
TEST(StringUtilsTest, ShouldSplitRangeFacet) {
  std::string range_facets_string =
      "score(fail:[0, 40], pass:[40, 100]), grade(A:[80,100], B:[60, 80], "
      "C:[40, 60])";
  ...
  std::string mixed_facets_string =
      "score, grade(A:[80,100], B:[60, 80], C:[40, 60]), rank";
...
void tokenizeTestHelper(const std::string& filter_query,
                        const std::vector<std::string>& tokenList) {
  std::queue<std::string> tokenizeOutput;
  auto tokenize_op =
      StringUtils::tokenize_filter_query(filter_query, tokenizeOutput);
...
TEST(StringUtilsTest, TokenizeFilterQuery) {
  ...
  filter_query =
      "((age: <5 || age: >10) && category:= [shoes]) || is_curated: true";
  tokenList = {"(",
               "(",
               "age: <5",
               "||",
  ...
  tokenList = {"(",       "(",  "age:<5",   "||",
               "age:>10", ")",  "&&",       "location:(48.906,2.343,5mi)",
               ")",       "||", "tags:AT&T"};
  tokenizeTestHelper(filter_query, tokenList);

  filter_query =
      "((age: <5 || age: >10) && category:= [shoes]) &&"
...
void splitIncludeTestHelper(const std::string& include_fields,
