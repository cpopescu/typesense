...
TEST_F(CollectionSortingTest, SortingOrder) {
  ...
  nlohmann::json results =
      coll_mul_fields
          ->search("the", query_fields, "", facets, sort_fields, {0}, 15, 1,
                   FREQUENCY, {false})
          .get();
  ASSERT_EQ(10, results["hits"].size());

  std::vector<std::string> ids = {"17", "13", "10", "4",  "0",
                                  "1",  "8",  "6",  "16", "11"};
  ...
  results = coll_mul_fields
                ->search("the", query_fields, "", facets, sort_fields, {0}, 5,
                         1, FREQUENCY, {false})
                .get();
  ...
  results = coll_mul_fields
                ->search("the", query_fields, "", facets, sort_fields, {0}, 15,
                         1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(10, results["hits"].size());
  ...
  // should be ordered desc on the default sorting field, since the match score
  // will be the same for all records.
  sort_fields = {};
  results = coll_mul_fields
                ->search("of", query_fields, "", facets, sort_fields, {0}, 10,
                         1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(5, results["hits"].size());
...
TEST_F(CollectionSortingTest, DefaultSortingFieldValidations) {
  ...
  Option<Collection*> collection_op = collectionManager.create_collection(
      "sample_collection", 4, fields, "name");
  ASSERT_FALSE(collection_op.ok());
  ASSERT_EQ("Default sorting field `name` is not a sortable type.",
            collection_op.error());
  ...
  collection_op = collectionManager.create_collection("sample_collection", 4,
                                                      fields, "NOT-DEFINED");
  ASSERT_FALSE(collection_op.ok());
  ASSERT_EQ(
      "Default sorting field is defined as `NOT-DEFINED` but is not found in "
      "the schema.",
      collection_op.error());
  collectionManager.drop_collection("sample_collection");
...
TEST_F(CollectionSortingTest, NoDefaultSortingField) {
  ...
  // without a default sorting field, matches should be sorted by (text_match,
  // seq_id)
  auto results = coll1
                     ->search("rocket", {"title"}, "", {}, {}, {1}, 10, 1,
                              FREQUENCY, {false})
                     .get();
  ...
  // try removing a document and doing wildcard (tests the seq_id array used for
  // wildcard searches)
  auto remove_op = coll1->remove("0");
  ASSERT_TRUE(remove_op.ok());

  results =
      coll1->search("*", {}, "", {}, {}, {1}, 30, 1, FREQUENCY, {false}).get();
  ...
  for (size_t i = 23; i >= 1; i--) {
    ...
    ASSERT_EQ(doc_id,
              results["hits"][23 - i]["document"]["id"].get<std::string>());
  }
}

TEST_F(CollectionSortingTest,
       FrequencyOrderedTokensWithoutDefaultSortingField) {
  // when no default sorting field is provided, tokens must be ordered on
  // frequency
  Collection* coll1;
  std::vector<field> fields = {field("title", field_types::STRING, false),
                               field("points", field_types::INT32, false)};
  ...
  // since only top 4 tokens are fetched for prefixes, the "enyzme" should not
  // show up in the results
  std::vector<std::string> tokens = {"enter",   "elephant", "enamel", "ercot",
                                     "enyzme",  "energy",   "epoch",  "epyc",
                                     "express", "everest",  "end"};
  ...
  auto results =
      coll1->search("e", {"title"}, "", {}, {}, {0}, 100, 1, NOT_SET, {true})
          .get();

  // [11 + 10 + 9 + 8] + 7 + 6 + 5 + 4 + 3 + 2
  ASSERT_EQ(38, results["found"].get<size_t>());

  // we have to ensure that no result contains the word "end" since it occurs
...
TEST_F(CollectionSortingTest, TokenOrderingOnFloatValue) {
  ...
  auto results =
      coll1->search("e", {"title"}, "", {}, {}, {0}, 3, 1, MAX_SCORE, {true})
          .get();
...
TEST_F(CollectionSortingTest, Int64AsDefaultSortingField) {
  ...
  auto doc_str1 =
      "{\"title\": \"foo\", \"starring\": \"bar\", \"points\": "
      "343234324234233234, \"cast\": [\"baz\"] }";
  const Option<nlohmann::json>& add_op = coll_mul_fields->add(doc_str1);
  ASSERT_TRUE(add_op.ok());

  auto doc_str2 =
      "{\"title\": \"foo\", \"starring\": \"bar\", \"points\": "
      "343234324234233232, \"cast\": [\"baz\"] }";
  auto doc_str3 =
      "{\"title\": \"foo\", \"starring\": \"bar\", \"points\": "
      "343234324234233235, \"cast\": [\"baz\"] }";
  auto doc_str4 =
      "{\"title\": \"foo\", \"starring\": \"bar\", \"points\": "
      "343234324234233231, \"cast\": [\"baz\"] }";
  ...
  nlohmann::json results =
      coll_mul_fields
          ->search("foo", query_fields, "", facets, sort_fields, {0}, 10, 1,
                   FREQUENCY, {false})
          .get();
  ASSERT_EQ(4, results["hits"].size());
  ...
  sort_fields = {sort_by("points", "desc")};
  results = coll_mul_fields
                ->search("foo", query_fields, "", facets, sort_fields, {0}, 10,
                         1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(4, results["hits"].size());
...
TEST_F(CollectionSortingTest, SortOnFloatFields) {
  ...
  std::vector<sort_by> sort_fields_desc = {sort_by("score", "DESC"),
                                           sort_by("average", "DESC")};
  ...
  if (coll_float_fields == nullptr) {
    coll_float_fields =
        collectionManager
            .create_collection("coll_float_fields", 4, fields, "score")
            .get();
  }
  ...
  std::vector<sort_by> sort_fields_asc = {sort_by("score", "ASC"),
                                          sort_by("average", "ASC")};
  ...
  std::vector<sort_by> sort_fields_asc_desc = {sort_by("score", "ASC"),
                                               sort_by("average", "DESC")};
  results = coll_float_fields
                ->search("Jeremy", query_fields, "", facets,
                         sort_fields_asc_desc, {0}, 10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(7, results["hits"].size());
...
TEST_F(CollectionSortingTest, ThreeSortFieldsLimit) {
  ...
  query_fields = {"title"};
  auto res_op = coll1->search("the", query_fields, "", {}, sort_fields_desc,
                              {0}, 10, 1, FREQUENCY, {false});

  ASSERT_FALSE(res_op.ok());
  ASSERT_STREQ("Only upto 3 sort_by fields can be specified.",
               res_op.error().c_str());
...
TEST_F(CollectionSortingTest, ThreeSortFieldsTextMatchLast) {
  ...
  std::vector<sort_by> sort_fields = {
      sort_by("popularity", "DESC"), sort_by("points", "DESC"),
      sort_by(sort_field_const::text_match, "DESC")};

  auto res =
      coll1
          ->search("grant", {"title", "artist"}, "", {}, sort_fields, {1}, 10,
...
TEST_F(CollectionSortingTest, SingleFieldTextMatchScoreDefault) {
  // when queried with a single field, _text_match score should be used
  // implicitly as the second sorting field
  Collection* coll1;
  ...
  auto results =
      coll1
          ->search("alpha", {"title"}, "", {}, sort_fields, {2}, 10, 1,
...
TEST_F(CollectionSortingTest, NegativeInt64Value) {
  ...
  std::vector<sort_by> sort_fields_desc = {sort_by("points", "DESC")};

  query_fields = {"title"};
  auto res = coll1
                 ->search("*", query_fields, "points:>=1577836800", {},
...
TEST_F(CollectionSortingTest, GeoPointSorting) {
  ...
  std::vector<std::string> expected_ids = {"9", "7", "4", "5", "3",
                                           "8", "0", "6", "1", "2"};

  for (size_t i = 0; i < expected_ids.size(); i++) {
    ASSERT_STREQ(
        expected_ids[i].c_str(),
        results["hits"][i]["document"]["id"].get<std::string>().c_str());
  }
  ...
  results =
      coll1->search("*", {}, "", {}, geo_sort_fields, {0}, 10, 1, FREQUENCY)
          .get();
  ...
  for (size_t i = 0; i < expected_ids.size(); i++) {
    ASSERT_STREQ(
        expected_ids[expected_ids.size() - 1 - i].c_str(),
        results["hits"][i]["document"]["id"].get<std::string>().c_str());
  }
  ...
  auto res_op = coll1->search("*", {}, "", {}, bad_geo_sort_fields, {0}, 10, 1,
                              FREQUENCY);

  ASSERT_FALSE(res_op.ok());
  ASSERT_STREQ("Bad syntax for sorting field `loc`", res_op.error().c_str());

  bad_geo_sort_fields = {sort_by("loc(x, y)", "ASC")};

  res_op = coll1->search("*", {}, "", {}, bad_geo_sort_fields, {0}, 10, 1,
                         FREQUENCY);

  ASSERT_FALSE(res_op.ok());
  ASSERT_STREQ("Bad syntax for sorting field `loc`", res_op.error().c_str());

  bad_geo_sort_fields = {sort_by("loc(", "ASC")};

  res_op = coll1->search("*", {}, "", {}, bad_geo_sort_fields, {0}, 10, 1,
                         FREQUENCY);

  ASSERT_FALSE(res_op.ok());
  ASSERT_STREQ("Could not find a field named `loc(` in the schema for sorting.",
               res_op.error().c_str());
  ...
  bad_geo_sort_fields = {sort_by("l()", "ASC")};

  res_op = coll1->search("*", {}, "", {}, bad_geo_sort_fields, {0}, 10, 1,
                         FREQUENCY);

  ASSERT_FALSE(res_op.ok());
  ASSERT_STREQ("Could not find a field named `l` in the schema for sorting.",
               res_op.error().c_str());

  // should not allow creation of collection with geo field as
...
TEST_F(CollectionSortingTest, GeoPointSortingWithExcludeRadius) {
  ...
  auto results = coll1
                     ->search("*", {}, "loc: (32.24348, 77.1893, 20 km)", {},
                              geo_sort_fields, {0}, 10, 1, FREQUENCY)
                     .get();

  ASSERT_EQ(6, results["found"].get<size_t>());

  std::vector<std::string> expected_ids = {"2", "1", "0", "3", "4", "5"};
  ...
  res_op = coll1->search("*", {},
                         "loc: (28.7040592, 77.10249019999999, 28.7040592, "
                         "77.10249019999999, 28.7040592, 77.10249019999999, "
  ...
  geo_sort_fields = {
      sort_by("loc(32.24348, 77.1893, exclude_radius: 1 meter)", "ASC")};
  res_op = coll1->search("*", {}, "loc: (32.24348, 77.1893, 20 km)", {},
                         geo_sort_fields, {0}, 10, 1, FREQUENCY);

  ASSERT_FALSE(res_op.ok());
  ASSERT_EQ("Sort field's parameter unit must be either `km` or `mi`.",
            res_op.error());

  geo_sort_fields = {
      sort_by("loc(32.24348, 77.1893, exclude_radius: -10 km)", "ASC")};
  res_op = coll1->search("*", {}, "loc: (32.24348, 77.1893, 20 km)", {},
                         geo_sort_fields, {0}, 10, 1, FREQUENCY);

  ASSERT_FALSE(res_op.ok());
  ASSERT_EQ("Sort field's parameter must be a positive number.",
            res_op.error());

  geo_sort_fields = {
      sort_by("loc(32.24348, 77.1893, exclude_radius: 10 km 20 mi)", "ASC")};
  res_op = coll1->search("*", {}, "loc: (32.24348, 77.1893, 20 km)", {},
                         geo_sort_fields, {0}, 10, 1, FREQUENCY);
  ...
  geo_sort_fields = {
      sort_by("loc(32.24348, 77.1893, exclude_radius: 1k)", "ASC")};
  res_op = coll1->search("*", {}, "loc: (32.24348, 77.1893, 20 km)", {},
                         geo_sort_fields, {0}, 10, 1, FREQUENCY);
  ASSERT_FALSE(res_op.ok());
  ASSERT_EQ("Sort field's parameter unit must be either `km` or `mi`.",
  ...
  geo_sort_fields = {
      sort_by("loc(32.24348, 77.1893, exclude_radius: 5)", "ASC")};
  res_op = coll1->search("*", {}, "loc: (32.24348, 77.1893, 20 km)", {},
                         geo_sort_fields, {0}, 10, 1, FREQUENCY);
  ASSERT_FALSE(res_op.ok());
  ASSERT_EQ("Bad syntax for sorting field `loc`", res_op.error());
...
TEST_F(CollectionSortingTest, GeoPointSortingWithPrecision) {
  ...
  std::vector<std::string> expected_ids = {"6", "2", "1", "0",
                                           "3", "4", "7", "5"};
  ...
  geo_sort_fields = {
      sort_by("loc(32.24348, 77.1893, precision: 1 meter)", "ASC")};
  res_op = coll1->search("*", {}, "loc: (32.24348, 77.1893, 20 km)", {},
                         geo_sort_fields, {0}, 10, 1, FREQUENCY);
  ...
  geo_sort_fields = {
      sort_by("loc(32.24348, 77.1893, precision: -10 km)", "ASC")};
  res_op = coll1->search("*", {}, "loc: (32.24348, 77.1893, 20 km)", {},
                         geo_sort_fields, {0}, 10, 1, FREQUENCY);
...
TEST_F(CollectionSortingTest, GeoPointArraySorting) {
  ...
  auto results = coll1
                     ->search("*", {}, "loc: (13.12631, 80.20252, 100 km)", {},
                              geo_sort_fields, {0}, 10, 1, FREQUENCY)
                     .get();
  ...
  results = coll1
                ->search("*", {}, "loc: (13.03388, 79.25868, 1000 km)", {},
                         geo_sort_fields, {0}, 10, 1, FREQUENCY)
                .get();
...
TEST_F(CollectionSortingTest, SortByTitle) {
  ...
  std::vector<field> fields = {
      field("title", field_types::STRING, false, false, true, "", true),
  ...
  if (coll1 == nullptr) {
    auto create_op =
        collectionManager.create_collection("coll1", 2, fields, "title");
    ASSERT_TRUE(create_op.ok());
    coll1 = create_op.get();
  }

  std::vector<std::vector<std::string>> records = {
      {"aaa", "ABCD"},  {"a", "ABCD"},   {"abcd", "ABCD"}, {"abdde", "ABCD"},
      {"b", "ABCD"},    {"bab", "ABCD"}, {"baa", "ABCD"},  {"bcma", "ABCD"},
      {"cdma", "ABCD"}, {"cc", "ABCD"},  {"c", "ABCD"},    {"cxya", "ABCD"},
  };
  ...
  std::vector<sort_by> sort_fields = {sort_by("title", "ASC")};

  std::vector<std::string> expected_order = {"a", "aaa", "abcd", "abdde",
                                             "b", "baa", "bab",  "bcma",
                                             "c", "cc",  "cdma", "cxya"};

  auto results = coll1
                     ->search("*", {}, "", {}, sort_fields, {0}, 20, 1,
                              FREQUENCY, {true}, 10)
                     .get();
  ...
  for (size_t i = 0; i < results["hits"].size(); i++) {
    ASSERT_EQ(expected_order[i],
              results["hits"][i]["document"]["title"].get<std::string>());
  }

  // descending order
  sort_fields = {sort_by("title", "DESC")};

  results = coll1
                ->search("*", {}, "", {}, sort_fields, {0}, 20, 1, FREQUENCY,
                         {true}, 10)
                .get();
  ...
  for (size_t i = 0; i < results["hits"].size(); i++) {
    ASSERT_EQ(expected_order[expected_order.size() - i - 1],
              results["hits"][i]["document"]["title"].get<std::string>());
  }

  // when querying for a string field with sort disabled
  sort_fields = {sort_by("artist", "DESC")};

  auto res_op = coll1->search("*", {}, "", {}, sort_fields, {0}, 20, 1,
                              FREQUENCY, {true}, 10);
  ASSERT_FALSE(res_op.ok());
  ASSERT_EQ("Could not find a field named `artist` in the schema for sorting.",
            res_op.error());

  // don't allow non-sort string field to be used as default sorting field

  fields = {
      field("title", field_types::STRING, false, false, true, "", false),
      field("artist", field_types::STRING, true),
      field("points", field_types::INT32, false),
  };

  auto create_op =
      collectionManager.create_collection("coll2", 2, fields, "title");
  ASSERT_FALSE(create_op.ok());
  ASSERT_EQ("Default sorting field `title` is not a sortable type.",
            create_op.error());
...
TEST_F(CollectionSortingTest, SortByIntegerAndString) {
  ...
  std::vector<std::vector<std::string>> records = {
      {"abdde", "2"}, {"b", "2"},    {"b", "1"},   {"a", "1"},
      {"c", "1"},     {"dd", "4"},   {"bab", "3"}, {"baa", "3"},
      {"bcma", "3"},  {"cdma", "3"}, {"c", "5"},   {"x", "6"},
  };
  ...
  ASSERT_EQ("abdde",
            results["hits"][3]["document"]["title"].get<std::string>());
  ...
  results = coll1
                ->search("b", {"title"}, "", {}, sort_fields, {0}, 20, 1,
                         FREQUENCY, {true}, 10)
                .get();
...
TEST_F(CollectionSortingTest, SortByStringEmptyValuesConfigFirstField) {
  ...
  coll1 = collectionManager.get_collection("coll1").get();
  if (coll1 == nullptr) {
    coll1 = collectionManager.create_collection("coll1", 1, fields, "points1")
                .get();
  }
  ...
  // without explicit arg, missing values will be deemed as having largest value
  // (same as SQL)
  sort_fields = {
      sort_by("title", "asc"),
  };
  ...
  res_op = coll1->search("*", {"title"}, "", {}, sort_fields, {0}, 10, 1,
                         MAX_SCORE, {true});
  ASSERT_FALSE(res_op.ok());
  ASSERT_EQ("Bad syntax for sorting field `title`", res_op.error());
}
...
TEST_F(CollectionSortingTest, TextMatchBucketRanking) {
  ...
  auto results =
      coll1
          ->search("mark", {"title"}, "", {}, sort_fields, {2}, 10, 1,
                   FREQUENCY, {true}, 10, spp::sparse_hash_set<std::string>(),
                   spp::sparse_hash_set<std::string>(), 10, "", 30, 4, "title",
                   20, {}, {}, {}, 0, "<mark>", "</mark>", {3}, 1000, true)
          .get();
  ...
  auto res_op =
      coll1->search("mark", {"title"}, "", {}, sort_fields, {2}, 10, 1,
                    FREQUENCY, {true}, 10, spp::sparse_hash_set<std::string>(),
                    spp::sparse_hash_set<std::string>(), 10, "", 30, 4, "title",
                    20, {}, {}, {}, 0, "<mark>", "</mark>", {3}, 1000, true);

  ASSERT_FALSE(res_op.ok());
  ASSERT_EQ("Invalid sorting parameter passed for _text_match.",
            res_op.error());
  ...
  ASSERT_FALSE(res_op.ok());
  ASSERT_EQ(
      "Could not find a field named `_text_match(foobar:` in the schema for "
      "sorting.",
      res_op.error());
  ...
  res_op =
      coll1->search("mark", {"title"}, "", {}, sort_fields, {2}, 10, 1,
                    ...
                    20, {}, {}, {}, 0, "<mark>", "</mark>", {3}, 1000, true);
  ASSERT_FALSE(res_op.ok());
  ASSERT_EQ("Invalid value passed for _text_match `buckets` configuration.",
            res_op.error());
...
TEST_F(CollectionSortingTest, TextMatchMoreDocsThanBuckets) {
  ...
  auto results =
      coll1
          ->search("mark", {"title"}, "", {}, sort_fields, {0}, 10, 1,
                   FREQUENCY, {true}, 10, spp::sparse_hash_set<std::string>(),
                   spp::sparse_hash_set<std::string>(), 10, "", 30, 4, "title",
                   20, {}, {}, {}, 0, "<mark>", "</mark>", {1}, 1000, true)
          .get();
...
TEST_F(CollectionSortingTest, RepeatingTokenRanking) {
  ...
  auto results =
      coll1
          ->search("mong mong", {"title"}, "", {}, sort_fields, {2}, 10, 1,
                   FREQUENCY, {true}, 10, spp::sparse_hash_set<std::string>(),
                   spp::sparse_hash_set<std::string>(), 10, "", 30, 4, "title",
                   20, {}, {}, {}, 0, "<mark>", "</mark>", {3}, 1000, true)
          .get();
  ...
  ASSERT_EQ(1157451471575842841,
            results["hits"][0]["text_match"].get<size_t>());
  ASSERT_EQ(1157451471575318553,
            results["hits"][1]["text_match"].get<size_t>());
  ASSERT_EQ(1157451471575318553,
            results["hits"][2]["text_match"].get<size_t>());
  ASSERT_EQ(1157451471575318553,
            results["hits"][3]["text_match"].get<size_t>());
...
TEST_F(CollectionSortingTest, SortingDoesNotHaveTextMatchComponent) {
  ...
  auto results = coll1
                     ->search("test", {"title"}, "", {}, sort_fields, {2}, 10,
                              1, FREQUENCY, {true})
                     .get();
  ...
  results =
      coll1->search("*", {}, "", {}, sort_fields, {2}, 10, 1, FREQUENCY, {true})
          .get();
  ASSERT_EQ(1, results["hits"].size());
  ASSERT_EQ(0, results["hits"][0].count("text_match"));
...
TEST_F(CollectionSortingTest, IntegerFloatAndBoolShouldDefaultSortTrue) {
  ...
  auto res_op =
      coll1->search("*", {"title"}, "", {}, {sort_by("points", "DESC")}, {2},
                    10, 1, FREQUENCY, {true}, 10);
  ASSERT_TRUE(res_op.ok());

  res_op = coll1->search("*", {"title"}, "", {}, {sort_by("timestamp", "DESC")},
                         {2}, 10, 1, FREQUENCY, {true}, 10);
  ASSERT_TRUE(res_op.ok());

  res_op = coll1->search("*", {"title"}, "", {}, {sort_by("max", "DESC")}, {2},
                         10, 1, FREQUENCY, {true}, 10);
  ASSERT_TRUE(res_op.ok());

  res_op = coll1->search("*", {"title"}, "", {}, {sort_by("is_valid", "DESC")},
                         {2}, 10, 1, FREQUENCY, {true}, 10);
  ASSERT_TRUE(res_op.ok());
...
TEST_F(CollectionSortingTest, DisallowSortingOnNonIndexedIntegerField) {
  ...
  auto res_op =
      coll1->search("*", {"title"}, "", {}, {sort_by("points", "DESC")}, {2},
                    10, 1, FREQUENCY, {true}, 10);
  ASSERT_FALSE(res_op.ok());
  ASSERT_EQ("Could not find a field named `points` in the schema for sorting.",
            res_op.error());

  res_op = coll1->search("*", {"title"}, "", {},
...
TEST_F(CollectionSortingTest, WildcardSearchSequenceIdSort) {
  ...
  auto res = coll1
                 ->search("*", {"category"}, "", {}, sort_fields, {2}, 10, 1,
                          FREQUENCY, {true}, 0)
                 .get();
...
TEST_F(CollectionSortingTest, OptionalFilteringViaSortingWildcard) {
  ...
  auto results = coll1
                     ->search("*", {"title"}, "", {}, sort_fields, {2}, 10, 1,
                              FREQUENCY, {true}, 10)
                     .get();
  ...
  for (size_t i = 0; i < expected_ids.size(); i++) {
    ASSERT_EQ(expected_ids[i],
              results["hits"][i]["document"]["id"].get<std::string>());
  }
  ...
  auto res_op = coll1->search("*", {"title"}, "", {}, sort_fields, {2}, 10, 1,
                              FREQUENCY, {true}, 10);
  ASSERT_FALSE(res_op.ok());
  ...
  res_op = coll1->search("*", {"title"}, "", {}, sort_fields, {2}, 10, 1,
                         FREQUENCY, {true}, 10);
  ASSERT_FALSE(res_op.ok());
  ...
  res_op = coll1->search("*", {"title"}, "", {}, sort_fields, {2}, 10, 1,
                         FREQUENCY, {true}, 10);
  ASSERT_FALSE(res_op.ok());
  ASSERT_EQ("Could not find a field named `)` in the schema for sorting.",
            res_op.error());
...
TEST_F(CollectionSortingTest, OptionalFilteringViaSortingSearch) {
  ...
  auto results = coll1
                     ->search("title", {"title"}, "", {}, sort_fields, {2}, 10,
                              1, FREQUENCY, {true}, 10)
                     .get();
  ...
  results = coll1
                ->search("title", {"title"}, "", {}, sort_fields, {2}, 10, 1,
                         FREQUENCY, {true}, 10)
                .get();
  ASSERT_EQ(5, results["hits"].size());
  ...
  auto res_op = coll1->search("title", {"title"}, "", {}, sort_fields, {2}, 10,
                              1, FREQUENCY, {true}, 10);
  ASSERT_FALSE(res_op.ok());
  ASSERT_EQ("Error parsing eval expression in sort_by clause.", res_op.error());
  ...
  res_op = coll1->search("title", {"title"}, "", {}, sort_fields, {2}, 10, 1,
                         FREQUENCY, {true}, 10);
  ASSERT_FALSE(res_op.ok());
  ASSERT_EQ("Could not find a field named `)` in the schema for sorting.",
            res_op.error());
...
TEST_F(CollectionSortingTest, DisallowIdAsDefaultSortingField) {
  ...
  ASSERT_EQ("Invalid `default_sorting_field` value: cannot be `id`.",
            coll_op.error());
}
...
TEST_F(CollectionSortingTest, AscendingVectorDistance) {
  ...
  std::vector<std::vector<float>> points = {
      {3.0, 4.0}, {9.0, 21.0}, {8.0, 15.0}, {1.0, 1.0}, {5.0, 7.0}};
...
TEST_F(CollectionSortingTest, InvalidVectorDistanceSorting) {
  ...
  auto results = coll1->search("title", {"title"}, "", {}, sort_fields, {2}, 10,
                               1, FREQUENCY, {true}, 10);

  ASSERT_FALSE(results.ok());

  ASSERT_EQ(
      "sort_by vector_distance is only supported for vector queries, semantic "
