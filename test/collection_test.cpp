...
class CollectionTest : public ::testing::Test {
 protected:
  ...
  void setupCollection() {
    ...
    system(("rm -rf " + state_dir_path + " && mkdir -p " + state_dir_path)
               .c_str());
    ...
    query_fields = {"title"};
    sort_fields = {sort_by(sort_field_const::text_match, "DESC"),
                   sort_by("points", "DESC")};
    ...
    if (collection == nullptr) {
      collection =
          collectionManager
              .create_collection("collection", 4, search_fields, "points")
              .get();
    }

    std::string json_line;

    // dummy record for record id 0: to make the test record IDs to match with
    // line numbers
    ...
    std::ifstream words_file(std::string(ROOT_DIR) +
...
TEST_F(CollectionTest, VerifyCountOfDocuments) {
  // we have 1 dummy record to match the line numbers on the fixtures file with
  // sequence numbers
  ASSERT_EQ(24 + 1, collection->get_num_documents());

  // check default no specific dirty values option is sent for a collection that
  // has explicit schema
  std::string empty_dirty_values;
  ASSERT_EQ(DIRTY_VALUES::REJECT,
            collection->parse_dirty_values_option(empty_dirty_values));
}
...
TEST_F(CollectionTest, ExactSearchShouldBeStable) {
  ...
  nlohmann::json results =
      collection
          ->search("the", query_fields, "", facets, sort_fields, {0}, 10, 1,
                   FREQUENCY, {false})
          .get();
  ...
  ASSERT_STREQ(
      "collection",
      results["request_params"]["collection_name"].get<std::string>().c_str());
  ASSERT_STREQ("the",
               results["request_params"]["q"].get<std::string>().c_str());
  ...
  results = collection
                ->search("the", query_fields, "", facets, sort_fields_asc, {0},
                         10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(7, results["hits"].size());
  ASSERT_EQ(7, results["found"].get<int>());
  ...
  // when a query does not return results, hits and found fields should still
  // exist in response
  results = collection
                ->search("zxsadqewsad", query_fields, "", facets,
                         sort_fields_asc, {0}, 10, 1, FREQUENCY, {false})
                .get();
...
TEST_F(CollectionTest, MultiTokenSearch) {
  ...
  nlohmann::json results =
      collection
          ->search("rocket launch", query_fields, "", facets, sort_fields, {0},
                   10, 1, FREQUENCY, {false}, 10,
                   spp::sparse_hash_set<std::string>(),
                   spp::sparse_hash_set<std::string>(), 10, "", 30, 5, "", 10)
          .get();
  ...
  */
  ...
  ASSERT_STREQ(
      results["hits"][0]["highlights"][0]["field"].get<std::string>().c_str(),
      "title");
  ASSERT_STREQ(
      results["hits"][0]["highlights"][0]["snippet"].get<std::string>().c_str(),
      "What is the power, requirement of a <mark>rocket</mark> "
      "<mark>launch</mark> these days?");

  // Check ASC sort order
  std::vector<sort_by> sort_fields_asc = {
      sort_by(sort_field_const::text_match, "DESC"), sort_by("points", "ASC")};
  results =
      collection
          ->search("rocket launch", query_fields, "", facets, sort_fields_asc,
                   {0}, 10, 1, FREQUENCY, {false}, 10,
                   spp::sparse_hash_set<std::string>(),
                   spp::sparse_hash_set<std::string>(), 10, "", 30, 5, "", 10)
          .get();
  ASSERT_EQ(5, results["hits"].size());
...
TEST_F(CollectionTest, SearchWithExcludedTokens) {
  ...
  nlohmann::json results =
      collection
          ->search("how -propellants -are", query_fields, "", facets,
                   sort_fields, {0}, 10, 1, FREQUENCY, {false}, 10,
                   spp::sparse_hash_set<std::string>(),
                   spp::sparse_hash_set<std::string>(), 10, "", 30, 5, "", 10)
          .get();
  ...
  results =
      collection
          ->search("-rocket", query_fields, "", facets, sort_fields, {0}, 50)
          .get();
  ...
  results = collection
                ->search("-rocket -cryovolcanism", query_fields, "", facets,
                         sort_fields, {0}, 50)
                .get();
...
TEST_F(CollectionTest, SkipUnindexedTokensDuringMultiTokenSearch) {
  ...
  std::vector<std::string> facets;
  nlohmann::json results = collection
                               ->search("DoesNotExist from", query_fields, "",
                                        facets, sort_fields, {0}, 10)
                               .get();
  ASSERT_EQ(2, results["hits"].size());
  ...
  // with non-zero cost
  results = collection
                ->search("DoesNotExist from", query_fields, "", facets,
                         sort_fields, {1}, 10)
                .get();
  ASSERT_EQ(2, results["hits"].size());
  ...
  results = collection
                ->search("from DoesNotExist insTruments", query_fields, "",
                         facets, sort_fields, {1}, 10)
                .get();
  ASSERT_EQ(2, results["hits"].size());
  ids = {"2", "17"};
  ...
  results = collection
                ->search("from DoesNotExist insTruments", query_fields, "",
                         facets, sort_fields, {1}, 10, 1, FREQUENCY, {true}, 1,
                         spp::sparse_hash_set<std::string>(),
                         spp::sparse_hash_set<std::string>(), 10, "", 30, 4, "",
                         1, {}, {}, {}, 0, "<mark>", "</mark>", {}, 1000, true,
                         false, true, "", true)
                .get();
  ASSERT_EQ(2, results["hits"].size());
  ids = {"2", "17"};
  ...
  results =
      collection
          ->search("the a", query_fields, "", facets, sort_fields, {0}, 10, 1,
  ...
  results.clear();
  results = collection
                ->search("the a", query_fields, "", facets, sort_fields, {0},
                         10, 1, FREQUENCY, {false}, 0)
                .get();
  ASSERT_EQ(3, results["hits"].size());
  ids = {"8", "16", "10"};
  ...
  results.clear();
  results = collection
                ->search("the a insurance", query_fields, "", facets,
                         sort_fields, {0}, 10, 1, FREQUENCY, {false}, 0)
                .get();
  ASSERT_EQ(0, results["hits"].size());
  ...
  results.clear();
  results = collection
                ->search("DoesNotExist1 DoesNotExist2", query_fields, "",
                         facets, sort_fields, {0}, 10)
                .get();
  ASSERT_EQ(0, results["hits"].size());
...
TEST_F(CollectionTest, PartialMultiTokenSearch) {
  ...
  nlohmann::json results =
      collection
          ->search("rocket research", query_fields, "", facets, sort_fields,
                   {0}, 10, 1, FREQUENCY, {false}, 10)
          .get();
...
TEST_F(CollectionTest, QueryWithTypo) {
  ...
  nlohmann::json results =
      collection
          ->search("kind biologcal", query_fields, "", facets, sort_fields, {2},
                   3, 1, FREQUENCY, {false}, 10,
                   spp::sparse_hash_set<std::string>(),
                   spp::sparse_hash_set<std::string>(), 10, "", 30, 5, "", 10)
          .get();
  ...
  results =
      collection
          ->search("lauxnch rcket", query_fields, "", facets, sort_fields, {1},
                   3, 1, FREQUENCY, {false}, 10,
                   spp::sparse_hash_set<std::string>(),
                   spp::sparse_hash_set<std::string>(), 10, "", 30, 5, "", 10)
          .get();
...
TEST_F(CollectionTest, TypoTokenRankedByScoreAndFrequency) {
  ...
  nlohmann::json results =
      collection
          ->search("loox", query_fields, "", facets, sort_fields, {1}, 2, 1,
                   MAX_SCORE, {false})
          .get();
  ASSERT_EQ(2, results["hits"].size());
  ...
  results = collection
                ->search("loox", query_fields, "", facets, sort_fields, {1}, 3,
                         1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(3, results["hits"].size());
  ids = {"22", "3", "12"};
  ...
  // Check pagination
  results = collection
                ->search("loox", query_fields, "", facets, sort_fields, {1}, 1,
                         1, FREQUENCY, {false})
                .get();
  ...
  results = collection
                ->search("loox", query_fields, "", facets, sort_fields, {1}, 2,
                         1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(5, results["found"].get<int>());
  ASSERT_EQ(2, results["hits"].size());
  ...
  results = collection
                ->search("loox", query_fields, "", facets, sort_fields, {1}, 10,
                         1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(5, results["hits"].size());
  ids = {"22", "3", "12", "23", "24"};
...
TEST_F(CollectionTest, TextContainingAnActualTypo) {
  // A line contains "ISSX" but not "what" - need to ensure that correction to
  // "ISSS what" happens
  std::vector<std::string> facets;
  nlohmann::json results =
      collection
          ->search("ISSX what", query_fields, "", facets, sort_fields, {1}, 4,
                   1, FREQUENCY, {false}, 20,
                   spp::sparse_hash_set<std::string>(),
                   spp::sparse_hash_set<std::string>(), 10, "", 30, 5, "", 20)
          .get();
  ...
  results =
      collection
          ->search("ISSX", query_fields, "", facets, sort_fields, {1}, 10, 1,
                   FREQUENCY, {false}, 10, spp::sparse_hash_set<std::string>(),
                   spp::sparse_hash_set<std::string>(), 10, "", 30, 5, "", 10)
          .get();
...
TEST_F(CollectionTest, Pagination) {
  nlohmann::json results =
      collection
          ->search("the", query_fields, "", {}, sort_fields, {0}, 3, 1,
                   FREQUENCY, {false})
          .get();
  ASSERT_EQ(3, results["hits"].size());
  ...
  results = collection
                ->search("the", query_fields, "", {}, sort_fields, {0}, 3, 2,
                         FREQUENCY, {false})
                .get();
  ASSERT_EQ(3, results["hits"].size());
  ASSERT_EQ(7, results["found"].get<uint32_t>());
  ...
  results = collection
                ->search("the", query_fields, "", {}, sort_fields, {0}, 3, 3,
                         FREQUENCY, {false})
                .get();
  ASSERT_EQ(1, results["hits"].size());
  ASSERT_EQ(7, results["found"].get<uint32_t>());
...
TEST_F(CollectionTest, WildcardQuery) {
  nlohmann::json results =
      collection
          ->search("*", query_fields, "points:>0", {}, sort_fields, {0}, 3, 1,
                   FREQUENCY, {false})
          .get();
  ...
  // when no filter is specified, fall back on default sorting field based
  // catch-all filter
  Option<nlohmann::json> results_op = collection->search(
      "*", query_fields, "", {}, sort_fields, {0}, 3, 1, FREQUENCY, {false});
  ...
  results = collection
                ->search("*", query_fields, "", {}, sort_fields, {0}, 3, 1,
                         FREQUENCY, {false})
                .get();
  ASSERT_EQ(3, results["hits"].size());
  ASSERT_EQ(25, results["found"].get<uint32_t>());
  ...
  results_op = collection->search("*", {}, "", {}, sort_fields, {0}, 3, 1,
                                  FREQUENCY, {false});
  ASSERT_TRUE(results_op.ok());
  ...
  results_op = collection->search("the", {}, "", {}, sort_fields, {0}, 3, 1,
                                  FREQUENCY, {false});
  ASSERT_FALSE(results_op.ok());
  ASSERT_STREQ("No search fields specified for the query.",
               results_op.error().c_str());
}
...
TEST_F(CollectionTest, PrefixSearching) {
  ...
  nlohmann::json results =
      collection
          ->search("ex", query_fields, "", facets, sort_fields, {0}, 10, 1,
                   FREQUENCY, {true})
          .get();
  ASSERT_EQ(2, results["hits"].size());
  ...
  results = collection
                ->search("ex", query_fields, "", facets, sort_fields, {0}, 10,
                         1, MAX_SCORE, {true})
                .get();
  ASSERT_EQ(2, results["hits"].size());
  ids = {"6", "12"};
  ...
  results =
      collection
          ->search("what ex", query_fields, "", facets, sort_fields, {0}, 10, 1,
                   MAX_SCORE, {true}, 10, spp::sparse_hash_set<std::string>(),
                   spp::sparse_hash_set<std::string>(), 10, "", 30, 5, "", 10)
          .get();
  ASSERT_EQ(9, results["hits"].size());
  ...
  // restrict to only 2 results and differentiate between MAX_SCORE and
  // FREQUENCY
  results =
      collection
          ->search("t", query_fields, "", facets, sort_fields, {0}, 2, 1,
                   MAX_SCORE, {true}, 10, spp::sparse_hash_set<std::string>(),
                   spp::sparse_hash_set<std::string>(), 10, "", 30, 5, "", 10)
          .get();
  ASSERT_EQ(2, results["hits"].size());
  ...
  results =
      collection
          ->search("t", query_fields, "", facets, sort_fields, {0}, 2, 1,
                   FREQUENCY, {true}, 10, spp::sparse_hash_set<std::string>(),
                   spp::sparse_hash_set<std::string>(), 10, "", 30, 5, "", 10)
          .get();
  ASSERT_EQ(2, results["hits"].size());
  ids = {"1", "2"};
  ...
  // only the last token in the query should be used for prefix search - so,
  // "math" should not match "mathematics"
  results = collection
                ->search("math fx", query_fields, "", facets, sort_fields, {0},
                         1, 1, FREQUENCY, {true}, 0)
                .get();
  ASSERT_EQ(0, results["hits"].size());

  // single and double char prefixes should set a ceiling on the num_typos
  // possible
  results = collection
                ->search("x", query_fields, "", facets, sort_fields, {2}, 2, 1,
                         FREQUENCY, {true})
                .get();
  ASSERT_EQ(0, results["hits"].size());
  ...
  // prefix with a typo
  results = collection
                ->search("late propx", query_fields, "", facets, sort_fields,
                         {2}, 1, 1, FREQUENCY, {true})
                .get();
  ASSERT_EQ(1, results["hits"].size());
...
TEST_F(CollectionTest, TypoTokensThreshold) {
  // Typo correction should happen only based on the `typo_tokens_threshold`
  // value
  auto results =
      collection
          ->search("redundant", {"title"}, "", {}, sort_fields, {2}, 10, 1,
                   token_ordering::FREQUENCY, {true}, 10,
                   spp::sparse_hash_set<std::string>(),
                   spp::sparse_hash_set<std::string>(), 10, "", 5, 5, "", 0)
          .get();
  ...
  results =
      collection
          ->search("redundant", {"title"}, "", {}, sort_fields, {2}, 10, 1,
                   token_ordering::FREQUENCY, {true}, 10,
                   spp::sparse_hash_set<std::string>(),
                   spp::sparse_hash_set<std::string>(), 10, "", 5, 5, "", 10)
          .get();
...
TEST_F(CollectionTest, MultiOccurrenceString) {
  ...
  coll_multi_string =
      collectionManager.get_collection("coll_multi_string").get();
  if (coll_multi_string == nullptr) {
    coll_multi_string =
        collectionManager
            .create_collection("coll_multi_string", 4, fields, "points")
            .get();
  }

  nlohmann::json document;
  document["title"] =
      "The brown fox was the tallest of the lot and the quickest of the trot.";
  document["points"] = 100;
  ...
  nlohmann::json results =
      coll_multi_string
          ->search("the", query_fields, "", {}, sort_fields, {0}, 10, 1,
                   FREQUENCY, {false}, 0)
          .get();
  ASSERT_EQ(1, results["hits"].size());
...
TEST_F(CollectionTest, ArrayStringFieldHighlight) {
  ...
  std::ifstream infile(std::string(ROOT_DIR) +
  ...
  if (coll_array_text == nullptr) {
    coll_array_text =
        collectionManager
            .create_collection("coll_array_text", 4, fields, "points")
            .get();
  }
  ...
  nlohmann::json results =
      coll_array_text
          ->search("truth about", query_fields, "", facets, sort_fields, {0},
                   10, 1, FREQUENCY, {false}, 0)
          .get();
  ASSERT_EQ(1, results["hits"].size());
  ...
  // an array's snippets must be sorted on match score, if match score is same,
  // priority to be given to lower indices
  ASSERT_EQ(3, results["hits"][0]["highlights"][0]["snippets"].size());
  ASSERT_STREQ("<mark>truth</mark> <mark>about</mark>",
               results["hits"][0]["highlights"][0]["snippets"][0]
                   .get<std::string>()
                   .c_str());
  ASSERT_STREQ("the <mark>truth</mark>",
               results["hits"][0]["highlights"][0]["snippets"][1]
                   .get<std::string>()
                   .c_str());
  ASSERT_STREQ("<mark>about</mark> forever",
               results["hits"][0]["highlights"][0]["snippets"][2]
                   .get<std::string>()
                   .c_str());
  ...
  results = coll_array_text
                ->search("forever truth", query_fields, "", facets, sort_fields,
  ...
  ASSERT_STREQ("the <mark>truth</mark>",
               results["hits"][0]["highlights"][0]["snippets"][0]
                   .get<std::string>()
                   .c_str());
  ASSERT_STREQ("about <mark>forever</mark>",
               results["hits"][0]["highlights"][0]["snippets"][1]
                   .get<std::string>()
                   .c_str());
  ASSERT_STREQ("<mark>truth</mark> about",
               results["hits"][0]["highlights"][0]["snippets"][2]
                   .get<std::string>()
                   .c_str());
  ASSERT_EQ(3, results["hits"][0]["highlights"][0]["indices"].size());
  ...
  results = coll_array_text
                ->search("truth", query_fields, "", facets, sort_fields, {0},
                         10, 1, FREQUENCY, {false}, 0)
                .get();
  ASSERT_EQ(2, results["hits"].size());
  ...
  results = coll_array_text
                ->search("asdadasd", query_fields, "", facets, sort_fields, {0},
                         10, 1, FREQUENCY, {false}, 0)
                .get();
  ASSERT_EQ(0, results["hits"].size());
  ...
  ASSERT_STREQ("<mark>truth</mark>",
               results["hits"][0]["highlights"][0]["snippets"][0]
                   .get<std::string>()
                   .c_str());
  ASSERT_STREQ("plain <mark>truth</mark>",
               results["hits"][0]["highlights"][0]["snippets"][1]
                   .get<std::string>()
                   .c_str());
  ASSERT_EQ(2, results["hits"][0]["highlights"][0]["matched_tokens"].size());
  ASSERT_STREQ("truth",
               results["hits"][0]["highlights"][0]["matched_tokens"][0][0]
                   .get<std::string>()
                   .c_str());
  ASSERT_STREQ("truth",
               results["hits"][0]["highlights"][0]["matched_tokens"][1][0]
                   .get<std::string>()
                   .c_str());
  ...
  ASSERT_STREQ(
      "title",
      results["hits"][0]["highlights"][1]["field"].get<std::string>().c_str());
  ASSERT_STREQ("Plain <mark>Truth</mark>",
               results["hits"][0]["highlights"][1]["snippet"]
                   .get<std::string>()
                   .c_str());
  ASSERT_EQ(1, results["hits"][0]["highlights"][1]["matched_tokens"].size());
  ASSERT_STREQ("Truth", results["hits"][0]["highlights"][1]["matched_tokens"][0]
                            .get<std::string>()
                            .c_str());
  ...
  ASSERT_STREQ(
      "title",
      results["hits"][1]["highlights"][0]["field"].get<std::string>().c_str());
  ASSERT_STREQ("The <mark>Truth</mark> About Forever",
               results["hits"][1]["highlights"][0]["snippet"]
                   .get<std::string>()
                   .c_str());
  ASSERT_EQ(1, results["hits"][1]["highlights"][0]["matched_tokens"].size());
  ASSERT_STREQ("Truth", results["hits"][1]["highlights"][0]["matched_tokens"][0]
                            .get<std::string>()
                            .c_str());

  ASSERT_EQ(4, results["hits"][1]["highlights"][1].size());
  ASSERT_STREQ(
      results["hits"][1]["highlights"][1]["field"].get<std::string>().c_str(),
      "tags");
  ASSERT_EQ(2, results["hits"][1]["highlights"][1]["snippets"].size());
  ASSERT_STREQ("the <mark>truth</mark>",
               results["hits"][1]["highlights"][1]["snippets"][0]
                   .get<std::string>()
                   .c_str());
  ASSERT_STREQ("<mark>truth</mark> about",
               results["hits"][1]["highlights"][1]["snippets"][1]
                   .get<std::string>()
                   .c_str());

  ASSERT_EQ(2, results["hits"][1]["highlights"][1]["matched_tokens"].size());
  ASSERT_STREQ("truth",
               results["hits"][1]["highlights"][1]["matched_tokens"][0][0]
                   .get<std::string>()
                   .c_str());
  ASSERT_STREQ("truth",
               results["hits"][1]["highlights"][1]["matched_tokens"][1][0]
                   .get<std::string>()
                   .c_str());
  ...
  results = coll_array_text
                ->search("amazing movie", query_fields, "", facets, sort_fields,
                         {0}, 10, 1, FREQUENCY, {false}, 0)
                .get();
  ASSERT_EQ(1, results["hits"].size());
  ASSERT_EQ(2, results["hits"][0]["highlights"].size());
  ...
  ASSERT_STREQ(
      "tags",
      results["hits"][0]["highlights"][0]["field"].get<std::string>().c_str());
  ASSERT_STREQ("<mark>amazing</mark> <mark>movie</mark>",
               results["hits"][0]["highlights"][0]["snippets"][0]
                   .get<std::string>()
                   .c_str());
  ...
  ASSERT_STREQ(
      ...
      "title");
  ASSERT_STREQ(
      results["hits"][0]["highlights"][1]["snippet"].get<std::string>().c_str(),
      "<mark>Amazing</mark> Spiderman is <mark>amazing</mark>");  // should
                                                                  // highlight
                                                                  // duplicating
                                                                  // tokens
  ...
  ASSERT_STREQ("Amazing",
               results["hits"][0]["highlights"][1]["matched_tokens"][0]
                   .get<std::string>()
                   .c_str());
  ASSERT_STREQ("amazing",
               results["hits"][0]["highlights"][1]["matched_tokens"][1]
                   .get<std::string>()
                   .c_str());

  // when query tokens are not found in an array field they should be ignored
  results = coll_array_text
                ->search("winds", query_fields, "", facets, sort_fields, {0},
                         10, 1, FREQUENCY, {false}, 0)
                .get();
  ASSERT_EQ(1, results["hits"].size());
...
TEST_F(CollectionTest, MultipleFields) {
  ...
  if (coll_mul_fields == nullptr) {
    coll_mul_fields =
        collectionManager
            .create_collection("coll_mul_fields", 4, fields, "points")
            .get();
  }
  ...
  nlohmann::json results =
      coll_mul_fields
          ->search("Will", query_fields, "", facets, sort_fields, {0}, 10, 1,
                   FREQUENCY, {false})
          .get();
  ASSERT_EQ(4, results["hits"].size());
  ...
  results =
      coll_mul_fields
          ->search("thomas", query_fields, "", facets, sort_fields, {0}, 10, 1,
                   FREQUENCY, {false}, 10, spp::sparse_hash_set<std::string>(),
  ...
  results = coll_mul_fields
                ->search("ben affleck", query_fields, "", facets, sort_fields,
                         {0}, 10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(1, results["hits"].size());

  query_fields = {"cast"};
  results = coll_mul_fields
                ->search("chris", query_fields, "", facets, sort_fields, {0},
                         10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(3, results["hits"].size());
  ...
  query_fields = {"cast"};
  results = coll_mul_fields
                ->search("chris pine", query_fields, "", facets, sort_fields,
                         {0}, 10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(1, results["hits"].size());
  ...
  query_fields = {"title"};
  results = coll_mul_fields
                ->search("captain", query_fields, "cast: chris", facets,
  ...
  // when a token exists in multiple fields of the same document, document and
  // facet should be returned only once
  query_fields = {"starring", "title", "cast"};
  facets = {"starring_facet"};

  results = coll_mul_fields
                ->search("myers", query_fields, "", facets, sort_fields, {0},
                         10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(1, results["hits"].size());
  ids = {"17"};
  ...
  ASSERT_STREQ(
      "starring_facet",
      results["facet_counts"][0]["field_name"].get<std::string>().c_str());
...
TEST_F(CollectionTest, KeywordQueryReturnsResultsBasedOnPerPageParam) {
  ...
  nlohmann::json results =
      coll_mul_fields
          ->search("w", query_fields, "", facets, sort_fields, {0}, 3, 1,
  ...
  auto res_op = coll_mul_fields->search("w", query_fields, "", facets,
                                        sort_fields, {0}, 251, 1, FREQUENCY,
                                        {true}, 1000, empty, empty, 10);
  ASSERT_FALSE(res_op.ok());
  ASSERT_EQ(422, res_op.code());
  ASSERT_STREQ("Only upto 250 hits can be fetched per page.",
               res_op.error().c_str());

  // when page number is zero, use the first page
  results = coll_mul_fields
                ->search("w", query_fields, "", facets, sort_fields, {0}, 3, 0,
                         FREQUENCY, {true}, 1000, empty, empty, 10)
                .get();
  ASSERT_EQ(3, results["hits"].size());
  ASSERT_EQ(6, results["found"].get<int>());
  ...
  results = coll_mul_fields
                ->search("w", query_fields, "", facets, sort_fields, {0}, 3, 2,
                         FREQUENCY, {true}, 1000, empty, empty, 10)
                .get();
...
std::vector<nlohmann::json> import_res_to_json(
    const std::vector<std::string> &imported_results) {
...
TEST_F(CollectionTest, ImportDocumentsUpsert) {
  ...
  if (coll_mul_fields == nullptr) {
    coll_mul_fields =
        collectionManager
            .create_collection("coll_mul_fields", 1, fields, "points")
            .get();
  }

  // try importing records
  nlohmann::json document;
  nlohmann::json import_response =
      coll_mul_fields->add_many(import_records, document);
  ...
  auto results =
      coll_mul_fields
          ->search("*", query_fields, "starring:= [Will Ferrell]", {"starring"},
                   sort_fields, {0}, 30, 1, FREQUENCY, {false})
          .get();
  ASSERT_EQ(2, results["hits"].size());

  // update existing record verbatim
  std::vector<std::string> existing_records = {
      R"({"id": "0", "title": "Wake Up, Ron Burgundy: The Lost Movie"})"};
  import_response =
      coll_mul_fields->add_many(existing_records, document, UPDATE);
  ASSERT_TRUE(import_response["success"].get<bool>());
  ASSERT_EQ(1, import_response["num_imported"].get<int>());

  // update + upsert records
  std::vector<std::string> more_records = {
      R"({"id": "0", "title": "The Fifth Harry", "starring": "Will Ferrell", "points":62, "cast":["Adam McKay","Steve Carell","Paul Rudd"]})",
  ...
  results = coll_mul_fields
                ->search("*", query_fields, "", {"starring"}, sort_fields, {0},
                         30, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(19, results["hits"].size());
  ASSERT_EQ(19, coll_mul_fields->get_num_documents());

  results = coll_mul_fields
                ->search("back again forest", query_fields, "", {"starring"},
                         sort_fields, {0}, 30, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(1, results["hits"].size());
  ...
  ASSERT_STREQ(
      "Back Again Forest",
      coll_mul_fields->get("18").get()["title"].get<std::string>().c_str());

  results = coll_mul_fields
                ->search("fifth", query_fields, "", {"starring"}, sort_fields,
                         {0}, 10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(2, results["hits"].size());

  ASSERT_STREQ("The <mark>Fifth</mark> Harry",
               results["hits"][0]["highlights"][0]["snippet"]
                   .get<std::string>()
                   .c_str());
  ASSERT_STREQ("The Woman in the <mark>Fifth</mark> from Kristin",
               results["hits"][1]["highlights"][0]["snippet"]
                   .get<std::string>()
                   .c_str());

  results = coll_mul_fields
                ->search("burgundy", query_fields, "", {}, sort_fields, {0}, 10,
                         1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(0, results["hits"].size());

  results = coll_mul_fields
                ->search("harry", query_fields, "", {}, sort_fields, {0}, 10, 1,
                         FREQUENCY, {false})
                .get();
  ASSERT_EQ(1, results["hits"].size());

  results = coll_mul_fields
                ->search("captain america", query_fields, "", {}, sort_fields,
                         {0}, 10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(1, results["hits"].size());
  ...
  more_records = {
      R"({"id": "1", "title": "Wake up, Harry", "cast":["Josh Lawson","Chris Parnell"],"points":63,"starring":"Will Ferrell"})",
      R"({"id": "90", "cast": ["Kim Werrel", "Random Wake"]})",  // missing
                                                                 // fields
  ...
  ASSERT_STREQ(
      "Field `points` has been declared as a default sorting field, but is not "
      "found in the document.",
      import_results[1]["error"].get<std::string>().c_str());
  ASSERT_STREQ(
      "Field `title` has been declared in the schema, but is not found in the "
      "document.",
  ...
  ASSERT_FALSE(import_results[1]["success"].get<bool>());
  ASSERT_STREQ("A document with id 1 already exists.",
               import_results[0]["error"].get<std::string>().c_str());
  ASSERT_STREQ("A document with id 5 already exists.",
               import_results[1]["error"].get<std::string>().c_str());

  // update document with verbatim fields, except for points
  more_records = {
      R"({"id": "3", "cast":["Matt Damon","Ben Affleck","Minnie Driver"],
  ...
  results = coll_mul_fields
                ->search("Good Will Hunting", query_fields, "", {"starring"},
                         sort_fields, {0}, 10, 1, FREQUENCY, {false})
                .get();
  ...
  ASSERT_STREQ("Could not find a document with id: 20",
               import_results[0]["error"].get<std::string>().c_str());
  ASSERT_EQ(404, import_results[0]["code"].get<size_t>());

  results = coll_mul_fields
                ->search("wake up harry", query_fields, "", {"starring"},
                         sort_fields, {0}, 10, 1, FREQUENCY, {false})
                .get();
  ...
  ASSERT_FALSE(import_results[0]["success"].get<bool>());
  ASSERT_FALSE(import_results[1]["success"].get<bool>());
  ASSERT_STREQ("A document with id 2 already exists.",
               import_results[0]["error"].get<std::string>().c_str());
  ASSERT_STREQ("A document with id 1 already exists.",
               import_results[1]["error"].get<std::string>().c_str());
...
TEST_F(CollectionTest, ImportDocumentsEmplace) {
  ...
  std::vector<std::string> records = {
      R"({"id": "0", "title": "The Matrix", "points":0})",
  ...
  auto import_response =
      coll1->add_many(records, document, EMPLACE, "",
                      DIRTY_VALUES::COERCE_OR_REJECT, true, true);
  ...
  res = coll1
            ->search("*", {}, "", {}, {}, {0}, 10, 1, token_ordering::FREQUENCY,
                     {true}, 10)
            .get();
  ...
  ASSERT_EQ("The Inception",
            res["hits"][1]["document"]["title"].get<std::string>());
  ...
  // can update individual document via "emplace" with only partial field
  // (missing points)
  ...
  ASSERT_EQ("The Superman",
            res["hits"][0]["document"]["title"].get<std::string>());

  // can create individual document via "emplace"
  std::string doc_4_create =
      R"({"id": "4", "title": "The Avengers", "points": 4})";
...
TEST_F(CollectionTest, DISABLED_CrashTroubleshooting) {
  ...
  if (coll1 == nullptr) {
    coll1 =
        collectionManager.create_collection("coll1", 4, fields, "points").get();
  }
...
TEST_F(CollectionTest, ImportDocuments) {
  ...
  auto x = coll_mul_fields->search("Will", query_fields, "", facets,
                                   sort_fields, {0}, 10, 1, FREQUENCY, {false});
  ...
  // verify that only bad records are rejected, rest must be imported (records 2
  // and 4 are bad)
  std::vector<std::string> more_records = {
      "{\"id\": \"id1\", \"title\": \"Test1\", \"starring\": \"Rand Fish\", "
      "\"points\": 12, "
      "\"cast\": [\"Tom Skerritt\"] }",
  ...
  ASSERT_STREQ("Field `title` must be a string.",
               import_results[1]["error"].get<std::string>().c_str());
  ASSERT_STREQ(
      "Field `starring` has been declared in the schema, but is not found in "
      "the document.",
      import_results[3]["error"].get<std::string>().c_str());
  ASSERT_STREQ(
      "{\"title\": 123, \"starring\": \"Jazz Gosh\", \"points\": 23, \"cast\": "
      "[\"Tom Skerritt\"] }",
  ...
  more_records = {
      "{\"id\": \"id2\", \"title\": \"Test1\", \"starring\": \"Rand Fish\", "
      "\"points\": 12, "
      "\"cast\": [\"Tom Skerritt\"] }",
      "{\"id\": \"id1\", \"title\": \"Test1\", \"starring\": \"Rand Fish\", "
      "\"points\": 12, "
  ...
  ASSERT_STREQ("A document with id id1 already exists.",
               import_results[1]["error"].get<std::string>().c_str());
  ASSERT_STREQ(
      "{\"id\": \"id1\", \"title\": \"Test1\", \"starring\": \"Rand Fish\", "
      "\"points\": 12, "
  ...
  ASSERT_STREQ("Bad JSON: not a properly formed document.",
               import_results[0]["error"].get<std::string>().c_str());
  ...
  ASSERT_EQ(false, import_results[0]["success"].get<bool>());
  ASSERT_STREQ(
      "Bad JSON: [json.exception.parse_error.101] parse error at line 1, "
      "column 2: syntax error "
      "while parsing object key - unexpected end of input; expected string "
      "literal",
      import_results[0]["error"].get<std::string>().c_str());
  ASSERT_STREQ("{", import_results[0]["document"].get<std::string>().c_str());
...
TEST_F(CollectionTest, SearchingWithMissingFields) {
  // return error without crashing when searching for fields that do not conform
  // to the schema
  Collection *coll_array_fields;

  std::ifstream infile(std::string(ROOT_DIR) +
                       "test/numeric_array_documents.jsonl");
  std::vector<field> fields = {
  ...
  coll_array_fields =
      collectionManager.get_collection("coll_array_fields").get();
  if (coll_array_fields == nullptr) {
    coll_array_fields =
        collectionManager
            .create_collection("coll_array_fields", 4, fields, "age")
            .get();
  }
  ...
  Option<nlohmann::json> res_op = coll_array_fields->search(
      "the", query_fields_not_found, "", facets, sort_fields, {0}, 10);
  ASSERT_FALSE(res_op.ok());
  ASSERT_EQ(404, res_op.code());
  ASSERT_STREQ("Could not find a field named `titlez` in the schema.",
               res_op.error().c_str());

  // when a query field is an integer field
  res_op = coll_array_fields->search("the", {"age"}, "", facets, sort_fields,
                                     {0}, 10);
  ASSERT_EQ(400, res_op.code());
  ASSERT_STREQ("Field `age` should be a string or a string array.",
               res_op.error().c_str());

  // when a facet field is not defined in the schema
  res_op = coll_array_fields->search("the", {"name"}, "", {"timestamps"},
                                     sort_fields, {0}, 10);
  ASSERT_EQ(404, res_op.code());
  ASSERT_STREQ("Could not find a facet field named `timestamps` in the schema.",
               res_op.error().c_str());

  // when a rank field is not defined in the schema
  res_op = coll_array_fields->search("the", {"name"}, "", {},
                                     {sort_by("timestamps", "ASC")}, {0}, 10);
  ASSERT_EQ(404, res_op.code());
  ASSERT_STREQ(
      "Could not find a field named `timestamps` in the schema for sorting.",
      res_op.error().c_str());

  res_op = coll_array_fields->search("the", {"name"}, "", {},
                                     {sort_by("_rank", "ASC")}, {0}, 10);
  ASSERT_EQ(404, res_op.code());
  ASSERT_STREQ(
      "Could not find a field named `_rank` in the schema for sorting.",
      res_op.error().c_str());
...
TEST_F(CollectionTest, IndexingWithBadData) {
  ...
  sample_collection =
      collectionManager.get_collection("sample_collection").get();
  if (sample_collection == nullptr) {
    sample_collection =
        collectionManager
            .create_collection("sample_collection", 4, fields, "age")
            .get();
  }

  const Option<nlohmann::json> &search_fields_missing_op1 =
      sample_collection->add(
          "{\"name\": \"foo\", \"age\": 29, \"average\": 78}");
  ASSERT_FALSE(search_fields_missing_op1.ok());
  ASSERT_STREQ(
      "Field `tags` has been declared in the schema, but is not found in the "
      "document.",
      search_fields_missing_op1.error().c_str());

  const Option<nlohmann::json> &search_fields_missing_op2 =
      sample_collection->add(
          "{\"namez\": \"foo\", \"tags\": [], \"age\": 34, \"average\": 78}");
  ASSERT_FALSE(search_fields_missing_op2.ok());
  ASSERT_STREQ(
      "Field `name` has been declared in the schema, but is not found in the "
      "document.",
      search_fields_missing_op2.error().c_str());

  const Option<nlohmann::json> &facet_fields_missing_op1 =
      sample_collection->add(
          "{\"name\": \"foo\", \"age\": 34, \"average\": 78}");
  ...
  const char *doc_str =
      "{\"name\": \"foo\", \"age\": 34, \"avg\": 78, \"tags\": [\"red\", "
      "\"blue\"]}";
  const Option<nlohmann::json> &sort_fields_missing_op1 =
      sample_collection->add(doc_str);
  ASSERT_FALSE(sort_fields_missing_op1.ok());
  ASSERT_STREQ(
      "Field `average` has been declared in the schema, but is not found in "
      "the document.",
  ...
  const Option<nlohmann::json> &bad_facet_field_op =
      sample_collection->add(doc_str);
  ASSERT_FALSE(bad_facet_field_op.ok());
  ASSERT_STREQ("Field `tags` must be an array.",
               bad_facet_field_op.error().c_str());

  doc_str =
      "{\"name\": \"foo\", \"age\": 34, \"tags\": [\"red\", 22], \"average\": "
      "78}";
  const Option<nlohmann::json> &bad_array_field_op =
      sample_collection->add(doc_str, CREATE, "", DIRTY_VALUES::REJECT);
  ASSERT_FALSE(bad_array_field_op.ok());
  ASSERT_STREQ("Field `tags` must be an array of string.",
               bad_array_field_op.error().c_str());

  // with coercion should work
  doc_str =
      "{\"name\": \"foo\", \"age\": 34, \"tags\": [\"red\", 22], \"average\": "
      "78}";
  const Option<nlohmann::json> &bad_array_field_coercion_op =
      sample_collection->add(doc_str, CREATE, "",
                             DIRTY_VALUES::COERCE_OR_REJECT);
  ASSERT_TRUE(bad_array_field_coercion_op.ok());

  doc_str = "{\"name\": \"foo\", \"age\": 34, \"tags\": [], \"average\": 34}";
  const Option<nlohmann::json> &empty_facet_field_op =
      sample_collection->add(doc_str);
  ASSERT_TRUE(empty_facet_field_op.ok());

  doc_str =
      "{\"name\": \"foo\", \"age\": [\"34\"], \"tags\": [], \"average\": 34 }";
  const Option<nlohmann::json> &bad_default_sorting_field_op1 =
      sample_collection->add(doc_str);
  ASSERT_FALSE(bad_default_sorting_field_op1.ok());
  ASSERT_STREQ("Field `age` must be an int32.",
               bad_default_sorting_field_op1.error().c_str());

  doc_str = "{\"name\": \"foo\", \"tags\": [], \"average\": 34 }";
  const Option<nlohmann::json> &bad_default_sorting_field_op3 =
      sample_collection->add(doc_str);
  ASSERT_FALSE(bad_default_sorting_field_op3.ok());
  ASSERT_STREQ(
      "Field `age` has been declared as a default sorting field, but is not "
      "found in the document.",
      bad_default_sorting_field_op3.error().c_str());

  doc_str =
      "{\"name\": \"foo\", \"age\": 34, \"tags\": [], \"average\": \"34\"}";
  const Option<nlohmann::json> &bad_rank_field_op =
      sample_collection->add(doc_str, CREATE, "", DIRTY_VALUES::REJECT);
  ASSERT_FALSE(bad_rank_field_op.ok());
  ASSERT_STREQ("Field `average` must be an int32.",
               bad_rank_field_op.error().c_str());

  doc_str =
      "{\"name\": \"foo\", \"age\": asdadasd, \"tags\": [], \"average\": 34 }";
  const Option<nlohmann::json> &bad_default_sorting_field_op4 =
      sample_collection->add(doc_str);
  ASSERT_FALSE(bad_default_sorting_field_op4.ok());
  ASSERT_STREQ(
      "Bad JSON: [json.exception.parse_error.101] parse error at line 1, "
      "column 24: syntax error "
  ...
  ASSERT_STREQ("A document with id 100 already exists.",
               add_op.error().c_str());
...
TEST_F(CollectionTest, EmptyIndexShouldNotCrash) {
  ...
  if (empty_coll == nullptr) {
    empty_coll =
        collectionManager.create_collection("empty_coll", 4, fields, "age")
            .get();
  }

  nlohmann::json results = empty_coll
                               ->search("a", {"name"}, "", {}, sort_fields, {0},
                                        10, 1, FREQUENCY, {false})
                               .get();
  ASSERT_EQ(0, results["hits"].size());
...
TEST_F(CollectionTest, IdFieldShouldBeAString) {
  ...
  if (coll1 == nullptr) {
    coll1 =
        collectionManager.create_collection("coll1", 4, fields, "age").get();
  }
  ...
  ASSERT_STREQ("Document's `id` field should be a string.",
               inserted_id_op.error().c_str());
...
TEST_F(CollectionTest, AnIntegerCanBePassedToAFloatField) {
  ...
  coll1 = collectionManager.get_collection("coll1").get();
  if (coll1 == nullptr) {
    coll1 = collectionManager.create_collection("coll1", 4, fields, "average")
                .get();
  }
...
TEST_F(CollectionTest, DeletionOfADocument) {
  ...
  collection_for_del =
      collectionManager.get_collection("collection_for_del").get();
  if (collection_for_del == nullptr) {
    collection_for_del =
        collectionManager
            .create_collection("collection_for_del", 4, search_fields, "points")
            .get();
  }
  ...
  results = collection_for_del
                ->search("cryogenic", query_fields, "", {}, sort_fields, {0}, 5,
                         1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(1, results["hits"].size());
  ...
  results = collection_for_del
                ->search("archives", query_fields, "", {}, sort_fields, {0}, 5,
                         1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(1, results["hits"].size());
  ...
  results = collection_for_del
                ->search("martian", query_fields, "", {}, sort_fields, {0}, 5,
                         1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(0, results["hits"].size());
  ASSERT_EQ(0, results["found"]);
...
TEST_F(CollectionTest, DeletionOfDocumentSingularFields) {
  ...
  if (coll1 == nullptr) {
    coll1 =
        collectionManager.create_collection("coll1", 4, fields, "int32").get();
  }
  ...
  nlohmann::json res = coll1
                           ->search("phone", {"str"}, "", {}, sort_fields, {0},
                                    10, 1, token_ordering::FREQUENCY, {true},
  ...
  res = coll1
            ->search("phone", {"str"}, "", {}, sort_fields, {0}, 10, 1,
                     token_ordering::FREQUENCY, {true}, 10,
                     spp::sparse_hash_set<std::string>(),
                     spp::sparse_hash_set<std::string>(), 10)
            .get();
  ...
  const Index *index =
      coll1->_get_index();  // seq id will always be zero for first document
...
TEST_F(CollectionTest, DeletionOfDocumentArrayFields) {
  ...
  std::vector<field> fields = {
      field("strarray", field_types::STRING_ARRAY, false),
  ...
  nlohmann::json doc;
  doc["id"] = "100";
  doc["strarray"] = {"Cell Phones", "Cell Phone Accessories",
  ...
  nlohmann::json res =
      coll1
          ->search("phone", {"strarray"}, "", {}, sort_fields, {0}, 10, 1,
                   token_ordering::FREQUENCY, {true}, 10,
                   spp::sparse_hash_set<std::string>(),
                   spp::sparse_hash_set<std::string>(), 10)
          .get();
...
TEST_F(CollectionTest, SearchLargeTextField) {
  ...
  std::vector<sort_by> sort_fields = {
      sort_by(sort_field_const::text_match, "DESC"), sort_by("age", "DESC")};

  coll_large_text = collectionManager.get_collection("coll_large_text").get();
  if (coll_large_text == nullptr) {
    coll_large_text =
        collectionManager.create_collection("coll_large_text", 4, fields, "age")
            .get();
  }
  ...
  Option<nlohmann::json> res_op = coll_large_text->search(
      "eguilazer", {"text"}, "", {}, sort_fields, {0}, 10);
  ASSERT_TRUE(res_op.ok());
  nlohmann::json results = res_op.get();
  ASSERT_EQ(1, results["hits"].size());

  res_op = coll_large_text->search("tristique", {"text"}, "", {}, sort_fields,
                                   {0}, 10);
  ASSERT_TRUE(res_op.ok());
  results = res_op.get();
  ASSERT_EQ(2, results["hits"].size());

  // query whose length exceeds maximum highlight window (match score's
  // WINDOW_SIZE)
  res_op = coll_large_text->search(
      "Phasellus non tristique elit Praesent non arcu id lectus accumsan "
      "venenatis at",
  ...
  ASSERT_STREQ("1",
               results["hits"][0]["document"]["id"].get<std::string>().c_str());

  // only single matched token in match window

  res_op = coll_large_text->search("molestie maecenas accumsan", {"text"}, "",
                                   {}, sort_fields, {0}, 10);
  ASSERT_TRUE(res_op.ok());
  results = res_op.get();
  ...
  ASSERT_STREQ(
      "non arcu id lectus <mark>accumsan</mark> venenatis at at justo.",
      results["hits"][0]["highlights"][0]["snippet"]
          .get<std::string>()
          .c_str());
...
TEST_F(CollectionTest, PruneFieldsFromDocument) {
  ...
  // when no inclusion is specified, should return all fields not mentioned by
  // exclusion list
  document = get_prune_doc();
  Collection::prune_doc(document, tsl::htrie_set<char>(),
                        tsl::htrie_set<char>({"three"}), "");
...
TEST_F(CollectionTest, StringArrayFieldShouldNotAllowPlainString) {
  ...
  std::vector<field> fields = {
      field("categories", field_types::STRING_ARRAY, true),
      field("points", field_types::INT32, false)};
...
TEST_F(CollectionTest, SearchHighlightShouldFollowThreshold) {
  ...
  doc["id"] = "100";
  doc["title"] =
      "The quick brown fox jumped over the lazy dog and ran straight to the "
      "forest to sleep.";
  doc["points"] = 25;
  ...
  auto res = coll1
                 ->search("lazy", {"title"}, "", {}, sort_fields, {0}, 10, 1,
                          token_ordering::FREQUENCY, {true}, 10,
                          spp::sparse_hash_set<std::string>(),
                          spp::sparse_hash_set<std::string>(), 10, "")
                 .get();
  ...
  ASSERT_STREQ(
      "The quick brown fox jumped over the <mark>lazy</mark> dog and ran "
      "straight to the forest to sleep.",
      res["hits"][0]["highlights"][0]["snippet"].get<std::string>().c_str());

  // now with with a small threshold (will show only 4 words either side of the
  // matched token)
  ...
  res = coll1
            ->search("lazy", {"title"}, "", {}, sort_fields, {0}, 10, 1,
                     token_ordering::FREQUENCY, {true}, 10,
                     spp::sparse_hash_set<std::string>(),
                     spp::sparse_hash_set<std::string>(), 10, "", 5)
            .get();

  ASSERT_STREQ(
      "fox jumped over the <mark>lazy</mark> dog and ran straight",
      res["hits"][0]["highlights"][0]["snippet"].get<std::string>().c_str());
  ...
  res = coll1
            ...
            .get();
  ASSERT_STREQ(
      "over the <mark>lazy</mark> dog and",
      res["hits"][0]["highlights"][0]["snippet"].get<std::string>().c_str());
...
TEST_F(CollectionTest, SearchHighlightShouldUseHighlightTags) {
  ...
  doc["id"] = "100";
  doc["title"] =
      "The quick brown  fox jumped over the  lazy fox. ";  // adding some extra
                                                           // spaces
  doc["points"] = 25;
  ...
  ASSERT_STREQ(
      "The quick brown  fox jumped over the  <em class=\"h\">lazy</em> fox. ",
      res["hits"][0]["highlights"][0]["snippet"].get<std::string>().c_str());
...
TEST_F(CollectionTest, SearchHighlightWithNewLine) {
  ...
  auto res = coll1
                 ->search("stark", {"title"}, "", {}, sort_fields, {0}, 10, 1,
                          token_ordering::FREQUENCY, {true}, 10,
                          spp::sparse_hash_set<std::string>(),
                          spp::sparse_hash_set<std::string>(), 10, "", 30, 4,
                          "", 40, {}, {}, {}, 0)
  ...
  ASSERT_STREQ("Stark", res["hits"][0]["highlights"][0]["matched_tokens"][0]
                            .get<std::string>()
                            .c_str());
...
TEST_F(CollectionTest, UpdateDocument) {
  ...
  if (coll1 == nullptr) {
    coll1 =
        collectionManager.create_collection("coll1", 1, fields, "points").get();
  }
  ...
  auto res =
      coll1
          ->search("lazy", {"title"}, "", {"tags"}, sort_fields, {0}, 10, 1,
                   token_ordering::FREQUENCY, {true}, 10,
                   spp::sparse_hash_set<std::string>(),
                   spp::sparse_hash_set<std::string>(), 10, "", 5, 5, "title")
          .get();
  ...
  ASSERT_STREQ("tags",
               res["facet_counts"][0]["field_name"].get<std::string>().c_str());
  ASSERT_EQ(2, res["facet_counts"][0]["counts"].size());

  ASSERT_STREQ(
      "LAZY",
      res["facet_counts"][0]["counts"][0]["value"].get<std::string>().c_str());
  ASSERT_EQ(1, (int)res["facet_counts"][0]["counts"][0]["count"]);

  ASSERT_STREQ(
      "NEWS",
      res["facet_counts"][0]["counts"][1]["value"].get<std::string>().c_str());
  ...
  res = coll1
            ->search("quick", {"title"}, "", {"title"}, sort_fields, {0}, 10, 1,
                     token_ordering::FREQUENCY, {true}, 10,
                     spp::sparse_hash_set<std::string>(),
                     spp::sparse_hash_set<std::string>(), 10, "", 5, 5, "title")
            .get();
  ...
  ASSERT_EQ(1, res["hits"].size());
  ASSERT_STREQ("The quick brown fox.",
               res["hits"][0]["document"]["title"].get<std::string>().c_str());
  ...
  ASSERT_FALSE(add_op.ok());
  ASSERT_STREQ("For update, the `id` key must be provided.",
               add_op.error().c_str());
  ...
  // check for old tag
  res = coll1
            ->search("NEWS", {"tags"}, "", {"tags"}, sort_fields, {0}, 10, 1,
                     token_ordering::FREQUENCY, {true}, 10,
                     spp::sparse_hash_set<std::string>(),
                     spp::sparse_hash_set<std::string>(), 10, "", 5, 5, "title")
            .get();
  ...
  res = coll1
            ->search("SENTENCE", {"tags"}, "", {"tags"}, sort_fields, {0}, 10,
                     1, token_ordering::FREQUENCY, {true}, 10,
                     spp::sparse_hash_set<std::string>(),
                     spp::sparse_hash_set<std::string>(), 10, "", 5, 5, "title")
            .get();
  ...
  res = coll1
            ->search("*", {"tags"}, "points: > 90", {"tags"}, sort_fields, {0},
                     10, 1, token_ordering::FREQUENCY, {true}, 10,
                     spp::sparse_hash_set<std::string>(),
                     spp::sparse_hash_set<std::string>(), 10, "", 5, 5, "title")
            .get();
  ...
  res = coll1
            ->search("*", {"tags"}, "points: > 101", {"tags"}, sort_fields, {0},
                     10, 1, token_ordering::FREQUENCY, {true}, 10,
                     spp::sparse_hash_set<std::string>(),
                     spp::sparse_hash_set<std::string>(), 10, "", 5, 5, "title")
            .get();
  ...
  // try to change a field with bad value and verify that old document is put
  // back
  doc4["points"] = "abc";
  add_op = coll1->add(doc4.dump(), UPDATE, "100");
  ASSERT_FALSE(add_op.ok());
...
TEST_F(CollectionTest, UpdateDocuments) {
  ...
  Collection *update_docs_collection =
      collectionManager.get_collection("update_docs_collection").get();
  ...
  auto res = update_docs_collection
                 ->search("cat data", {"content"}, "", {}, sort_fields, {0}, 10)
                 .get();
  ASSERT_EQ(2, res["hits"].size());
  for (size_t i = 0; i < res["hits"].size(); i++) {
    ASSERT_EQ("fat_cat",
              res["hits"][i]["document"]["user_name"].get<std::string>());
  }
  ...
  auto update_op = update_docs_collection->update_matching_filter(
      "user_name:=fat_cat", document.dump(), dirty_values);
  ASSERT_TRUE(update_op.ok());
  ASSERT_EQ(2, update_op.get()["num_updated"]);

  res = update_docs_collection
            ->search("cat data", {"content"}, "", {}, sort_fields, {0}, 10)
            .get();
  ASSERT_EQ(2, res["hits"].size());
  for (size_t i = 0; i < res["hits"].size(); i++) {
  ...
  // Test batching
  res = update_docs_collection
            ->search("dog data", {"content"}, "", {}, sort_fields, {0}, 10)
            .get();
  ...
  update_op = update_docs_collection->update_matching_filter(
      "user_name:=fast_dog", document.dump(), dirty_values, 2);
  ASSERT_TRUE(update_op.ok());
  ...
  res = update_docs_collection
            ->search("*", {}, "user_name:=slim_cat", {}, sort_fields, {0}, 10)
            .get();
  ASSERT_EQ(2, res["hits"].size());
  for (size_t i = 0; i < res["hits"].size(); i++) {
    ASSERT_EQ(
        "cat data " + std::to_string(i + 1),
        res["hits"][i]["document"]["content"]["title"].get<std::string>());
  }
  ...
  update_op = update_docs_collection->update_matching_filter(
      "user_name:=slim_cat", document.dump(), dirty_values, 2);
  ASSERT_TRUE(update_op.ok());
  ASSERT_EQ(2, update_op.get()["num_updated"]);
  ...
  res = update_docs_collection->search("*", {}, "", {}, sort_fields, {0}, 10)
            .get();
...
TEST_F(CollectionTest, UpdateDocumentSorting) {
  ...
  auto res =
      coll1
          ->search("*", {"tags"}, "", {"tags"}, sort_fields, {0}, 10, 1,
                   token_ordering::FREQUENCY, {true}, 10,
                   spp::sparse_hash_set<std::string>(),
                   spp::sparse_hash_set<std::string>(), 10, "", 5, 5, "title")
          .get();
  ...
  ASSERT_STREQ("101",
               res["hits"][0]["document"]["id"].get<std::string>().c_str());

  ASSERT_EQ(100, res["hits"][1]["document"]["points"].get<size_t>());
  ASSERT_STREQ("100",
               res["hits"][1]["document"]["id"].get<std::string>().c_str());
...
TEST_F(CollectionTest, UpdateDocumentUnIndexedField) {
  ...
  ASSERT_EQ(1, res["hits"].size());
  ASSERT_STREQ("foo2",
               res["hits"][0]["document"]["foo"].get<std::string>().c_str());
...
TEST_F(CollectionTest, SearchHighlightFieldFully) {
  ...
  res = coll1
            ->search("lazy", {"title"}, "", {}, sort_fields, {0}, 10, 1,
                     token_ordering::FREQUENCY, {true}, 10,
                     spp::sparse_hash_set<std::string>(),
                     spp::sparse_hash_set<std::string>(), 10, "", 5, 5, "")
            .get();
  ...
  ASSERT_EQ("<mark>LAZY</mark>",
            res["hits"][0]["highlights"][0]["values"][0].get<std::string>());
  ASSERT_EQ(1, res["hits"][0]["highlights"][0]["snippets"].size());
  ASSERT_EQ("<mark>LAZY</mark>",
            res["hits"][0]["highlights"][0]["snippets"][0].get<std::string>());
  ...
  ASSERT_STREQ("lazy", res["hits"][0]["highlights"][1]["matched_tokens"][0]
                           .get<std::string>()
                           .c_str());
  ...
  res = coll1
            ->search("lazy", {"title", "tags"}, "", {}, sort_fields, {0}, 10, 1,
                     token_ordering::FREQUENCY, {true}, 10,
                     spp::sparse_hash_set<std::string>(), excluded_fields, 10,
                     "", 5, 5, "title, tags")
            .get();
...
TEST_F(CollectionTest, OptionalFields) {
  ...
  if (coll1 == nullptr) {
    coll1 =
        collectionManager.create_collection("coll1", 4, fields, "max").get();
  }
  ...
  auto res =
      coll1->search("*", {"title"}, "", {}, {}, {0}, 10, 1, FREQUENCY, {false})
          .get();
  ASSERT_EQ(6, res["found"].get<size_t>());

  // search on optional `description` field
  res = coll1
            ->search("book", {"description"}, "", {}, {}, {0}, 10, 1, FREQUENCY,
                     {false})
            .get();
  ASSERT_EQ(5, res["found"].get<size_t>());
  ...
  res = coll1
            ->search("the", {"title"}, "average: >0", {}, {}, {0}, 10, 1,
                     FREQUENCY, {false})
            .get();
  ASSERT_EQ(5, res["found"].get<size_t>());
  ...
  res = coll1
            ->search("the", {"title"}, "", {"description"}, {}, {0}, 10, 1,
                     FREQUENCY, {false})
            .get();
  ASSERT_EQ(6, res["found"].get<size_t>());
  ASSERT_EQ(5, res["facet_counts"][0]["counts"][0]["count"].get<size_t>());
  ASSERT_STREQ("description",
               res["facet_counts"][0]["field_name"].get<std::string>().c_str());

  // sort_by optional `average` field should be allowed (default used for
  // missing values)
  std::vector<sort_by> sort_fields = {sort_by("average", "DESC")};
  ...
  ASSERT_EQ(6, res["found"].get<size_t>());
  ASSERT_EQ(0, res["hits"][5]["document"].count(
                   "average"));  // record with missing average is last
  ...
  ASSERT_STREQ("title",
               coll_summary["fields"][0]["name"].get<std::string>().c_str());
  ASSERT_STREQ("string",
               coll_summary["fields"][0]["type"].get<std::string>().c_str());
  ASSERT_FALSE(coll_summary["fields"][0]["facet"].get<bool>());
  ASSERT_FALSE(coll_summary["fields"][0]["optional"].get<bool>());

  ASSERT_STREQ("description",
               coll_summary["fields"][1]["name"].get<std::string>().c_str());
  ASSERT_STREQ("string",
               coll_summary["fields"][1]["type"].get<std::string>().c_str());
  ...
  auto create_op =
      collectionManager.create_collection("coll2", 4, fields, "score");

  ASSERT_FALSE(create_op.ok());
  ASSERT_STREQ("Default sorting field `score` cannot be an optional field.",
...
TEST_F(CollectionTest, EmptyStringNotIndexed) {
  ...
  ASSERT_EQ(0, coll1->_get_index()->_get_search_index().at("artist")->size);
  ASSERT_EQ(0,
            coll1->_get_index()->_get_search_index().at("launch_year")->size);
  ASSERT_EQ(0, coll1->_get_index()->_get_search_index().at("genres")->size);
...
TEST_F(CollectionTest, WildcardQueryReturnsResultsBasedOnPerPageParam) {
  ...
  nlohmann::json results =
      collection
          ->search("*", query_fields, "", facets, sort_fields, {0}, 12, 1,
  ...
  results = collection
                ->search("*", query_fields, "", facets, sort_fields, {0}, 100,
                         1, FREQUENCY, {false}, 1000, empty, empty, 10)
                .get();
  ...
  // cannot fetch more than in-built limit of 250
  auto res_op =
      collection->search("*", query_fields, "", facets, sort_fields, {0}, 251,
                         1, FREQUENCY, {false}, 1000, empty, empty, 10);
  ASSERT_FALSE(res_op.ok());
  ASSERT_EQ(422, res_op.code());
  ASSERT_STREQ("Only upto 250 hits can be fetched per page.",
  ...
  results = collection
                ->search("*", query_fields, "", facets, sort_fields, {0}, 10, 0,
                         FREQUENCY, {false}, 1000, empty, empty, 10)
                .get();
  ...
  results = collection
                ->search("*", query_fields, "", facets, sort_fields, {0}, 10, 1,
                         FREQUENCY, {false}, 1000, empty, empty, 10)
                .get();
  ...
  results = collection
                ->search("*", query_fields, "", facets, sort_fields, {0}, 10, 2,
                         FREQUENCY, {false}, 1000, empty, empty, 10)
                .get();
  ...
  results = collection
                ->search("*", query_fields, "", facets, sort_fields, {0}, 10, 3,
                         FREQUENCY, {false}, 1000, empty, empty, 10)
                .get();
  ...
  // enforce limit_hits
  res_op = collection->search(
      "*", query_fields, "", facets, sort_fields, {0}, 10, 3, FREQUENCY,
      {false}, 1000, spp::sparse_hash_set<std::string>(),
      spp::sparse_hash_set<std::string>(), 10, "", 30, 4, "", 40, {}, {}, {}, 0,
      "<mark>", "</mark>", {1}, 20);

  ASSERT_FALSE(res_op.ok());
  ASSERT_STREQ(
      "Only upto 20 hits can be fetched. Ensure that `page` and `per_page` "
...
TEST_F(CollectionTest, RemoveIfFound) {
  ...
  auto res = coll1
                 ->search("*", {"title"}, "", {}, sort_fields, {0}, 10, 1,
                          token_ordering::FREQUENCY, {true}, 10,
                          spp::sparse_hash_set<std::string>(),
                          spp::sparse_hash_set<std::string>(), 10, "", 30, 4,
                          "", 40, {}, {}, {}, 0)
...
TEST_F(CollectionTest, CreateCollectionInvalidFieldType) {
  ...
  auto create_op =
      collectionManager.create_collection("coll1", 4, fields, "points");

  ASSERT_FALSE(create_op.ok());
  ASSERT_STREQ(
      "Field `title` has an invalid data type `blah`, see docs for supported "
      "data types.",
      create_op.error().c_str());
...
TEST_F(CollectionTest, MultiFieldRelevance) {
  ...
  auto results = coll1
                     ->search("Dustin Kensrue Down There by the Train",
                              {"title", "artist"}, "", {}, {}, {0}, 10, 1,
                              FREQUENCY, {true}, 10)
                     .get();
  ...
  for (size_t i = 0; i < expected_ids.size(); i++) {
    ASSERT_EQ(
        expected_ids[i],
        std::stoi(results["hits"][i]["document"]["id"].get<std::string>()));
  }

  ASSERT_STREQ(
      "<mark>Down</mark> <mark>There</mark> <mark>by</mark> <mark>the</mark> "
      "<mark>Train</mark>",
      results["hits"][0]["highlights"][0]["snippet"]
          .get<std::string>()
          .c_str());
  ...
  ASSERT_STREQ("<mark>Dustin</mark> <mark>Kensrue</mark>",
               results["hits"][2]["highlights"][0]["snippet"]
                   .get<std::string>()
                   .c_str());
  ...
  results = coll1
                ->search("-downie dustin kensrue down there by the train",
                         {"title", "artist"}, "", {}, {}, {0}, 10, 1, FREQUENCY,
                         {true}, 10)
                .get();
...
TEST_F(CollectionTest, MultiFieldRelevance2) {
  ...
  auto results = coll1
                     ->search("on a jetplane", {"title", "artist"}, "", {}, {},
                              {0}, 10, 1, FREQUENCY, {true}, 10)
                     .get();
  ...
  ASSERT_STREQ("1",
               results["hits"][0]["document"]["id"].get<std::string>().c_str());
  ASSERT_STREQ("0",
               results["hits"][1]["document"]["id"].get<std::string>().c_str());

  // changing weights to favor artist still favors title because it contains all
  // tokens of the query

  results =
      coll1
          ->search("on a jetplane", {"title", "artist"}, "", {}, {}, {0}, 10, 1,
                   FREQUENCY, {true}, 10, spp::sparse_hash_set<std::string>(),
                   spp::sparse_hash_set<std::string>(), 10, "", 30, 4, "", 40,
                   {}, {}, {}, 0, "<mark>", "</mark>", {1, 4})
          .get();
  ...
  // add weights to favor artist without all tokens in a query being found in a
  // field

  results = coll1
                ->search("on a helicopter", {"title", "artist"}, "", {}, {},
                         {0}, 10, 1, FREQUENCY, {true}, 10,
                         spp::sparse_hash_set<std::string>(),
                         spp::sparse_hash_set<std::string>(), 10, "", 30, 4, "",
                         40, {}, {}, {}, 0, "<mark>", "</mark>", {1, 4})
...
TEST_F(CollectionTest, FieldWeightsNotProper) {
  ...
  auto results_op = coll1->search(
      "on a jetplane", {"title", "artist"}, "", {}, {}, {0}, 10, 1, FREQUENCY,
  ...
  results_op = coll1->search(
      "on a jetplane", {"title"}, "", {}, {}, {0}, 10, 1, FREQUENCY, {true}, 10,
      spp::sparse_hash_set<std::string>(), spp::sparse_hash_set<std::string>(),
      10, "", 30, 4, "", 40, {}, {}, {}, 0, "<mark>", "</mark>", {2, 1});
...
TEST_F(CollectionTest, MultiFieldRelevance3) {
  ...
  auto results =
      coll1
          ->search("style taylor swift", {"title", "artist"}, "", {}, {}, {0},
                   10, 1, FREQUENCY, {true}, 10,
                   spp::sparse_hash_set<std::string>(),
                   spp::sparse_hash_set<std::string>(), 10, "", 30, 4, "", 40,
                   {}, {}, {}, 0, "<mark>", "</mark>", {1, 1})
          .get();
  ...
  results =
      coll1
          ->search("swift", {"title", "artist"}, "", {}, {}, {0}, 10, 1,
                   FREQUENCY, {true}, 10, spp::sparse_hash_set<std::string>(),
                   spp::sparse_hash_set<std::string>(), 10, "", 30, 4, "", 40,
                   {}, {}, {}, 0, "<mark>", "</mark>", {1, 1})
          .get();
...
TEST_F(CollectionTest, MultiFieldRelevance4) {
  ...
  auto results =
      coll1
          ->search("maddras", {"title", "artist"}, "", {}, {}, {2}, 10, 1,
                   FREQUENCY, {true}, 10, spp::sparse_hash_set<std::string>(),
                   spp::sparse_hash_set<std::string>(), 10, "", 30, 4, "", 40,
                   {}, {}, {}, 0, "<mark>", "</mark>", {1, 1})
          .get();
...
TEST_F(CollectionTest, MultiFieldRelevance5) {
  ...
  if (coll1 == nullptr) {
    coll1 =
        collectionManager.create_collection("coll1", 1, fields, "num_employees")
            .get();
  }
  ...
  auto results =
      coll1
          ->search("Canada", {"company_name", "country", "field_a"}, "", {}, {},
                   {2}, 10, 1, FREQUENCY, {true}, 10,
                   spp::sparse_hash_set<std::string>(),
                   spp::sparse_hash_set<std::string>(), 10, "", 30, 4, "", 40,
                   {}, {}, {}, 0, "<mark>", "</mark>", {1, 1, 1})
          .get();
  ...
  ASSERT_STREQ("2",
               results["hits"][1]["document"]["id"].get<std::string>().c_str());
  ASSERT_STREQ("1",
               results["hits"][2]["document"]["id"].get<std::string>().c_str());

  results = coll1
                ->search("Canada", {"company_name", "field_a", "country"}, "",
                         {}, {}, {2}, 10, 1, FREQUENCY, {true}, 10,
                         spp::sparse_hash_set<std::string>(),
                         spp::sparse_hash_set<std::string>(), 10, "", 30, 4, "",
                         40, {}, {}, {}, 0, "<mark>", "</mark>", {1, 1, 1})
                .get();
  ...
  ASSERT_EQ(2, results["hits"][0]["highlights"].size());
  ASSERT_EQ("field_a",
            results["hits"][0]["highlights"][0]["field"].get<std::string>());
  ASSERT_EQ("<mark>Canadia</mark>",
            results["hits"][0]["highlights"][0]["snippet"].get<std::string>());
  ASSERT_EQ("country",
            results["hits"][0]["highlights"][1]["field"].get<std::string>());
  ASSERT_EQ("<mark>Canada</mark>",
            results["hits"][0]["highlights"][1]["snippet"].get<std::string>());

  ASSERT_EQ(1, results["hits"][1]["highlights"].size());
  ASSERT_EQ("field_a",
            results["hits"][1]["highlights"][0]["field"].get<std::string>());
  ASSERT_EQ("<mark>Canadoo</mark>",
            results["hits"][1]["highlights"][0]["snippet"].get<std::string>());

  ASSERT_EQ(2, results["hits"][2]["highlights"].size());
  ASSERT_EQ("field_a",
            results["hits"][2]["highlights"][0]["field"].get<std::string>());
  ASSERT_EQ("<mark>Canadoo</mark>",
            results["hits"][2]["highlights"][0]["snippet"].get<std::string>());
  ASSERT_EQ("company_name",
            results["hits"][2]["highlights"][1]["field"].get<std::string>());
  ASSERT_EQ("<mark>Canaida</mark> Corp",
            results["hits"][2]["highlights"][1]["snippet"].get<std::string>());

  collectionManager.drop_collection("coll1");
}

TEST_F(CollectionTest, MultiFieldRelevance6) {
  // with exact match, the number of fields with exact match will not be
  // considered as a ranking signal
  Collection *coll1;
  ...
  auto results =
      coll1
          ->search("taylor swift", {"title", "artist"}, "", {}, {}, {2}, 10, 1,
                   FREQUENCY, {true}, 10, spp::sparse_hash_set<std::string>(),
                   spp::sparse_hash_set<std::string>(), 10, "", 30, 4, "", 40,
                   {}, {}, {}, 0, "<mark>", "</mark>", {1, 1})
          .get();
...
TEST_F(CollectionTest, ExactMatch) {
  ...
  auto results = coll1
                     ->search("alpha beta", {"title"}, "", {}, {}, {2}, 10, 1,
                              FREQUENCY, {true}, 10)
                     .get();
  ...
  results = coll1
                ->search("alpha", {"title"}, "", {}, {}, {2}, 10, 1, FREQUENCY,
                         {true}, 10)
                .get();
...
TEST_F(CollectionTest, MultiFieldHighlighting) {
  ...
  std::vector<std::vector<std::string>> records = {
      ...
      {"Annie's Song", "John Denver", "Album > Compilation"},
  };
  ...
  auto results =
      coll1
          ->search("charger", {"name", "description", "categories"}, "", {}, {},
                   {2}, 10, 1, FREQUENCY, {true}, 10,
                   spp::sparse_hash_set<std::string>(),
                   spp::sparse_hash_set<std::string>(), 10, "", 30, 4, "", 40,
                   {}, {}, {}, 0, "<mark>", "</mark>", {1, 1, 1})
  ...
  ASSERT_EQ("description",
            results["hits"][0]["highlights"][1]["field"].get<std::string>());
  ASSERT_EQ(
      "Easily replenish your cell phone with this wireless "
      "<mark>charger</mark>.",
      results["hits"][0]["highlights"][1]["snippet"].get<std::string>());

  ASSERT_EQ("categories",
            results["hits"][0]["highlights"][2]["field"].get<std::string>());
  ASSERT_EQ(
      "Car <mark>Charger</mark>s",
      results["hits"][0]["highlights"][2]["snippets"][0].get<std::string>());

  results =
      coll1
          ->search("John With Denver", {"description"}, "", {}, {}, {0}, 10, 1,
  ...
  results = coll1
                ->search("Annies song John Denver", {"name", "description"}, "",
                         {}, {}, {0}, 10, 1, FREQUENCY, {true}, 1,
                         spp::sparse_hash_set<std::string>(),
                         spp::sparse_hash_set<std::string>(), 10, "", 30, 4, "",
                         40, {}, {}, {}, 0, "<mark>", "</mark>", {1, 1})
                .get();
...
TEST_F(CollectionTest, MultiFieldMatchRanking) {
  ...
  auto results = coll1
                     ->search("taylor swift style", {"artist", "title"}, "", {},
                              {}, {0}, 3, 1, FREQUENCY, {true}, 5)
                     .get();
...
TEST_F(CollectionTest, MultiFieldMatchRankingOnArray) {
  ...
  std::vector<std::vector<std::vector<std::string>>> records = {
      ...
      {{"Jack Dan"},
       {"Golang", "Phoenix", "React"},
       {"Docker", "Vue", "Kubernetes"}},
  };
  ...
  auto results = coll1
                     ->search("golang vue", {"strong_skills", "skills"}, "", {},
                              {}, {0}, 3, 1, FREQUENCY, {true}, 1)
                     .get();
...
TEST_F(CollectionTest, MultiFieldMatchRankingOnFieldOrder) {
  ...
  auto results =
      coll1
          ->search("michael jackson toxic", {"title", "artist"}, "", {}, {},
                   {0}, 3, 1, FREQUENCY, {true}, 5,
                   spp::sparse_hash_set<std::string>(),
                   spp::sparse_hash_set<std::string>(), 10, "", 30, 4, "", 10,
                   {}, {}, {}, 0, "<mark>", "</mark>", {1, 6})
          .get();
...
TEST_F(CollectionTest, HighlightWithAccentedCharacters) {
  ...
  auto results =
      coll1->search("à jour", {"title"}, "", {}, {}, {0}, 10, 1, FREQUENCY)
          .get();
  ...
  ASSERT_EQ(2, results["hits"][0]["highlights"][0]["matched_tokens"].size());
  ASSERT_STREQ("à", results["hits"][0]["highlights"][0]["matched_tokens"][0]
                        .get<std::string>()
                        .c_str());
  ASSERT_STREQ("jour", results["hits"][0]["highlights"][0]["matched_tokens"][1]
                           .get<std::string>()
                           .c_str());

  results =
      coll1
          ->search("by train", {"title"}, "", {}, {}, {0}, 10, 1, FREQUENCY,
                   {true}, 10, spp::sparse_hash_set<std::string>(),
                   spp::sparse_hash_set<std::string>(), 10, "", 30, 4, "title")
          .get();
  ...
  ASSERT_STREQ("Down There <mark>by</mark> the <mark>T.r.a.i.n</mark>",
               results["hits"][0]["highlights"][0]["snippet"]
                   ...
                   .c_str());
  ASSERT_STREQ(
      "Down There <mark>by</mark> the <mark>T.r.a.i.n</mark>",
      results["hits"][0]["highlights"][0]["value"].get<std::string>().c_str());

  results = coll1
                ->search("state trooper", {"title"}, "", {}, {}, {0}, 10, 1,
                         FREQUENCY)
                .get();
  ...
  results =
      coll1->search("q", {"title"}, "", {}, {}, {0}, 10, 1, FREQUENCY).get();
  ASSERT_EQ(1, results["found"].get<size_t>());
...
TEST_F(CollectionTest, DISABLED_SearchingForRecordsWithSpecialChars) {
  ...
  ASSERT_EQ(2, results["hits"][0]["highlights"].size());
  ASSERT_EQ("<mark>Google</mark> Home",
            results["hits"][0]["highlights"][0]["snippet"].get<std::string>());
  ASSERT_EQ("https://<mark>google</mark>.com///",
            results["hits"][0]["highlights"][1]["snippet"].get<std::string>());

  results = coll1
                ->search("amazon.com", {"title", "url"}, "", {}, {}, {2}, 10, 1,
                         FREQUENCY)
                .get();
  ...
  results = coll1
                ->search("typesense", {"title", "url"}, "", {}, {}, {2}, 10, 1,
                         FREQUENCY)
                .get();
  ...
  results = coll1
                ->search("nb_sb_noss_2", {"title", "url"}, "", {}, {}, {2}, 10,
                         1, FREQUENCY)
                .get();
  ...
  ASSERT_EQ(1, results["hits"][0]["highlights"].size());
  ASSERT_EQ(
      "https://www.amazon.com/s?k=phone&ref=<mark>nb</mark>_<mark>sb</"
      "mark>_<mark>noss</mark>_<mark>2</mark>",
      results["hits"][0]["highlights"][0]["snippet"].get<std::string>());
...
TEST_F(CollectionTest, FieldSpecificNumTypos) {
  ...
  auto results =
      coll1
          ->search("tayylor", {"title", "artist"}, "", {}, {}, {1, 1}, 10, 1,
                   FREQUENCY, {true}, 10, spp::sparse_hash_set<std::string>(),
                   spp::sparse_hash_set<std::string>(), 10, "", 30, 4, "", 40,
                   {}, {}, {}, 0, "<mark>", "</mark>", {1, 1})
          .get();
  ...
  results =
      coll1
          ->search("tayylor", {"title", "artist"}, "", {}, {}, {0, 1}, 10, 1,
                   FREQUENCY, {true}, 10, spp::sparse_hash_set<std::string>(),
                   spp::sparse_hash_set<std::string>(), 10, "", 30, 4, "", 40,
                   {}, {}, {}, 0, "<mark>", "</mark>", {1, 1})
          .get();
  ...
  // must return error when num_typos does not match length of search fields
  // queried
  auto res_op = coll1->search(
      "tayylor", {"title"}, "", {}, {}, {0, 1}, 10, 1, FREQUENCY, {true}, 10,
      spp::sparse_hash_set<std::string>(), spp::sparse_hash_set<std::string>(),
      10, "", 30, 4, "", 40, {}, {}, {}, 0, "<mark>", "</mark>", {1, 1});

  ASSERT_FALSE(res_op.ok());
  ASSERT_EQ(
      "Number of weights in `query_by_weights` does not match number of "
      "`query_by` fields.",
      res_op.error());
  ...
  results =
      coll1
          ->search("tayylor", {"title", "artist"}, "", {}, {}, {1}, 10, 1,
                   FREQUENCY, {true}, 10, spp::sparse_hash_set<std::string>(),
                   spp::sparse_hash_set<std::string>(), 10, "", 30, 4, "", 40,
                   {}, {}, {}, 0, "<mark>", "</mark>", {1, 1})
          .get();
  ...
  results = coll1
                ->search("*", {}, "", {}, {}, {1}, 10, 1, FREQUENCY, {true}, 10,
                         spp::sparse_hash_set<std::string>(),
                         spp::sparse_hash_set<std::string>(), 10, "", 30, 4, "",
                         40, {}, {}, {}, 0, "<mark>", "</mark>", {1, 1})
                .get();
...
TEST_F(CollectionTest, BadHighlightingOnText) {
  ...
  doc["id"] = "0";
  doc["text"] =
      "include destruction of natural marine and estuarine\\nhabitats, loss of "
      "productive agricultural "
      "land,\\nand soil erosion. 90 When interviewed, multiple\\nexperts "
      "stated that inappropriate land use "
      "and\\nmanagement is a central factor contributing to\\nenvironmental "
      "degradation in the "
      "Castries-Gros\\nIslet Corridor. 91 The construction is placing "
      "greater\\nstress on natural resources "
      "and biodiversity, and\\nthe capacity to produce food and retain "
      "freshwater\\nhas been diminished. "
      "92 Moreover, increased\\nwater consumption by the tourism sector, "
      "when\\ncompounded by climate "
      "change, is increasing food\\nand water insecurity throughout Saint "
      "Lucia, as well\\nas suppressing "
  ...
  auto results = coll1
                     ->search("natural saint lucia", {"text"}, "", {}, {}, {1},
                              10, 1, FREQUENCY, {true}, 10)
                     .get();
  ...
  ASSERT_STREQ(
      "food\\nand water insecurity throughout <mark>Saint</mark> "
      "<mark>Lucia</mark>, as well\\nas suppressing long-term",
      results["hits"][0]["highlights"][0]["snippet"]
          .get<std::string>()
          .c_str());

  ASSERT_EQ(2, results["hits"][0]["highlights"][0]["matched_tokens"].size());
  ASSERT_STREQ("Saint", results["hits"][0]["highlights"][0]["matched_tokens"][0]
                            .get<std::string>()
                            .c_str());
  ASSERT_STREQ("Lucia", results["hits"][0]["highlights"][0]["matched_tokens"][1]
                            .get<std::string>()
                            .c_str());
...
TEST_F(CollectionTest, FieldLevelPrefixConfiguration) {
  ...
  auto results =
      coll1
          ->search("taylo", {"title", "artist"}, "", {}, {}, {0}, 10, 1,
                   FREQUENCY, {true, false}, 10,
                   spp::sparse_hash_set<std::string>(),
                   spp::sparse_hash_set<std::string>(), 10, "", 30, 4, "", 40,
                   {}, {}, {}, 0, "<mark>", "</mark>", {1, 1})
          .get();
...
TEST_F(CollectionTest, QueryParsingForPhraseSearch) {
  ...
  /*coll1->parse_search_query(q, q_include_tokens, q_exclude_tokens, q_phrases,
  "en", false);
...
TEST_F(CollectionTest, WildcardQueryBy) {
  ...
  ASSERT_EQ("Hi! I'm <mark>user_a</mark>",
            result["hits"][0]["highlight"]["user"]["bio"]["snippet"]
                .get<std::string>());
  ASSERT_EQ(
      "<mark>user_a</mark>",
  ...
  ASSERT_EQ(
      "No string or string array field found matching the pattern `foo*` in "
      "the schema.",
      error);
}
...
TEST_F(CollectionTest, WildcardHighlightFields) {
  ...
  auto add_op = coll->add(
      R"({"id": "124","user_name": "user_a","user": {"rank": 100,"phone": "+91 123123123"}})");
  if (!add_op.ok()) {
    LOG(INFO) << add_op.error();
  }
  ASSERT_TRUE(add_op.ok());
  ...
  auto result = coll->search("123", {"user"}, "", {}, {}, {0}, 10, 1, FREQUENCY,
                             {true}, Index::DROP_TOKENS_THRESHOLD,
                             dummy_include_exclude, dummy_include_exclude, 10,
                             "", 30, 4, "", Index::TYPO_TOKENS_THRESHOLD, "",
                             "", {}, 3, "<mark>", "</mark>", {}, UINT32_MAX,
                             true, false, true, highlight_fields)
                    .get();

  ASSERT_EQ(1, result["found"].get<size_t>());
  ASSERT_EQ(1, result["hits"].size());
  ASSERT_EQ(1, result["hits"][0]["highlight"].size());
  ASSERT_EQ("+91 <mark>123</mark>123123",
            result["hits"][0]["highlight"]["user"]["phone"]["snippet"]
                .get<std::string>());

  highlight_fields = "user.*";
  // user.* matches user.rank and user.phone
  result = coll->search("+91", {"user"}, "", {}, {}, {0}, 10, 1, FREQUENCY,
                        {true}, Index::DROP_TOKENS_THRESHOLD,
                        dummy_include_exclude, dummy_include_exclude, 10, "",
                        30, 4, "", Index::TYPO_TOKENS_THRESHOLD, "", "", {}, 3,
                        "<mark>", "</mark>", {}, UINT32_MAX, true, false, true,
                        highlight_fields)
               .get();
  ...
  highlight_fields = "user*";
  // user* matches user_name, user.rank and user.phone
  result = coll->search("user_a", {"user_name"}, "", {}, {}, {0}, 10, 1,
  ...
  ASSERT_EQ("<mark>user_a</mark>",
            result["hits"][0]["highlight"]["user_name"]["snippet"]
                .get<std::string>());
...
TEST_F(CollectionTest, WildcardHighlightFullFields) {
  ...
  std::string bio =
      "Once there was a middle-aged boy named User_a who was an avid swimmer."
      "He had been swimming competitively for most of his life, and had even "
      "competed in several national competitions."
      "However, despite his passion and talent for the sport, he had never "
      "quite managed to win that elusive gold medal."
      "Determined to change that, User_a began training harder than ever "
      "before."
      "He woke up early every morning to swim laps before work and spent his "
      "evenings at the pool as well."
      "Despite the grueling schedule, he never once complained."
      "Instead, he reminded himself of his goal: to become a national "
      "champion.";
  json["user"]["bio"] = bio;
  ...
  auto result =
      coll->search("user_a", {"*"}, "", {}, {}, {0}, 10, 1, FREQUENCY, {true},
                   Index::DROP_TOKENS_THRESHOLD, dummy_include_exclude,
                   dummy_include_exclude, 10, "", 30, 4, highlight_full_fields)
          .get();
  ...
  std::string highlighted_value =
      "Once there was a middle-aged boy named <mark>User_a</mark> who was an "
      "avid swimmer."
      "He had been swimming competitively for most of his life, and had even "
      "competed in several national competitions."
      "However, despite his passion and talent for the sport, he had never "
      "quite managed to win that elusive gold medal."
      "Determined to change that, <mark>User_a</mark> began training harder "
      "than ever before."
      "He woke up early every morning to swim laps before work and spent his "
      "evenings at the pool as well."
      "Despite the grueling schedule, he never once complained."
      "Instead, he reminded himself of his goal: to become a national "
      "champion.";
  ASSERT_EQ(highlighted_value,
            result["hits"][0]["highlight"]["user"]["bio"]["value"]
                .get<std::string>());
  ASSERT_EQ(
      "<mark>user_a</mark>",
...
TEST_F(CollectionTest, SemanticSearchTest) {
  ...
  auto search_res_op =
      coll->search("apple", {"embedding"}, "", {}, {}, {0}, 10, 1, FREQUENCY,
                   {true}, Index::DROP_TOKENS_THRESHOLD, dummy_include_exclude,
                   dummy_include_exclude, 10, "", 30, 4, "");
  ...
  ASSERT_EQ("apple",
            search_res["hits"][0]["document"]["name"].get<std::string>());
...
TEST_F(CollectionTest, InvalidSemanticSearch) {
  ...
  auto search_res_op = coll->search(
      "apple", {"embedding", "embedding"}, "", {}, {}, {0}, 10, 1, FREQUENCY,
      {true}, Index::DROP_TOKENS_THRESHOLD, dummy_include_exclude,
      dummy_include_exclude, 10, "", 30, 4, "");
...
TEST_F(CollectionTest, HybridSearch) {
  ...
  spp::sparse_hash_set<std::string> dummy_include_exclude;
  auto search_res_op = coll->search(
      "apple", {"name", "embedding"}, "", {}, {}, {0}, 10, 1, FREQUENCY, {true},
      Index::DROP_TOKENS_THRESHOLD, dummy_include_exclude,
      dummy_include_exclude, 10, "", 30, 4, "");
  ASSERT_TRUE(search_res_op.ok());
  auto search_res = search_res_op.get();
...
TEST_F(CollectionTest, HybridSearchRankFusionTest) {
  ...
  spp::sparse_hash_set<std::string> dummy_include_exclude;
  auto search_res_op =
      coll->search("butter", {"embedding"}, "", {}, {}, {0}, 10, 1, FREQUENCY,
                   {true}, Index::DROP_TOKENS_THRESHOLD, dummy_include_exclude,
                   dummy_include_exclude, 10, "", 30, 4, "");
  ASSERT_TRUE(search_res_op.ok());
  auto search_res = search_res_op.get();
  ...
  ASSERT_EQ("butter",
            search_res["hits"][0]["document"]["name"].get<std::string>());
  ASSERT_EQ("butterball",
            search_res["hits"][1]["document"]["name"].get<std::string>());
  ASSERT_EQ("butterfly",
            search_res["hits"][2]["document"]["name"].get<std::string>());

  search_res_op =
      coll->search("butter", {"name"}, "", {}, {}, {0}, 10, 1, FREQUENCY,
                   {true}, Index::DROP_TOKENS_THRESHOLD, dummy_include_exclude,
                   dummy_include_exclude, 10, "", 30, 4, "");
  ASSERT_TRUE(search_res_op.ok());
  ...
  search_res_op = coll->search(
      "butter", {"name", "embedding"}, "", {}, {}, {0}, 10, 1, FREQUENCY,
      {true}, Index::DROP_TOKENS_THRESHOLD, dummy_include_exclude,
      dummy_include_exclude, 10, "", 30, 4, "");
  ASSERT_TRUE(search_res_op.ok());
  search_res = search_res_op.get();
  ...
  ASSERT_FLOAT_EQ(
      (1.0 / 1.0 * 0.7) + (1.0 / 1.0 * 0.3),
      search_res["hits"][0]["hybrid_search_info"]["rank_fusion_score"]
          .get<float>());
  ASSERT_FLOAT_EQ(
      (1.0 / 2.0 * 0.7) + (1.0 / 3.0 * 0.3),
      search_res["hits"][1]["hybrid_search_info"]["rank_fusion_score"]
          .get<float>());
  ASSERT_FLOAT_EQ(
      (1.0 / 3.0 * 0.7) + (1.0 / 2.0 * 0.3),
      search_res["hits"][2]["hybrid_search_info"]["rank_fusion_score"]
          .get<float>());
}
...
TEST_F(CollectionTest, WildcardSearchWithEmbeddingField) {
  ...
  spp::sparse_hash_set<std::string> dummy_include_exclude;
  auto search_res_op = coll->search(
      "*", {"name", "embedding"}, "", {}, {}, {0}, 10, 1, FREQUENCY, {true},
      Index::DROP_TOKENS_THRESHOLD, dummy_include_exclude,
      dummy_include_exclude, 10, "", 30, 4, "");
...
TEST_F(CollectionTest, WrongTypeInEmbedFrom) {
  ...
  auto op = collectionManager.create_collection(schema);
  ASSERT_FALSE(op.ok());
  ASSERT_EQ("Property `embed.from` must contain only field names as strings.",
            op.error());
}
...
TEST_F(CollectionTest, WrongTypeForEmbedding) {
  ...
  auto add_op = validator_t::validate_embed_fields(
      doc, coll->get_embedding_fields(), coll->get_schema(), true);
  ASSERT_FALSE(add_op.ok());
...
TEST_F(CollectionTest, UpdateEmbeddingsForUpdatedDocument) {
  ...
  auto update_op = coll->update_matching_filter(
      "id:=" + id.get<std::string>(), update_doc.dump(), dirty_values);
  ASSERT_TRUE(update_op.ok());
  ...
  auto updated_embedding_field =
      get_op.get()["embedding"].get<std::vector<float>>();
...
TEST_F(CollectionTest, CreateOpenAIEmbeddingField) {
  ...
  ASSERT_EQ("openai/text-embedding-ada-002",
            summary["fields"][1]["embed"]["model_config"]["model_name"]);
...
TEST_F(CollectionTest, HideOpenAIApiKey) {
  ...
  ASSERT_EQ(summary["fields"][1]["embed"]["model_config"]["api_key"]
                .get<std::string>(),
            api_key.replace(5, api_key.size() - 5, api_key.size() - 5, '*'));
}
...
TEST_F(CollectionTest, PrefixSearchDisabledForOpenAI) {
  ...
  spp::sparse_hash_set<std::string> dummy_include_exclude;
  auto search_res_op = op.get()->search(
      "dummy", {"embedding"}, "", {}, {}, {0}, 10, 1, FREQUENCY, {true},
      Index::DROP_TOKENS_THRESHOLD, dummy_include_exclude,
      dummy_include_exclude, 10, "", 30, 4, "");

  ASSERT_FALSE(search_res_op.ok());
  ASSERT_EQ(
      "Prefix search is not supported for remote embedders. Please set "
      "`prefix=false` as an additional search parameter to disable prefix "
      "searching.",
      search_res_op.error());

  search_res_op = op.get()->search(
      "dummy", {"embedding"}, "", {}, {}, {0}, 10, 1, FREQUENCY, {false},
      Index::DROP_TOKENS_THRESHOLD, dummy_include_exclude,
      dummy_include_exclude, 10, "", 30, 4, "");
  ASSERT_TRUE(search_res_op.ok());
}
...
TEST_F(CollectionTest, MoreThanOneEmbeddingField) {
  ...
  auto add_op = validator_t::validate_embed_fields(
      doc, op.get()->get_embedding_fields(), op.get()->get_schema(), true);

  ASSERT_TRUE(add_op.ok());
  spp::sparse_hash_set<std::string> dummy_include_exclude;

  auto search_res_op = coll->search(
      "butter", {"name", "embedding", "embedding2"}, "", {}, {}, {0}, 10, 1,
      FREQUENCY, {true}, Index::DROP_TOKENS_THRESHOLD, dummy_include_exclude,
      dummy_include_exclude, 10, "", 30, 4, "");

  ASSERT_FALSE(search_res_op.ok());

  ASSERT_EQ("Only one embedding field is allowed in the query.",
            search_res_op.error());
}
...
TEST_F(CollectionTest, EmbeddingFieldEmptyArrayInDocument) {
  ...
  auto update_op = coll->update_matching_filter("id:=" + id.get<std::string>(),
                                                doc.dump(), dirty_values);
  ASSERT_TRUE(update_op.ok());
  ASSERT_EQ(1, update_op.get()["num_updated"]);
