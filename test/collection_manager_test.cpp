...
TEST_F(CollectionManagerTest, CollectionCreation) {
  ...
  // we already call `collection1->get_next_seq_id` above, which is
  // side-effecting
...
TEST_F(CollectionManagerTest, ParallelCollectionCreation) {
  ...
  for (size_t i = 0; i < 10; i++) {
    threads.emplace_back([i, &collectionManager = collectionManager]() {
      ...
      coll_json["name"] =
          coll_json["name"].get<std::string>() + std::to_string(i + 1);
...
TEST_F(CollectionManagerTest, ShouldInitCollection) {
  nlohmann::json collection_meta1 = nlohmann::json::parse(
      "{\"name\": \"foobar\", \"id\": 100, \"fields\": [{\"name\": \"org\", "
      "\"type\": "
      "\"string\", \"facet\": false}], \"default_sorting_field\": \"foo\"}");

  Collection* collection =
      collectionManager.init_collection(collection_meta1, 100, store, 1.0f);
  ...
  nlohmann::json collection_meta2 = nlohmann::json::parse(
      "{\"name\": \"foobar\", \"id\": 100, \"fields\": [{\"name\": \"org\", "
      "\"type\": "
      "\"string\", \"facet\": false, \"infix\": true, \"sort\": true, "
      "\"locale\": \"en\"}], \"created_at\": 12345,"
      "\"default_sorting_field\": \"foo\","
      "\"symbols_to_index\": [\"+\"], \"token_separators\": [\"-\"]}");

  collection =
      collectionManager.init_collection(collection_meta2, 100, store, 1.0f);
...
TEST_F(CollectionManagerTest, RestoreRecordsOnRestart) {
  ...
  ASSERT_TRUE(
      collection1
          ->add_synonym(
              R"({"id": "id1", "root": "smart phone", "synonyms": ["iphone"]})"_json)
          .ok());
  ASSERT_TRUE(
      collection1
          ->add_synonym(
              R"({"id": "id2", "root": "mobile phone", "synonyms": ["samsung phone"]})"_json)
          .ok());
  ASSERT_TRUE(
      collection1
          ->add_synonym(
              R"({"id": "id3", "synonyms": ["football", "foot ball"]})"_json)
          .ok());
  ...
  nlohmann::json results =
      collection1
          ->search("thomas", search_fields, "", facets, sort_fields, {0}, 10, 1,
                   FREQUENCY, {false})
          .get();
  ...
  // recreate collection manager to ensure that it restores the records from the
  // disk backed store
  collectionManager.dispose();
  delete store;
  ...
  ASSERT_STREQ("exclude-rule",
               collection1->get_overrides()["exclude-rule"].id.c_str());
  ASSERT_STREQ("include-rule",
               collection1->get_overrides()["include-rule"].id.c_str());
  ...
  results = collection1
                ->search("thomas", search_fields, "", facets, sort_fields, {0},
...
TEST_F(CollectionManagerTest, VerifyEmbeddedParametersOfScopedAPIKey) {
  ...
  auto results =
      coll1->search("*", {"title"}, "", {}, {}, {0}, 3, 1, FREQUENCY, {true}, 5)
          .get();
  ...
  auto search_op = collectionManager.do_search(req_params, embedded_params,
                                               json_res, now_ts);
...
TEST_F(CollectionManagerTest, QuerySuggestionsShouldBeTrimmed) {
  ...
  auto create_op =
      AnalyticsManager::get_instance().create_rule(analytics_rule, false, true);
  ...
  ASSERT_EQ(
      2, popular_queries["top_queries"]->get_user_prefix_queries()[""].size());
  ASSERT_EQ(
      "tom",
      popular_queries["top_queries"]->get_user_prefix_queries()[""][0].query);
  ASSERT_EQ(
      "",
      popular_queries["top_queries"]->get_user_prefix_queries()[""][1].query);
...
TEST_F(CollectionManagerTest, RestoreAutoSchemaDocsOnRestart) {
  ...
  std::vector<field> fields = {field("max", field_types::INT32, false)};

  coll1 = collectionManager.get_collection("coll1").get();
  if (coll1 == nullptr) {
    coll1 =
        collectionManager
            .create_collection("coll1", 1, fields, "max", 0, field_types::AUTO)
            .get();
  }
  ...
  Option<nlohmann::json> add_op =
      coll1->add(doc_json, CREATE, "", DIRTY_VALUES::COERCE_OR_DROP);
  ASSERT_TRUE(add_op.ok());

  tsl::htrie_map<char, field> schema = collection1->get_schema();

  // create a new collection manager to ensure that it restores the records from
  ...
  auto results = restored_coll
                     ->search("12345", {"title"}, "", {}, {}, {0}, 10, 1,
                              FREQUENCY, {false})
                     .get();

  ASSERT_EQ(1, results["hits"].size());

  // int to string conversion should be done for `title` while `average` field
  // must be dropped
  ASSERT_STREQ(
      "12345",
...
TEST_F(CollectionManagerTest, RestoreNestedDocsOnRestart) {
  ...
  res_op = restored_coll->search("foobar", {"company.name"}, "", {}, {}, {0},
...
TEST_F(CollectionManagerTest, RestoreCoercedDocValuesOnRestart) {
  ...
  res_op = restored_coll->search("*", {}, "product.price:>0", {}, {}, {0}, 10,
...
TEST_F(CollectionManagerTest, DropCollectionCleanly) {
  ...
  ASSERT_FALSE(nullptr ==
               collectionManager.get_collection("collection1").get());
...
TEST_F(CollectionManagerTest, AuthWithMultiSearchKeys) {
  api_key_t key1("api_key", "some key", {"documents:create"}, {"foo"},
                 64723363199);
  ...
  ASSERT_TRUE(collectionManager.auth_key_matches(
      "", "documents:create", collection_keys, params, embedded_params_vec));

  // should work with bootstrap key
  collection_keys = {collection_key_t("foo", "auth_key")};
  ...
  collection_keys = {collection_key_t("foo", "")};

  ASSERT_FALSE(collectionManager.auth_key_matches(
      "", "documents:create", collection_keys, params, embedded_params_vec));
}
...
TEST_F(CollectionManagerTest, Symlinking) {
  ...
  system(
      ("rm -rf " + state_dir_path + " && mkdir -p " + state_dir_path).c_str());
  ...
  Option<std::string> collection_option =
      cmanager.resolve_symlink("collection");
  ...
  Option<bool> inserted =
      cmanager.upsert_symlink("collection1", "collection_2018");
  ASSERT_FALSE(inserted.ok());
  ASSERT_STREQ("Name `collection1` conflicts with an existing collection name.",
...
TEST_F(CollectionManagerTest, LoadMultipleCollections) {
  // to prevent fixture tear down from running as we are fudging with
  ...
  for (size_t i = 0; i < 100; i++) {
    auto schema = {field("title", field_types::STRING, false),
    ...
    cmanager
        .create_collection("collection" + std::to_string(i), 4, schema,
                           "points")
        .get();
  }
...
TEST_F(CollectionManagerTest, ParseSortByClause) {
  std::vector<sort_by> sort_fields;
  bool sort_by_parsed = CollectionManager::parse_sort_by_str(
      "points:desc,loc(24.56,10.45):ASC", sort_fields);
  ...
  sort_by_parsed = CollectionManager::parse_sort_by_str(
      " points:desc , loc(24.56,10.45):ASC", sort_fields);
  ASSERT_TRUE(sort_by_parsed);
  ...
  sort_by_parsed = CollectionManager::parse_sort_by_str(
      " loc(24.56,10.45):ASC, points: desc ", sort_fields);
  ASSERT_TRUE(sort_by_parsed);
  ...
  sort_fields.clear();
  sort_by_parsed = CollectionManager::parse_sort_by_str(
      " location(48.853, 2.344, exclude_radius: 2mi):asc,popularity:desc",
      sort_fields);
  ASSERT_TRUE(sort_by_parsed);
  ASSERT_EQ("location(48.853, 2.344, exclude_radius: 2mi)",
            sort_fields[0].name);
  ASSERT_STREQ("ASC", sort_fields[0].order.c_str());

  sort_fields.clear();
  sort_by_parsed = CollectionManager::parse_sort_by_str(
      " location(48.853, 2.344, precision: 2mi):asc,popularity:desc",
      sort_fields);
  ASSERT_TRUE(sort_by_parsed);
  ...
  sort_fields.clear();
  sort_by_parsed = CollectionManager::parse_sort_by_str(
      " _text_match(buckets: 10):ASC, points:desc ", sort_fields);
  ASSERT_TRUE(sort_by_parsed);
  ...
  sort_fields.clear();
  sort_by_parsed = CollectionManager::parse_sort_by_str(
      "_eval(brand:nike && foo:bar):DESC,points:desc ", sort_fields);
  ASSERT_TRUE(sort_by_parsed);
  ...
  sort_fields.clear();
  sort_by_parsed =
      CollectionManager::parse_sort_by_str("foobar:,bar:desc", sort_fields);
  ASSERT_FALSE(sort_by_parsed);
...
TEST_F(CollectionManagerTest, CloneCollection) {
  ...
  nlohmann::json override_json = {
      ...
      {"rule", {{"query", "{categories}"}, {"match", override_t::MATCH_EXACT}}},
