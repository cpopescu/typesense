...
class PostingListTest : public ::testing::Test {
 protected:
  ...
  virtual void SetUp() { pool = new ThreadPool(4); }
...
TEST_F(PostingListTest, RemovalsOnFirstBlock) {
  ...
  // delete elements from first block: blocks should not be merged until it
  // falls below 50% occupancy
  pl.erase(1);
  ASSERT_EQ(2, pl.num_blocks());
...
TEST_F(PostingListTest, RemovalsOnLaterBlocks) {
  ...
  for (size_t i = 0; i < pl.get_root()->next->next->offset_index.getLength();
       i++) {
  ...
  // only part of the next node contents can be moved over when we delete 8
  // since (1 + 5) > 5
  pl.erase(8);
...
TEST_F(PostingListTest, IntersectionBasics) {
  ...
  posting_t::block_intersector_t(raw_lists, iter_state)
      .intersect([&](auto id, auto& its) {
  ...
  posting_t::block_intersector_t(raw_lists, iter_state2)
      .intersect([&](auto id, auto& its) {
        std::unique_lock lk(vecm);
        result_ids.push_back(id);
      });

  std::sort(result_ids.begin(),
            result_ids.end());  // because of concurrent intersection order is
                                // not guaranteed
  ...
  posting_t::block_intersector_t(raw_lists, iter_state3)
      .intersect([&](auto id, auto& its) {
        std::unique_lock lk(vecm);
        result_ids.push_back(id);
      });
...
TEST_F(PostingListTest, ResultsAndOffsetsBasics) {
  // NOTE: due to the way offsets1 are parsed, the actual positions are 1 less
  // than the offset values stored (to account for the special offset `0` which
  // indicates last offset
  ...
  std::vector<std::unordered_map<size_t, std::vector<token_positions_t>>>
  array_token_positions_vec; posting_list_t::get_offsets(iter_state,
  array_token_positions_vec); ASSERT_EQ(2, array_token_positions_vec.size());

  ASSERT_EQ(actual_offsets_3[0].positions,
  array_token_positions_vec[0].at(0)[0].positions);
  ASSERT_EQ(actual_offsets_3[1].positions,
  array_token_positions_vec[0].at(0)[1].positions);
  ASSERT_EQ(actual_offsets_3[2].positions,
  array_token_positions_vec[0].at(0)[2].positions);

  ASSERT_EQ(actual_offsets_20[0].positions,
  array_token_positions_vec[1].at(0)[0].positions);
  ASSERT_EQ(actual_offsets_20[1].positions,
  array_token_positions_vec[1].at(0)[1].positions);
  ASSERT_EQ(actual_offsets_20[2].positions,
  array_token_positions_vec[1].at(0)[2].positions);
  */
}
...
TEST_F(PostingListTest, IntersectionSkipBlocks) {
  ...
  size_t temp_len = ArrayUtils::and_scalar(
      &p1_ids[0], p1_ids.size(), &p2_ids[0], p2_ids.size(), &p1_p2_intersected);
  size_t final_len = ArrayUtils::and_scalar(
      &p3_ids[0], p3_ids.size(), p1_p2_intersected, temp_len, &final_results);
...
TEST_F(PostingListTest, CompactPostingListUpsertAppends) {
  ...
  compact_posting_list_t* list =
      compact_posting_list_t::create(3, ids, offset_index, 9, offsets);
...
TEST_F(PostingListTest, CompactPostingListUpserts) {
  ...
  uint32_t expected_id_offsets[] = {2,   1, 2, 2, 3, 0,    3, 4, 3, 2, 1,   2,
                                    999, 3, 0, 3, 4, 1000, 3, 0, 3, 4, 1002};
...
TEST_F(PostingListTest, CompactPostingListUpdateWithLessOffsets) {
  ...
  ASSERT_EQ(1002, list->last_id());
  ASSERT_EQ(3, list->num_ids());
  uint32_t expected_id_offsets[] = {3, 0,    3, 4, 0, 2, 1,
                                    2, 1000, 3, 0, 3, 4, 1002};
  ...
  ASSERT_EQ(1002, list->last_id());
  ASSERT_EQ(3, list->num_ids());
  uint32_t expected_id_offsets2[] = {2,    2, 4, 0, 2, 1,   2,
                                     1000, 3, 0, 3, 4, 1002};
  for (size_t i = 0; i < list->length; i++) {
...
TEST_F(PostingListTest, CompactPostingListUpdateWithMoreOffsets) {
  ...
  ASSERT_EQ(1002, list->last_id());
  ASSERT_EQ(3, list->num_ids());
  uint32_t expected_id_offsets[] = {3, 0, 3,    4, 0, 4, 1, 2,
                                    3, 4, 1000, 3, 0, 3, 4, 1002};
  for (size_t i = 0; i < list->length; i++) {
    ASSERT_EQ(expected_id_offsets[i], list->id_offsets[i]);
  }
  ...
  ASSERT_EQ(1002, list->last_id());
  ASSERT_EQ(3, list->num_ids());
  uint32_t expected_id_offsets2[] = {4, 1, 2,    3, 4, 0, 4, 1,   2,
                                     3, 4, 1000, 3, 0, 3, 4, 1002};
  for (size_t i = 0; i < list->length; i++) {
    ASSERT_EQ(expected_id_offsets2[i], list->id_offsets[i]);
  }
  ...
  ASSERT_EQ(1002, list->last_id());
  ASSERT_EQ(3, list->num_ids());
  uint32_t expected_id_offsets3[] = {4, 1, 2,    3, 4, 0, 4, 1, 2,
                                     3, 4, 1000, 4, 1, 2, 3, 4, 1002};
  for (size_t i = 0; i < list->length; i++) {
    ASSERT_EQ(expected_id_offsets3[i], list->id_offsets[i]);
  }
...
TEST_F(PostingListTest, CompactPostingListContainsAtleastOne) {
  ...
  compact_posting_list_t* list1 =
      compact_posting_list_t::create(4, ids, offset_index, 12, offsets);
  ASSERT_TRUE(list1->contains_atleast_one(&target_ids1[0], target_ids1.size()));
  ASSERT_FALSE(
      list1->contains_atleast_one(&target_ids2[0], target_ids2.size()));
  free(list1);

  compact_posting_list_t* list2 = static_cast<compact_posting_list_t*>(
      malloc(sizeof(compact_posting_list_t)));
  ...
  ASSERT_TRUE(COMPACT_POSTING_PTR(obj)->contains_atleast_one(
      &target_ids3[0], target_ids3.size()));
  ASSERT_FALSE(COMPACT_POSTING_PTR(obj)->contains_atleast_one(
      &target_ids4[0], target_ids4.size()));

  std::vector<uint32_t> target_ids5 = {2, 3};
  ASSERT_TRUE(COMPACT_POSTING_PTR(obj)->contains_atleast_one(
      &target_ids5[0], target_ids5.size()));

  std::vector<uint32_t> target_ids6 = {0, 1, 2};
  ASSERT_FALSE(COMPACT_POSTING_PTR(obj)->contains_atleast_one(
      &target_ids6[0], target_ids6.size()));
...
TEST_F(PostingListTest, DISABLED_RandInsertAndErase) {
  ...
  LOG(INFO) << "Num ids: " << pl.num_ids()
            << ", num bocks: " << pl.num_blocks();
}
...
TEST_F(PostingListTest, DISABLED_Benchmark) {
  ...
  timeMicros = std::chrono::duration_cast<std::chrono::microseconds>(
                   std::chrono::high_resolution_clock::now() - begin)
                   .count();
...
TEST_F(PostingListTest, DISABLED_BenchmarkIntersection) {
  ...
  uint32_t* ab;
  size_t ab_len =
      ArrayUtils::and_scalar(a, arr1.getLength(), b, arr2.getLength(), &ab);

  uint32_t* abc;
  size_t abc_len =
      ArrayUtils::and_scalar(ab, ab_len, c, arr3.getLength(), &abc);
