...
TEST_F(PopularQueriesTest, PrefixQueryCompaction) {
  ...
  // compaction after user has typed first prefix but before compaction interval
  // has happened
  ...
  pq.compact_user_queries(
      now_ts_us + PopularQueries::QUERY_FINALIZATION_INTERVAL_MICROS + 100);
  queries = pq.get_user_prefix_queries();
  ...
  pq.add(
      "b", true, "0",
      now_ts_us + 3 + PopularQueries::QUERY_FINALIZATION_INTERVAL_MICROS + 100);
  pq.compact_user_queries(now_ts_us + 3 +
                          PopularQueries::QUERY_FINALIZATION_INTERVAL_MICROS +
                          100 + 1);
  queries = pq.get_user_prefix_queries();
  ASSERT_EQ(1, queries.size());
  ASSERT_EQ(1, queries["0"].size());
  ...
  auto prev_ts = now_ts_us + 3 +
                 PopularQueries::QUERY_FINALIZATION_INTERVAL_MICROS + 100 + 1;
  pq.add("ba", true, "0", prev_ts + 1);
  pq.add("bar", true, "0", prev_ts + 2);
  pq.compact_user_queries(
      prev_ts + 2 + PopularQueries::QUERY_FINALIZATION_INTERVAL_MICROS + 1);
  queries = pq.get_user_prefix_queries();
  ASSERT_EQ(0, queries.size());
  local_counts = pq.get_local_counts();
