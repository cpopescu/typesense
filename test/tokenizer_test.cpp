...
TEST(TokenizerTest, ShouldTokenizeNormalizeDifferentStrings) {
  ...
  std::vector<std::string> withaccent_tokens = {"mise", "tj",     "a", "jour",
                                                "timy", "depuis", "pc"};
  ...
  // special chars
  const std::string specialchar_tokens =
      "https://www.amazon.com/s?k=phone&ref=nb_sb_noss_2";
  tokens.clear();
  ...
  tokens.clear();
  const std::string withspecialchars =
      "Special ½¥ and தமிழ் 你好吗 abcÅà123ß12 here.";
...
TEST(TokenizerTest, ShouldTokenizeNormalizeUnicodeStrings) {
  ...
  const std::string withspecialchars =
      "Special ½¥ and -thenதமிழ், 你2好吗 abcÅà123ß12 verläßlich here.";
  tokens.clear();
...
TEST(TokenizerTest, ShouldTokenizeIteratively) {
  ...
  std::vector<std::string> expected_tokens = {"michael", "jordan", "welcome",
                                              "everybody", "welcome"};
...
TEST(TokenizerTest, ShouldTokenizeChineseText) {
  ...
  tokens.clear();
  Tokenizer("很久以前，傳說在臺中北屯的一個地方", false, false, "zh")
      .tokenize(tokens);
