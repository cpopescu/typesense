...
TEST(ConfigTest, LoadConfigFile) {
  ...
  std::vector<std::string> args = {
      "./typesense-server", std::string("--config=") + std::string(ROOT_DIR) +
                                "test/valid_config.ini"};
  std::vector<char*> argv = get_argv(args);
  init_cmdline_options(options, argv.size() - 1, argv.data());
  options.parse(argv.size() - 1, argv.data());
...
TEST(ConfigTest, CmdLineArgsOverrideConfigFileAndEnvVars) {
  ...
  std::vector<std::string> args = {"./typesense-server",
                                   "--data-dir=/tmp/data",
                                   "--api-key=abcd",
  ...
  ASSERT_EQ("abcd",
            config.get_api_key());  // cli parameter overrides file config
  ASSERT_EQ(
      1,
      config.get_cors_domains().size());  // cli parameter overrides file config
...
TEST(ConfigTest, CorsDefaults) {
  ...
  std::vector<std::string> args = {
      "./typesense-server", "--data-dir=/tmp/data", "--api-key=abcd",
      "--listen-address=192.168.10.10",
      std::string("--config=") + std::string(ROOT_DIR) +
          "test/valid_sparse_config.ini"};
