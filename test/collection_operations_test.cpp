...
TEST_F(CollectionOperationsTest, IncrementInt32Value) {
  ...
  auto res = coll->search("*", {"title"}, "points:101", {}, {}, {0}, 3, 1,
                          FREQUENCY, {false})
                 .get();
  ...
  res = coll->search("*", {"title"}, "points:111", {}, {}, {0}, 3, 1, FREQUENCY,
                     {false})
            .get();
  ASSERT_EQ(1, res["hits"].size());
  ASSERT_EQ(3, res["hits"][0]["document"].size());
  ...
  res = coll->search("*", {"title"}, "points:101", {}, {}, {0}, 3, 1, FREQUENCY,
                     {false})
            .get();
  ASSERT_EQ(1, res["hits"].size());
  ASSERT_EQ(3, res["hits"][0]["document"].size());
  ...
  ASSERT_TRUE(coll->add(doc.dump(), UPDATE).ok());
  res = coll->search("*", {"title"}, "", {}, {}, {0}, 3, 1, FREQUENCY, {false})
            .get();
  ASSERT_EQ(1, res["hits"].size());
  ASSERT_EQ(3, res["hits"][0]["document"].size());
  ASSERT_EQ("0", res["hits"][0]["document"]["id"].get<std::string>());
  ASSERT_EQ("The Sherlock Holmes",
            res["hits"][0]["document"]["title"].get<std::string>());
  ASSERT_EQ(101, res["hits"][0]["document"]["points"].get<size_t>());
}
...
TEST_F(CollectionOperationsTest, IncrementInt32ValueCreationViaOptionalField) {
  ...
  auto res = coll->search("*", {"title"}, "points:1", {}, {}, {0}, 3, 1,
                          FREQUENCY, {false})
                 .get();
  ASSERT_EQ(1, res["hits"].size());
  ASSERT_EQ(3, res["hits"][0]["document"].size());
  ...
  res = coll->search("*", {"title"}, "points:10", {}, {}, {0}, 3, 1, FREQUENCY,
                     {false})
            .get();
  ASSERT_EQ(1, res["hits"].size());
  ASSERT_EQ(3, res["hits"][0]["document"].size());
  ASSERT_EQ("1", res["hits"][0]["document"]["id"].get<std::string>());
  ASSERT_EQ("Harry Potter",
            res["hits"][0]["document"]["title"].get<std::string>());
