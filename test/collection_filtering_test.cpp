...
TEST_F(CollectionFilteringTest, FilterOnTextFields) {
  ...
  nlohmann::json results =
      coll_array_fields
          ->search("Jeremy", query_fields, "tags: gold", facets, sort_fields,
  ...
  results = coll_array_fields
                ->search("Jeremy", query_fields, "tags : fine PLATINUM", facets,
                         sort_fields, {0}, 10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(1, results["hits"].size());

  // using just ":", filtering should return documents that contain ALL tokens
  // in the filter expression
  results = coll_array_fields
                ->search("Jeremy", query_fields, "tags : PLATINUM", facets,
                         sort_fields, {0}, 10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(1, results["hits"].size());
  ...
  results = coll_array_fields
                ->search("Jeremy", query_fields, "tags : WHITE PLATINUM",
                         facets, sort_fields, {0}, 10, 1, FREQUENCY, {false})
                .get();
  ...
  results = coll_array_fields
                ->search("Jeremy", query_fields, "tags : bronze", facets,
                         sort_fields, {0}, 10, 1, FREQUENCY, {false})
                .get();
  ...
  results = coll_array_fields
                ->search("Jeremy", query_fields, "tags: [bronze,   silver]",
                         facets, sort_fields, {0}, 10, 1, FREQUENCY, {false})
                .get();
  ...
  auto res_op =
      coll_array_fields->search("Jeremy", query_fields, "tags:=", facets,
                                sort_fields, {0}, 10, 1, FREQUENCY, {false});
  ASSERT_FALSE(res_op.ok());
  ASSERT_EQ("Error with filter field `tags`: Filter value cannot be empty.",
            res_op.error());
...
TEST_F(CollectionFilteringTest, FacetFieldStringFiltering) {
  ...
  if (coll_str == nullptr) {
    coll_str =
        collectionManager.create_collection("coll_str", 1, fields, "points")
            .get();
  }
  ...
  auto results = coll_str
                     ->search("*", query_fields, "starring:= samuel", facets,
                              sort_fields, {0}, 10, 1, FREQUENCY, {false})
                     .get();
  ASSERT_EQ(0, results["hits"].size());
  ...
  results = coll_str
                ->search("*", query_fields, "starring:= ssamuel l. Jackson",
                         facets, sort_fields, {0}, 10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(0, results["hits"].size());
  ASSERT_EQ(0, results["found"].get<size_t>());
  ...
  results = coll_str
                ->search("*", query_fields, "starring:= samuel l. Jackson",
                         facets, sort_fields, {0}, 10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(2, results["hits"].size());
  ...
  results = coll_str
                ->search("*", query_fields, "starring: samuel johnson", facets,
                         sort_fields, {0}, 10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(0, results["hits"].size());
...
TEST_F(CollectionFilteringTest, FacetFieldStringArrayFiltering) {
  ...
  if (coll_array_fields == nullptr) {
    coll_array_fields =
        collectionManager
            .create_collection("coll_array_fields", 1, fields, "age")
            .get();
  }
  ...
  auto results = coll_array_fields
                     ->search("Jeremy", query_fields, "tags:= PLATINUM", facets,
                              sort_fields, {0}, 10, 1, FREQUENCY, {false})
                     .get();
  ASSERT_EQ(0, results["hits"].size());
  ...
  results = coll_array_fields
                ->search("Jeremy", query_fields, "tags:= FINE", facets,
                         sort_fields, {0}, 10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(0, results["hits"].size());

  results = coll_array_fields
                ->search("Jeremy", query_fields, "tags:= FFINE PLATINUM",
                         facets, sort_fields, {0}, 10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(0, results["hits"].size());
  ...
  results = coll_array_fields
                ->search("Jeremy", query_fields, "name:= Jeremy Howard", facets,
                         sort_fields, {0}, 10, 1, FREQUENCY, {false})
                .get();
  ...
  results = coll_array_fields
                ->search("Jeremy", query_fields, "tags:= [Gold, bronze]",
                         facets, sort_fields, {0}, 10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(3, results["hits"].size());
  ...
  results = coll_array_fields
                ->search("Jeremy", query_fields,
                         "tags:= [Gold, bronze, fine PLATINUM]", facets,
                         sort_fields, {0}, 10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(4, results["hits"].size());
  ...
  results = coll_array_fields
                ->search("Jeremy", query_fields, "tags:= [fine PLATINUM]",
                         facets, sort_fields, {0}, 10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(1, results["hits"].size());
  ASSERT_EQ(1, results["found"].get<size_t>());
...
TEST_F(CollectionFilteringTest, FilterOnTextFieldWithColon) {
  ...
  auto res = coll1
                 ->search("*", query_fields, "url:= https://example.com/1",
                          facets, sort_fields, {0}, 10, 1, FREQUENCY, {false})
                 .get();
  ...
  res = coll1
            ->search("*", query_fields, "url: https://example.com/1", facets,
                     sort_fields, {0}, 10, 1, FREQUENCY, {false})
            .get();
  ASSERT_EQ(1, res["hits"].size());
  ASSERT_STREQ("1",
...
TEST_F(CollectionFilteringTest, HandleBadlyFormedFilterQuery) {
  ...
  nlohmann::json results =
      coll_array_fields
          ->search("Jeremy", query_fields, "tagzz: gold", facets, sort_fields,
                   {0}, 10, 1, FREQUENCY, {false})
          .get();
  ASSERT_EQ(0, results["hits"].size());
  ...
  results = coll_array_fields
                ->search("Jeremy", query_fields,
                         "(age:>0 ||  timestamps:> 0) || tagzz: gold", facets,
                         sort_fields, {0}, 10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(0, results["hits"].size());
  ...
  results =
      coll_array_fields
          ->search("Jeremy", query_fields, "(age:>0 ||  timestamps:> 0) || ",
                   facets, sort_fields, {0}, 10, 1, FREQUENCY, {false})
          .get();
  ASSERT_EQ(0, results["hits"].size());
  ...
  results = coll_array_fields
                ->search("Jeremy", query_fields, "age: abcdef", facets,
                         sort_fields, {0}, 10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(0, results["hits"].size());
  ...
  results = coll_array_fields
                ->search("Jeremy", query_fields, "timestamps: abcdef", facets,
                         sort_fields, {0}, 10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(0, results["hits"].size());
  ...
  results = coll_array_fields
                ->search("Jeremy", query_fields, "age: '21'", facets,
                         sort_fields, {0}, 10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(0, results["hits"].size());
  ...
  auto res_op =
      coll_array_fields->search("Jeremy", query_fields, "age:", facets,
                                sort_fields, {0}, 10, 1, FREQUENCY, {false});
  ASSERT_FALSE(res_op.ok());
  ASSERT_EQ(
      "Error with filter field `age`: Numerical field has an invalid "
      "comparator.",
      res_op.error());
...
TEST_F(CollectionFilteringTest, FilterAndQueryFieldRestrictions) {
  ...
  Option<nlohmann::json> result_op =
      coll_mul_fields->search("anton", query_fields, "", facets, sort_fields,
                              {0}, 10, 1, FREQUENCY, {false});
  ...
  query_fields = {"title"};
  result_op = coll_mul_fields->search(
      "captain", query_fields, "starring: Samuel L. Jackson", facets,
      sort_fields, {0}, 10, 1, FREQUENCY, {false});
  ...
  // filtering on facet field should be possible (supports partial word search
  // but without typo tolerance)
  query_fields = {"title"};
  result_op =
      coll_mul_fields->search("*", query_fields, "cast: chris", facets,
                              sort_fields, {0}, 10, 1, FREQUENCY, {false});
  ASSERT_EQ(true, result_op.ok());
  results = result_op.get();
  ASSERT_EQ(3, results["hits"].size());
  ...
  // bad query string
  result_op =
      coll_mul_fields->search("captain", query_fields, "BLAH", facets,
                              sort_fields, {0}, 10, 1, FREQUENCY, {false});
  ...
  // missing field
  result_op =
      coll_mul_fields->search("captain", query_fields, "age: 100", facets,
                              sort_fields, {0}, 10, 1, FREQUENCY, {false});
  ASSERT_EQ(false, result_op.ok());
  ASSERT_STREQ("Could not find a filter field named `age` in the schema.",
               result_op.error().c_str());

  // bad filter value type
  result_op = coll_mul_fields->search("captain", query_fields,
                                      "points: \"100\"", facets, sort_fields,
                                      {0}, 10, 1, FREQUENCY, {false});
  ASSERT_EQ(false, result_op.ok());
  ASSERT_STREQ(
      "Error with filter field `points`: Numerical field has an invalid "
      "comparator.",
      result_op.error().c_str());

  // bad filter value type - equaling float on an integer field
  result_op =
      coll_mul_fields->search("captain", query_fields, "points: 100.34", facets,
                              sort_fields, {0}, 10, 1, FREQUENCY, {false});
  ASSERT_EQ(false, result_op.ok());
  ASSERT_STREQ("Error with filter field `points`: Not an int32.",
               result_op.error().c_str());

  // bad filter value type - less than float on an integer field
  result_op =
      coll_mul_fields->search("captain", query_fields, "points: <100.0", facets,
                              sort_fields, {0}, 10, 1, FREQUENCY, {false});
  ASSERT_EQ(false, result_op.ok());
  ASSERT_STREQ("Error with filter field `points`: Not an int32.",
               result_op.error().c_str());
  ...
  result_op = coll_mul_fields->search(
      "captain", query_fields, "points: <2230070399", facets, sort_fields, {0},
      10, 1, FREQUENCY, {false});
  ASSERT_EQ(false, result_op.ok());
  ASSERT_STREQ("Error with filter field `points`: Not an int32.",
               result_op.error().c_str());
  ...
  result_op = coll_mul_fields->search("captain", query_fields,
                                      "points: <sdsdfsdf", facets, sort_fields,
                                      {0}, 10, 1, FREQUENCY, {false});
  ASSERT_EQ(false, result_op.ok());

  // large negative number
  result_op = coll_mul_fields->search(
      "captain", query_fields, "points: >-3230070399", facets, sort_fields, {0},
      10, 1, FREQUENCY, {false});
  ASSERT_EQ(false, result_op.ok());

  // but should allow small negative number
  result_op =
      coll_mul_fields->search("captain", query_fields, "points: >-3230", facets,
                              sort_fields, {0}, 10, 1, FREQUENCY, {false});
  ASSERT_EQ(true, result_op.ok());
...
TEST_F(CollectionFilteringTest, FilterOnNumericFields) {
  ...
  if (coll_array_fields == nullptr) {
    ...
    auto coll_op = collectionManager.create_collection("coll_array_fields", 4,
                                                       fields, "years");
    ASSERT_EQ(false, coll_op.ok());
    ASSERT_STREQ("Default sorting field `years` is not a sortable type.",
                 coll_op.error().c_str());

    // let's try again properly
    coll_op = collectionManager.create_collection("coll_array_fields", 4,
                                                  fields, "age");
  ...
  results = coll_array_fields
                ->search("Jeremy", query_fields, "age:>24", facets, sort_fields,
                         {0}, 10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(3, results["hits"].size());
  ...
  results = coll_array_fields
                ->search("Jeremy", query_fields, "years:>2002", facets,
                         sort_fields, {0}, 10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(3, results["hits"].size());
  ...
  results = coll_array_fields
                ->search("Jeremy", query_fields, "years:<1989", facets,
                         sort_fields, {0}, 10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(1, results["hits"].size());
  ...
  // multiple filters
  results = coll_array_fields
                ->search("Jeremy", query_fields, "years:<2005 && years:>1987",
                         facets, sort_fields, {0}, 10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(1, results["hits"].size());
  ...
  // multiple search values (works like SQL's IN operator) against a single int
  // field
  results = coll_array_fields
                ->search("Jeremy", query_fields, "age:[21, 24, 63]", facets,
                         sort_fields, {0}, 10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(3, results["hits"].size());
  ...
  results = coll_array_fields
                ->search("Jeremy", query_fields, "age: [!=21, >30]", facets,
                         sort_fields, {0}, 10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(4, results["hits"].size());
  ...
  // negate multiple search values (works like SQL's NOT IN) against a single
  // int field
  results = coll_array_fields
                ->search("Jeremy", query_fields, "age:!= [21, 24, 63]", facets,
                         sort_fields, {0}, 10, 1, FREQUENCY, {false})
  ...
  // individual comparators can still be applied.
  results = coll_array_fields
                ->search("Jeremy", query_fields, "age: != [<30, >60]", facets,
                         sort_fields, {0}, 10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(2, results["hits"].size());
  ...
  // multiple search values against an int32 array field - also use extra
  // padding between symbols
  results = coll_array_fields
                ->search("Jeremy", query_fields, "years : [ 2015, 1985 , 1999]",
                         facets, sort_fields, {0}, 10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(4, results["hits"].size());
  ...
  // searching on an int64 array field - also ensure that padded space causes no
  // issues
  results = coll_array_fields
                ->search("Jeremy", query_fields, "timestamps : > 475205222",
                         facets, sort_fields, {0}, 10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(4, results["hits"].size());
  ...
  // range based filter
  results = coll_array_fields
                ->search("Jeremy", query_fields, "age: 21..32", facets,
                         sort_fields, {0}, 10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(3, results["hits"].size());
  ...
  results = coll_array_fields
                ->search("Jeremy", query_fields, "age: 0 .. 100", facets,
                         sort_fields, {0}, 10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(5, results["hits"].size());

  results = coll_array_fields
                ->search("Jeremy", query_fields, "age: [21..24, 40..65]",
                         facets, sort_fields, {0}, 10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(4, results["hits"].size());
  ...
  results = coll_array_fields
                ->search("Jeremy", query_fields, "rating: 7.812 .. 9.999",
                         facets, sort_fields, {0}, 10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(2, results["hits"].size());
  ...
  results = coll_array_fields
                ->search("Jeremy", query_fields,
                         "rating: [7.812 .. 9.999, 1.05 .. 1.09]", facets,
                         sort_fields, {0}, 10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(3, results["hits"].size());
  ...
  results = coll_array_fields
                ->search("Jeremy", query_fields, "timestamps:>1591091288061",
                         facets, sort_fields, {0}, 10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(0, results["hits"].size());
...
TEST_F(CollectionFilteringTest, FilterOnFloatFields) {
  ...
  // Plain search with no filters - results should be sorted by rating field
  // DESC
  query_fields = {"name"};
  std::vector<std::string> facets;
  nlohmann::json results =
      coll_array_fields
          ->search("Jeremy", query_fields, "", facets, sort_fields_desc, {0},
                   10, 1, FREQUENCY, {false})
          .get();
  ASSERT_EQ(5, results["hits"].size());
  ...
  results = coll_array_fields
                ->search("Jeremy", query_fields, "", facets, sort_fields_asc,
                         {0}, 10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(5, results["hits"].size());
  ...
  results = coll_array_fields
                ->search("Jeremy", query_fields, "rating:!=0", facets,
  ...
  results = coll_array_fields
                ->search("Jeremy", query_fields, "rating:>0.0", facets,
  ...
  results = coll_array_fields
                ->search("Jeremy", query_fields, "top_3:>7.8", facets,
                         sort_fields_desc, {0}, 10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(2, results["hits"].size());
  ...
  results =
      coll_array_fields
          ->search("Jeremy", query_fields, "top_3:>7.8 && rating:>7.9", facets,
                   sort_fields_desc, {0}, 10, 1, FREQUENCY, {false})
          .get();
  ASSERT_EQ(1, results["hits"].size());
  ...
  // multiple search values (works like SQL's IN operator) against a single
  // float field
  results = coll_array_fields
                ->search("Jeremy", query_fields, "rating:[1.09, 7.812]", facets,
                         sort_fields_desc, {0}, 10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(2, results["hits"].size());
  ...
  // negate multiple search values (works like SQL's NOT IN operator) against a
  // single float field
  results =
      coll_array_fields
          ->search("Jeremy", query_fields, "rating:!= [1.09, 7.812]", facets,
  ...
  results =
      coll_array_fields
          ->search("Jeremy", query_fields, "rating: != [<5.4, >9]", facets,
                   sort_fields_desc, {0}, 10, 1, FREQUENCY, {false})
          .get();
  ASSERT_EQ(2, results["hits"].size());
  ...
  // multiple search values against a float array field - also use extra padding
  // between symbols
  results = coll_array_fields
                ->search("Jeremy", query_fields,
                         "top_3 : [ 5.431, 0.001 , 7.812, 11.992]", facets,
                         sort_fields_desc, {0}, 10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(3, results["hits"].size());
  ...
  // when filters don't match any record, no results should be returned
  auto results_op = coll_array_fields->search(
      "Jeremy", query_fields, "rating:<-2.78", facets, sort_fields_desc, {0},
      10, 1, FREQUENCY, {false});
  ...
  results_op =
      coll_array_fields->search("j", query_fields, "", facets, sort_fields_desc,
                                {0}, 10, 1, MAX_SCORE, {true});
  ASSERT_TRUE(results_op.ok());
  results = results_op.get();
  ASSERT_EQ(5, results["hits"].size());
...
TEST_F(CollectionFilteringTest, FilterOnNegativeNumericalFields) {
  ...
  if (coll1 == nullptr) {
    coll1 =
        collectionManager.create_collection("coll1", 1, fields, "int32_field")
            .get();
  }
  ...
  results = coll1
                ->search("*", {}, "float_field:<0", {},
                         {sort_by("float_field", "desc")}, {0}, 10, 1,
                         FREQUENCY, {true}, 10)
                .get();
...
TEST_F(CollectionFilteringTest, ComparatorsOnMultiValuedNumericalField) {
  ...
  nlohmann::json results =
      coll_array_fields
          ->search("Jeremy", query_fields, "age: [24, >32]", facets,
                   sort_fields_desc, {0}, 10, 1, FREQUENCY, {false})
          .get();
  ...
  results = coll_array_fields
                ->search("Jeremy", query_fields, "age: [<=24, >=44]", facets,
                         sort_fields_desc, {0}, 10, 1, FREQUENCY, {false})
                .get();
...
TEST_F(CollectionFilteringTest, GeoPointFiltering) {
  ...
  results =
      coll1
          ->search("*", {}, "loc: (48.85825332869331, 2.303816427653377, 1 mi)",
                   {}, {}, {0}, 10, 1, FREQUENCY)
          .get();
  ...
  auto gop = coll1->search("*", {}, "loc: (NaN, nan, 1 mi)", {}, {}, {0}, 10, 1,
                           FREQUENCY);

  ASSERT_FALSE(gop.ok());
  ASSERT_EQ(
      "Value of filter field `loc`: must be in the `(-44.50, 170.29, 0.75 km)` "
      "or "
  ...
  ASSERT_FALSE(add_op.ok());
  ASSERT_EQ("Field `loc` must be a 2 element array: [lat, lng].",
            add_op.error());
  ...
  bad_doc["loc"] = {"foo", "bar"};
  add_op =
      coll1->add(bad_doc.dump(), CREATE, "", DIRTY_VALUES::COERCE_OR_REJECT);
  ASSERT_FALSE(add_op.ok());
  ASSERT_EQ("Field `loc` must be a geopoint.", add_op.error());
...
TEST_F(CollectionFilteringTest, GeoPointArrayFiltering) {
  ...
  for (size_t i = 0; i < records.size(); i++) {
    ...
    for (size_t k = 0; k < records[i].size(); k++) {
      ...
      std::vector<double> lat_lng = {std::stod(lat_lng_str[0]),
  ...
  auto results = coll1
                     ->search("*", {}, "loc: (13.12631, 80.20252, 100km)", {},
                              {}, {0}, 10, 1, FREQUENCY)
                     .get();
  ...
  // pick location close to none of the spots
  results = coll1
                ->search("*", {}, "loc: (13.62601, 79.39559, 10 km)", {}, {},
                         {0}, 10, 1, FREQUENCY)
                .get();
  ...
  results = coll1
                ->search("*", {}, "loc: (12.98941, 80.23073, 1mi)", {}, {}, {0},
                         10, 1, FREQUENCY)
                .get();
  ...
  auto add_op = coll1->add(bad_doc.dump(), CREATE, "", DIRTY_VALUES::REJECT);
  ASSERT_FALSE(add_op.ok());
  ASSERT_EQ("Field `loc` must contain 2 element arrays: [ [lat, lng],... ].",
            add_op.error());
...
TEST_F(CollectionFilteringTest, FilteringWithPrefixSearch) {
  ...
  // pick a location close to only the Sacre Coeur
  auto res_op = coll1->search("e", {"title"}, "points: 23", {}, {}, {0}, 10, 1,
                              FREQUENCY, {true});
...
TEST_F(CollectionFilteringTest, NumericalFilteringWithAnd) {
  ...
  auto results =
      coll1
          ->search("*", {}, "num_employees:>=100 && num_employees:<=300", {},
  ...
  results = coll1
                ->search("*", {}, "num_employees:>=100 && num_employees:<=10",
                         {}, sort_fields, {0}, 10, 1, FREQUENCY, {true})
                .get();
  ...
  // check boundaries
  results = coll1
                ->search("*", {}, "num_employees:>=150 && num_employees:<=250",
                         {}, sort_fields, {0}, 10, 1, FREQUENCY, {true})
                .get();
  ...
  results = coll1
                ->search("*", {}, "num_employees:>50 && num_employees:<250", {},
                         sort_fields, {0}, 10, 1, FREQUENCY, {true})
                .get();
  ...
  results = coll1
                ->search("*", {}, "num_employees:>50 && num_employees:<=500",
                         {}, sort_fields, {0}, 10, 1, FREQUENCY, {true})
                .get();
  ...
  // no match
  results = coll1
                ->search("*", {}, "num_employees:>3000 && num_employees:<10",
                         {}, sort_fields, {0}, 10, 1, FREQUENCY, {true})
                .get();
...
TEST_F(CollectionFilteringTest, FilteringViaDocumentIds) {
  ...
  auto results = coll1
                     ->search("*", {}, "id: 123", {}, sort_fields, {0}, 10, 1,
                              FREQUENCY, {true})
                     .get();
  ...
  results = coll1
                ->search("*", {}, "id: 125 && num_employees: 150", {},
                         sort_fields, {0}, 10, 1, FREQUENCY, {true})
                .get();
  ...
  results =
      coll1
          ->search("*", {}, "id: [123, 125, 127, 129] && num_employees: <300",
                   {}, sort_fields, {0}, 10, 1, FREQUENCY, {true})
          .get();
  ...
  results =
      coll1
          ->search("*", {}, "id:= [129, 123, 127, 125] && num_employees: <300",
                   {}, sort_fields, {0}, 10, 1, FREQUENCY, {true})
          .get();
  ...
  auto res_op = coll1->search("*", {}, "id:=", {}, sort_fields, {0}, 10, 1,
                              FREQUENCY, {true});
  ASSERT_FALSE(res_op.ok());
  ASSERT_EQ("Error with filter field `id`: Filter value cannot be empty.",
            res_op.error());

  res_op = coll1->search("*", {}, "id:= ", {}, sort_fields, {0}, 10, 1,
                         FREQUENCY, {true});
  ASSERT_FALSE(res_op.ok());
  ASSERT_EQ("Error with filter field `id`: Filter value cannot be empty.",
            res_op.error());
  ...
  res_op = coll1->search("*", {}, "id:!= [123,125] && num_employees: <300", {},
                         sort_fields, {0}, 10, 1, FREQUENCY, {true});
  ASSERT_FALSE(res_op.ok());
  ASSERT_EQ("Not equals filtering is not supported on the `id` field.",
            res_op.error());

  // when no IDs exist
  results = coll1
                ->search("*", {}, "id: [1000] && num_employees: <300", {},
                         sort_fields, {0}, 10, 1, FREQUENCY, {true})
                .get();
  ...
  results = coll1
                ->search("*", {}, "id: 1000", {}, sort_fields, {0}, 10, 1,
                         FREQUENCY, {true})
                .get();
...
TEST_F(CollectionFilteringTest, NumericalFilteringWithArray) {
  ...
  auto results =
      coll1->search("*", {}, "prices:1", {}, {}, {0}, 10, 1, FREQUENCY, {true})
          .get();
  ...
  results =
      coll1
          ->search("*", {}, "prices:>=1", {}, {}, {0}, 10, 1, FREQUENCY, {true})
          .get();
  ...
  results =
      coll1
          ->search("*", {}, "prices:>=2", {}, {}, {0}, 10, 1, FREQUENCY, {true})
          .get();
  ...
  results =
      coll1->search("*", {}, "prices:<4", {}, {}, {0}, 10, 1, FREQUENCY, {true})
          .get();
...
TEST_F(CollectionFilteringTest, NegationOperatorBasics) {
  ...
  auto results = coll1
                     ->search("*", {"artist"}, "artist:!=Michael Jackson", {},
  ...
  results =
      coll1
          ->search("*", {"artist"}, "artist:!= Michael Jackson && points: >0",
                   {}, {}, {0}, 10, 1, FREQUENCY, {true}, 10)
          .get();
  ASSERT_EQ(2, results["found"].get<size_t>());
  ASSERT_STREQ("3",
  ...
  results =
      coll1
          ->search("*", {"artist"}, "artist:!= [Michael Jackson, Taylor Swift]",
                   {}, {}, {0}, 10, 1, FREQUENCY, {true}, 10)
          .get();
  ASSERT_EQ(1, results["found"].get<size_t>());
  ASSERT_STREQ("3",
  ...
  results = coll1
                ->search("*", {"artist"}, "artist:!=Foobar", {}, {}, {0}, 10, 1,
                         FREQUENCY, {true}, 10)
                .get();
  ASSERT_EQ(4, results["found"].get<size_t>());

  // empty value (bad filtering)
  auto res_op = coll1->search("*", {"artist"}, "artist:!=", {}, {}, {0}, 10, 1,
                              FREQUENCY, {true}, 10);
  ASSERT_FALSE(res_op.ok());
  ASSERT_EQ("Error with filter field `artist`: Filter value cannot be empty.",
            res_op.error());

  res_op = coll1->search("*", {"artist"}, "artist:!= ", {}, {}, {0}, 10, 1,
                         FREQUENCY, {true}, 10);
  ASSERT_FALSE(res_op.ok());
  ASSERT_EQ("Error with filter field `artist`: Filter value cannot be empty.",
            res_op.error());
...
TEST_F(CollectionFilteringTest, FilterStringsWithComma) {
  ...
  std::vector<field> fields = {
      field("place", field_types::STRING, true),
  ...
  auto results = coll1
                     ->search("*", {"place"},
                              "place:= St. John's Cathedral, Denver, Colorado",
                              {}, {}, {0}, 10, 1, FREQUENCY, {true}, 10)
                     .get();
  ...
  results =
      coll1
          ->search("*", {"place"}, "place: [`Cathedral, Denver, Colorado`]", {},
                   {}, {0}, 10, 1, FREQUENCY, {true}, 10)
          .get();
...
TEST_F(CollectionFilteringTest, NumericalRangeFilter) {
  std::vector<field> fields = {
      field("company", field_types::STRING, true),
      field("num_employees", field_types::INT32, false),
  };
  ...
  auto results =
      coll1
          ->search("*", {}, "num_employees:>=100 && num_employees:<=300", {},
                   sort_fields, {0}, 10, 1, FREQUENCY, {true}, 10)
          .get();
...
TEST_F(CollectionFilteringTest, RangeFilterOnTimestamp) {
  ...
  Collection *coll1 =
      collectionManager
          .create_collection("coll1", 1, fields, "", 0, "", {}, {"."})
          .get();
  ...
  auto results = coll1
                     ->search("*", {}, "ts:[1646092800000..1648771199000]", {},
                              {}, {0}, 10, 1, FREQUENCY, {false})
                     .get();
...
TEST_F(CollectionFilteringTest, QueryBoolFields) {
  ...
  std::vector<sort_by> sort_fields = {sort_by("popular", "DESC"),
                                      sort_by("rating", "DESC")};

  coll_bool = collectionManager.get_collection("coll_bool").get();
  if (coll_bool == nullptr) {
    coll_bool =
        collectionManager.create_collection("coll_bool", 1, fields, "rating")
            .get();
  }
  ...
  results = coll_bool
                ->search("the", query_fields, "popular:true", facets,
                         sort_fields, {0}, 10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(3, results["hits"].size());
  ...
  results = coll_bool
                ->search("the", query_fields, "popular:false", facets,
                         sort_fields, {0}, 10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(2, results["hits"].size());
  ...
  Option<nlohmann::json> res_op =
      coll_bool->search("the", query_fields, "bool_array:[true, false]", facets,
                        sort_fields, {0}, 10, 1, FREQUENCY, {false});
  ...
  results = coll_bool
                ->search("the", query_fields, "bool_array: true", facets,
                         sort_fields, {0}, 10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(4, results["hits"].size());
  ids = {"1", "4", "9", "2"};
  ...
  ASSERT_FALSE(res_op.ok());
  ASSERT_EQ(
      "Error with filter field `bool_array`: Filter value cannot be empty.",
      res_op.error());
...
TEST_F(CollectionFilteringTest, FilteringWithTokenSeparators) {
  ...
  Collection *coll2 =
      collectionManager
          .create_collection("coll2", 1, fields, "", 0, "", {"."}, {})
          .get();
...
TEST_F(CollectionFilteringTest, ExactFilteringSingleQueryTerm) {
  ...
  results = coll1
                ->search("*", {}, "tags:=AT&T", {}, {}, {0}, 10, 1, FREQUENCY,
                         {false})
                .get();
  ASSERT_EQ(1, results["hits"].size());
  ASSERT_EQ("1", results["hits"][0]["document"]["id"].get<std::string>());
  ...
  results = coll1
                ->search("*", {}, "tags:=Phone", {}, {}, {0}, 10, 1, FREQUENCY,
                         {false})
                .get();
  ASSERT_EQ(1, results["hits"].size());
  ASSERT_EQ("2", results["hits"][0]["document"]["id"].get<std::string>());
...
TEST_F(CollectionFilteringTest, ExactFilteringRepeatingTokensSingularField) {
  ...
  auto results =
      coll1
          ->search("*", {}, "name:=Cardiology - Interventional Cardiology", {},
                   {}, {0}, 10, 1, FREQUENCY, {false})
          .get();
  ASSERT_EQ(1, results["hits"].size());
  ASSERT_EQ("0", results["hits"][0]["document"]["id"].get<std::string>());
  ...
  results = coll1
                ->search("*", {}, "name:=Cardiology - Interventional", {}, {},
                         {0}, 10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(1, results["hits"].size());
  ASSERT_EQ("1", results["hits"][0]["document"]["id"].get<std::string>());
  ...
  results = coll1
                ->search("*", {}, "name:=Interventional Cardiology", {}, {},
                         {0}, 10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(0, results["hits"].size());

  results = coll1
                ->search("*", {}, "name:=Cardiology", {}, {}, {0}, 10, 1,
                         FREQUENCY, {false})
                .get();
  ASSERT_EQ(0, results["hits"].size());
...
TEST_F(CollectionFilteringTest, ExcludeMultipleTokens) {
  ...
  auto results =
      coll1
          ->search(
              "-TXBT0eiYnFhkJHqz02Wv0PWN5hp1 -3u7RtEn5S9fcnizoUojWUwW23Yf2 "
              "-HpPALvzDDVc3zMmlAAUySwp8Ir33 "
              "-9oF2qhYI8sdBa2xJSerfmntpvBr2 -5fAnLlld5obG4vhhNIbIeoHe1uB2 "
              "-4OlIYKbzwIUoAOYy6dfDzCREezg1 "
              "-4JK1BvoqCuTeMwEZorlKj8hnSl02 -3tQBmRH0AQPEWyoKcDNYJyIxQQe2 "
              "-3Mvl5HZgNwQkHykAqL77oMfo8DW2 "
              "-3Ipnw5JATpYFyCcdUKTBhCicjoH3 -2rizUF2ntNSUVpaXwPdHmSBB6C63 "
              "-2kMHFOUQhAQK9cQbFNoXGpcAFVD2",
...
TEST_F(CollectionFilteringTest,
       FilteringAfterUpsertOnArrayWithTokenSeparators) {
  std::vector<field> fields = {field("name", field_types::STRING, false),
                               field("tags", field_types::STRING_ARRAY, false),
  ...
  auto results = coll1
                     ->search("david", {"name"}, "tags:=[foo-bar-baz]", {}, {},
                              {0}, 10, 1, FREQUENCY, {false})
                     .get();
  ASSERT_EQ(1, results["hits"].size());
  ASSERT_EQ("0", results["hits"][0]["document"]["id"].get<std::string>());
  ...
  results = coll1
                ->search("david", {"name"}, "tags:=[foo-bar-baz]", {}, {}, {0},
                         10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(0, results["hits"].size());

  results = coll1
                ->search("david", {"name"}, "tags:=[bar-foo-baz]", {}, {}, {0},
                         10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(1, results["hits"].size());
  ASSERT_EQ("1", results["hits"][0]["document"]["id"].get<std::string>());
  ...
  results = coll1
                ->search("david", {"name"}, "tag:=[foo-bar-baz]", {}, {}, {0},
                         10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(0, results["hits"].size());
...
TEST_F(CollectionFilteringTest, ComplexFilterQuery) {
  ...
  std::vector<sort_by> sort_fields_desc = {sort_by("rating", "DESC")};
  nlohmann::json results =
      coll->search("Jeremy", {"name"}, "(rating:>=0 && years:>2000) && age:>50",
                   {}, sort_fields_desc, {0}, 10, 1, FREQUENCY, {false})
          .get();
  ASSERT_EQ(0, results["hits"].size());

  results =
      coll->search("Jeremy", {"name"}, "(age:>50 || rating:>5) && years:<2000",
                   {}, sort_fields_desc, {0}, 10, 1, FREQUENCY, {false})
          .get();
  ASSERT_EQ(2, results["hits"].size());
  ...
  results = coll->search("Jeremy", {"name"},
                         "(age:<50 && rating:10) || (years:>2000 && rating:<5)",
                         {}, sort_fields_desc, {0}, 10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(1, results["hits"].size());
  ...
  results = coll->search("Jeremy", {"name"},
                         "years:>2000 && ((age:<30 && rating:>5) || (age:>50 "
                         "&& rating:<5))",
                         {}, sort_fields_desc, {0}, 10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(1, results["hits"].size());
  ...
  auto search_op =
      coll->search("Jeremy", {"name"}, extreme_filter, {}, sort_fields_desc,
                   {0}, 10, 1, FREQUENCY, {false});
...
TEST_F(CollectionFilteringTest, LargeFilterToken) {
  ...
  json.clear();
  std::string token =
      "rade/aols/insolvenzrecht/"
      "persoenliche-risiken-fuer-organe-von-kapitalgesellschaften-gmbh-"
      "geschaeftsfuehrer-ag-vorstand";
  json["uri"] = token;
  auto add_op = coll->add(json.dump());
  ASSERT_TRUE(add_op.ok());

  auto results = coll->search("*", query_fields, "", {}, sort_fields, {0}, 10,
                              1, FREQUENCY, {false})
                     .get();
  ASSERT_EQ(1, results["hits"].size());

  results = coll->search("*", query_fields, "uri:" + token, {}, sort_fields,
                         {0}, 10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(1, results["hits"].size());

  token.erase(
      100);  // Max token length that's indexed is 100, we'll still get a match.
  results = coll->search("*", query_fields, "uri:" + token, {}, sort_fields,
                         {0}, 10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(1, results["hits"].size());
