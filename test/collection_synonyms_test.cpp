...
TEST_F(CollectionSynonymsTest, SynonymParsingFromJson) {
  nlohmann::json syn_json = {
      {"id", "syn-1"}, {"root", "Ocean"}, {"synonyms", {"Sea"}}};
  ...
  nlohmann::json syn_json_without_root = {{"id", "syn-1"},
                                          {"synonyms", {"Sea", "ocean"}}};
  ...
  nlohmann::json syn_json_without_id = {{"root", "Ocean"},
                                        {"synonyms", {"Sea"}}};
  ...
  // synonyms missing
  nlohmann::json syn_json_without_synonyms = {{"id", "syn-1"},
                                              {"root", "Ocean"}};
  ...
  ASSERT_FALSE(syn_op.ok());
  ASSERT_STREQ("Could not find a valid string array of `synonyms`",
               syn_op.error().c_str());

  nlohmann::json syn_json_bad_type3 = {
      {"id", "syn-1"}, {"root", "Ocean"}, {"synonyms", {}}};
  ...
  nlohmann::json syn_json_root_bad_type = {
      {"id", "syn-1"}, {"root", 120}, {"synonyms", {"Sea"}}};
  ...
  nlohmann::json syn_json_bad_symbols = {{"id", "syn-1"},
                                         {"root", "Ocean"},
                                         {"synonyms", {"Sea"}},
                                         {"symbols_to_index", {}}};
  ...
  ASSERT_FALSE(syn_op.ok());
  ASSERT_STREQ("Synonym `symbols_to_index` should be an array of strings.",
               syn_op.error().c_str());

  syn_json_bad_symbols = {{"id", "syn-1"},
                          {"root", "Ocean"},
                          {"synonyms", {"Sea"}},
                          {"symbols_to_index", {"%^"}}};
  ...
  ASSERT_FALSE(syn_op.ok());
  ASSERT_STREQ(
      "Synonym `symbols_to_index` should be an array of single character "
      "symbols.",
      syn_op.error().c_str());
}
...
TEST_F(CollectionSynonymsTest, SynonymReductionOneWay) {
  ...
  std::vector<std::string> red_new_york_tshirts = {"red", "new", "york",
                                                   "tshirt"};
...
TEST_F(CollectionSynonymsTest, SynonymReductionMultiWay) {
  ...
  std::vector<std::string> red_new_york_tshirts = {"united", "states", "of",
                                                   "america"};
  for (size_t i = 0; i < red_new_york_tshirts.size(); i++) {
...
TEST_F(CollectionSynonymsTest, OneWaySynonym) {
  ...
  auto res =
      coll_mul_fields->search("ocean", {"title"}, "", {}, {}, {0}, 10).get();
...
TEST_F(CollectionSynonymsTest, SynonymQueryVariantWithDropTokens) {
  std::vector<field> fields = {
      field("category", field_types::STRING_ARRAY, false),
  ...
  nlohmann::json syn_json = {
      {"id", "syn-1"}, {"root", "us"}, {"synonyms", {"united states"}}};
  ...
  auto res = coll1
                 ->search("us sneakers", {"category", "location"}, "", {}, {},
...
TEST_F(CollectionSynonymsTest, SynonymsTextMatchSameAsRootQuery) {
  ...
  nlohmann::json syn_json = {{"id", "syn-1"},
                             {"root", "ceo"},
  ...
  ASSERT_EQ(res["hits"][1]["text_match"].get<size_t>(),
            res["hits"][0]["text_match"].get<size_t>());
...
TEST_F(CollectionSynonymsTest, MultiWaySynonym) {
  nlohmann::json syn_json = {
      {"id", "syn-1"}, {"synonyms", {"Home Land", "Homeland", "homǝland"}}};
  ...
  auto res =
      coll_mul_fields->search("homǝland", {"title"}, "", {}, {}, {0}, 10).get();
  ASSERT_EQ(0, res["hits"].size());
  ASSERT_EQ(0, res["found"].get<uint32_t>());
  ...
  res = coll_mul_fields
            ->search("samuel leroy jackson", {"starring"}, "", {}, {}, {0}, 10,
                     1, FREQUENCY, {false}, 0)
            .get();
  ASSERT_EQ(0, res["hits"].size());
  ...
  ASSERT_STREQ(
      "<mark>Samuel</mark> <mark>L</mark>. <mark>Jackson</mark>",
      res["hits"][0]["highlights"][0]["snippet"].get<std::string>().c_str());
  ASSERT_STREQ(
      "<mark>Samuel</mark> <mark>L</mark>. <mark>Jackson</mark>",
      res["hits"][1]["highlights"][0]["snippet"].get<std::string>().c_str());

  // for now we don't support synonyms on ANY prefix

  res = coll_mul_fields
            ->search("ler", {"starring"}, "", {}, {}, {0}, 10, 1, FREQUENCY,
                     {true})
            .get();
  ASSERT_EQ(0, res["hits"].size());
  ASSERT_EQ(0, res["found"].get<uint32_t>());
...
TEST_F(CollectionSynonymsTest, ExactMatchVsSynonymMatchCrossFields) {
  ...
  auto res = coll1
                 ->search("cmo", {"title", "description"}, "", {}, {}, {0}, 10,
                          1, FREQUENCY, {false}, 0)
                 .get();
...
TEST_F(CollectionSynonymsTest, SynonymFieldOrdering) {
  // Synonym match on a field earlier in the fields list should rank above exact
  // match of another field
  Collection *coll1;
  ...
  auto res = coll1
                 ->search("laughing", {"title", "description"}, "", {}, {}, {0},
...
TEST_F(CollectionSynonymsTest, DeleteAndUpsertDuplicationOfSynonms) {
  coll_mul_fields->add_synonym(
      R"({"id": "ipod-synonyms", "root": "ipod", "synonyms": ["i pod", "ipod"]})"_json);
  coll_mul_fields->add_synonym(
      R"({"id": "samsung-synonyms", "root": "s3", "synonyms": ["s3 phone", "samsung"]})"_json);
  ...
  ASSERT_STREQ("samsung-synonyms",
               coll_mul_fields->get_synonyms()["samsung-synonyms"].id.c_str());

  // try to upsert synonym with same ID

  auto upsert_op = coll_mul_fields->add_synonym(
      R"({"id": "samsung-synonyms", "root": "s3 smartphone",
...
TEST_F(CollectionSynonymsTest, SynonymSingleTokenExactMatch) {
  ...
  coll1->add_synonym(
      R"({"id": "syn-1", "root": "lulu lemon", "synonyms": ["lululemon"]})"_json);

  auto res = coll1
                 ->search("lulu lemon", {"title"}, "", {}, {}, {2}, 10, 1,
                          FREQUENCY, {true}, 0)
                 .get();
...
TEST_F(CollectionSynonymsTest, SynonymExpansionAndCompressionRanking) {
  ...
  coll1->add_synonym(
      R"({"id": "syn-1", "root": "lululemon", "synonyms": ["lulu lemon"]})"_json);

  auto res = coll1
                 ->search("lululemon", {"title"}, "", {}, {}, {2}, 10, 1,
                          FREQUENCY, {true}, 0)
                 .get();
  ASSERT_EQ(2, res["hits"].size());
  ASSERT_EQ(2, res["found"].get<uint32_t>());

  // Even thought "lulu lemon" has two token synonym match, it should have same
  // text match score as "lululemon" and hence must be tied and then ranked on
  // "points"
  ASSERT_EQ("2", res["hits"][0]["document"]["id"].get<std::string>());
  ASSERT_EQ("1", res["hits"][1]["document"]["id"].get<std::string>());

  ASSERT_EQ(res["hits"][0]["text_match"].get<size_t>(),
            res["hits"][1]["text_match"].get<size_t>());
  ...
  res = coll1
            ->search("lulu lemon", {"title"}, "", {}, {}, {2}, 10, 1, FREQUENCY,
                     {true}, 0)
            .get();
  ASSERT_EQ(2, res["hits"].size());
  ASSERT_EQ(2, res["found"].get<uint32_t>());

  // Even thought "lululemon" has single token synonym match, it should have
  // same text match score as "lulu lemon" and hence must be tied and then
  // ranked on "points"
  ASSERT_EQ("2", res["hits"][0]["document"]["id"].get<std::string>());
  ASSERT_EQ("1", res["hits"][1]["document"]["id"].get<std::string>());
...
TEST_F(CollectionSynonymsTest, SynonymQueriesMustHavePrefixEnabled) {
  ...
  coll1->add_synonym(
      R"({"id": "syn-1", "root": "ns", "synonyms": ["nonstick"]})"_json);

  auto res = coll1
                 ->search("ns cook", {"title"}, "", {}, {}, {2}, 10, 1,
                          FREQUENCY, {true}, 0)
                 .get();
  ASSERT_EQ(1, res["hits"].size());
  ASSERT_EQ(1, res["found"].get<uint32_t>());
  ...
  res = coll1
            ->search("ns cook", {"title"}, "", {}, {}, {2}, 10, 1, FREQUENCY,
                     {false}, 0)
            .get();
  ASSERT_EQ(0, res["hits"].size());
...
TEST_F(CollectionSynonymsTest, HandleSpecialSymbols) {
  ...
  nlohmann::json syn_plus_json = {{"id", "syn-1"},
                                  {"root", "plus"},
                                  {"synonyms", {"+"}},
                                  {"symbols_to_index", {"+"}}};
  ...
  auto res = coll1
                 ->search("plus", {"title"}, "", {}, {}, {0}, 10, 1, FREQUENCY,
                          {true}, 0)
                 .get();
  ASSERT_EQ(1, res["hits"].size());
  ASSERT_EQ("0", res["hits"][0]["document"]["id"].get<std::string>());
...
TEST_F(CollectionSynonymsTest, SynonymForNonAsciiLanguage) {
  ...
  nlohmann::json syn_plus_json = {
      {"id", "syn-1"}, {"root", "எல்லோருக்கும்"}, {"synonyms", {"அனைவருக்கும்"}}};

  ASSERT_TRUE(coll1->add_synonym(syn_plus_json).ok());

  auto res = coll1
                 ->search("எல்லோருக்கும்", {"title"}, "", {}, {}, {0}, 10, 1,
                          FREQUENCY, {true}, 0)
                 .get();
  ASSERT_EQ(1, res["hits"].size());
  ASSERT_EQ("0", res["hits"][0]["document"]["id"].get<std::string>());
...
TEST_F(CollectionSynonymsTest, SynonymForKorean) {
  ...
  auto res = coll1
                 ->search("도쿄구울", {"title"}, "", {}, {}, {0}, 10, 1,
                          FREQUENCY, {true}, 0)
                 .get();
  ASSERT_EQ(3, res["hits"].size());

  res = coll1
            ->search("도쿄 구울", {"title"}, "", {}, {}, {0}, 10, 1, FREQUENCY,
                     {true}, 0)
            .get();
  ASSERT_EQ(3, res["hits"].size());

  res = coll1
            ->search("구울", {"title"}, "", {}, {}, {0}, 10, 1, FREQUENCY,
                     {true}, 0)
            .get();
  ASSERT_EQ(3, res["hits"].size());
}
