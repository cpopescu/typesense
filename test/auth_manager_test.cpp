...
class AuthManagerTest : public ::testing::Test {
 protected:
  ...
  virtual void TearDown() { delete store; }
};
...
TEST_F(AuthManagerTest, CreateListDeleteAPIKeys) {
  ...
  api_key_t api_key1("abcd1", "test key 1", {"read", "write"},
                     {"collection1", "collection2"}, FUTURE_TS);
...
TEST_F(AuthManagerTest, VerifyAuthentication) {
  ...
  ASSERT_FALSE(auth_manager.authenticate(
      "", {collection_key_t("", "jdlaslasdasd")}, sparams, embedded_params));

  // wildcard permission
  api_key_t wildcard_all_key =
      api_key_t("abcd1", "wildcard all key", {"*"}, {"*"}, FUTURE_TS);
  auth_manager.create_key(wildcard_all_key);

  ASSERT_FALSE(auth_manager.authenticate(
      "documents:create", {collection_key_t("collection1", "jdlaslasdasd")},
      sparams, embedded_params));
  ASSERT_TRUE(auth_manager.authenticate(
      "metrics:get", {collection_key_t("", wildcard_all_key.value)}, sparams,
      embedded_params));

  // long API key
  std::string long_api_key_str = StringUtils::randstring(50);
  api_key_t long_api_key =
      api_key_t(long_api_key_str, "long api key", {"*"}, {"*"}, FUTURE_TS);
  auth_manager.create_key(long_api_key);

  ASSERT_TRUE(auth_manager.authenticate(
      "metrics:get",
      {collection_key_t(long_api_key_str, wildcard_all_key.value)}, sparams,
      embedded_params));

  // wildcard on a collection
  api_key_t wildcard_coll_key = api_key_t("abcd2", "wildcard coll key", {"*"},
                                          {"collection1"}, FUTURE_TS);
  auth_manager.create_key(wildcard_coll_key);

  ASSERT_FALSE(auth_manager.authenticate(
      "documents:create", {collection_key_t("collection1", "adasda")}, sparams,
      embedded_params));
  ASSERT_TRUE(auth_manager.authenticate(
      "documents:get",
      {collection_key_t("collection1", wildcard_coll_key.value)}, sparams,
      embedded_params));
  ASSERT_FALSE(auth_manager.authenticate(
      "documents:get",
      {collection_key_t("collection2", wildcard_coll_key.value)}, sparams,
      embedded_params));

  // wildcard on multiple collections
  api_key_t wildcard_colls_key =
      api_key_t("abcd3", "wildcard coll key", {"*"},
                {"collection1", "collection2", "collection3"}, FUTURE_TS);
  auth_manager.create_key(wildcard_colls_key);

  ASSERT_TRUE(auth_manager.authenticate(
      "documents:get",
      {collection_key_t("collection1", wildcard_colls_key.value)}, sparams,
      embedded_params));
  ASSERT_TRUE(auth_manager.authenticate(
      "documents:search",
      {collection_key_t("collection2", wildcard_colls_key.value)}, sparams,
      embedded_params));
  ASSERT_TRUE(auth_manager.authenticate(
      "documents:create",
      {collection_key_t("collection3", wildcard_colls_key.value)}, sparams,
      embedded_params));
  ASSERT_FALSE(auth_manager.authenticate(
      "documents:get",
      {collection_key_t("collection4", wildcard_colls_key.value)}, sparams,
      embedded_params));
  ASSERT_FALSE(auth_manager.authenticate(
      "documents:get", {collection_key_t("*", wildcard_colls_key.value)},
      sparams, embedded_params));

  // only 1 action on multiple collections
  api_key_t one_action_key =
      api_key_t("abcd4", "one action key", {"documents:search"},
                {"collection1", "collection2"}, FUTURE_TS);
  auth_manager.create_key(one_action_key);

  ASSERT_TRUE(auth_manager.authenticate(
      "documents:search",
      {collection_key_t("collection1", one_action_key.value)}, sparams,
      embedded_params));
  ASSERT_FALSE(auth_manager.authenticate(
      "documents:get", {collection_key_t("collection2", one_action_key.value)},
      sparams, embedded_params));
  ASSERT_FALSE(auth_manager.authenticate(
      "documents:search",
      {collection_key_t("collection5", one_action_key.value)}, sparams,
      embedded_params));
  ASSERT_FALSE(auth_manager.authenticate(
      "*", {collection_key_t("collection2", one_action_key.value)}, sparams,
      embedded_params));

  // multiple actions on multiple collections
  api_key_t mul_acoll_key =
      api_key_t("abcd5", "multiple action/collection key",
                {"documents:get", "collections:list"},
                {"metacollection", "collection2"}, FUTURE_TS);
  auth_manager.create_key(mul_acoll_key);

  ASSERT_TRUE(auth_manager.authenticate(
      "documents:get",
      {collection_key_t("metacollection", mul_acoll_key.value)}, sparams,
      embedded_params));
  ASSERT_TRUE(auth_manager.authenticate(
      "collections:list",
      {collection_key_t("collection2", mul_acoll_key.value)}, sparams,
      embedded_params));
  ASSERT_TRUE(auth_manager.authenticate(
      "collections:list",
      {collection_key_t("metacollection", mul_acoll_key.value)}, sparams,
  ...
  ASSERT_FALSE(auth_manager.authenticate(
      ...
      embedded_params));
  ASSERT_FALSE(auth_manager.authenticate(
      "documents:get", {collection_key_t("collection5", mul_acoll_key.value)},
      sparams, embedded_params));
  ASSERT_FALSE(auth_manager.authenticate(
      "*", {collection_key_t("*", mul_acoll_key.value)}, sparams,
      embedded_params));

  // regexp match

  api_key_t regexp_colls_key1 =
      api_key_t("abcd6", "regexp coll key", {"*"}, {"coll.*"}, FUTURE_TS);
  auth_manager.create_key(regexp_colls_key1);
  ASSERT_TRUE(auth_manager.authenticate(
      "collections:list",
      {collection_key_t("collection2", regexp_colls_key1.value)}, sparams,
      embedded_params));
  ASSERT_TRUE(auth_manager.authenticate(
      "documents:get",
      {collection_key_t("collection5", regexp_colls_key1.value)}, sparams,
      embedded_params));

  api_key_t regexp_colls_key2 =
      api_key_t("abcd7", "regexp coll key", {"*"}, {".*meta.*"}, FUTURE_TS);
  auth_manager.create_key(regexp_colls_key2);
  ASSERT_TRUE(auth_manager.authenticate(
      "collections:list",
      {collection_key_t("metacollection", regexp_colls_key2.value)}, sparams,
      embedded_params));
  ASSERT_TRUE(auth_manager.authenticate(
      "collections:list",
      {collection_key_t("ametacollection", regexp_colls_key2.value)}, sparams,
      embedded_params));

  // check for expiry

  api_key_t expired_key1 =
      api_key_t("abcd8", "expiry key", {"*"}, {"*"}, 1606542716);
  auth_manager.create_key(expired_key1);
  ASSERT_FALSE(auth_manager.authenticate(
      "collections:list", {collection_key_t("collection", expired_key1.value)},
      sparams, embedded_params));

  api_key_t unexpired_key1 =
      api_key_t("abcd9", "expiry key", {"*"}, {"*"}, 2237712220);
  auth_manager.create_key(unexpired_key1);
  ASSERT_TRUE(auth_manager.authenticate(
      "collections:list",
      {collection_key_t("collection", unexpired_key1.value)}, sparams,
      embedded_params));

  // wildcard action on any collection
  api_key_t wildcard_action_coll_key =
      api_key_t("abcd10", "wildcard coll action key", {"collections:*"}, {"*"},
                FUTURE_TS);
  auth_manager.create_key(wildcard_action_coll_key);

  ASSERT_TRUE(auth_manager.authenticate(
      "collections:create",
      {collection_key_t("collection1", wildcard_action_coll_key.value)},
      sparams, embedded_params));
  ASSERT_TRUE(auth_manager.authenticate(
      "collections:delete",
      {collection_key_t("collection1", wildcard_action_coll_key.value),
       collection_key_t("collection2", wildcard_action_coll_key.value)},
      sparams, embedded_params));
  ASSERT_FALSE(auth_manager.authenticate(
      "documents:create",
      {collection_key_t("collection1", wildcard_action_coll_key.value)},
  ...
  api_key_t create_action_coll_key =
      api_key_t("abcd11", "create action+coll key", {"collections:create"},
                {"collection1"}, FUTURE_TS);
  auth_manager.create_key(create_action_coll_key);

  ASSERT_TRUE(auth_manager.authenticate(
      "collections:create",
      {collection_key_t("collection1", create_action_coll_key.value)}, sparams,
      embedded_params));
  ASSERT_FALSE(auth_manager.authenticate(
      "collections:create",
      {collection_key_t("collection2", create_action_coll_key.value)}, sparams,
      embedded_params));

  // two keys against 2 different collections: both should be valid
  api_key_t coll_a_key =
      api_key_t("coll_a", "one action key", {"documents:search"},
                {"collectionA"}, FUTURE_TS);
  api_key_t coll_b_key =
      api_key_t("coll_b", "one action key", {"documents:search"},
                {"collectionB"}, FUTURE_TS);
  ...
  api_key_t coll_c_key = api_key_t(
      "coll_c", "one action key", {"documents:search"}, {"*coll_c"}, FUTURE_TS);
...
TEST_F(AuthManagerTest, GenerationOfAPIAction) {
  route_path rpath_search =
      route_path("GET", {"collections", ":collection", "documents", "search"},
                 nullptr, false, false);
  route_path rpath_multi_search =
      route_path("POST", {"multi_search"}, nullptr, false, false);
  route_path rpath_coll_create =
      route_path("POST", {"collections"}, nullptr, false, false);
  route_path rpath_coll_get =
      route_path("GET", {"collections", ":collection"}, nullptr, false, false);
  route_path rpath_coll_list =
      route_path("GET", {"collections"}, nullptr, false, false);
  route_path rpath_keys_post =
      route_path("POST", {"keys"}, nullptr, false, false);
  route_path rpath_doc_delete =
      route_path("DELETE", {"collections", ":collection", "documents", ":id"},
                 nullptr, false, false);
  route_path rpath_override_upsert =
      route_path("PUT", {"collections", ":collection", "overrides", ":id"},
                 nullptr, false, false);
  route_path rpath_doc_patch =
      route_path("PATCH", {"collections", ":collection", "documents", ":id"},
                 nullptr, false, false);
...
TEST_F(AuthManagerTest, ScopedAPIKeys) {
  ...
  api_key_t key_search_coll1("KeyVal", "test key", {"documents:search"},
                             {"coll1"}, FUTURE_TS);
  ...
  ASSERT_FALSE(auth_manager.authenticate(
      "documents:search", {collection_key_t("coll2", scoped_key)}, params,
      embedded_params));

  // should scope to search action only
  ASSERT_FALSE(auth_manager.authenticate(
      "documents:create", {collection_key_t("coll1", scoped_key)}, params,
      embedded_params));

  // check with corrupted key
  ASSERT_FALSE(auth_manager.authenticate(
      "documents:search", {collection_key_t("coll1", "asdasasd")}, params,
      embedded_params));

  // with multiple collections, all should be authenticated
  ASSERT_FALSE(
      auth_manager.authenticate("documents:search",
                                {collection_key_t("coll1", scoped_key),
  ...
  ASSERT_TRUE(auth_manager.authenticate("documents:search",
                                        {collection_key_t("coll1", scoped_key)},
                                        empty_params, embedded_params));
  ASSERT_EQ("user_id:1080", embedded_params[0]["filter_by"].get<std::string>());

  // when more than a single key prefix matches, must pick the correct
  // underlying key
  embedded_params.clear();
  embedded_params.push_back(nlohmann::json::object());
  api_key_t key_search_coll2("KeyVal2", "test key", {"documents:search"},
                             {"coll2"}, FUTURE_TS);
  ...
  ASSERT_TRUE(auth_manager.authenticate("documents:search",
                                        ...
                                        empty_params, embedded_params));
  ASSERT_FALSE(auth_manager.authenticate(
      "documents:search", {collection_key_t("coll2", scoped_key)}, empty_params,
      embedded_params));

  // scoped key generated from key_search_coll2
  embedded_params.clear();
  embedded_params.push_back(nlohmann::json::object());
  std::string scoped_key_prefix2 =
      "QmNlNXdkUThaeDJFZXNiOXB4VUFCT1BmN01GSEJnRUdiMng2aTJESjJqND1LZXlWeyJmaWx0"
      "ZXJfYnkiOiAidXNlcl9pZDoxMDgwIn0=";
  ASSERT_TRUE(auth_manager.authenticate(
      "documents:search", {collection_key_t("coll2", scoped_key_prefix2)},
      empty_params, embedded_params));
  ASSERT_FALSE(auth_manager.authenticate(
      "documents:search", {collection_key_t("coll1", scoped_key_prefix2)},
      empty_params, embedded_params));

  // should only allow scoped API keys derived from parent key with
  ...
  std::string scoped_key2 = StringUtils::base64_encode(
      "BXbsk+xLT1gxOjDyip6+PE4MtOzOm/H7kbkN1d/j/s4=Admi{\"filter_by\": "
      "\"user_id:1080\"}");
  ASSERT_FALSE(auth_manager.authenticate(
      "documents:search", {collection_key_t("coll2", scoped_key2)},
      empty_params, embedded_params));

  // expiration of scoped api key

  // {"filter_by": "user_id:1080", "expires_at": 2237712220} (NOT expired)
  api_key_t key_expiry("ExpireKey", "expire key", {"documents:search"}, {"*"},
                       FUTURE_TS);
  ...
  std::string scoped_key3 =
      "K1M2STRDelZYNHpxNGVWUTlBTGpOWUl4dk8wNU8xdnVEZi9aSUcvZE5tcz1FeHBpeyJmaWx0"
      "ZXJfYnkiOi"
      "AidXNlcl9pZDoxMDgwIiwgImV4cGlyZXNfYXQiOiAyMjM3NzEyMjIwfQ==";

  ASSERT_TRUE(auth_manager.authenticate(
      "documents:search", {collection_key_t("coll1", scoped_key3)},
      empty_params, embedded_params));
  ASSERT_EQ("user_id:1080", embedded_params[0]["filter_by"].get<std::string>());
  ...
  api_key_t key_expiry2("ExpireKey2", "expire key", {"documents:search"}, {"*"},
                        FUTURE_TS);
  ...
  std::string scoped_key4 =
      "SXFKNldZZWRiWkVKVmI2RCt3OTlKNHpBZ24yWlRUbEdJdERtTy9IZ2REZz1FeHBpeyJmaWx0"
      "ZXJfYnkiOiAidXN"
      "lcl9pZDoxMDgwIiwgImV4cGlyZXNfYXQiOiAxNjA2NTYzMzE2fQ==";

  ASSERT_FALSE(auth_manager.authenticate(
      "documents:search", {collection_key_t("coll1", scoped_key4)},
      empty_params, embedded_params));

  // {"filter_by": "user_id:1080", "expires_at": 64723363200} (greater than
  // parent key expiry) embedded key's param cannot exceed parent's expiry

  api_key_t key_expiry3("ExpireKey3", "expire key", {"documents:search"}, {"*"},
                        1606563841);
  ...
  std::string scoped_key5 =
      "V3JMNFJlZHRMVStrZHphNFVGZDh4MWltSmx6Yzk2R3QvS2ZwSE8weGRWQT1FeHBpeyJmaWx0"
      "ZXJfYnkiOiAidX"
      "Nlcl9pZDoxMDgwIiwgImV4cGlyZXNfYXQiOiA2NDcyMzM2MzIwMH0=";

  ASSERT_FALSE(auth_manager.authenticate(
      "documents:search", {collection_key_t("coll1", scoped_key5)},
      empty_params, embedded_params));

  // bad scoped API key
  ASSERT_FALSE(auth_manager.authenticate(
      "documents:search",
      {collection_key_t("coll1", " XhsdBdhehdDheruyhvbdhwjhHdhgyeHbfheR")},
      empty_params, embedded_params));
  ASSERT_FALSE(auth_manager.authenticate(
      "documents:search",
      {collection_key_t("coll1",
                        "cXYPvkNKRlQrBzVTEgY4a3FrZfZ2MEs4kFJ6all3eldwM "
                        "GhKZnRId3Y3TT1RZmxZeYJmaWx0ZXJfYnkiOkJ1aWQ6OElVm1lUVm1"
                        "5SG9ZOHM4NUx2VFk4S2drNHJIMiJ9")},
      empty_params, embedded_params));
  ASSERT_FALSE(auth_manager.authenticate(
      "documents:search",
      {collection_key_t("coll1",
                        "SXZqcVdOWjVNNUVsY3ZiTW9YajQ1QnhrUXJaRzRaS0VhTlFvUmlvQ3"
                        "gycz1LZXlWeyJmaWx0ZXJfYnkiOiAidXNlcl9pZDoxMDgw In0=")},
      empty_params, embedded_params));
}
...
TEST_F(AuthManagerTest, ValidateBadKeyProperties) {
  ...
  ASSERT_FALSE(validate_op.ok());
  ASSERT_STREQ("Key description must be a string.",
               validate_op.error().c_str());
}