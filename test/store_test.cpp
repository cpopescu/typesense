...
TEST(StoreTest, GetUpdatesSince) {
  ...
  system(("rm -rf " + primary_store_path + " && mkdir -p " + primary_store_path)
             .c_str());
  ...
  Option<std::vector<std::string>*> updates_op =
      primary_store.get_updates_since(0, 10);
  ...
  ASSERT_EQ(
      "Unable to fetch updates. Master's latest sequence number is 0 but "
      "requested sequence number is 2",
      updates_op.error());
  ...
  system(("rm -rf " + replica_store_path + " && mkdir -p " + replica_store_path)
             .c_str());
  ...
  for (const std::string& update : *updates_op.get()) {
    ...
    const std::string update_decoded =
        StringUtils::base64_decode(update_encoded);
  ...
  ASSERT_FALSE(updates_op.ok());
  ASSERT_EQ(
      "Unable to fetch updates. Master's latest sequence number is 3 but "
      "requested sequence number is 50",
      updates_op.error());
}
...
TEST(StoreTest, GetUpdateSinceInvalidIterator) {
  ...
  Option<std::vector<std::string>*> updates_op =
      primary_store.get_updates_since(2, 10);
  ASSERT_FALSE(updates_op.ok());
  ASSERT_EQ(
      "Invalid iterator. Master's latest sequence number is 4 but updates are "
      "requested from sequence number 2. "
      "The master's WAL entries might have expired (they are kept only for 24 "
      "hours).",
      updates_op.error());
}
