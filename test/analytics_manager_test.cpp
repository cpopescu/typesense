...
TEST_F(AnalyticsManagerTest, AddSuggestion) {
  ...
  Collection* titles_coll =
      collectionManager.create_collection(titles_schema).get();
  ...
  Collection* suggestions_coll =
      collectionManager.create_collection(suggestions_schema).get();
  ...
  auto userQueries =
      popularQueries["top_queries"]->get_user_prefix_queries()["1"];
...
TEST_F(AnalyticsManagerTest, GetAndDeleteSuggestions) {
  ...
  ASSERT_EQ(
      "top_queriesUpdated",
      existing_rule["params"]["destination"]["collection"].get<std::string>());

  // reject when upsert is not enabled
  create_op = analyticsManager.create_rule(analytics_rule, false, true);
  ASSERT_FALSE(create_op.ok());
  ASSERT_EQ(
      "There's already another configuration with the name "
      "`top_search_queries2`.",
      create_op.error());
