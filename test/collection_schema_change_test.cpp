...
TEST_F(CollectionSchemaChangeTest, AddNewFieldsToCollection) {
  ...
  results = coll1
                ->search("eriment", {"tags"}, "", {}, {}, {0}, 3, 1, FREQUENCY,
                         {true}, 5, spp::sparse_hash_set<std::string>(),
  ...
  results = coll1
                ->search("*", {}, "", {}, sort_fields, {0}, 3, 1, FREQUENCY,
                         {true}, 5, spp::sparse_hash_set<std::string>(),
                         spp::sparse_hash_set<std::string>(), 10, "", 30, 4,
                         "title", 20, {}, {}, {}, 0, "<mark>", "</mark>", {},
                         1000, true, false, true, "", false, 6000 * 1000, 4, 7,
  ...
  results =
      coll1
          ->search("*", {}, "quantity: 100", {"quantity"}, {}, {0}, 3, 1,
  ...
  ASSERT_STREQ("100", results["facet_counts"][0]["counts"][0]["value"]
                          .get<std::string>()
                          .c_str());
  ...
  results =
      coll1
          ->search("*", {}, "on_sale_bool: true", {}, {}, {0}, 3, 1, FREQUENCY,
                   {true}, 5, spp::sparse_hash_set<std::string>(),
                   spp::sparse_hash_set<std::string>(), 10, "", 30, 4, "title",
                   20, {}, {}, {}, 0, "<mark>", "</mark>", {}, 1000, true,
                   false, true, "", false, 6000 * 1000, 4, 7, fallback, 4,
  ...
  ASSERT_TRUE(coll1->add(doc.dump()).ok());
  results = coll1
                ->search("*", {}, "foobar: 123", {}, {}, {0}, 3, 1, FREQUENCY,
                         {true}, 5, spp::sparse_hash_set<std::string>(),
                         spp::sparse_hash_set<std::string>(), 10, "", 30, 4,
                         "title", 20, {}, {}, {}, 0, "<mark>", "</mark>", {},
                         1000, true, false, true, "", false, 6000 * 1000, 4, 7,
...
TEST_F(CollectionSchemaChangeTest, DropFieldsFromCollection) {
  ...
  Collection* coll1 =
      collectionManager
          .create_collection("coll1", 1, fields, "points", 0, "auto")
          .get();
  ...
  auto results =
      coll1->search("*", {}, "", {}, {}, {0}, 3, 1, FREQUENCY, {true}, 5).get();
  ...
  auto res_op = coll1->search("quick", {"title"}, "", {}, {}, {0}, 3, 1,
                              FREQUENCY, {true}, 5);
  ASSERT_FALSE(res_op.ok());
  ASSERT_EQ("Could not find a field named `title` in the schema.",
            res_op.error());
  ...
  res_op = coll1->search("quick", {"title"}, "", {}, {}, {0}, 3, 1, FREQUENCY,
                         {true}, 5);
  ASSERT_FALSE(res_op.ok());
  ASSERT_EQ("Could not find a field named `title` in the schema.",
            res_op.error());
...
TEST_F(CollectionSchemaChangeTest, AlterValidations) {
  ...
  Collection* coll1 =
      collectionManager.create_collection("coll1", 1, fields, "points", 0, "")
          .get();
  ...
  ASSERT_FALSE(alter_op.ok());
  ASSERT_EQ(
      "Field `title` is already part of the schema: To change this field, drop "
  ...
  ASSERT_FALSE(alter_op.ok());
  ASSERT_EQ(
      "Wrong format for `fields`. It should be an array of objects containing "
      "`name`, `type`, "
      "`optional` and `facet` properties.",
      alter_op.error());
  ...
  alter_op = coll1->alter(schema_changes);
  ASSERT_FALSE(alter_op.ok());
  ASSERT_EQ("Field `title` must have a drop value of `true`.",
            alter_op.error());
  ...
  ASSERT_FALSE(alter_op.ok());
  ASSERT_EQ(
      "Schema change is incompatible with the type of documents already stored "
      "in this collection. "
      "Existing data for field `desc` cannot be coerced into an int32.",
      alter_op.error());
  ...
  ASSERT_FALSE(alter_op.ok());
  ASSERT_EQ(
      "Field `desc` has been declared in the schema, but is not found in the "
      "documents already present "
      "in the collection. If you still want to add this field, set it as "
      "`optional: true`.",
      alter_op.error());
  ...
  ASSERT_FALSE(alter_op.ok());
  ASSERT_EQ(
      "The `fields` value should be an array of objects containing the field "
      "`name` "
  ...
  ASSERT_FALSE(alter_op.ok());
  ASSERT_EQ(
      "Field `title` has an invalid data type `foobar`, see docs for supported "
      "data types.",
      alter_op.error());
...
TEST_F(CollectionSchemaChangeTest,
       DropPropertyShouldNotBeAllowedInSchemaCreation) {
  ...
  ASSERT_EQ(
      "Invalid property `drop` on field `title`: it is allowed only during "
      "schema update.",
      coll1_op.error());
...
TEST_F(CollectionSchemaChangeTest, AbilityToDropAndReAddIndexAtTheSameTime) {
  ...
  ASSERT_EQ(
      "Schema change is incompatible with the type of documents already stored "
      "in this collection. "
      "Existing data for field `title` cannot be coerced into an int32.",
      alter_op.error());

  // existing data should not have been touched
  auto res = coll1
                 ->search("he", {"title"}, "", {}, {}, {0}, 10, 1, FREQUENCY,
                          {true}, 10)
                 .get();
  ...
  res = coll1->search("*", {}, "", {"title"}, {}, {0}, 3, 1, FREQUENCY, {true})
            .get();
  ...
  ASSERT_EQ("Hello",
            res["facet_counts"][0]["counts"][0]["value"].get<std::string>());
...
TEST_F(CollectionSchemaChangeTest, AddAndDropFieldImmediately) {
  ...
  results = coll1
                ->search("*", {}, "quantity_int: 1000", {}, {}, {0}, 3, 1,
                         FREQUENCY, {true}, 5)
                .get();
  ASSERT_EQ(1, results["found"].get<size_t>());
  ...
  ASSERT_EQ(
      "Schema change is incompatible with the type of documents already stored "
      "in this collection. "
      "Existing data for field `some_txt` cannot be coerced into an int32.",
      alter_op.error());
...
TEST_F(CollectionSchemaChangeTest, AddDynamicFieldMatchingMultipleFields) {
  ...
  results = coll1
                ->search("*", {}, "", {"year_int"}, {}, {0}, 3, 1, FREQUENCY,
                         {true}, 5)
                .get();
  ASSERT_EQ(1, results["found"].get<size_t>());
  ASSERT_EQ(1, results["facet_counts"].size());
...
TEST_F(CollectionSchemaChangeTest, DropFieldNotExistingInDocuments) {
  ...
  std::vector<field> fields = {
      field("title", field_types::STRING, false, true, true, "", 1, 1),
      field("points", field_types::INT32, true),
  };
...
TEST_F(CollectionSchemaChangeTest, IndexFalseToTrue) {
  ...
  auto res_op = coll1->search("type", {"title"}, "", {"title"}, {}, {0}, 3, 1,
                              FREQUENCY, {true}, 5);
...
TEST_F(CollectionSchemaChangeTest, DropGeoPointArrayField) {
  // when a value is `null` initially, and is altered, subsequent updates should
  // not fail
...
TEST_F(CollectionSchemaChangeTest, DropIntegerFieldAndAddStringValues) {
  ...
  // now we have documents which have both string and integer for the same field
  // :BOOM: schema change operation should not be allowed at this point
  schema_changes = R"({
        "fields": [
  ...
  ASSERT_EQ(
      "Schema change is incompatible with the type of documents already stored "
      "in this collection. "
      "Existing data for field `label` cannot be coerced into an int64.",
      alter_op.error());
  ...
  auto res_op = coll1->search("xyz", {"label"}, "", {}, {}, {0}, 3, 1,
                              FREQUENCY, {true}, 5);
  ASSERT_TRUE(res_op.ok());
  ASSERT_EQ(1, res_op.get()["found"].get<size_t>());
}
...
TEST_F(CollectionSchemaChangeTest, NestedFieldSchemaAdditions) {
  ...
  auto res_op = coll1->search("jack", {"person.name"}, "", {}, {}, {0}, 3, 1,
                              FREQUENCY, {true}, 5);
  ASSERT_TRUE(res_op.ok());
  ASSERT_EQ(1, res_op.get()["found"].get<size_t>());

  res_op = coll1->search("nyc", {"school.city"}, "", {}, {}, {0}, 3, 1,
                         FREQUENCY, {true}, 5);
  ASSERT_TRUE(res_op.ok());
  ASSERT_EQ(1, res_op.get()["found"].get<size_t>());
}
...
TEST_F(CollectionSchemaChangeTest,
       AlterShouldBeAbleToHandleFieldValueCoercion) {
  nlohmann::json schema = R"({
        "name": "coll1",
        "enable_nested_fields": true,
        "fields": [
...
TEST_F(CollectionSchemaChangeTest, GeoFieldSchemaAddition) {
  ...
  // try searching on new fields
  auto res_op =
      coll1->search("*", {}, "location:(22.848641, 89.5406279, 50 km)", {}, {},
                    {0}, 3, 1, FREQUENCY, {true});
  ASSERT_TRUE(res_op.ok());
...
TEST_F(CollectionSchemaChangeTest, DropFieldUsedForEmbedding) {
  ...
  ASSERT_EQ(2, embedding_fields["embedding"]
                   .embed[fields::from]
                   .get<std::vector<std::string>>()
                   .size());
  ASSERT_EQ(1, embedding_fields["embedding2"]
                   .embed[fields::from]
                   .get<std::vector<std::string>>()
                   .size());
  ...
  ASSERT_EQ(1, embedding_fields["embedding"]
                   .embed[fields::from]
                   .get<std::vector<std::string>>()
                   .size());
  ASSERT_EQ("category", embedding_fields["embedding"]
                            .embed[fields::from]
...
TEST_F(CollectionSchemaChangeTest, EmbeddingFieldsMapTest) {
  ...
  ASSERT_EQ("embedding", embedding_field_it.value().name);
  ASSERT_EQ(1, embedding_field_it.value()
                   .embed[fields::from]
                   .get<std::vector<std::string>>()
                   .size());
  ASSERT_EQ("name", embedding_field_it.value()
                        .embed[fields::from]
                        .get<std::vector<std::string>>()[0]);
