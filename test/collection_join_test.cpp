...
TEST_F(CollectionJoinTest, SchemaReferenceField) {
  ...
  ASSERT_EQ("Wildcard field cannot have a reference.",
  ...
  ASSERT_FALSE(collection_create_op.ok());
  ASSERT_EQ("Field `.*` cannot be a reference field.",
            collection_create_op.error());
...
TEST_F(CollectionJoinTest, IndexDocumentHavingReferenceField) {
  ...
  auto collection_create_op =
      collectionManager.create_collection(customers_schema_json);
  ASSERT_TRUE(collection_create_op.ok());
  ...
  ASSERT_EQ(
      "Missing the required reference field `reference_id` in the document.",
      add_doc_op.error());
  ...
  collection_create_op =
      collectionManager.create_collection(products_schema_json);
  ASSERT_TRUE(collection_create_op.ok());

  add_doc_op = customer_collection->add(customer_json.dump());
  ASSERT_FALSE(add_doc_op.ok());
  ASSERT_EQ("Referenced field `id` not found in the collection `Products`.",
            add_doc_op.error());
  collectionManager.drop_collection("Customers");
  ...
  ASSERT_FALSE(add_doc_op.ok());
  ASSERT_EQ(
      "Referenced field `product_id` in the collection `Products` must be "
      "indexed.",
      add_doc_op.error());
  ...
  add_doc_op = customer_collection->add(customer_json.dump());
  ASSERT_EQ(
      "Referenced document having `product_id: a` not found in the collection "
      "`Products`.",
      add_doc_op.error());
  ...
  add_doc_op = customer_collection->add(customer_json.dump());
  ASSERT_EQ(
      "Multiple documents having `product_id: product_a` found in the "
      "collection `Products`.",
      add_doc_op.error());
  ...
  ASSERT_EQ(
      customer_collection->get("0").get().count("reference_id_sequence_id"), 1);

  nlohmann::json document;
  // Referenced document's sequence_id must be valid.
  auto get_op =
      collectionManager.get_collection("Products")
          ->get_document_from_store(customer_collection->get("0")
                                        .get()["reference_id_sequence_id"]
                                        .get<uint32_t>(),
                                    document);
...
TEST_F(CollectionJoinTest, FilterByReference_SingleMatch) {
  ...
  auto search_op =
      coll->search("s", {"product_name"}, "$foo:=customer_a", {}, {}, {0}, 10,
  ...
  search_op =
      coll->search("s", {"product_name"}, "$Customers(foo:=customer_a)", {}, {},
                   {0}, 10, 1, FREQUENCY, {true}, Index::DROP_TOKENS_THRESHOLD);
  ASSERT_FALSE(search_op.ok());
  ASSERT_EQ(search_op.error(),
            "Failed to apply reference filter on `Customers` collection: Could "
            "not find a filter field named `foo` in the schema.");

  auto result =
      coll->search("s", {"product_name"},
                   "$Customers(customer_id:=customer_a && product_price:<100)",
  ...
  ASSERT_EQ("soap",
            result["hits"][0]["document"]["product_name"].get<std::string>());
...
TEST_F(CollectionJoinTest, FilterByReference_MultipleMatch) {
  ...
  auto result =
      coll->search("R", {"user_name"}, "$Links(repo_id:=repo_b)", {}, {}, {0},
  ...
  ASSERT_EQ("user_b",
            result["hits"][0]["document"]["user_id"].get<std::string>());
  ASSERT_EQ("user_a",
            result["hits"][1]["document"]["user_id"].get<std::string>());
...
TEST_F(CollectionJoinTest, AndFilterResults_WithReferences) {
  ...
  std::vector<uint32_t> docs = {3, 6}, foo_reference = {7, 4},
                        bar_reference = {6, 12};
  ...
  for (size_t i = 0; i < result.count; i++) {
    ...
    ASSERT_EQ(foo_reference[i],
              result.reference_filter_results["foo"][i].docs[0]);
    ASSERT_EQ(1, result.reference_filter_results["bar"][i].count);
    ASSERT_EQ(bar_reference[i],
              result.reference_filter_results["bar"][i].docs[0]);
  }
}
...
TEST_F(CollectionJoinTest, OrFilterResults_WithReferences) {
  ...
  for (size_t i = 0; i < result1.count; i++) {
    ...
    ASSERT_EQ(foo_reference[i],
              result1.reference_filter_results["foo"][i].docs[0]);
  }
  ...
  // doc_id -> reference_id
  std::map<uint32_t, uint32_t> foo_map = {{3, 6}, {6, 12}, {9, 18}},
                               bar_map = {{0, 10}, {1, 9}, {2, 8},
                                          {3, 7},  {4, 6}, {5, 5},
                                          {6, 4},  {7, 3}, {8, 2}};
  for (size_t i = 0; i < result2.count; i++) {
    ASSERT_EQ(expected[i], result2.docs[i]);
    ...
    if (foo_map.count(i) != 0) {
      ...
      // Reference count should be 0 for the docs that were not present in the a
      // result.
  ...
  c.reference_filter_results["baz"] =
      new reference_filter_result_t[baz_map.size()];
  auto j = 0;
  for (auto i : baz_map) {
    c.docs[j] = i.first;
...
TEST_F(CollectionJoinTest, FilterByNReferences) {
  ...
  auto result =
      coll->search("R", {"user_name"},
                   "$Participants(org_id:=org_a) && $Links(repo_id:=repo_b)",
                   {}, {}, {0}, 10, 1, FREQUENCY, {true},
                   Index::DROP_TOKENS_THRESHOLD)
          .get();
...
TEST_F(CollectionJoinTest, IncludeFieldsByReference_SingleMatch) {
  ...
  ASSERT_FALSE(search_op.ok());
  ASSERT_EQ(
      "Invalid reference in include_fields, expected `$CollectionName(fieldA, "
      "...)`.",
      search_op.error());
  ...
  //    req_params["include_fields"] = "$foo(bar)";
  //    search_op = collectionManager.do_search(req_params, embedded_params,
  //    json_res, now_ts); ASSERT_FALSE(search_op.ok()); ASSERT_EQ("Referenced
  //    collection `foo` not found.", search_op.error());
  //
  //    req_params["include_fields"] = "$Customers(bar)";
  //    search_op = collectionManager.do_search(req_params, embedded_params,
  //    json_res, now_ts); ASSERT_TRUE(search_op.ok());
  //
  ...
  //    ASSERT_EQ("customer_a",
  //    res_obj["hits"][0]["document"].at("customer_id"));
  //
  //    req_params["include_fields"] = "*, $Customers(product_price,
  //    customer_id)"; search_op = collectionManager.do_search(req_params,
  //    embedded_params, json_res, now_ts); ASSERT_TRUE(search_op.ok());
  //
  //    res_obj = nlohmann::json::parse(json_res);
  //    ASSERT_EQ(1, res_obj["found"].get<size_t>());
  //    ASSERT_EQ(1, res_obj["hits"].size());
  ...
  //    ASSERT_EQ(5, res_obj["hits"][0]["document"].size());
  //    ASSERT_EQ(1,
  //    res_obj["hits"][0]["document"].count("product_id_sequence_id"));
}
