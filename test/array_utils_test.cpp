...
TEST(SortedArrayTest, AndScalar) {
  ...
  uint32_t results_size =
      ArrayUtils::and_scalar(arr1, size1, arr2, arr2_len, &results);
...
TEST(SortedArrayTest, OrScalarMergeShouldRemoveDuplicates) {
  ...
  uint32_t results_size =
      ArrayUtils::or_scalar(arr1, size1, arr2, arr2_len, &results);
...
TEST(SortedArrayTest, OrScalarMergeShouldRemoveDuplicatesAtBoundary) {
  ...
  uint32_t *results = nullptr;
  uint32_t results_size =
      ArrayUtils::or_scalar(arr1, size1, arr2, vec2.size(), &results);
...
TEST(SortedArrayTest, OrScalarWithEitherArrayAsNull) {
  ...
  uint32_t *results = nullptr;
  uint32_t results_size =
      ArrayUtils::or_scalar(arr1, size1, nullptr, 0, &results);
  ASSERT_EQ(9, results_size);
...
TEST(SortedArrayTest, FilterArray) {
  ...
  uint32_t *results = nullptr;
  uint32_t results_size =
      ArrayUtils::exclude_scalar(arr1, size1, arr2, vec2.size(), &results);
  ...
  std::vector<uint32_t> filter_ids = {58,  103, 116, 117, 137, 154,
                                      191, 210, 211, 284, 299, 302,
                                      306, 309, 332, 334, 360};
  std::vector<uint32_t> expected_res = {118, 185, 260, 322, 353};

  results_size = ArrayUtils::exclude_scalar(
      &vec3[0], vec3.size(), &filter_ids[0], filter_ids.size(), &results);
