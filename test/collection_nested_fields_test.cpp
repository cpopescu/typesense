...
tsl::htrie_map<char, field> get_nested_map(
...
TEST_F(CollectionNestedFieldsTest, FlattenJSONObject) {
  ...
  ASSERT_TRUE(field::flatten_doc(doc, get_nested_map(nested_fields), {}, false,
                                 flattened_fields)
                  .ok());
  ...
  // handle order of generation differences between compilers (due to iteration
  // of unordered map)
  ...
  nested_fields = {field("company", field_types::OBJECT, false)};
  ...
  flattened_fields.clear();
  doc = nlohmann::json::parse(json_str);
  nested_fields = {field("locations.address", field_types::OBJECT, false)};

  ASSERT_FALSE(field::flatten_doc(doc, get_nested_map(nested_fields), {}, false,
                                  flattened_fields)
                   .ok());  // must be of type object_array
  ...
  flattened_fields.clear();
  doc = nlohmann::json::parse(json_str);
  nested_fields = {field("company.name", field_types::STRING, false)};
...
TEST_F(CollectionNestedFieldsTest, TestNestedArrayField) {
  ...
  nested_fields = {field("employees", field_types::OBJECT, false)};
  ...
  std::sort(flattened_fields.begin(), flattened_fields.end(),
            [](field& a, field& b) { return a.name < b.name; });
...
TEST_F(CollectionNestedFieldsTest, FlattenJSONObjectHandleErrors) {
  ...
  nlohmann::json doc = nlohmann::json::parse(json_str);
  auto flatten_op = field::flatten_doc(doc, get_nested_map(nested_fields), {},
                                       false, flattened_fields);
  ASSERT_FALSE(flatten_op.ok());
  ASSERT_EQ("Field `locations` not found.", flatten_op.error());

  nested_fields = {field("company", field_types::INT32, false)};

  flattened_fields.clear();
  flatten_op = field::flatten_doc(doc, get_nested_map(nested_fields), {}, false,
                                  flattened_fields);
  ASSERT_FALSE(flatten_op.ok());
...
TEST_F(CollectionNestedFieldsTest, FlattenStoredDoc) {
  ...
  schema.emplace("employees.num",
                 field("employees.num", field_types::INT32, false));
  schema.emplace("details.name",
                 field("details.name", field_types::STRING_ARRAY, false));
  schema.emplace("details.year",
                 field("details.year", field_types::INT32_ARRAY, false));
...
TEST_F(CollectionNestedFieldsTest, CompactNestedFields) {
  ...
  tsl::htrie_map<char, field> schema;
  schema.emplace(
      "location_addresses.city",
      field("location_addresses.city", field_types::STRING_ARRAY, true));
  schema.emplace("location_addresses",
                 field("location_addresses", field_types::OBJECT_ARRAY, true));
  schema.emplace("primary_address",
                 field("primary_address", field_types::OBJECT, true));
  schema.emplace("primary_address.city",
                 field("primary_address.city", field_types::STRING, true));
  schema.emplace(
      "location_addresses.street",
      field("location_addresses.street", field_types::STRING_ARRAY, true));
  schema.emplace("primary_address.street",
                 field("primary_address.street", field_types::STRING, true));
...
TEST_F(CollectionNestedFieldsTest, SearchOnFieldsOnWildcardSchema) {
  ...
  auto op = collectionManager.create_collection(
      "coll1", 1, fields, "", 0, field_types::AUTO, {}, {}, true);
  ASSERT_TRUE(op.ok());
  ...
  auto results = coll1
                     ->search("electrician", {"employees"}, "", {}, sort_fields,
  ...
  results = coll1
                ->search("electrician commerce", {"employees", "locations"}, "",
  ...
  // raw document in the store will not have the .flat meta key or actual flat
  // fields
  ...
  // search specific nested fields, only matching field is highlighted by
  // default
  results =
      coll1
          ->search("one shoe", {"locations.address.street", "employees.tags"},
                   "", {}, sort_fields, {0}, 10, 1, FREQUENCY, {true})
          .get();
  ASSERT_EQ(1, results["hits"].size());
  ASSERT_EQ(doc, results["hits"][0]["document"]);
  ...
  auto res_op = coll1->search("one shoe", {"locations.address.str"}, "", {},
                              sort_fields, {0}, 10, 1, FREQUENCY, {true});
  ...
  res_op = coll1->search("one shoe", {"locations.address.foo"}, "", {},
                         sort_fields, {0}, 10, 1, FREQUENCY, {true});
  ASSERT_FALSE(res_op.ok());
  ASSERT_EQ(
      "Could not find a field named `locations.address.foo` in the schema.",
      res_op.error());

  res_op = coll1->search("one shoe", {"locations.foo.street"}, "", {},
                         sort_fields, {0}, 10, 1, FREQUENCY, {true});
  ASSERT_FALSE(res_op.ok());
  ASSERT_EQ(
      "Could not find a field named `locations.foo.street` in the schema.",
      res_op.error());
}
...
TEST_F(CollectionNestedFieldsTest, IncludeExcludeFieldsPruning) {
  ...
  doc = nlohmann::json::parse(doc_str);
  Collection::prune_doc(doc, {"locations.address.city"},
                        tsl::htrie_set<char>());
  ASSERT_EQ(
      R"({"locations":[{"address":{"city":"Beaverton"}},{"address":{"city":"Thornhill"}}]})",
      doc.dump());

  // select 2 fields within nested array object
  doc = nlohmann::json::parse(doc_str);
  Collection::prune_doc(
      doc, {"locations.address.city", "locations.address.products"},
      tsl::htrie_set<char>());
  ASSERT_EQ(
      R"({"locations":[{"address":{"city":"Beaverton","products":["shoes","tshirts"]}},{"address":{"city":"Thornhill","products":["sneakers","shoes"]}}]})",
      doc.dump());
  ...
  Collection::prune_doc(
      ...
      {"locations.address.city"});
  ASSERT_EQ(
      R"({"locations":[{"address":{"products":["shoes","tshirts"]}},{"address":{"products":["sneakers","shoes"]}}]})",
      doc.dump());
}
...
TEST_F(CollectionNestedFieldsTest, HighlightNestedFieldFully) {
  ...
  auto results = coll1
                     ->search("One", {"locations.address"}, "", {}, sort_fields,
  ...
  results = coll1
                ->search("drive", {"locations.address"}, "", {}, sort_fields,
                         {0}, 10, 1, token_ordering::FREQUENCY, {true}, 10,
                         spp::sparse_hash_set<std::string>(),
                         spp::sparse_hash_set<std::string>(), 10, "", 30, 4,
                         "locations.address")
  ...
  results =
      coll1
          ->search("shoes", {"locations.address.products"}, "", {}, sort_fields,
                   {0}, 10, 1, token_ordering::FREQUENCY, {true}, 10,
                   spp::sparse_hash_set<std::string>(),
                   spp::sparse_hash_set<std::string>(), 10, "", 30, 4,
                   "locations.address", 20, {}, {}, {}, 0, "<mark>", "</mark>",
                   {}, 1000, true, false, true, "locations.address")
          .get();
  ...
  results =
      coll1
          ->search("drive",
                   {"company.names", "company_names", "locations.address"}, "",
                   {}, sort_fields, {0}, 10, 1, token_ordering::FREQUENCY,
  ...
  // if highlight fields not provided, only matching sub-fields should appear in
  // highlight
  ...
  results = coll1
                ->search("shoes", {"locations.address.products"}, "", {},
                         sort_fields, {0}, 10, 1, token_ordering::FREQUENCY,
                         {true}, 10, spp::sparse_hash_set<std::string>(),
                         spp::sparse_hash_set<std::string>(), 10, "", 30, 4,
                         "id", 20, {}, {}, {}, 0, "<mark>", "</mark>", {}, 1000,
                         true, false, true, "id")
                .get();
...
TEST_F(CollectionNestedFieldsTest, FieldsWithExplicitSchema) {
  ...
  auto results =
      coll1
          ->search("brown fox", {"details", "locations"}, "", {}, sort_fields,
                   {0}, 10, 1, token_ordering::FREQUENCY, {true}, 10,
                   spp::sparse_hash_set<std::string>(),
                   spp::sparse_hash_set<std::string>(), 10, "", 30, 4)
          .get();
  ...
  results = coll1
                ->search("fix", {"company.name"}, "", {}, sort_fields, {0}, 10,
  ...
  results = coll2
                ->search("brown", {"locations.address.street"}, "", {},
                         sort_fields, {0}, 10, 1, token_ordering::FREQUENCY,
                         {true}, 10, spp::sparse_hash_set<std::string>(),
                         spp::sparse_hash_set<std::string>(), 10, "", 30, 4)
                .get();
  ...
  results = coll3
                ->search("brown", {"locations.address"}, "", {}, sort_fields,
                         {0}, 10, 1, token_ordering::FREQUENCY, {true}, 10,
                         spp::sparse_hash_set<std::string>(),
                         spp::sparse_hash_set<std::string>(), 10, "", 30, 4)
                .get();
  ...
  for (size_t i = 0; i < coll_fields.size(); i++) {
    ...
    if (i <= 2) {
      // original 3 explicit fields will be non-optional, but the sub-properties
      // will be optional
...
TEST_F(CollectionNestedFieldsTest, ExplicitSchemaForNestedArrayTypeValidation) {
  ...
  ASSERT_FALSE(add_op.ok());
  ASSERT_EQ(
      "Field `blocks.text.description` has an incorrect type. "
...
TEST_F(CollectionNestedFieldsTest,
  ...
  auto results =
      coll1
          ->search("*", {}, "company.num_employees: 2000", {}, sort_fields, {0},
  ...
  results =
      coll1
          ->search("*", {}, "company.num_employees: 1000", {}, sort_fields, {0},
                   10, 1, token_ordering::FREQUENCY, {true})
          .get();
  ASSERT_EQ(0, results["found"].get<size_t>());
  ...
  // nested array object
  results = coll1
                ->search("foo", {"details.name"}, "", {}, sort_fields, {0}, 10,
                         1, token_ordering::FREQUENCY, {true})
                .get();
  ASSERT_EQ(1, results["found"].get<size_t>());

  results = coll1
                ->search("bar", {"details.name"}, "", {}, sort_fields, {0}, 10,
                         1, token_ordering::FREQUENCY, {true})
                .get();
  ASSERT_EQ(0, results["found"].get<size_t>());
  ...
  // nested simple array
  results = coll1
                ->search("*", {}, "company.ids: 10", {}, sort_fields, {0}, 10,
                         1, token_ordering::FREQUENCY, {true})
                .get();
  ASSERT_EQ(1, results["found"].get<size_t>());

  results = coll1
                ->search("*", {}, "company.ids: 1", {}, sort_fields, {0}, 10, 1,
...
TEST_F(CollectionNestedFieldsTest, NestedFieldWithExplicitWeight) {
  ...
  auto results =
      coll1
          ->search("college", {"studies"}, "", {}, {}, {0}, 10, 1, FREQUENCY,
...
TEST_F(CollectionNestedFieldsTest, NestedFieldWithGeopointArray) {
  ...
  auto results =
      coll1->search("*", {}, "", {}, {}, {0}, 10, 1, FREQUENCY, {false}, 0)
          .get();
  ASSERT_EQ(1, results["found"].get<size_t>());

  results = coll1
                ->search("*", {}, "addresses.geoPoint: (12.911, 23.5, 1 mi)",
                         {}, {}, {0}, 10, 1, FREQUENCY)
                .get();
  ASSERT_EQ(1, results["found"].get<size_t>());
  ...
  // this technically cannot be allowed but it's really tricky to detect so we
  // allow
  auto doc4 = R"({
        "addresses": {"geoPoint": [1.91, 23.5]}
    })"_json;
  ...
  ASSERT_EQ("Field `addresses.geoPoint` has an incorrect type.",
            create_op.error());
  ...
  ASSERT_FALSE(create_op.ok());
  ASSERT_EQ("Field `addresses.geoPoint` must be an array of geopoint.",
            create_op.error());
}
...
TEST_F(CollectionNestedFieldsTest, NestedFieldWithGeopoint) {
  ...
  results = coll1
                ->search("*", {}, "address.geoPoint: (19.07, 72.882, 1 mi)", {},
                         {}, {0}, 10, 1, FREQUENCY)
                .get();
  ASSERT_EQ(1, results["found"].get<size_t>());
  ...
  create_op = coll1->add(bad_doc.dump(), CREATE);
  ASSERT_FALSE(create_op.ok());
  ASSERT_EQ("Field `address.geoPoint` must be a 2 element array: [lat, lng].",
            create_op.error());
  ...
  ASSERT_FALSE(create_op.ok());
  ASSERT_EQ(
      "Field `address.geoPoint` has an incorrect type. "
      "Hint: field inside an array of objects must be an array type as well.",
      create_op.error());
}
...
TEST_F(CollectionNestedFieldsTest, GroupByOnNestedFieldsWithWildcardSchema) {
  ...
  auto results =
      coll1
          ->search("school", {"education"}, "", {}, {}, {0}, 10, 1, FREQUENCY,
                   {false}, 10, spp::sparse_hash_set<std::string>(),
  ...
  ASSERT_EQ("X High School",
            results["grouped_hits"][0]["group_key"][0][0].get<std::string>());
  ASSERT_EQ("Z University",
            results["grouped_hits"][0]["group_key"][0][1].get<std::string>());
  ASSERT_EQ(1, results["grouped_hits"][0]["hits"].size());
  ASSERT_EQ("1", results["grouped_hits"][0]["hits"][0]["document"]["id"]
                     .get<std::string>());

  ASSERT_EQ(1, results["grouped_hits"][1]["group_key"].size());
  ASSERT_EQ(2, results["grouped_hits"][1]["group_key"][0].size());
  ASSERT_EQ("X High School",
            results["grouped_hits"][1]["group_key"][0][0].get<std::string>());
  ASSERT_EQ("Y University",
            results["grouped_hits"][1]["group_key"][0][1].get<std::string>());
  ASSERT_EQ(1, results["grouped_hits"][1]["hits"].size());
  ASSERT_EQ("0", results["grouped_hits"][1]["hits"][0]["document"]["id"]
                     .get<std::string>());
...
TEST_F(CollectionNestedFieldsTest, WildcardWithExplicitSchema) {
  ...
  auto results = coll1
                     ->search("*", {}, "company.id: 1000", {}, {}, {0}, 10, 1,
                              FREQUENCY, {false})
                     .get();
  ASSERT_EQ(1, results["found"].get<size_t>());

  results = coll1
                ->search("*", {}, "studies.year: 1997", {}, {}, {0}, 10, 1,
                         FREQUENCY, {false})
                .get();
  ASSERT_EQ(1, results["found"].get<size_t>());
}
...
TEST_F(CollectionNestedFieldsTest, UpdateOfNestFields) {
  ...
  auto results =
      coll1->search("*", {}, "", {}, {}, {0}, 10, 1, FREQUENCY, {false}).get();
  ASSERT_EQ(1, results["found"].get<size_t>());
  ASSERT_EQ(6, results["hits"][0]["document"].size());
  ASSERT_EQ(2, results["hits"][0]["document"]["brand"].size());
  ASSERT_EQ("Rempel",
            results["hits"][0]["document"]["brand"]["name"].get<std::string>());
  ...
  ASSERT_FALSE(add_op.ok());
  ASSERT_EQ(
      "Field `brand.id` has been declared in the schema, but is not found in "
      "the document.",
      add_op.error());
...
TEST_F(CollectionNestedFieldsTest, UpdateOfNestFieldsWithWildcardSchema) {
  ...
  auto results = coll1
                     ->search("*", {}, "company.year: 2000", {}, {}, {0}, 10, 1,
                              FREQUENCY, {false})
                     .get();
  ASSERT_EQ(1, results["found"].get<size_t>());

  results = coll1
                ->search("*", {}, "studies.year: 1967", {}, {}, {0}, 10, 1,
                         FREQUENCY, {false})
                .get();
  ASSERT_EQ(1, results["found"].get<size_t>());

  results = coll1
                ->search("*", {}, "studies.year: 1978", {}, {}, {0}, 10, 1,
                         FREQUENCY, {false})
                .get();
  ASSERT_EQ(1, results["found"].get<size_t>());

  results = coll1
                ->search("alpha", {"studies.name"}, "", {}, {}, {0}, 10, 1,
                         FREQUENCY, {false})
                .get();
  ASSERT_EQ(1, results["found"].get<size_t>());

  results = coll1
                ->search("beta", {"studies.name"}, "", {}, {}, {0}, 10, 1,
                         FREQUENCY, {false})
                .get();
  ASSERT_EQ(1, results["found"].get<size_t>());
  ...
  ASSERT_EQ(
      4000,
      results["hits"][0]["document"]["company"]["num_employees"].get<size_t>());
  ASSERT_EQ(1976,
            results["hits"][0]["document"]["company"]["founded"].get<size_t>());
  ASSERT_EQ(1, results["hits"][0]["document"]["studies"].size());
  ASSERT_EQ(1, results["hits"][0]["document"]["studies"][0].size());
  ASSERT_EQ(
      "College Alpha",
      results["hits"][0]["document"]["studies"][0]["name"].get<std::string>());
  ...
  results = coll1
                ->search("*", {}, "company.founded: 1976", {}, {}, {0}, 10, 1,
                         FREQUENCY, {false})
                .get();
  ASSERT_EQ(1, results["found"].get<size_t>());
  ASSERT_EQ(3, results["hits"][0]["document"].size());
  ...
  results = coll1
                ->search("*", {}, "company.num_employees: 2000", {}, {}, {0},
  ...
  ASSERT_EQ(
      ...
      results["hits"][0]["document"]["studies"][0]["name"].get<std::string>());
  ASSERT_EQ(1977,
            results["hits"][0]["document"]["studies"][0]["year"].get<size_t>());
}
...
TEST_F(CollectionNestedFieldsTest, NestedSchemaWithSingularType) {
  ...
  ASSERT_FALSE(add_op.ok());
  ASSERT_EQ(
      "Field `studies.year` has an incorrect type. "
      "Hint: field inside an array of objects must be an array type as well.",
      add_op.error());
...
TEST_F(CollectionNestedFieldsTest, ArrayOfObjectsFaceting) {
  ...
  auto results = coll1
                     ->search("*", {}, "", {"location_addresses.city"}, {}, {0},
                              10, 1, FREQUENCY, {false})
                     .get();
  ...
  results = coll1
                ->search("*", {}, "", {"location_addresses.city"}, {}, {0}, 10,
                         1, FREQUENCY, {false})
                .get();
...
TEST_F(CollectionNestedFieldsTest,
       ErrorWhenObjectTypeUsedWithoutEnablingNestedFields) {
  nlohmann::json schema = R"({
        "name": "coll1",
        "fields": [
          {"name": "details", "type": "object", "optional": false }
  ...
  ASSERT_FALSE(op.ok());
  ASSERT_EQ(
      "Type `object` or `object[]` can be used only when nested fields are "
      "enabled by setting` "
      "enable_nested_fields` to true.",
      op.error());
...
TEST_F(CollectionNestedFieldsTest, FieldsWithDotsButNotNested) {
  ...
  auto results = coll1
                     ->search("beta", {"name.first"}, "", {}, {}, {0}, 10, 1,
                              FREQUENCY, {false})
                     .get();
  ASSERT_EQ(1, results["found"].get<size_t>());
  ASSERT_EQ("Alpha <mark>Beta</mark> Gamma",
            results["hits"][0]["highlight"]["name.first"]["snippet"]
                .get<std::string>());
}
...
TEST_F(CollectionNestedFieldsTest, NullValuesWithExplicitSchema) {
  ...
  ASSERT_EQ("Jack",
            results["hits"][0]["document"]["name"]["first"].get<std::string>());
}
...
TEST_F(CollectionNestedFieldsTest,
       EmplaceWithMissingArrayValueOnOptionalField) {
  nlohmann::json schema = R"({
        "name": "coll1",
        "enable_nested_fields": true,
        "fields": [
  ...
  ASSERT_EQ(
      10000,
      results["hits"][0]["document"]["currency"][0]["us"].get<uint32_t>());
}
...
TEST_F(CollectionNestedFieldsTest, UpdateNestedDocument) {
  ...
  results = coll1
                ->search("gamma", {"title"}, "", {}, {}, {0}, 10, 1, FREQUENCY,
                         {false})
                .get();
  ASSERT_EQ(1, results["found"].get<size_t>());
  ...
  results = coll1
                ->search("shaw", {"contributors"}, "", {}, {}, {0}, 10, 1,
                         FREQUENCY, {false})
                .get();
  ASSERT_EQ(1, results["found"].get<size_t>());

  results = coll1
                ->search("john", {"contributors.first_name"}, "", {}, {}, {0},
                         10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(1, results["found"].get<size_t>());
  ...
  results = coll1
                ->search("galt", {"contributors"}, "", {}, {}, {0}, 10, 1,
                         FREQUENCY, {false})
                .get();
  ASSERT_EQ(0, results["found"].get<size_t>());
}
...
TEST_F(CollectionNestedFieldsTest, UpdateNestedDocumentAutoSchema) {
  ...
  auto results = coll1
                     ->search("us", {"price.country"}, "", {}, {}, {0}, 10, 1,
                              FREQUENCY, {false})
                     .get();
  ASSERT_EQ(1, results["found"].get<size_t>());
}
...
TEST_F(CollectionNestedFieldsTest,
       ImproveErrorMessageForNestedArrayNumericalFields) {
  nlohmann::json schema = R"({
        "name": "coll1",
        "enable_nested_fields": true,
       "fields": [
  ...
  ASSERT_FALSE(add_op.ok());
  ASSERT_EQ(
      "Field `variants.sellingPrice` has an incorrect type. "
      "Hint: field inside an array of objects must be an array type as well.",
      add_op.error());
  ...
  ASSERT_FALSE(add_op.ok());
  ASSERT_EQ(
      "Field `variants.timestamp` has an incorrect type. "
      "Hint: field inside an array of objects must be an array type as well.",
      add_op.error());
  ...
  ASSERT_FALSE(add_op.ok());
  ASSERT_EQ(
      "Field `variants.is_deleted` has an incorrect type. "
      "Hint: field inside an array of objects must be an array type as well.",
      add_op.error());
  ...
  ASSERT_FALSE(add_op.ok());
  ASSERT_EQ(
      "Field `variants.price` has an incorrect type. "
      "Hint: field inside an array of objects must be an array type as well.",
      add_op.error());
}
...
TEST_F(CollectionNestedFieldsTest, HighlightArrayOfObjects) {
  ...
  auto results =
      coll1
          ->search("james", {"details.name"}, "", {}, {}, {0}, 10, 1, FREQUENCY,
...
TEST_F(CollectionNestedFieldsTest, NestedFieldWithRegexName) {
  ...
  auto results = coll1
                     ->search("foobar", {"titles.en"}, "start_date.year: 2020",
...
TEST_F(CollectionNestedFieldsTest, HighlightOnFlatFieldWithSnippeting) {
  ...
  doc1["body"] =
      "on left side of chin under the corner of my mouth i keep getting huge "
      "pimples. they’ll go away for "
      "a few days but come back every time and i don’t quit it. I have oily "
      "skin and acne prone. i "
      "also just started using twice a week";

  ASSERT_TRUE(coll1->add(doc1.dump()).ok());

  auto results = coll1
                     ->search("pimples", {"title", "body"}, "", {}, {}, {2}, 10,
                              1, FREQUENCY, {true})
                     .get();
  ...
  highlight_doc["body"]["value"] =
      "on left side of chin under the corner of my mouth i keep getting huge "
      "<mark>pimples</mark>. they’ll go away for a few days but come back "
      "every time "
      "and i don’t quit it. I have oily skin and acne prone. i also just "
      "started "
      "using twice a week";

  results =
      coll1
          ->search("pimples", {"title", "body"}, "", {}, {}, {2}, 10, 1,
