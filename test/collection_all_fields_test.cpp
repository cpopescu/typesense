...
TEST_F(CollectionAllFieldsTest, IndexDocsWithoutSchema) {
  ...
  auto bad_coll_op =
      collectionManager.create_collection("coll_bad", 1, fields, "", 0, "blah");
  ASSERT_FALSE(bad_coll_op.ok());
  ASSERT_EQ("Field `.*` has an invalid type.", bad_coll_op.error());

  coll1 = collectionManager.get_collection("coll1").get();
  if (coll1 == nullptr) {
    auto coll_op = collectionManager.create_collection("coll1", 1, fields, "",
                                                       0, field_types::AUTO);
    coll1 = coll_op.get();
  }
  ...
  // has schema detection enabled
  std::string dirty_values;
  ASSERT_EQ(DIRTY_VALUES::COERCE_OR_REJECT,
            coll1->parse_dirty_values_option(dirty_values));
  ...
  ASSERT_EQ(DIRTY_VALUES::COERCE_OR_DROP,
            coll1->parse_dirty_values_option(dirty_values));
  ...
  auto results = coll1
                     ->search("will", query_fields, "", facets, sort_fields,
  ...
  results = coll1
                ->search("chris", {"cast"}, "", facets, sort_fields, {0}, 10, 1,
                         FREQUENCY, {false})
                .get();
  ...
  // unable to coerce
  auto doc_json =
      R"({"cast":"William Barnes","points":63,"starring":"Will Ferrell",
  ...
  results = coll1
                ->search("300", {"title"}, "", {}, sort_fields, {0}, 10, 1,
                         FREQUENCY, {false})
                .get();
  ASSERT_EQ(1, results["hits"].size());
  ASSERT_STREQ(
      "300",
      results["hits"][0]["document"]["title"].get<std::string>().c_str());
  ...
  results = coll1
                ->search("With bad cast field", {"title"}, "", {}, sort_fields,
                         {0}, 10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(1, results["hits"].size());
  ASSERT_STREQ(
  ...
  // no coercion should happen, `title` field will just be dropped, but record
  // indexed
  doc_json =
      R"({"cast": ["Jeremy Livingston"],"points":63,"starring":"Will Ferrell",
  ...
  results = coll1
                ->search("1200", {"title"}, "", {}, sort_fields, {0}, 10, 1,
                         FREQUENCY, {false})
                .get();
  ASSERT_EQ(0, results["hits"].size());

  results = coll1
                ->search("Jeremy Livingston", {"cast"}, "", {}, sort_fields,
                         {0}, 10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(1, results["hits"].size());
  ...
  ASSERT_EQ(
      "Could not find a field named `not-found` in the schema for sorting.",
      res_op.error());
...
TEST_F(CollectionAllFieldsTest, CoerceDynamicStringField) {
  ...
  coll1 = collectionManager.get_collection("coll1").get();
  if (coll1 == nullptr) {
    coll1 = collectionManager.create_collection("coll1", 1, fields, "", 0, "")
                .get();
  }
...
TEST_F(CollectionAllFieldsTest, HandleArrayTypes) {
  ...
  if (coll1 == nullptr) {
    coll1 = collectionManager
                .create_collection("coll1", 1, {}, "", 0, field_types::AUTO)
                .get();
  }
  ...
  auto results = coll1
                     ->search("second", {"title"}, "", {}, sort_fields, {0}, 10,
                              1, FREQUENCY, {false})
                     .get();
  ASSERT_EQ(1, results["hits"].size());
  ...
  ASSERT_TRUE(add_op.ok());
  results = coll1
                ->search("third", {"title"}, "", {}, sort_fields, {0}, 10, 1,
                         FREQUENCY, {false})
                .get();
  ASSERT_EQ(1, results["hits"].size());
...
TEST_F(CollectionAllFieldsTest, NonOptionalFieldShouldNotBeDropped) {
  ...
  if (coll1 == nullptr) {
    coll1 =
        collectionManager.create_collection("coll1", 1, fields, "", 0).get();
  }
  ...
  Option<nlohmann::json> add_op =
      coll1->add(doc.dump(), CREATE, "0", DIRTY_VALUES::DROP);
  ASSERT_FALSE(add_op.ok());
...
TEST_F(CollectionAllFieldsTest, ShouldBeAbleToUpdateSchemaDetectedDocs) {
  ...
  if (coll1 == nullptr) {
    coll1 = collectionManager
                .create_collection("coll1", 4, fields, "", 0, field_types::AUTO)
                .get();
  }
  ...
  Option<nlohmann::json> add_op =
      coll1->add(doc.dump(), CREATE, "0", DIRTY_VALUES::REJECT);
  ASSERT_TRUE(add_op.ok());
  ...
  for (size_t i = 0; i < NUM_DOCS; i++) {
    ...
    doc["title"] =
        std::string("upserted ") +
        std::to_string(StringUtils::hash_wy(i_str.c_str(), i_str.size()));
    doc["scores"] = {i};
    doc["max"] = i;
...
TEST_F(CollectionAllFieldsTest, StringifyAllValues) {
  ...
  if (coll1 == nullptr) {
    coll1 =
        collectionManager.create_collection("coll1", 1, {}, "", 0, "string*")
            .get();
  }
  ...
  auto results = coll1
                     ->search("first", {"title"}, "", {}, sort_fields, {0}, 10,
                              1, FREQUENCY, {false})
                     .get();
  ASSERT_EQ(1, results["hits"].size());
  ASSERT_EQ("FIRST",
  ...
  ASSERT_EQ("1",
            results["hits"][0]["document"]["int_values"][0].get<std::string>());
  ASSERT_EQ("2",
            results["hits"][0]["document"]["int_values"][1].get<std::string>());
  ...
  results = coll1
                ->search("second", {"title"}, "", {}, sort_fields, {0}, 10, 1,
                         FREQUENCY, {false})
                .get();
  ASSERT_EQ(1, results["hits"].size());
  ASSERT_EQ("SECOND",
            results["hits"][0]["document"]["title"].get<std::string>());
  ASSERT_EQ(1, results["hits"][0]["document"].count("int_values"));
  ASSERT_EQ(0, results["hits"][0]["document"]["int_values"]
                   .size());  // since both array values are dropped
...
TEST_F(CollectionAllFieldsTest, IntegerAllValues) {
  ...
  coll1 = collectionManager.get_collection("coll1").get();
  if (coll1 == nullptr) {
    coll1 = collectionManager.create_collection("coll1", 1, {}, "", 0, "int32")
                .get();
  }
  ...
  auto results =
      coll1
          ->search("*", {}, "", {}, sort_fields, {0}, 10, 1, FREQUENCY, {false})
          .get();
  ASSERT_EQ(1, results["hits"].size());
...
TEST_F(CollectionAllFieldsTest, SearchStringifiedField) {
  ...
  coll1 = collectionManager.get_collection("coll1").get();
  if (coll1 == nullptr) {
    const Option<Collection *> &coll_op =
  ...
  auto results_op = coll1->search("stark", {"company_name"}, "", {},
                                  sort_fields, {0}, 10, 1, FREQUENCY, {false});
  ASSERT_TRUE(results_op.ok());
  ASSERT_EQ(1, results_op.get()["hits"].size());

  results_op = coll1->search("engineering", {"department"}, "", {}, sort_fields,
                             {0}, 10, 1, FREQUENCY, {false});
  ASSERT_TRUE(results_op.ok());
  ASSERT_EQ(1, results_op.get()["hits"].size());
...
TEST_F(CollectionAllFieldsTest, StringSingularAllValues) {
  ...
  coll1 = collectionManager.get_collection("coll1").get();
  if (coll1 == nullptr) {
    coll1 = collectionManager.create_collection("coll1", 1, {}, "", 0, "string")
                .get();
  }
  ...
  ASSERT_EQ("FIRST",
            results["hits"][0]["document"]["title"].get<std::string>());
  ASSERT_EQ("123",
            results["hits"][0]["document"]["int_values"].get<std::string>());
...
TEST_F(CollectionAllFieldsTest, UpdateOfDocumentsInAutoMode) {
  ...
  auto add_op =
      coll1->add(doc.dump(), CREATE, "0", DIRTY_VALUES::COERCE_OR_REJECT);
  ASSERT_TRUE(add_op.ok());
  ...
  add_op = coll1->add(update_doc.dump(), UPDATE, "0",
                      DIRTY_VALUES::COERCE_OR_REJECT);
  ASSERT_TRUE(add_op.ok());
...
TEST_F(CollectionAllFieldsTest, NormalFieldWithAutoType) {
  ...
  auto res_op = coll1->search("austin", {"city"}, "publication_year: 2010", {},
                              sort_fields, {0}, 10, 1, FREQUENCY, {false});
...
TEST_F(CollectionAllFieldsTest, JsonFieldsToFieldsConversion) {
  ...
  auto parse_op = field::json_fields_to_fields(false, fields_json,
  ...
  parse_op = field::json_fields_to_fields(false, fields_json,
                                          fallback_field_type, fields);
  ASSERT_TRUE(parse_op.ok());
  ASSERT_EQ(true, fields[0].optional);
...
TEST_F(CollectionAllFieldsTest, WildcardFacetFieldsOnAutoSchema) {
  ...
  if (coll1 == nullptr) {
    coll1 = collectionManager
                .create_collection("coll1", 1, fields, "", 0, field_types::AUTO)
                .get();
  }
  ...
  auto results = coll1
                     ->search("org", {"title"}, "", {"org_name"}, sort_fields,
                              {0}, 10, 1, FREQUENCY, {false})
                     .get();

  ASSERT_EQ(2, results["hits"].size());
  ASSERT_EQ("Walmart",
            results["hits"][0]["document"]["org_name"].get<std::string>());
  ASSERT_EQ("Amazon",
            results["hits"][1]["document"]["org_name"].get<std::string>());
  ...
  results = coll1
                ->search("*", {"title"}, "", {"company_name", "org_name"},
                         sort_fields, {0}, 10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(3, results["hits"].size());
  ...
  ASSERT_EQ("org_name",
            results["facet_counts"][1]["field_name"].get<std::string>());
  ASSERT_EQ(2, results["facet_counts"][1]["counts"].size());
  ASSERT_EQ(
      "Amazon",
      results["facet_counts"][1]["counts"][0]["value"].get<std::string>());
  ASSERT_EQ(1, (int)results["facet_counts"][1]["counts"][0]["count"]);

  ASSERT_EQ(
      "Walmart",
      results["facet_counts"][1]["counts"][1]["value"].get<std::string>());
...
TEST_F(CollectionAllFieldsTest, WildcardFacetFieldsWithoutAutoSchema) {
  ...
  auto res_op = coll1->search("*", {"description"}, "", {}, sort_fields, {0},
                              10, 1, FREQUENCY, {false});
  ASSERT_FALSE(res_op.ok());
  ASSERT_EQ("Could not find a field named `description` in the schema.",
            res_op.error());
...
TEST_F(CollectionAllFieldsTest, RegexpExplicitFieldTypeCoercion) {
  ...
  auto results = coll1
                     ->search("rand", {"title"}, "i_age: 28", {}, sort_fields,
                              {0}, 10, 1, FREQUENCY, {false})
                     .get();
  ASSERT_EQ(1, results["hits"].size());
...
TEST_F(CollectionAllFieldsTest, BothFallbackAndDynamicFields) {
  ...
  coll1 = collectionManager.get_collection("coll1").get();
  if (coll1 == nullptr) {
    auto op = collectionManager.create_collection("coll1", 1, fields, "", 0,
                                                  field_types::AUTO);
    ASSERT_TRUE(op.ok());
    coll1 = op.get();
  }
  ...
  auto res_op = coll1->search("Amazon", {"org_name"}, "", {"org_name"},
                              sort_fields, {0}, 10, 1, FREQUENCY, {false});
  ASSERT_FALSE(res_op.ok());
  ASSERT_EQ("Could not find a facet field named `org_name` in the schema.",
            res_op.error());

  auto results = coll1
                     ->search("Amazon", {"org_name"}, "", {"org_year"},
                              sort_fields, {0}, 10, 1, FREQUENCY, {false})
                     .get();
  ASSERT_EQ(1, results["hits"].size());

  res_op = coll1->search("fizzbuzz", {"rand_str"}, "", {"rand_str"},
                         sort_fields, {0}, 10, 1, FREQUENCY, {false});
  ASSERT_EQ("Could not find a facet field named `rand_str` in the schema.",
            res_op.error());

  results = coll1
                ->search("fizzbuzz", {"rand_str"}, "", {"org_year"},
                         sort_fields, {0}, 10, 1, FREQUENCY, {false})
                .get();
  ASSERT_EQ(1, results["hits"].size());
...
TEST_F(CollectionAllFieldsTest, WildcardFieldAndDictionaryField) {
  ...
  auto results = coll1
                     ->search("*", {}, "year: 2000", {}, sort_fields, {0}, 10,
                              1, FREQUENCY, {false})
                     .get();
  ASSERT_EQ(1, results["hits"].size());
  ...
  results = coll1
                ->search("*", {}, "kinds.CGXX: 13", {}, sort_fields, {0}, 10, 1,
                         FREQUENCY, {false})
                .get();
  ASSERT_EQ(1, results["hits"].size());
...
TEST_F(CollectionAllFieldsTest, DoNotIndexFieldMarkedAsNonIndex) {
  ...
  auto res_op = coll1->search("Amazon", {"description_txt"}, "", {},
                              sort_fields, {0}, 10, 1, FREQUENCY, {false});
  ASSERT_FALSE(res_op.ok());
  ASSERT_EQ("Could not find a field named `description_txt` in the schema.",
            res_op.error());

  res_op = coll1->search("Amazon", {"post"}, "", {}, sort_fields, {0}, 10, 1,
                         FREQUENCY, {false});
  ASSERT_FALSE(res_op.ok());
  ASSERT_EQ("Field `post` is marked as a non-indexed field in the schema.",
            res_op.error());
  ...
  auto res = coll1
                 ->search("Amazon", {"company_name"}, "", {}, sort_fields, {0},
                          10, 1, FREQUENCY, {false})
                 .get();
  ASSERT_EQ("Some post updated.",
            res["hits"][0]["document"]["post"].get<std::string>());
  ...
  auto fs_op =
      coll1->search("Amazon", {"company_name"}, "", {"description_txt"},
                    sort_fields, {0}, 10, 1, FREQUENCY, {false});
  ASSERT_FALSE(fs_op.ok());
  ASSERT_EQ(
      "Could not find a facet field named `description_txt` in the schema.",
      fs_op.error());
  ...
  auto op = collectionManager.create_collection("coll2", 1, fields, "", 0,
                                                field_types::AUTO);
  ASSERT_FALSE(op.ok());
  ASSERT_EQ(
      "Field `post` must be optional since it is marked as non-indexable.",
      op.error());
  ...
  op = collectionManager.create_collection("coll2", 1, fields, "", 0,
                                           field_types::AUTO);
  ASSERT_FALSE(op.ok());
  ...
  op = collectionManager.create_collection("coll3", 1, fields, "", 0,
                                           field_types::AUTO);
  ASSERT_TRUE(op.ok());
  ...
  op = collectionManager.create_collection("coll4", 1, fields, "", 0,
                                           field_types::AUTO);
  ASSERT_FALSE(op.ok());
  ASSERT_EQ(
      "Field `facet_noindex` cannot be a facet since it's marked as "
      "non-indexable.",
      op.error());
...
TEST_F(CollectionAllFieldsTest, NullValueUpdate) {
  ...
  ASSERT_EQ("Hello",
            results["hits"][0]["document"]["unindexed"].get<std::string>());
...
TEST_F(CollectionAllFieldsTest, NullValueArrayUpdate) {
  ...
  std::vector<field> fields = {
      field("titles", field_types::STRING_ARRAY, false, true),
...
TEST_F(CollectionAllFieldsTest,
       EmptyArrayShouldBeAcceptedAsFirstValueOfAutoField) {
  Collection *coll1;
...
TEST_F(CollectionAllFieldsTest,
       DISABLED_SchemaUpdateShouldBeAtomicForAllFields) {
  // when a given field in a document is "bad", other fields should not be
  // partially added to schema
  Collection *coll1;
...
TEST_F(CollectionAllFieldsTest, FieldNameMatchingRegexpShouldNotBeIndexed) {
  ...
  auto results =
      coll1
          ->search("one", {"title"}, "", {}, {}, {2}, 10, 1, FREQUENCY, {true},
...
TEST_F(CollectionAllFieldsTest,
       FieldNameMatchingRegexpShouldNotBeIndexedInNonAutoSchema) {
  std::vector<field> fields = {
      field("title", field_types::STRING, false),
      field("name.*", field_types::STRING, true, true)};
...
TEST_F(CollectionAllFieldsTest, EmbedFromFieldJSONInvalidField) {
  ...
  auto field_op =
      field::json_fields_to_fields(false, arr, fallback_field_type, fields);

  ASSERT_FALSE(field_op.ok());
  ASSERT_EQ(
      "Property `embed.from` can only refer to string or string array fields.",
      field_op.error());
}
...
TEST_F(CollectionAllFieldsTest, ModelParametersWithoutEmbedFrom) {
  ...
  auto field_op =
      field::json_fields_to_fields(false, arr, fallback_field_type, fields);
  ASSERT_FALSE(field_op.ok());
  ASSERT_EQ("Property `embed` must contain a `from` property.",
            field_op.error());
}
