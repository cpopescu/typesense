...
TEST_F(CoreAPIUtilsTest, StatefulRemoveDocs) {
  ...
  deletion_state.index_ids.emplace_back(filter_results.count,
                                        filter_results.docs);
...
TEST_F(CoreAPIUtilsTest, MultiSearchEmbeddedKeys) {
  ...
  // ensure that req params are appended to (embedded params are also rolled
  // into req params)
  ASSERT_EQ("((user_id: 100) && (age: > 100)) && (foo: bar)",
  ...
  ASSERT_EQ(
      "{\"message\": \"Number of multi searches exceeds `limit_multi_searches` "
      "parameter.\"}",
      res->body);
...
TEST_F(CoreAPIUtilsTest, SearchEmbeddedPresetKey) {
  ...
  Option<bool> success_op =
      collectionManager.upsert_preset("apple", preset_value);
...
TEST_F(CoreAPIUtilsTest, ExtractCollectionsFromRequestBody) {
  ...
  route_path rpath("POST", {"collections"}, post_create_collection, false,
                   false);
...
TEST_F(CoreAPIUtilsTest, ExtractCollectionsFromRequestBodyExtended) {
  ...
  get_collections_for_auth(req_params, "{]", rpath_multi_search, "",
                           collections, embedded_params_vec);
  ...
  collections.clear();
  embedded_params_vec.clear();
  get_collections_for_auth(req_params, sample_search_body.dump(),
                           rpath_multi_search, "", collections,
                           embedded_params_vec);
  ...
  // when one of the search arrays don't have an explicit collection, use the
  // collection name from req param
  ...
  get_collections_for_auth(req_params, sample_search_body.dump(), rpath_search,
                           "", collections, embedded_params_vec);
...
TEST_F(CoreAPIUtilsTest, MultiSearchWithPresetShouldUsePresetForAuth) {
  ...
  get_collections_for_auth(req_params, search_body, rpath_multi_search, "",
                           collections, embedded_params_vec);
...
TEST_F(CoreAPIUtilsTest, SearchPagination) {
  ...
  // when offset is used we should expect the same but "offset" should be
  ...
  ASSERT_EQ("Parameter `page` must be an unsigned integer.",
            results["error"].get<std::string>());
  ...
  results = nlohmann::json::parse(res->body)["results"][0];
  ASSERT_EQ(400, results["code"].get<size_t>());
  ASSERT_EQ("Parameter `offset` must be an unsigned integer.",
            results["error"].get<std::string>());
...
TEST_F(CoreAPIUtilsTest, TestParseAPIKeyIPFromMetadata) {
  ...
  Option<std::pair<std::string, std::string>> res =
      get_api_key_and_ip(valid_metadata);
  EXPECT_TRUE(res.ok());
...
TEST_F(CoreAPIUtilsTest, TestProxy) {
  ...
  long expected_status_code =
      HttpClient::get_instance().get_response(url, res, res_headers, headers);
...
TEST_F(CoreAPIUtilsTest, TestProxyInvalid) {
  ...
  ASSERT_EQ("URL and method must be non-empty strings.",
            nlohmann::json::parse(resp->body)["message"]);
  ...
  ASSERT_EQ(400, resp->status_code);
  ASSERT_EQ("Parameter `method` must be one of GET, POST, PUT, DELETE.",
            nlohmann::json::parse(resp->body)["message"]);
  ...
  ASSERT_EQ(400, resp->status_code);
  ASSERT_EQ("Body must be a string.",
            nlohmann::json::parse(resp->body)["message"]);
  ...
  ASSERT_EQ(400, resp->status_code);
  ASSERT_EQ("Headers must be a JSON object.",
            nlohmann::json::parse(resp->body)["message"]);
}
...
TEST_F(CoreAPIUtilsTest, TestProxyTimeout) {
  ...
  ASSERT_EQ("Server error on remote server. Please try again later.",
            nlohmann::json::parse(resp->body)["message"]);
}