...
class SystemMetrics {
 private:
  ...
  size_t get_total_time(const cpu_data_t &e) {
    return e.times[S_USER] + e.times[S_NICE] + e.times[S_SYSTEM] +
           e.times[S_IDLE] + e.times[S_IOWAIT] + e.times[S_IRQ] +
  ...
  std::vector<cpu_stat_t> compute_cpu_stats(
      const std::vector<cpu_data_t> &cpu_data_prev,
    ...
    for (size_t i = 0; i < NUM_ENTRIES; ++i) {
      ...
      float active_percentage =
          (now_total == prev_total) ? 0 : ((active_diff / total_diff) * 100);
 ...
 public:
  ...
  static void linux_get_network_data(const std::string &stat_path,
