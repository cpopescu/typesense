...
using array_mapped_single_val_facet_t =
    std::array<single_val_facet_map_t*, ARRAY_FACET_DIM>;
...
struct token_t {
  ...
  token_t(size_t position, const std::string& value, bool is_prefix_searched,
          uint32_t root_len, uint32_t num_typos)
      : position(position),
        value(value),
        is_prefix_searched(is_prefix_searched),
        root_len(root_len),
        num_typos(num_typos) {}
};
...
enum enable_t { always, fallback, off };

enum text_match_type_t { max_score, max_weight };
...
struct search_args {
  ...
  search_args(std::vector<query_tokens_t> field_query_tokens,
              ...
              std::vector<std::pair<uint32_t, uint32_t>>& included_ids,
              std::vector<uint32_t> excluded_ids,
              std::vector<sort_by>& sort_fields_std, facet_query_t facet_query,
              const std::vector<uint32_t>& num_typos, size_t max_facet_values,
              size_t max_hits, size_t per_page, size_t offset,
              ...
              size_t group_limit, const string& default_sorting_field,
              bool prioritize_exact_match, const bool prioritize_token_position,
              bool exhaustive_search, size_t concurrency,
              size_t search_cutoff_ms, size_t min_len_1typo,
              ...
              vector_query_t& vector_query, size_t facet_sample_percent,
              size_t facet_sample_threshold)
      : field_query_tokens(field_query_tokens),
        search_fields(search_fields),
        match_type(match_type),
        filter_tree_root(filter_tree_root),
        facets(facets),
    ...
    const size_t topster_size =
        std::max((size_t)1,
                 max_hits);  // needs to be atleast 1 since scoring is mandatory
...
struct index_record {
  ...
  nlohmann::json
      del_doc;  // document containing the fields that should be deleted
  ...
  index_record(size_t record_pos, uint32_t seq_id, const nlohmann::json& doc,
               index_operation_t operation, const DIRTY_VALUES& dirty_values)
      : position(record_pos),
        seq_id(seq_id),
        doc(doc),
        operation(operation),
  ...
  void index_success() { indexed = Option<bool>(true); }
};
...
class VectorFilterFunctor : public hnswlib::BaseFilterFunctor {
 ...
 public:
  explicit VectorFilterFunctor(const uint32_t* filter_ids,
...
struct hnsw_index_t {
  ...
  hnsw_index_t(size_t num_dim, size_t init_size,
               vector_distance_type_t distance_type)
      : space(new hnswlib::InnerProductSpace(num_dim)),
        vecdex(new hnswlib::HierarchicalNSW<float>(space, init_size, 16, 200,
                                                   100, true)),
        num_dim(num_dim),
  ...
  static void normalize_vector(const std::vector<float>& src,
                               std::vector<float>& norm_dest) {
    float norm = 0.0f;
    for (float i : src) {
      norm += i * i;
    }
...
class Index {
 private:
  ...
  spp::sparse_hash_map<
      std::string, spp::sparse_hash_map<std::string, std::vector<uint32_t>>*>
      geopoint_index;

  // geo_array_field => (seq_id => values) used for exact filtering of geo array
  // records
  spp::sparse_hash_map<std::string, spp::sparse_hash_map<uint32_t, int64_t*>*>
      geo_array_index;
  ...
  spp::sparse_hash_map<std::string, array_mapped_single_val_facet_t>
      single_val_facet_index_v3;
  ...
  bool static_filter_query_eval(const override_t* override,
                                std::vector<std::string>& tokens,
                                filter_node_t*& filter_tree_root) const;

  bool resolve_override(const std::vector<std::string>& rule_tokens,
  ...
  bool check_for_overrides(
      ...
      bool slide_window, bool exact_rule_match,
  ...
  void search_field(
      ...
      const std::string& field_name, const uint32_t* filter_ids,
      size_t filter_ids_length, const std::vector<uint32_t>& curated_ids,
      std::vector<sort_by>& sort_fields, int last_typo, int max_typos,
      std::vector<std::vector<art_leaf*>>& searched_queries, Topster* topster,
      spp::sparse_hash_map<uint64_t, uint32_t>& groups_processed,
      uint32_t** all_result_ids, size_t& all_result_ids_len,
      size_t& field_num_results, size_t group_limit,
      const std::vector<std::string>& group_by_fields,
      bool prioritize_exact_match, size_t concurrency,
      std::set<uint64>& query_hashes, token_ordering token_order,
      const bool prefix, size_t drop_tokens_threshold,
      size_t typo_tokens_threshold, bool exhaustive_search,
  ...
  void search_all_candidates(
      ...
      std::set<uint64>& query_hashes, std::vector<uint32_t>& id_buff) const;
  ...
  void search_candidates(
      ...
      std::vector<token_candidates>& token_to_candidates,
      std::vector<std::vector<art_leaf*>>& searched_queries, Topster* topster,
      spp::sparse_hash_map<uint64_t, uint32_t>& groups_processed,
      uint32_t** all_result_ids, size_t& all_result_ids_len,
      ...
      bool exhaustive_search, int syn_orig_num_tokens, size_t concurrency,
      std::set<uint64>& query_hashes, std::vector<uint32_t>& id_buff) const;

  static void popular_fields_of_token(
  ...
  void numeric_not_equals_filter(num_tree_t* const num_tree,
                                 ...
                                 uint32_t* const& context_ids, size_t& ids_len,
                                 uint32_t*& ids) const;

  bool field_is_indexed(const std::string& field_name) const;

  Option<bool> do_filtering(filter_node_t* const root, filter_result_t& result,
  ...
  /// \param filter_result
  /// \param collection_name Name of the collection to which current index
  /// belongs. Used to find the reference field in other collection. \param
  /// context_ids_length Number of docs matching the search query. \param
  /// context_ids Array of doc ids matching the search query.
  ...
  static void tokenize_string_with_facets(
  ...
  static void tokenize_string_array_with_facets(
  ...
  void collate_included_ids(
      const std::vector<token_t>& q_included_tokens,
      const std::map<size_t, std::map<size_t, uint32_t>>& included_ids_map,
      Topster* curated_topster,
      std::vector<std::vector<art_leaf*>>& searched_queries) const;

  static uint64_t facet_token_hash(const field& a_field,
                                   const std::string& token);

  static void compute_facet_stats(facet& a_facet, uint64_t raw_value,
  ...
  static void remove_facet_token(
  ...
  static void batch_embed_fields(
      std::vector<index_record*>& documents,
      const tsl::htrie_map<char, field>& embedding_fields,
 ...
 public:
  ...
  // If the number of results found is less than this threshold, Typesense will
  // attempt to drop the tokens in the query that have the least individual hits
  // one by one until enough results are found.
  ...
  int64_t score_results2(
      ...
      const size_t field_id, const bool field_is_array,
      const uint32_t total_cost, int64_t& match_score, const uint32_t seq_id,
      const int sort_order[3], const bool prioritize_exact_match,
      const bool single_exact_query_token, const bool prioritize_token_position,
      size_t num_query_tokens, int syn_orig_num_tokens,
      const std::vector<posting_list_t::iterator_t>& posting_lists) const;

  void score_results(
      const std::vector<sort_by>& sort_fields, const uint16_t& query_index,
      const uint8_t& field_id, bool field_is_array, const uint32_t total_cost,
      ...
      bool prioritize_exact_match, bool single_exact_query_token,
      int syn_orig_num_tokens,
      const std::vector<posting_list_t::iterator_t>& posting_lists) const;

  static int64_t get_points_from_doc(const nlohmann::json& document,
                                     const std::string& default_sorting_field);

  const spp::sparse_hash_map<std::string, art_tree*>& _get_search_index() const;

  const spp::sparse_hash_map<std::string, num_tree_t*>& _get_numerical_index()
      const;

  const spp::sparse_hash_map<std::string, array_mapped_infix_t>&
  _get_infix_index() const;

  const spp::sparse_hash_map<std::string, hnsw_index_t*>& _get_vector_index()
      const;

  static int get_bounded_typo_cost(const size_t max_cost,
                                   const size_t token_len, size_t min_len_1typo,
                                   size_t min_len_2typo);
  ...
  static void compute_token_offsets_facets(
  ...
  void remove_field(uint32_t seq_id, const nlohmann::json& document,
  ...
  static void validate_and_preprocess(
  ...
  static size_t batch_memory_index(
      Index* index, std::vector<index_record>& iter_batch,
      const std::string& default_sorting_field,
      const tsl::htrie_map<char, field>& search_schema,
      const tsl::htrie_map<char, field>& embedding_fields,
  ...
  void index_field_in_memory(const field& afield,
                             std::vector<index_record>& iter_batch);

  template <class T>
  void iterate_and_index_numerical_field(std::vector<index_record>& iter_batch,
                                         const field& afield, T func);

  // static bool is_point_in_polygon(const Geofence& poly, const GeoCoord&
  // point);
  ...
  art_leaf* get_token_leaf(const std::string& field_name,
                           const unsigned char* token, uint32_t token_len);

  Option<bool> do_filtering_with_lock(
      filter_node_t* const filter_tree_root, filter_result_t& filter_result,
      const std::string& collection_name = "") const;

  /// Traverses through filter tree and gets an approximate doc count for each
  /// filter. Also arranges the children of each operator in ascending order
  /// based on their approx doc count.
  ///
  /// \param filter_tree_root
  /// \param approx_filter_ids_length Approximate count of docs that would match
  /// the whole filter_by clause. \param collection_name Name of the collection
  /// to which current index belongs. Used to find the reference field in other
  /// collection.
  Option<bool> rearrange_filter_tree(
      filter_node_t* const filter_tree_root, uint32_t& approx_filter_ids_length,
      const std::string& collection_name = "") const;

  Option<bool> _approximate_filter_ids(
      const filter& a_filter, uint32_t& filter_ids_length,
      const std::string& collection_name = "") const;

  Option<bool> do_reference_filtering_with_lock(
      filter_node_t* const filter_tree_root, filter_result_t& filter_result,
      const std::string& collection_name,
      const std::string& reference_helper_field_name) const;

  /// Get approximate count of docs matching a reference filter on foo
  /// collection when $foo(...) filter is encountered.
  Option<bool> get_approximate_reference_filter_ids_with_lock(
  ...
  // the following methods are not synchronized because their parent calls are
  // synchronized or they are const/static
  ...
  void search_infix(const std::string& query, const std::string& field_name,
                    std::vector<uint32_t>& ids, size_t max_extra_prefix,
                    size_t max_extra_suffix) const;

  void curate_filtered_ids(
  ...
  void populate_sort_mapping(
      int* sort_order, std::vector<size_t>& geopoint_indices,
      std::vector<sort_by>& sort_fields_std,
      std::array<spp::sparse_hash_map<uint32_t, int64_t>*, 3>& field_values)
      const;

  static void remove_matched_tokens(
      std::vector<std::string>& tokens,
      const std::set<std::string>& rule_token_set);

  void compute_facet_infos(const std::vector<facet>& facets,
                           facet_query_t& facet_query,
                           const size_t facet_query_num_typos,
  ...
  void search_across_fields(
      ...
      const std::vector<std::string>& group_by_fields,
      bool prioritize_exact_match, const bool search_all_candidates,
      const uint32_t* filter_ids, uint32_t filter_ids_length,
      ...
      std::array<spp::sparse_hash_map<uint32_t, int64_t>*, 3>& field_values,
      const std::vector<size_t>& geopoint_indices,
      std::vector<uint32_t>& id_buff, uint32_t*& all_result_ids,
      size_t& all_result_ids_len) const;

  void search_fields(
      const std::vector<filter>& filters,
      const std::map<size_t, std::map<size_t, uint32_t>>& included_ids_map,
      const std::vector<sort_by>& sort_fields_std, const size_t min_typo,
      const std::vector<uint32_t>& num_typos, Topster* topster,
      Topster* curated_topster, const token_ordering& token_order,
      const std::vector<bool>& prefixes, const size_t drop_tokens_threshold,
      spp::sparse_hash_map<uint64_t, uint32_t>& groups_processed,
      std::vector<std::vector<art_leaf*>>& searched_queries,
      const size_t typo_tokens_threshold, const size_t group_limit,
      const std::vector<std::string>& group_by_fields,
      bool prioritize_exact_match, const bool exhaustive_search,
      const size_t concurrency, size_t min_len_1typo, size_t min_len_2typo,
      const size_t max_candidates, const std::vector<enable_t>& infixes,
      const size_t max_extra_prefix, const size_t max_extra_suffix,
      uint32_t* filter_ids, uint32_t filter_ids_length,
      const std::set<uint32_t>& curated_ids,
      const std::vector<uint32_t>& curated_ids_sorted,
      const size_t num_search_fields, const uint32_t* exclude_token_ids,
      size_t exclude_token_ids_size, std::vector<Topster*>& ftopsters,
      bool is_wildcard_query, bool split_join_tokens,
      std::vector<query_tokens_t>& field_query_tokens,
...
template <class T>
void Index::iterate_and_index_numerical_field(
    std::vector<index_record>& iter_batch, const field& afield, T func) {
