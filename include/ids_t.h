...
struct compact_id_list_t {
  ...
  static compact_id_list_t* create(uint32_t num_ids,
...
class ids_t {
 private:
  static void to_expanded_id_lists(const std::vector<void*>& raw_id_lists,
 ...
 public:
  ...
  struct block_intersector_t {
    ...
    block_intersector_t(const std::vector<void*>& raw_id_lists,
                        id_list_t::result_iter_state_t& iter_state,
                        ThreadPool* thread_pool, size_t parallelize_min_ids = 1)
      ...
      if (id_lists.size() > 1) {
        std::sort(this->id_lists.begin(), this->id_lists.end(),
                  [](id_list_t* a, id_list_t* b) {
  ...
  static void merge(const std::vector<void*>& id_lists,
                    std::vector<uint32_t>& result_ids);

  static void intersect(const std::vector<void*>& id_lists,
                        std::vector<uint32_t>& result_ids);
...
bool ids_t::block_intersector_t::intersect(T func, size_t concurrency) {
  ...
  // 3. On each window, pick the last block's last ID and identify blocks from
  // other lists containing that ID
  // 4. Construct N groups of iterators this way (the last block must overlap on
  ...
  for (size_t i = 0; i < partial_its_vec.size(); i++) {
    ...
    thread_pool->enqueue([this, i, &func, &partial_its, &num_processed,
  ...
  cv_process.wait(lock_process,
                  [&]() { return num_processed == num_non_empty; });
