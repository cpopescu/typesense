...
struct reference_pair {
  ...
  reference_pair(std::string collection, std::string field)
...
class Collection {
 private:
  ...
  struct highlight_t {
    ...
    bool operator<(const highlight_t& a) const {
      return std::tie(match_score, field_index) >
             std::tie(a.match_score, field_index);
    }
  };
  ...
  struct match_index_t {
    ...
    match_index_t(Match match, uint64_t match_score, size_t index)
        : match(match), match_score(match_score), index(index) {}
  ...
  // Auto incrementing record ID used internally for indexing - not exposed to
  // the client
  ...
  /// "field name" -> reference_pair(referenced_collection_name,
  /// referenced_field_name)
  spp::sparse_hash_map<std::string, reference_pair> reference_fields;

  // Keep index as the last field since it is initialized in the constructor via
  // init_index(). Add a new field before it.
  Index* index;
  ...
  void highlight_result(
      const std::string& h_obj, const field& search_field,
  ...
  void remove_document(const nlohmann::json& document, const uint32_t seq_id,
                       bool remove_from_store);

  void process_remove_field_for_embedding_fields(
  ...
  void curate_results(
      ...
      bool& filter_curated_hits, std::string& curated_sort_by) const;

  static Option<bool> detect_new_fields(
      ...
      std::vector<field>& new_fields, bool enable_nested_fields);
  ...
  static Option<bool> parse_pinned_hits(
  ...
  Option<bool> validate_and_standardize_sort_fields(
      const std::vector<sort_by>& sort_fields,
      std::vector<sort_by>& sort_fields_std, bool is_wildcard_query,
      const bool is_vector_query, bool is_group_by_query = false) const;
  ...
  void populate_text_match_info(nlohmann::json& info, uint64_t match_score,
                                const text_match_type_t match_type) const;

  bool handle_highlight_text(
      ...
      bool highlight_fully, const size_t snippet_threshold,
      bool is_infix_search, std::vector<std::string>& raw_query_tokens,
      size_t last_valid_offset, const std::string& highlight_start_tag,
      const std::string& highlight_end_tag, const uint8_t* index_symbols,
      const match_index_t& match_index) const;

  static Option<bool> extract_field_name(
      const std::string& field_name,
      const tsl::htrie_map<char, field>& search_schema,
      std::vector<std::string>& processed_search_fields,
      bool extract_only_string_fields, bool enable_nested_fields,
      const bool handle_wildcard = true);

  bool is_nested_array(const nlohmann::json& obj,
                       std::vector<std::string> path_parts,
                       size_t part_i) const;

  template <class T>
  static bool highlight_nested_field(const nlohmann::json& hdoc,
                                     nlohmann::json& hobj,
                                     std::vector<std::string>& path_parts,
                                     size_t path_index, bool is_arr_obj_ele,
                                     int array_index, T func);

  static Option<bool> resolve_field_type(
      ...
      bool enable_nested_fields, std::vector<field>& new_fields);

  static uint64_t extract_bits(uint64_t value, unsigned lsb_offset, unsigned n);

  Option<bool> populate_include_exclude_fields(
  ...
  Option<std::string> get_reference_field(
      const std::string& collection_name) const;

  static void hide_credential(nlohmann::json& json,
 ...
 public:
  ...
  // Using a $ prefix so that these meta keys stay above record entries in a
  // lexicographically ordered KV store
  ...
  static constexpr const char* COLLECTION_DEFAULT_SORTING_FIELD_KEY =
      "default_sorting_field";
  static constexpr const char* COLLECTION_CREATED = "created_at";
  static constexpr const char* COLLECTION_NUM_MEMORY_SHARDS =
      "num_memory_shards";
  static constexpr const char* COLLECTION_FALLBACK_FIELD_TYPE =
      "fallback_field_type";
  static constexpr const char* COLLECTION_ENABLE_NESTED_FIELDS =
  ...
  Collection(const std::string& name, const uint32_t collection_id,
  ...
  static std::string get_override_key(const std::string& collection_name,
  ...
  Option<doc_seq_id_t> to_doc(const std::string& json_str,
  ...
  Option<bool> get_document_from_store(const std::string& seq_id_key,
                                       nlohmann::json& document,
                                       bool raw_doc = false) const;

  Option<bool> get_document_from_store(const uint32_t& seq_id,
                                       nlohmann::json& document,
                                       bool raw_doc = false) const;
  ...
  void batch_index(std::vector<index_record>& index_records,
                   std::vector<std::string>& json_out, size_t& num_indexed,
  ...
  Option<nlohmann::json> add(
      const std::string& json_str, const index_operation_t& operation = CREATE,
      const std::string& id = "",
      const DIRTY_VALUES& dirty_values = DIRTY_VALUES::COERCE_OR_REJECT);

  nlohmann::json add_many(
  ...
  Option<bool> populate_include_exclude_fields_lk(
      const spp::sparse_hash_set<std::string>& include_fields,
      const spp::sparse_hash_set<std::string>& exclude_fields,
      tsl::htrie_set<char>& include_fields_full,
      tsl::htrie_set<char>& exclude_fields_full) const;

  Option<nlohmann::json> search(
      std::string query, const std::vector<std::string>& search_fields,
      const std::string& filter_query,
      const std::vector<std::string>& facet_fields,
      const std::vector<sort_by>& sort_fields,
      const std::vector<uint32_t>& num_typos, size_t per_page = 10,
      size_t page = 1, token_ordering token_order = FREQUENCY,
      ...
      const spp::sparse_hash_set<std::string>& exclude_fields =
          spp::sparse_hash_set<std::string>(),
      size_t max_facet_values = 10, const std::string& simple_facet_query = "",
      ...
      size_t group_limit = 3, const std::string& highlight_start_tag = "<mark>",
      const std::string& highlight_end_tag = "</mark>",
      std::vector<uint32_t> raw_query_by_weights = {},
      size_t limit_hits = UINT32_MAX, bool prioritize_exact_match = true,
      bool pre_segmented_query = false, bool enable_overrides = true,
      const std::string& highlight_fields = "",
      const bool exhaustive_search = false,
      size_t search_stop_millis = 6000 * 1000, size_t min_len_1typo = 4,
      size_t min_len_2typo = 7, enable_t split_join_tokens = fallback,
      size_t max_candidates = 4, const std::vector<enable_t>& infixes = {off},
      ...
      const size_t facet_sample_threshold = 0, const size_t page_offset = 0,
  ...
  Option<bool> get_approximate_reference_filter_ids(
      const std::string& filter_query, uint32_t& filter_ids_length) const;

  Option<bool> get_reference_filter_ids(
  ...
  Option<bool> parse_facet(const std::string& facet_field,
  ...
  static void copy_highlight_doc(
...
template <class T>
bool Collection::highlight_nested_field(const nlohmann::json& hdoc,
                                        nlohmann::json& hobj,
                                        std::vector<std::string>& path_parts,
                                        size_t path_index, bool is_arr_obj_ele,
                                        int array_index, T func) {
  ...
  if (it != hobj.end()) {
    if (it.value().is_array()) {
      ...
      for (size_t i = 0; i < it.value().size(); i++) {
        ...
        resolved =
            highlight_nested_field(hdoc, h_ele, path_parts, path_index + 1,
                                   is_arr_obj_ele, i, func) ||
            resolved;
      }
      return resolved;
    } else {
      return highlight_nested_field(hdoc, it.value(), path_parts,
                                    path_index + 1, is_arr_obj_ele, 0, func);
    }
  }
  { return false; }
}
