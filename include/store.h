...
class UInt64AddOperator : public rocksdb::AssociativeMergeOperator {
 public:
  virtual bool Merge(const rocksdb::Slice& key,
    ...
    *new_value = StringUtils::serialize_uint32_t(
        existing + StringUtils::deserialize_uint32_t(value.ToString()));
    return true;
  }
...
enum StoreStatus { FOUND, NOT_FOUND, ERROR };
...
class Store {
 private:
  ...
  // So we use unique lock only for assignment, but shared locks for all other
  // operations on DB
  ...
  rocksdb::Status init_db() {
    ...
    if (!s.ok()) {
      ...
      if (s.code() == rocksdb::Status::Code::kIOError) {
        LOG(ERROR) << "It seems like the data directory " << state_dir_path
                   << " is already being used by "
                   << "another Typesense server. ";
        LOG(ERROR) << "If you are SURE that this is not the case, delete the "
                      "LOCK file "
 ...
 public:
  ...
  bool contains(const std::string& key) const {
    ...
    bool value_found;
    bool key_may_exist =
        db->KeyMayExist(rocksdb::ReadOptions(), key, &value, &value_found);
  ...
  StoreStatus get(const std::string& key, std::string& value) const {
    ...
    LOG(ERROR) << "Error while fetching the key: " << key
  ...
  void scan_fill(const std::string& prefix_start, const std::string& prefix_end,
    ...
    std::shared_lock lock(mutex);
    rocksdb::Iterator* iter = db->NewIterator(read_opts);
    for (iter->Seek(prefix_start);
         iter->Valid() && iter->key().starts_with(prefix_start); iter->Next()) {
  ...
  Option<std::vector<std::string>*> get_updates_since(
      const uint64_t seq_number_org, const uint64_t max_updates) const {
    std::shared_lock lock(mutex);
    ...
    if (!status.ok()) {
      LOG(ERROR) << "Error while fetching updates for replication: "
                 << status.ToString();

      std::ostringstream error;
      error << "Unable to fetch updates. "
            << "Master's latest sequence number is " << local_latest_seq_num
    ...
    if (!iter->Valid()) {
      std::ostringstream error;
      error << "Invalid iterator. Master's latest sequence number is "
            << local_latest_seq_num << " but "
            << "updates are requested from sequence number " << seq_number
            << ". "
            << "The master's WAL entries might have expired (they are kept "
               "only for 24 hours).";
      LOG(ERROR) << error.str();
      return Option<std::vector<std::string>*>(400, error.str());
    }
    ...
    while (iter->Valid() && num_updates < max_updates) {
      ...
      if (first_iteration) {
        ...
        if (batch.sequence != seq_number) {
          std::ostringstream error;
          error << "Invalid iterator. Requested sequence number is "
                << seq_number << " but "
                << "updates are available only from sequence number "
                << batch.sequence << ". "
                << "The master's WAL entries might have expired (they are kept "
                   "only for 24 hours).";
          LOG(ERROR) << error.str();
  ...
  int reload(bool clear_state_dir, const std::string& snapshot_path) {
    ...
    // we don't use close() to avoid nested lock and because lock is required
    // until db is re-initialized
    delete db;
    db = nullptr;
    ...
    if (!snapshot_path.empty()) {
      ...
      if (!copy_dir(snapshot_path, state_dir_path)) {
        LOG(WARNING) << "copy snapshot " << snapshot_path << " to "
                     << state_dir_path << " failed";
        return -1;
      }

      LOG(INFO) << "copy snapshot " << snapshot_path << " to " << state_dir_path
                << " success";
    }
    ...
    if (!status.ok()) {
      LOG(WARNING) << "Open DB " << state_dir_path
                   << " failed, msg: " << status.ToString();
      return -1;
    }
  ...
  rocksdb::Status create_check_point(rocksdb::Checkpoint** checkpoint_ptr,
                                     const std::string& db_snapshot_path) {
    std::shared_lock lock(mutex);
  ...
  rocksdb::Status delete_range(const std::string& begin_key,
                               const std::string& end_key) {
    std::shared_lock lock(mutex);
    return db->DeleteRange(rocksdb::WriteOptions(), db->DefaultColumnFamily(),
                           begin_key, end_key);
  }

  rocksdb::Status compact_range(const rocksdb::Slice& begin_key,
                                const rocksdb::Slice& end_key) {
    std::shared_lock lock(mutex);
    return db->CompactRange(rocksdb::CompactRangeOptions(), &begin_key,
                            &end_key);
  }

  // Only for internal tests
  rocksdb::DB* _get_db_unsafe() const { return db; }

  const std::string& get_state_dir_path() const { return state_dir_path; }

  const rocksdb::Options& get_db_options() const { return options; }
