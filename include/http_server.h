...
struct h2o_custom_generator_t {
  ...
  std::shared_ptr<http_req>& req() { return request; }

  std::shared_ptr<http_res>& res() { return response; }
};
...
struct deferred_req_res_t {
  ...
  deferred_req_res_t(const std::shared_ptr<http_req>& req,
                     const std::shared_ptr<http_res>& res, HttpServer* server,
                     bool destroy_after_use)
      : req(req),
        res(res),
        server(server),
...
struct async_req_res_t {
  // NOTE: care must be taken to ensure that concurrent writes are protected as
  // some fields are also used by http lib
 private:
 ...
 public:
  ...
  async_req_res_t(const std::shared_ptr<http_req>& h_req,
                  const std::shared_ptr<http_res>& h_res,
                  const bool destroy_after_use)
    ...
    h2o_custom_generator_t* res_generator =
    ...
    res_state.is_req_early_exit = (res_generator->rpath->async_req &&
                                   res->final && !req->last_chunk_aggregate);
    res_state.send_state =
        res->final ? H2O_SEND_STATE_FINAL : H2O_SEND_STATE_IN_PROGRESS;
    res_state.generator =
        (res_generator == nullptr) ? nullptr : &res_generator->h2o_generator;
    res_state.set_response(res->status_code, res->content_type_header,
                           res->body);
  }

  bool is_alive() { return res->is_alive; }

  void req_notify() { return req->notify(); }

  void res_notify() { return res->notify(); }

  stream_response_state_t& get_res_state() { return req->res_state; }
};
...
struct defer_processing_t {
  ...
  defer_processing_t(const std::shared_ptr<http_req>& req,
...
class HttpServer {
 private:
  ...
  static bool initialize_ssl_ctx(const char* cert_file, const char* key_file,
                                 h2o_accept_ctx_t* accept_ctx);
  ...
  static int send_response(h2o_req_t* req, int status_code,
                           const std::string& message);

  static int async_req_cb(void* ctx, int is_end_stream);

  static bool is_write_request(const std::string& root_resource,
                               const std::string& http_method);

 public:
  HttpServer(const std::string& version, const std::string& listen_address,
  ...
  void set_auth_handler(bool (*handler)(
  ...
  void get(const std::string& path,
           bool (*handler)(const std::shared_ptr<http_req>& req,
                           const std::shared_ptr<http_res>& res),
           bool async_req = false, bool async_res = false);
  ...
  uint64_t find_route(const std::vector<std::string>& path_parts,
                      const std::string& http_method, route_path** found_rpath);
  ...
  void clear_timeouts(const std::vector<h2o_timer_t*>& timers,
  ...
  static int process_request(const std::shared_ptr<http_req>& request,
