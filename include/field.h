...
// Some models require additional parameters to be passed to the model during
// indexing/querying For e.g. e5-small model requires prefix "passage:" for
// indexing and "query:" for querying
...
enum vector_distance_type_t { ip, cosine };
...
struct field {
  ...
  // third state is used to diff between array of object and array within object
  // during write
  int nested_array;
  ...
  field(const std::string& name, const std::string& type, const bool facet,
        const bool optional = false, bool index = true, std::string locale = "",
        int sort = -1, int infix = -1, bool nested = false,
        int nested_array = 0, size_t num_dim = 0,
        vector_distance_type_t vec_dist = cosine, std::string reference = "",
        const nlohmann::json& embed = nlohmann::json())
      : name(name),
        type(type),
        facet(facet),
        optional(optional),
  ...
  bool operator<(const field& f) const { return name < f.name; }

  bool operator==(const field& f) const { return name == f.name; }

  bool is_auto() const { return (type == field_types::AUTO); }

  bool is_single_integer() const {
    return (type == field_types::INT32 || type == field_types::INT64);
  }

  bool is_single_float() const { return (type == field_types::FLOAT); }

  bool is_single_bool() const { return (type == field_types::BOOL); }

  bool is_single_geopoint() const { return (type == field_types::GEOPOINT); }
  ...
  bool is_string_star() const { return field_types::is_string_or_array(type); }

  bool is_facet() const { return facet; }

  bool is_array() const {
    return (
        type == field_types::STRING_ARRAY || type == field_types::INT32_ARRAY ||
        type == field_types::FLOAT_ARRAY || type == field_types::INT64_ARRAY ||
        type == field_types::BOOL_ARRAY ||
        type == field_types::GEOPOINT_ARRAY ||
        type == field_types::OBJECT_ARRAY);
  }

  bool is_singular() const { return !is_array(); }
  ...
  bool is_dynamic() const { return is_dynamic(name, type); }
  ...
  bool is_num_sortable() const { return sort && is_num_sort_field(); }

  bool is_str_sortable() const { return sort && type == field_types::STRING; }

  bool is_sortable() const { return is_num_sortable() || is_str_sortable(); }

  bool has_valid_type() const {
    bool is_basic_type = is_string() || is_integer() || is_float() ||
                         is_bool() || is_geopoint() || is_object() || is_auto();
    if (!is_basic_type) {
  ...
  static bool get_single_type(const nlohmann::json& obj,
  ...
  static Option<bool> fields_to_json_fields(
    ...
    for (const field& field : fields) {
      ...
      if (!field.has_valid_type()) {
        return Option<bool>(
            400, "Field `" + field.name + "` has an invalid data type `" +
                     field.type + "`, see docs for supported data types.");
      }

      if (field.name == default_sorting_field && !field.is_sortable()) {
        return Option<bool>(400, "Default sorting field `" +
      ...
      if (field.name == default_sorting_field) {
        ...
        if (field.is_geopoint()) {
          return Option<bool>(
              400, "Default sorting field cannot be of type geopoint.");
        }
      ...
      if (field.is_dynamic() && !field.nested && !field.optional) {
        return Option<bool>(
            400, "Field `" + field.name + "` must be an optional field.");
      }

      if (!field.index && !field.optional) {
        return Option<bool>(
            400, "Field `" + field.name +
                     "` must be optional since it is marked as non-indexable.");
      }

      if (field.name == ".*" && !field.index) {
        return Option<bool>(400, "Field `" + field.name +
                                     "` cannot be marked as non-indexable.");
      }

      if (!field.index && field.facet) {
        return Option<bool>(400, "Field `" + field.name +
                                     "` cannot be a facet since "
                                     "it's marked as non-indexable.");
      }

      if (!field.is_sort_field() && field.sort) {
        return Option<bool>(
            400, "Field `" + field.name + "` cannot be a sortabale field.");
      }
    }

    if (!default_sorting_field.empty() && !found_default_sorting_field) {
      return Option<bool>(400, "Default sorting field is defined as `" +
                                   default_sorting_field +
  ...
  static Option<bool> json_field_to_field(bool enable_nested_fields,
  ...
  static Option<bool> validate_and_init_embed_field(
      const tsl::htrie_map<char, field>& search_schema,
      nlohmann::json& field_json, const nlohmann::json& fields_json,
      field& the_field);

  static bool flatten_obj(
  ...
  static Option<bool> flatten_field(
  ...
  static Option<bool> flatten_doc(
...
struct filter {
  ...
  // Would be set when `field: != ...` is encountered with a string field or
  // `field: != [ ... ]` is encountered in the case of int and float fields.
  // During filtering, all the results of matching the field against the values
  // are aggregated and then this flag is checked if negation on the aggregated
  // result is required.
  ...
  static const std::string RANGE_OPERATOR() { return ".."; }

  static Option<bool> validate_numerical_filter_value(
      field _field, const std::string& raw_value) {
    if (_field.is_int32() && !StringUtils::is_int32_t(raw_value)) {
      return Option<bool>(
          400, "Error with filter field `" + _field.name + "`: Not an int32.");
    }

    else if (_field.is_int64() && !StringUtils::is_int64_t(raw_value)) {
      return Option<bool>(
          400, "Error with filter field `" + _field.name + "`: Not an int64.");
    }

    else if (_field.is_float() && !StringUtils::is_float(raw_value)) {
      return Option<bool>(
          400, "Error with filter field `" + _field.name + "`: Not a float.");
    }

    return Option<bool>(true);
  }

  static Option<NUM_COMPARATOR> extract_num_comparator(
      std::string& comp_and_value) {
    auto num_comparator = EQUALS;

    if (StringUtils::is_integer(comp_and_value) ||
        StringUtils::is_float(comp_and_value)) {
    ...
    // the ordering is important - we have to compare 2-letter operators first
      ...
      return Option<NUM_COMPARATOR>(
          400, "Numerical field has an invalid comparator.");
    }

    if (num_comparator == LESS_THAN || num_comparator == GREATER_THAN) {
      comp_and_value = comp_and_value.substr(1);
    } else if (num_comparator == LESS_THAN_EQUALS ||
               num_comparator == GREATER_THAN_EQUALS ||
               num_comparator == NOT_EQUALS) {
...
struct filter_node_t {
  ...
  filter_node_t(FILTER_OPERATOR filter_operator, filter_node_t* left,
                filter_node_t* right)
      : filter_operator(filter_operator),
        isOperator(true),
        left(left),
...
struct reference_filter_result_t {
  ...
  reference_filter_result_t& operator=(
      const reference_filter_result_t& obj) noexcept {
  ...
  ~reference_filter_result_t() { delete[] docs; }
};
...
struct filter_result_t {
  ...
  filter_result_t& operator=(const filter_result_t& obj) noexcept {
    ...
    for (const auto& item : obj.reference_filter_results) {
      reference_filter_results[item.first] =
          new reference_filter_result_t[count];
  ...
  static void and_filter_results(const filter_result_t& a,
                                 const filter_result_t& b,
                                 filter_result_t& result);

  static void or_filter_results(const filter_result_t& a,
                                const filter_result_t& b,
                                filter_result_t& result);
};
...
struct sort_by {
  ...
  sort_by(const std::string& name, const std::string& order,
          uint32_t text_match_buckets, int64_t geopoint,
          uint32_t exclude_radius, uint32_t geo_precision)
      : name(name),
        order(order),
        text_match_buckets(text_match_buckets),
        geopoint(geopoint),
...
class GeoPoint {
 ...
 public:
  static uint64_t pack_lat_lng(double lat, double lng) {
    ...
    // during int32_t -> uint64_t, higher order bits will be 1, so we have to
    // mask that
    const uint64_t lat_lng =
        (uint64_t(ilat) << 32) | (uint64_t)(ilng & MASK_H32_BITS);
    return lat_lng;
  }

  static void unpack_lat_lng(uint64_t packed_lat_lng, S2LatLng& latlng) {
    const double lat =
        double(int32_t((packed_lat_lng >> 32) & MASK_H32_BITS)) / 1000000;
    const double lng =
        double(int32_t(packed_lat_lng & MASK_H32_BITS)) / 1000000;
...
struct facet_stats_t {
  double fvmin = std::numeric_limits<double>::max(),
         fvmax = -std::numeric_limits<double>::min(), fvcount = 0, fvsum = 0;
};
...
struct facet {
  ...
  explicit facet(const std::string& field_name,
                 std::map<int64_t, std::string> facet_range = {},
                 bool is_range_q = false)
...
struct facet_hash_values_t {
  ...
  uint64_t back() const { return hashes[length - 1]; }
};
