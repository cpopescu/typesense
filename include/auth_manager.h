...
struct api_key_t {
  ...
  static constexpr const uint64_t FAR_FUTURE_TIMESTAMP =
      64723363199;  // year 4020

  api_key_t() {}

  api_key_t(const std::string& value, const std::string& description,
...
struct collection_key_t {
  ...
  explicit collection_key_t(const std::string& collection,
...
class AuthManager {
 private:
  ...
  Option<bool> authenticate_parse_params(const collection_key_t& scoped_api_key,
                                         const std::string& action,
                                         nlohmann::json& embedded_params) const;

  bool auth_against_key(const std::string& req_collection,
                        const std::string& action, const api_key_t& api_key,
 ...
 public:
  ...
  static bool add_item_to_params(
