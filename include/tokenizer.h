...
class Tokenizer {
 private:
  ...
  // tracks start of a text segment that can span multiple unicode tokens due to
  // use of custom symbols
  ...
  inline size_t get_stream_mode(char c) {
    return (std::isalnum(c) || index_symbols[uint8_t(c)] == 1)
               ? INDEX
 ...
 public:
  explicit Tokenizer(const std::string& input, bool normalize = true,
                     bool no_op = false, const std::string& locale = "",
  ...
  bool next(std::string& token, size_t& token_index, size_t& start_index,
            size_t& end_index);
  ...
  static inline bool is_ascii_char(char c) { return (c & ~0x7f) == 0; }
