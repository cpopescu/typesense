...
struct http_res {
  ...
  void set_content(uint32_t status_code, const std::string& content_type_header,
                   const std::string& body, const bool final) {
...
struct cached_res_t {
  ...
  bool operator==(const cached_res_t& res) const { return hash == res.hash; }

  bool operator!=(const cached_res_t& res) const { return hash != res.hash; }

  void load(uint32_t status_code, const std::string& content_type_header,
            const std::string& body, const TimePoint created_at,
            const uint32_t ttl, uint64_t hash) {
    this->status_code = status_code;
    this->content_type_header = content_type_header;
    this->body = body;
...
struct stream_response_state_t {
 ...
 public:
  ...
  void set_response(uint32_t status_code, const std::string& content_type,
                    std::string& body) {
  ...
  void set_req(h2o_req_t* _req) { req = _req; }

  h2o_req_t* get_req() { return req; }
};
...
struct http_req {
  ...
  // was the request aborted *without a result* because of wait time exceeding
  // search cutoff threshold?
  ...
  // stores http lib related datastructures to avoid race conditions between
  // indexing and http write threads
  ...
  http_req(h2o_req_t* _req, const std::string& http_method,
           const std::string& path_without_query, uint64_t route_hash,
           const std::map<std::string, std::string>& params,
           std::vector<nlohmann::json>& embedded_params_vec,
           const std::string& api_auth_key, const std::string& body,
      ...
      : _req(_req),
        ...
        params(params),
        embedded_params_vec(embedded_params_vec),
  ...
  ~http_req() {
    ...
    if (_req != nullptr) {
      ...
      const std::string metric_identifier =
          http_method + " " + path_without_query;
      AppMetrics::get_instance().increment_duration(metric_identifier,
                                                    ms_since_start);
      AppMetrics::get_instance().increment_write_metrics(route_hash,
                                                         ms_since_start);

      bool log_slow_searches =
          config.get_log_slow_searches_time_ms() >= 0 &&
      ...
      bool log_slow_requests =
          config.get_log_slow_requests_time_ms() >= 0 &&
          int(ms_since_start) >= config.get_log_slow_requests_time_ms();

      if (overloaded) {
        AppMetrics::get_instance().increment_count(AppMetrics::OVERLOADED_LABEL,
                                                   1);
        ...
        if (is_multi_search_query) {
          ...
          // ignore params map of multi_search since it is mutated for every
          // search object in the POST body
  ...
  // Take care to check for existence of key to ensure backward compatibility
  // during upgrade
  ...
  void load_from_json(const std::string& serialized_content) {
    ...
    if (start_ts == 0) {
      // Serialized request from an older version (v0.21 and below) which
      // serializes import data differently.
      body = content["body"];
    } else {
      body += content["body"];
    }

    for (nlohmann::json::iterator it = content["params"].begin();
         it != content["params"].end(); ++it) {
      params.emplace(it.key(), it.value());
    }

    metadata = content.count("metadata") != 0 ? content["metadata"] : "";
    first_chunk_aggregate = content.count("first_chunk_aggregate") != 0
                                ? content["first_chunk_aggregate"].get<bool>()
                                : true;
    last_chunk_aggregate = content.count("last_chunk_aggregate") != 0
                               ? content["last_chunk_aggregate"].get<bool>()
                               : false;
    start_ts = content.count("start_ts") != 0
                   ? content["start_ts"].get<uint64_t>()
                   : 0;
    log_index = content.count("log_index") != 0
                    ? content["log_index"].get<int64_t>()
                    : 0;
  }
...
struct route_path {
  ...
  bool (*handler)(const std::shared_ptr<http_req>&,
                  const std::shared_ptr<http_res>&) = nullptr;
  bool async_req;
  bool async_res;
  std::string action;

  route_path(const std::string& httpMethod,
  ...
  inline bool operator<(const route_path& rhs) const { return true; }

  uint64_t route_hash() {
    std::string path = StringUtils::join(path_parts, "/");
    std::string method_path = http_method + path;
    uint64_t hash =
        StringUtils::hash_wy(method_path.c_str(), method_path.size());
    return (hash > 100) ? hash
                        : (hash + 100);  // [0-99] reserved for special codes
  }
  ...
  std::string _get_action() {
    ...
    if (resource_index != path_parts.size() - 1 &&
        path_parts[resource_index + 1][0] != ':') {
      // e.g. /collections/:collection/documents/search
      operation = path_parts[resource_index + 1];
    } else {
      // e.g /collections or /collections/:collection/foo or
...
struct http_message_dispatcher {
  ...
  void init(h2o_loop_t* loop) {
    ...
    h2o_multithread_register_receiver(message_queue, message_receiver,
                                      on_message);
  }
  ...
  static void on_message(h2o_multithread_receiver_t* receiver,
    ...
    while (!h2o_linklist_is_empty(messages)) {
      h2o_multithread_message_t* message = H2O_STRUCT_FROM_MEMBER(
          h2o_multithread_message_t, link, messages->next);
      h2o_custom_res_message_t* custom_message =
          reinterpret_cast<h2o_custom_res_message_t*>(message);
  ...
  void send_message(const std::string& type, void* data) {
    h2o_custom_res_message_t* message = new h2o_custom_res_message_t{
        {{nullptr, nullptr}}, &message_handlers, type, data};
