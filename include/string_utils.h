...
struct StringUtils {
  ...
  static size_t split(
      const std::string& s, std::vector<std::string>& result,
      const std::string& delim, const bool keep_empty = false,
      const bool trim_space = true, const size_t start_index = 0,
  ...
  static std::string join(std::vector<std::string> vec,
  ...
  static void split_to_values(const std::string& vals_str,
  ...
  static bool is_int64_t(const std::string& s) {
    ...
    return (*p == 0) && val >= std::numeric_limits<int64_t>::min() &&
  ...
  static bool is_int32_t(const std::string& s) {
    ...
    return (*p == 0) && val >= std::numeric_limits<int32_t>::min() &&
  ...
  static uint32_t deserialize_uint32_t(std::string serialized_num) {
    uint32_t seq_id = ((serialized_num[0] & 0xFF) << 24) |
                      ((serialized_num[1] & 0xFF) << 16) |
                      ((serialized_num[2] & 0xFF) << 8) |
                      (serialized_num[3] & 0xFF);
    return seq_id;
  }
  ...
  static uint64_t hash_wy(const void* key, uint64_t len) {
    ...
    return hash != std::numeric_limits<uint64_t>::max()
               ? hash
  ...
  static Option<bool> tokenize_filter_query(const std::string& filter_query,
                                            std::queue<std::string>& tokens);

  static Option<bool> split_include_fields(const std::string& include_fields,
