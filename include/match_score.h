...
struct TokenOffset {
  ...
  bool operator>(const TokenOffset &a) const { return offset > a.offset; }
...
struct Match {
  ...
  Match(uint8_t words_present, uint8_t distance, uint8_t max_offset,
        uint8_t exact_match = 0)
  ...
  // Construct a single match score from individual components (for multi-field
  // sort)
  inline uint64_t get_match_score(const uint32_t total_cost,
                                  const uint32_t unique_words) const {
    uint64_t match_score =
        ((int64_t(unique_words) << 40) | (int64_t(words_present) << 32) |
         (int64_t(255 - total_cost) << 24) | (int64_t(100 - distance) << 16) |
         (int64_t(exact_match) << 8) | (int64_t(255 - max_offset) << 0));

    return match_score;
  }

  static void print_token_offsets(
  ...
  /*
      Given *sorted offsets* of each target token in a *single* document
     (token_offsets), generates a score indicating: a) How many tokens are
     present within a match window b) The proximity between the tokens within
     the match window
      ...
      Calculate distance, use only tokens within max window size from lowest
     offset. Reassign best window and distance if found. Pop end of vector
     (smallest offset). Push to vector next offset of token just popped. Until
     queue size is 1.
  */
  ...
  Match(uint32_t doc_id, const std::vector<token_positions_t> &token_offsets,
    ...
    for (size_t token_id = 0; token_id < tokens_size; token_id++) {
      window[token_id] = TokenOffset{static_cast<uint8_t>(token_id),
                                     token_offsets[token_id].positions[0], 0};
    }
    ...
    while (window.size() > 1) {
      ...
      if (int(min_offset) < prev_min_offset) {
        // indicates that one of the offsets are wrapping around (e.g. long
        // document)
        break;
      }
      ...
      for (size_t i = 0; i < window.size(); i++) {
        ...
        if ((window[i].offset - min_offset) <= WINDOW_SIZE) {
          uint16_t next_offset = (i == window.size() - 1)
                                     ? window[i].offset
                                     : window[i + 1].offset;
        ...
        all_offsets_are_same =
            all_offsets_are_same && (window[i].offset == prev_offset);
      }

      if (((this_num_match > best_num_match) ||
           (this_num_match == best_num_match &&
            this_displacement < best_displacement))) {
      ...
      if (best_num_match == tokens_size &&
          best_displacement == (window.size() - 1)) {
        // this is the best we can get, so quit early!
        break;
      }

      // fill window with next possible smallest offset across available token
      // this_token_offsets
      const TokenOffset &smallest_offset = window.back();
      window.pop_back();

      const uint8_t token_id = smallest_offset.token_id;
      const std::vector<uint16_t> &this_token_offsets =
          token_offsets[token_id].positions;
      ...
      TokenOffset token_offset{token_id, this_token_offsets[next_offset_index],
                               next_offset_index};
    ...
    if (check_exact_match) {
      ...
      for (const auto &token_positions : token_offsets) {
        ...
        if (total_offsets > token_offsets.size() &&
            distance == token_offsets.size() - 1) {
          // if total offsets exceed query length, there cannot possibly be an
          // exact match
          return;
        }
      }
