...
struct KV {
  ...
  KV(uint16_t queryIndex, uint64_t key, uint64_t distinct_key,
     int8_t match_score_index, const int64_t* scores,
     reference_filter_result_t* reference_filter_result = nullptr)
      : match_score_index(match_score_index),
        ...
        reference_filter_result(reference_filter_result) {
...
struct Topster {
  ...
  explicit Topster(size_t capacity, size_t distinct)
      : MAX_SIZE(capacity), size(0), distinct(distinct) {
    // we allocate data first to get a memory block whose indices are then
    // assigned to `kvs` we use separate **kvs for easier pointer swaps
  ...
  int add(KV* kv) {
    /*LOG(INFO) << "kv_map size: " << kv_map.size() << " -- kvs[0]: " <<
    kvs[0]->scores[kvs[0]->match_score_index]; for(auto& mkv: kv_map) {
        LOG(INFO) << "kv key: " << mkv.first << " => " <<
    mkv.second->scores[mkv.second->match_score_index];
    }*/
    ...
    if (distinct) {
      ...
      // Grouping cannot be a streaming operation, so aggregate the KVs
      ...
      */
      ...
      if (is_duplicate_key) {
        ...
        // LOG(INFO) << "existing_kv: " << existing_kv->key << " -> " <<
        // existing_kv->match_score;
  ...
  static bool is_greater_kv_group(const std::vector<KV*>& i,
                                  const std::vector<KV*>& j) {
    return std::tie(i[0]->scores[0], i[0]->scores[1], i[0]->scores[2],
                    i[0]->key) > std::tie(j[0]->scores[0], j[0]->scores[1],
                                          j[0]->scores[2], j[0]->key);
  }
  ...
  uint64_t getKeyAt(uint32_t index) { return kvs[index]->key; }

  uint64_t getDistinctKeyAt(uint32_t index) { return kvs[index]->distinct_key; }

  KV* getKV(uint32_t index) { return kvs[index]; }
};