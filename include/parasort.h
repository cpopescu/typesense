...
template <typename T>
static void _count(size_t sz, const T* list, unsigned np, const T* range,
                   size_t* bucket) {
...
void parasort(size_t sz, T* list, unsigned np = 2, unsigned sf = 100) {
  ...
  for (unsigned i = 0; i < np; i++) {
    ...
    threads[i] = std::thread(_count<T>, length, &list[start], np, &range[0],
                             &bucket[i * np]);
  }
  ...
  for (unsigned i = 0; i < np; i++) {
    size_t start = i * part;
    size_t length = (i + 1 == np) ? sz - i * part : part;
    threads[i] = std::thread(_reorder<T>, length, &list[start], np, &range[0],
                             &tmp[i * np], &sorted[0]);
  }
  for (auto&& thread : threads) thread.join();
  ...
  for (auto&& thread : threads) thread.join();
  // Step 6: Copy the temporary container for the sorted array into the input
  // array.
