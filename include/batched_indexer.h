...
class BatchedIndexer {
 private:
  struct req_res_t {
    uint64_t start_ts;
    std::string prev_req_body;  // used to handle partial JSON documents caused
                                // by chunking
    ...
    req_res_t(uint64_t start_ts, const std::string& prev_req_body,
              ...
              const std::shared_ptr<http_res>& res, uint64_t last_updated,
              uint32_t num_chunks, uint32_t next_chunk_index, bool is_complete)
        : start_ts(start_ts),
          prev_req_body(prev_req_body),
          req(req),
          res(res),
  ...
  std::string skip_index_upper_bound_key =
      std::string(SKIP_INDICES_PREFIX) + "`";  // cannot inline this
  rocksdb::Slice* skip_index_iter_upper_bound = nullptr;

  // When set, all writes (both live and log serialized) are skipped with 422
  // response
 ...
 public:
  ...
  BatchedIndexer(HttpServer* server, Store* store, Store* meta_store,
                 size_t num_threads, const Config& config,
                 const std::atomic<bool>& skip_writes);

  ~BatchedIndexer();

  void enqueue(const std::shared_ptr<http_req>& req,
